...
\dot{\varepsilon}_{i} = \gamma J^{(n-1)/2} A_{ij} \sigma_{j} \text{ ,}
```
...
$\gamma_0=1.1\times 10^{5}$ is the isotropic fluidity, $Q=530$ $kJ/mol$ is the activation energy, $R=8.314$ m$^3 \cdot$ Pa $\cdot$ K$^{−1} \cdot$ mol$^{−1}$ is the gas constant, $d=0.001$ m is the grain size, and $m=0.73$ is the grain size exponent. These values for olivine are taken from rock experiments performed by {cite:t}`hansen:etal:2016` and {cite:t}`HK04`. $J$ is the yield potential (scalar), which is computed with the anisotropic stress $\sigma_{ij}$:

```{math}
:label: eqn:yield_potential
J=\frac{2}{3}\left[F(\sigma_{22}-\sigma_{33})^2 + G(\sigma_{33}-\sigma_{11})^2 + H(\sigma_{11} - \sigma_{22})^2 + 2L\sigma_{23}^2 + 2M\sigma_{13}^2 + 2N\sigma_{12}^2\right]
```
