# This python script will reproduce the figures for the shell_simple_2d_smoothing cookbook,
# which should be run prior to executing this code. The script assumes the results are located
# in the output folder (output-shell_simple_2d_smoothing) one directory level above this folder. The
# script can be modified to plot different time steps by changing the pvtu file defined in the
# variable 'solution_file' below. It is recommended to run this script using the conda environment
# provided in contrib/python/env-py_aspect.yml
...
# inputs/config
solution_file = "../output-shell_simple_2d_smoothing/solution/solution-00185.pvtu"
output_png    = "../doc/smoothing.png"
...
x_labels = [f"{v:g}" for v in x_ticks / 1e3]
y_labels = [f"{v:g}" for v in y_ticks / 1e3]
z_labels = [f"{v:g}" for v in z_ticks / 1e3]
...
titles = [
    (to_px((xmax / 2, ymax + 6.5 * tick, 0)), "X (km)"),
    (to_px((xmax + 6.5 * tick, ymax / 2, 0)), "Y (km)"),
    (to_px((xmax + 6 * tick, -6 * tick, warp_factor / 2)), "Z (km)"),
]
