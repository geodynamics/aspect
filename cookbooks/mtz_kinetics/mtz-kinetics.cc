...
namespace aspect
{
  namespace MaterialModel
  {
      ...
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        {
          ...
          // Compute phase mass fractions from cumulative reaction progress values
    ...
    template <int dim>
    void
    MTZKinetics<dim>::clamp_reaction_progress_fields(MaterialModel::MaterialModelInputs<dim> &in) const
    {
      const unsigned int n_reactions = reaction_chain.n_reactions();
      ...
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        {
          ...
          for (unsigned int r = 0; r < n_reactions; ++r)
            in.composition[q][reaction_field_indices[r]] = reaction_progress_clamped[r];
        }
    }
