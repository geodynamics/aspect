...
To plot pole figures from the output in out_drex_olivineA/particles_cpo, you could use [MTEX](https://mtex-toolbox.github.io/) or [cpo_analyzer](https://github.com/MFraters/cpo_analyzer.git). Inside the git clone cpo_analyzer folder, run:
```{code-block} console
...
{numref}`fig:olivinefabrics`), we can see the results reproduce the pole figures in Figure 3 of {cite:t}`fraters_billen_2021_cpo`. There is minor difference (the girdle is less pronounced) in type D fabric when compared to Fig. 3 of (Karato et al., 2008), which requires further investigation.
