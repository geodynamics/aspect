...
//out.force_vector[i][0] = cos(z) - cos(x + z) + sin(z);
//out.force_vector[i][1] = sin(x) - cos(x + z) + x * cos(z);
          ...
          for (unsigned int i=0;i<in.position.size();++i)
            {
              ...
              melt_out->permeabilities[i] = 0.0; //0.1000000000e-1 + 0.2000000000e0 * exp(-0.200e2 * pow(x + 0.20e1 * z, 0.2e1)); //porosity;// K_D
		//	std::max(0.001953125*this->get_triangulation().begin_active()->diameter()*this->get_triangulation().begin_active()->diameter(),0.0);
...
gradient[0] = -0.2e1 * cos(x + z) + sin(z);
gradient[1] = -0.2e1 * cos(x + z) + x * cos(z);
//**********
