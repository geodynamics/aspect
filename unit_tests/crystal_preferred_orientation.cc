...
TEST_CASE("LPO elastic tensor decomposition")
{
  ...
  using namespace aspect::Particle::Property;
  {
    ...
    const Tensor<1,21> mono_and_higher_vector = Utility::projection_matrix_tric_to_mono * full_elastic_vector;
    ...
    const Tensor<1,9> ortho_and_higher_vector = Utility::projection_matrix_mono_to_ortho*mono_and_higher_vector_croped;
    ...
    const Tensor<1,9> tetra_and_higher_vector = Utility::projection_matrix_ortho_to_tetra*ortho_and_higher_vector;
    ...
    const Tensor<1,9> hexa_and_higher_vector = Utility::projection_matrix_tetra_to_hexa*tetra_and_higher_vector;
    ...
    const Tensor<1,9> iso_vector = Utility::projection_matrix_hexa_to_iso*hexa_and_higher_vector;
    for (size_t iii = 0; iii < 9; iii++)
      {
...

  {
    ...
    const SymmetricTensor<2,3> dilatation_stiffness_tensor_full = Utility::compute_dilatation_stiffness_tensor(full_elastic_matrix);
    for (size_t iii = 0; iii < 3; iii++)
      {
        for (size_t jjj = 0; jjj < 3; jjj++)
          {
    ...
    const SymmetricTensor<2,3> voigt_stiffness_tensor_full = Utility::compute_voigt_stiffness_tensor(full_elastic_matrix);
    for (size_t iii = 0; iii < 3; iii++)
      {
        for (size_t jjj = 0; jjj < 3; jjj++)
          {
    ...
    Tensor<2,3> unpermutated_SCCS = Utility::compute_unpermutated_SCCS(dilatation_stiffness_tensor_full, voigt_stiffness_tensor_full);
    for (size_t iii = 0; iii < 3; iii++)
      {
  ...
  }
  {
    ...
    std::array<std::array<double,3>,7 > norms = Utility::compute_elastic_tensor_SCCS_decompositions(unpermutated_SCCS, full_elastic_matrix);
