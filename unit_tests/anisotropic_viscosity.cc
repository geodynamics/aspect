...
TEST_CASE("OrthotropicRheology Functions in 3D")
{
  ...
  const double F_iso = 0.5;
  const double G_iso = 0.5;
  const double H_iso = 0.5;
  const double L_iso = 1.5;
  const double M_iso = 1.5;
  const double N_iso = 1.5;

  const double F_rand = 0.3;
  const double G_rand = 0.654;
  const double H_rand = 1.5 - F_rand - G_rand;
  const double L_rand = 2;
  const double M_rand = 1.2;
  const double N_rand = 1.6;
