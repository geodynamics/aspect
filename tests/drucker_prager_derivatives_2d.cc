...
namespace aspect
{
  ...
  f(const aspect::SimulatorAccess<dim> &simulator_access, aspect::Assemblers::Manager<dim> &)
  {
    ...
    const aspect::MaterialModel::DruckerPrager<dim> const_material_model =
      dynamic_cast<const aspect::MaterialModel::DruckerPrager<dim> &>(simulator_access.get_material_model());
    aspect::MaterialModel::DruckerPrager<dim> material_model =
      const_cast<aspect::MaterialModel::DruckerPrager<dim> &>(const_material_model);
        ...
        for (unsigned int i = 0; i < 5; i++)
          {
            in_dviscositydstrainrate.strain_rate[i] =
              in_base.strain_rate[i] + std::fabs(in_base.strain_rate[i][strain_rate_indices]) * finite_difference_accuracy *
  ...
  template <>
  void
  f(const aspect::SimulatorAccess<3> &, aspect::Assemblers::Manager<3> &)
  {
