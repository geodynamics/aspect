...
namespace aspect
{
    ...
    class AdditionalOutputs1 : public AdditionalMaterialOutputs<dim>
    {
      public:
                ...
                const FullMatrix<double>                   &expansion_matrix)
        {
          ...
          average_property(operation, projection_matrix, expansion_matrix, additional_material_output1);
        }
    ...
    class Material1 : public MaterialModel::Simple<dim>
    {
      public:
        ...
        evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const override
        {
          ...
          const std::shared_ptr<AdditionalOutputs1<dim>> additional = out.template get_additional_output_object<AdditionalOutputs1<dim>>();
          if (additional)
            ++counter_with;
          else
            ++counter_without;
  ...
  template <int dim>
  class TestAssembler : public aspect::Assemblers::Interface<dim>
  {
    public:
      virtual void
              ...
              internal::Assembly::CopyData::CopyDataBase<dim> & /*data_base*/) const
      {
        ...
        if (additional != nullptr)
          std::cout << "   value = " << additional->additional_material_output1[0] << ' ' << additional->additional_material_output1[1]
                    << std::endl;
      }
  };
  ...
  template <int dim>
  void
  set_assemblers1(const SimulatorAccess<dim> &, Assemblers::Manager<dim> &assemblers)
  {
...
namespace aspect
{
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(Material1, "Material1", "")
  }
}
