...
int f(double parameter)
{
  ...
  for(unsigned int i = 0; i < 3; i++)
  {
      ...
      if(temp > derivatives->dviscosities_dpressure[i] * finite_difference_factor || temp < derivatives->dviscosities_dpressure[i] * (2-finite_difference_factor))
      {
  ...
  for(unsigned int i = 0; i < 3; i++)
  {
	  ...
	  std::cout << "zerozero " << i << ": Finite difference = " << temp << ". Analytical derivative = " << derivatives->dviscosities_dstrain_rate[i][0][0]  << std::endl;
	  if(std::fabs(temp - derivatives->dviscosities_dstrain_rate[i][0][0]) > 1e-3 * (std::fabs(temp) + std::fabs(derivatives->dviscosities_dstrain_rate[i][0][0])))
		  {
  ...
  for(unsigned int i = 0; i < 3; i++)
  {
	  ...
	  std::cout << "onezero " << i << ": Finite difference = " << temp << ". Analytical derivative = " << derivatives->dviscosities_dstrain_rate[i][1][0]   << std::endl;
	  	  if(std::fabs(temp - derivatives->dviscosities_dstrain_rate[i][1][0]) > 1e-3 * (std::fabs(temp) + std::fabs(derivatives->dviscosities_dstrain_rate[i][1][0])) )
		  {
			  std::cout << "   Error: The derivative of the viscosity to the strain rate is too different from the analitical value." << std::endl;
              Error = true;
		  }
  ...
  for(unsigned int i = 0; i < 3; i++)
  {
	  ...
	  std::cout << "oneone " << i << ": Finite difference = " << temp << ". Analytical derivative = " << derivatives->dviscosities_dstrain_rate[i][1][1]  << std::endl;
  	  if(std::fabs(temp - derivatives->dviscosities_dstrain_rate[i][1][1]) > 1e-3 * (std::fabs(temp) + std::fabs(derivatives->dviscosities_dstrain_rate[i][1][1])) )
	  {
		  std::cout << "   Error: The derivative of the viscosity to the strain rate is too different from the analitical value." << std::endl;
		  Error = true;
		  }
