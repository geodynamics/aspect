...
{
  template <int dim>
  class MeltingRate : public MaterialModel::MeltInterface<dim>, public ::aspect::SimulatorAccess<dim>
  {
      virtual bool
            ...
            for (unsigned int c = 0; c < in.composition[i].size(); ++c)
              {
                ...
                if (c == peridotite_idx &&
                    ...
                    this->get_timestep_number() != numbers::invalid_unsigned_int && deltaT > 0.0)
...
namespace aspect
{
  ...
  ASPECT_REGISTER_MATERIAL_MODEL(MeltingRate, "melting rate", "")
}
