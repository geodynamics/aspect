  ...
  namespace Postprocess
  {
    ...
    class VolumeOfFluidSpecifiedSolutionDiff : public Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
        ...
        double
        get_next_t(const double curr_time, const double interval);
      ...
      private:
        ...
        error_abrev();
        std::vector<double>
        calc_error_level_set(const Function<dim> &func, const unsigned int n_samp, const unsigned int f_ind);
    ...
    template <int dim>
    double
    VolumeOfFluidSpecifiedSolutionDiff<dim>::get_next_t(const double curr_time, const double interval)
    {
          ...
          for (unsigned int f = 0; f < n_volume_of_fluid_fields; ++f)
            {
              for (unsigned int i = 0; i < n_err; ++i)
                {
                  err_str << std::scientific << std::setprecision(8) << global_err_vals[f * n_err + i];
                  if (i + 1 < n_err)
                    err_str << " / ";
                }
    ...
    VolumeOfFluidSpecifiedSolutionDiff<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Time between error estimates", "1e8", Patterns::Double(0.), "Time interval between error estimates.");

          prm.declare_entry("Number error samples",
                            "3",
                            Patterns::Integer(1),
                                                                  ...
                                                                  const unsigned int   f_ind)
    {
      ...
      FEValues<dim>        fe_err(this->get_mapping(), finite_element, quadrature, update_JxW_values | update_quadrature_points);

      const unsigned int volume_of_fluid_index =
        this->get_volume_of_fluid_handler().field_struct_for_field_index(f_ind).volume_fraction.first_component_index;
      const unsigned int volume_of_fluidN_c_index =
        this->get_volume_of_fluid_handler().field_struct_for_field_index(f_ind).reconstruction.first_component_index;
      ...
      for (auto cell : dof_handler.active_cell_iterators())
        {
          ...
          const double cell_volume_of_fluid =
            solution(local_dof_indices[finite_element.component_to_system_index(volume_of_fluid_index, 0)]);
          ...
          if (abs(d_compute) >= 0.5 * nnorm1 && abs(d_true) >= 0.5 * cell_diam && d_compute * d_true >= 0.0)
            {
              continue;
            }
          ...
          for (unsigned int i = 0; i < fe_err.n_quadrature_points; ++i)
            {
              ...
              const double computed_fluid_fraction_at_point =
                VolumeOfFluid::Utilities::compute_fluid_fraction(h * normal, (d_compute - dot));
