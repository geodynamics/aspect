...
class MyAspectLandlabModel(LandlabTemplate):
    ...
    def set_mesh_information(self, grid_dictionary):
        if not self.model_grid:
            ...
            self.model_grid = landlab.IcosphereGlobalGrid(radius=20, mesh_densification_level=2)

            print("* The number of Landlab grid nodes is: ", len(self.model_grid.x_of_node), flush=True)
            print("* The node coordinates (in radians) are:", self.model_grid.phi_of_node, self.model_grid.theta_of_node, flush=True)

            print("* Creating topographic elevation ...", flush=True)
            self.elevation = self.model_grid.add_zeros("topographic__elevation", at="node")
            self.elevation[self.model_grid.theta_of_node < np.pi/2] += 1.0
...
if __name__ == "__main__":
    ...
    print("grid coordinates:", model.get_grid_x(0), model.get_grid_y(0), model.get_grid_z(0), flush=True)
