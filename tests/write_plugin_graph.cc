...
template <int dim>
void
post_constraints_creation(const SimulatorAccess<dim> &sim, AffineConstraints<double> &)
{
  ...
  if (first)
    {
      ...
      const char *checks[] = {"digraph Plugins",
                              "label=\"TestMaterial\"",
...
namespace aspect
{
  ...
  ASPECT_REGISTER_MATERIAL_MODEL(TestMaterial, "TestMaterial", "A material model.")
}
