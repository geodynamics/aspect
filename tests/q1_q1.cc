...
namespace aspect
{
  namespace DoneaHuertaBenchmark
  {
    namespace AnalyticSolutions
    {
      ...
      Tensor<1, 2>
      DoneaHuerta_velocity(const Point<2> &pos, const double /*beta*/)
      {
      ...
      double
      DoneaHuerta_pressure(const Point<2> &pos, const double /*beta*/)
      {
        const double x = pos[0];
        return x * (1. - x) - 1. / 6.;
      }
    ...
    template <int dim>
    bool
    DoneaHuertaMaterial<dim>::is_compressible() const
    {
      return false;
    }
    ...
    DoneaHuertaMaterial<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("Viscosity parameter", "1", Patterns::Double(), "Viscosity parameter beta in the DoneaHuerta benchmark.");
      }
    ...
    template <>
    Tensor<1, 2>
    DoneaHuertaBoundary<2>::boundary_velocity(const types::boundary_id, const Point<2> &p) const
    {
      const DoneaHuertaMaterial<2> *material_model = dynamic_cast<const DoneaHuertaMaterial<2> *>(&this->get_material_model());
    ...
    template <>
    Tensor<1, 3>
    DoneaHuertaBoundary<3>::boundary_velocity(const types::boundary_id, const Point<3> &) const
    {
    ...
    template <int dim>
    Tensor<1, dim>
    DoneaHuertaGravity<dim>::gravity_vector(const Point<dim> &pos) const
    {
      const double   x = pos[0];
      const double   y = pos[1];
      Tensor<1, dim> g;
      ...
      g[1] =
        ((8. - 48. * y + 48. * y * y) * x * x * x + (-12. + 72. * y - 72 * y * y) * x * x +
    ...
    DoneaHuertaPostprocessor<dim>::execute(TableHandler &)
    {
      ...
      {
        const DoneaHuertaMaterial<dim> *material_model = dynamic_cast<const DoneaHuertaMaterial<dim> *>(&this->get_material_model());
