  ...
  namespace InitialTemperature
  {
    ...
    class MeltingCurve
    {
      public:
         ...
         */
        void
        read(const std::string &filename, const MPI_Comm comm);

        /**
         * Get the melting temperature.
         */
        double
        T(const double p, const double radius) const;
    ...
    void
    MeltingCurve::read(const std::string &filename, const MPI_Comm comm)
    {
          ...
          if (!in.fail())
            {
              ...
              if (T_Unit == "C")
                ...
                {
                  AssertThrow(false,
                              ExcMessage("Unit of the first column of melting curve data "
              ...
              if (P_Unit == "kbar")
              ...
              else
                {
                  ...
                  if (P_Unit == "km")
                    ...
                    AssertThrow(false,
                                ExcMessage("Unit of the second column of melting curve data "
    ...
    MeltingCurve::T(const double p, const double radius) const
    {
      ...
      for (unsigned i = 1; i < n_points; i++)
        {
          if ((i == n_points - 1) || (is_radius && P_or_R_value > P_or_R_array[i]) || (!is_radius && P_or_R_value < P_or_R_array[i]))
            {
              T_value = T_array[i - 1] +
                        (T_array[i] - T_array[i - 1]) / (P_or_R_array[i] - P_or_R_array[i - 1]) * (P_or_R_value - P_or_R_array[i - 1]);
              return (T_value);
            }
        }
    ...
    template <int dim>
    double
    Solidus<dim>::initial_temperature(const Point<dim> &position) const
    {
      ...
      if (dim == 2)
      ...
      else if (dim == 3)
        {
          ...
          std::pair<double, double> sph_harm_vals =
            Utilities::real_spherical_harmonic(lateral_wave_number_1, lateral_wave_number_2, scoord[2], scoord[1]);
    ...
    template <int dim>
    const Tensor<1, dim>
    Solidus<dim>::spherical_surface_coordinates(const Tensor<1, dim> &position) const
    {
      Tensor<1, dim> scoord;
    ...
    Solidus<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Supersolidus",
                            "0e0",
                            Patterns::Double(),
          ...
          prm.enter_subsection("Perturbation");
          {
            ...
            prm.declare_entry("Lateral wave number two",
                              "2",
                              Patterns::Integer(),
