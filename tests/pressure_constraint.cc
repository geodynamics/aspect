...
namespace aspect
{
  template <int dim>
  void
  modify_constraints(const SimulatorAccess<dim> &simulator_access, AffineConstraints<double> &current_constraints)
  {
    ...
    const types::global_dof_index first_pressure_dof = simulator_access.introspection().system_dofs_per_block[0];
    ...
    if (current_constraints.can_store_line(first_pressure_dof))
      {
        ...
        std::cout << "adding constraint idx= " << first_pressure_dof << " to value= " << value << std::endl;
      }
  }
