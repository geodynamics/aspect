...
namespace aspect
{
  ...
  my_signal(std::vector<VariableDeclaration<dim>> &variables)
  {
    ...
    VariableDeclaration<dim> dummy("dummy", std::make_shared<FE_DGQ<dim>>(4), 1, 1);
    variables.insert(variables.begin() + 6, dummy);

    for (unsigned int i = 0; i < variables.size(); ++i)
      {
        std::cout << " name=" << variables[i].name << " fe=" << variables[i].fe->get_name() << " multiplicity=" << variables[i].multiplicity
                  << " n_blocks=" << variables[i].n_blocks << " n_components=" << variables[i].n_components() << std::endl;
      }
...
namespace aspect
{
  ...
  ASPECT_REGISTER_MATERIAL_MODEL(MeltMaterial, "MeltMaterial", "")
}
