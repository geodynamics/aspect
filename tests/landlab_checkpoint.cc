...
int f()
{
  ...
  command = ("cd output-landlab_checkpoint ; "
             "(cat " ASPECT_SOURCE_DIR "/tests/landlab_checkpoint.prm "
             " ; "
  ...
  command = ("cd output-landlab_checkpoint ; "
             ...
             " mv output1.tmp/landlab_checkpoints* output2.tmp/");
  std::cout << "Executing the following command:\n"
            << command
            << std::endl;
  ret = run_command(command);
  ...
  ret = run_command("cd output-landlab_checkpoint ; "
                    "cp output1.tmp/topography/topography.00010 topography1.00010;"
                    "cp output2.tmp/topography/topography.00010 topography2.00010;"
