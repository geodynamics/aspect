...
namespace aspect
{
  ...
  f()
  {
    ...
    std::cout << "Testing distance to polygon function with the following parameters: (polygon 1) " << polygon[0] << ", " << polygon[1]
              << ", " << polygon[2] << ", " << polygon[3] << ", "
              << "(polygon 2) " << concave_polygon[0] << ", " << concave_polygon[1] << ", " << concave_polygon[2] << ", "
              << concave_polygon[3] << ", " << concave_polygon[4] << ", "
              << "(polygon 3) " << crossing_polygon[0] << ", " << crossing_polygon[1] << ", " << crossing_polygon[2] << ", "
              << crossing_polygon[3] << ", (points) " << points[0] << ", " << points[1] << ", " << points[2] << ", " << points[3] << ", "
              << points[4] << std::endl;

    for (unsigned int i = 0; i < 5; i++)
      {
        std::cout << "Minimal distance of point " << points[i] << " to polygon 1 = " << signed_distance_to_polygon<dim>(polygon, points[i])
                  << std::endl;
        std::cout << "Minimal distance of point " << points[i]
                  << " to polygon 2 = " << signed_distance_to_polygon<dim>(concave_polygon, points[i]) << std::endl;
        std::cout << "Minimal distance of point " << points[i]
                  << " to polygon 3 = " << signed_distance_to_polygon<dim>(crossing_polygon, points[i]) << std::endl;
      }
