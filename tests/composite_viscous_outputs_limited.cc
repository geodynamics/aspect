...
namespace aspect
{
  ...
  f(const aspect::SimulatorAccess<dim> &simulator_access, aspect::Assemblers::Manager<dim> &)
  {
    ...
    Rheology::DruckerPragerParameters p =
      drucker_prager_power->compute_drucker_prager_parameters(composition, phase_function_values, n_phase_transitions_per_composition);
    ...
    std::cout << "temperature (K)   eta (Pas)   creep stress (Pa)   edot_ii (/s)   edot_ii fractions (diff, disl, prls, drpr, max)"
              << std::endl;
    ...
    for (unsigned int i = 0; i <= 10; i++)
      {
        ...
        viscosity =
          composite_creep->compute_viscosity(pressure, temperature, grain_size, volume_fractions, strain_rate, partial_strain_rates);
        ...
        disl_stress =
          2. * partial_strain_rates[1] * dislocation_creep->compute_viscosity(partial_strain_rates[1], pressure, temperature, composition);
        prls_stress =
