...
bool
test_point(const GeometryModel::internal::EllipsoidalChunkGeometry<3> ellipsoidal_manifold, const Point<3> &test_point)
{
  ...
  std::cout << "Point: " << test_point << " becomes: " << converted_point << ". Back transformed: " << twice_converted_point
...
f()
{
  ...
  std::shared_ptr<InitialTopographyModel::ZeroTopography<dim>> topography = std::make_shared<InitialTopographyModel::ZeroTopography<dim>>();
  {
    ...
    GeometryModel::internal::EllipsoidalChunkGeometry<dim> ellipsoidal_manifold(topography, 6371000.0, 0.0, 6371000.0, 2890000.0, corners);
  ...
  const double semi_minor =
    std::sqrt((1 - dealii::Utilities::fixed_power<2>(eccentricity)) * dealii::Utilities::fixed_power<2>(semi_major_axis_a));
  {
    std::cout << "WGS84 test" << std::endl;

    GeometryModel::internal::EllipsoidalChunkGeometry<dim> ellipsoidal_manifold(
      topography, semi_major_axis_a, eccentricity, semi_minor, 2890000.0, corners);
...

  {
    ...
    GeometryModel::internal::EllipsoidalChunkGeometry<dim> ellipsoidal_manifold(
      topography, semi_major_axis_a, eccentricity, semi_minor, 500000.0, corners);
