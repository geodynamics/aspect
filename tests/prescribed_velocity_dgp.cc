...
namespace aspect
{
  ...
  {
    prm.declare_entry("Prescribe internal velocities",
                      "false",
                      Patterns::Bool(),
  ...
  template <int dim>
  void
  constrain_internal_velocities(const SimulatorAccess<dim> &simulator_access, AffineConstraints<double> &current_constraints)
  {
                ...
                if (current_constraints.can_store_line(local_dof_indices[q]) && !current_constraints.is_constrained(local_dof_indices[q]))
                  {
                    ...
                    if ((c_idx >= simulator_access.introspection().component_indices.velocities[0]) &&
                        (c_idx <= simulator_access.introspection().component_indices.velocities[dim - 1]))
                      {
                        ...
                        const unsigned int component_direction = (c_idx - simulator_access.introspection().component_indices.velocities[0]);
                        ...
                        if (dim == 2)
                          {
                            indicator =
                              prescribed_velocity_indicator_function_2d.value(reinterpret_cast<const Point<2> &>(p), component_direction);
                            u_i = prescribed_velocity_function_2d.value(reinterpret_cast<const Point<2> &>(p), component_direction);
                          }
                        else
                          {
                            indicator =
                              prescribed_velocity_indicator_function_3d.value(reinterpret_cast<const Point<3> &>(p), component_direction);
                            u_i = prescribed_velocity_function_3d.value(reinterpret_cast<const Point<3> &>(p), component_direction);
                          }
