...
namespace aspect
{
  ...
  ConvergenceMeltPostprocessor<dim>::execute(TableHandler &statistics)
  {
    ...
    os << std::scientific << " h = " << this->get_triangulation().begin_active()->diameter() << " ndofs= " << this->get_solution().size()
       << " u_L2= " << u_l2 << " p_L2= " << p_l2 << " p_f_L2= " << p_f_l2 << " p_c_L2= " << p_c_l2 << " phi_L2= " << phi_l2
       << " u_f_L2= " << u_f_l2;
  ...
  template <int dim>
  class PressureBdry : public BoundaryFluidPressure::Interface<dim>
  {
    public:
      virtual void
      fluid_pressure_gradient(const dealii::types::boundary_id,
