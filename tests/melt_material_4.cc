...
{
  template <int dim>
  class MeltMaterial : public MaterialModel::MeltInterface<dim>, public ::aspect::SimulatorAccess<dim>
  {
      virtual bool
  ...
  MMPostprocessor<dim>::execute(TableHandler &statistics)
  {
    ...
    os << std::scientific << u_l2 << ", " << p_l2 << ", " << poro_l2;
...
namespace aspect
{
  ...
  ASPECT_REGISTER_MATERIAL_MODEL(MeltMaterial, "MeltMaterial4", "")

  ASPECT_REGISTER_POSTPROCESSOR(MMPostprocessor, "MMPostprocessor", "")
