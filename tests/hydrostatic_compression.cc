...
{
  template <int dim>
  class TestMaterial : public MaterialModel::Interface<dim>, public ::aspect::SimulatorAccess<dim>
  {
    public:
      virtual bool
  ...
  class Gravity : public aspect::GravityModel::Interface<dim>
  {
    public:
      ...
      gravity_vector(const Point<dim> &pos) const
      {
        ...
        gravity[0] = -(0.1e1 + 0.2e1 * alpha * (double)pi * std::cos((double)(2 * pi * x)) * std::cos((double)(pi * z))) /
                     std::exp(alpha * (T0 + std::sin((double)(2 * pi * x)) * std::cos((double)(pi * z))));
        gravity[1] = -(0.1e1 - alpha * std::sin((double)(2 * pi * x)) * (double)pi * std::sin((double)(pi * z))) /
                     std::exp(alpha * (T0 + std::sin((double)(2 * pi * x)) * std::cos((double)(pi * z))));
  ...
  class RefFunction : public Function<dim>
  {
    public:
      ...
      vector_value(const Point<dim> &p, Vector<double> &values) const
      {
        ...
        values[0] = -2.0 * u1 * pi * std::sin((double)(2 * pi * x)) + u1 * std::cos((double)(2 * pi * x)) -
                    w1 * (pi * pi + 1.0) * std::cos((double)(pi * z));
  ...
  ConvergencePostprocessor<dim>::execute(TableHandler &statistics)
  {
    ...
    os << std::scientific << "ndofs= " << this->get_solution().size() << " u_L2= " << u_l2 << " p_L2= " << p_l2;

    return std::make_pair("Errors", os.str());
  }

  template <int dim>
  class ForceAssembler : public aspect::Assemblers::Interface<dim>, public SimulatorAccess<dim>
  {
    public:
      virtual void
      execute(internal::Assembly::Scratch::ScratchBase<dim> &scratch_base, internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
      {
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
          {
            ...
            force_u[0] = -0.32e2 / 0.3e1 * u1 * std::pow(pi, 0.3e1) * std::sin(0.2e1 * pi * x) +
                         0.16e2 / 0.3e1 * u1 * pi * pi * std::cos(0.2e1 * pi * x) - w1 * (pi * pi + 0.1e1) * pi * pi * std::cos(pi * z) +
                         (0.5e0 - z) * (0.1e1 + 0.2e1 * alpha * pi * std::cos(0.2e1 * pi * x) * std::cos(pi * z));

            force_u[1] = -0.4e1 / 0.3e1 * w1 * std::pow(pi, 0.3e1) * std::sin(pi * z) + 0.4e1 / 0.3e1 * w1 * pi * pi * std::cos(pi * z) -
                         0.4e1 * u1 * (0.4e1 * pi * pi + 0.1e1) * pi * pi * std::cos(0.2e1 * pi * x) - 0.1e1 +
                         (0.5e0 - z) * (0.1e1 - alpha * std::sin(0.2e1 * pi * x) * pi * std::sin(pi * z));
            ...
            for (unsigned int i = 0; i < stokes_dofs_per_cell; ++i)
              {
                data.local_rhs(i) += ((force_u * scratch.phi_u[i] + pressure_scaling * force_p * scratch.phi_p[i])) * JxW;
              }
          }
      }
  };
  ...
  template <int dim>
  void
  add_force_assembler(const SimulatorAccess<dim> &simulator_access, Assemblers::Manager<dim> &assemblers)
  {
...
namespace aspect
{
  ...
  ASPECT_REGISTER_MATERIAL_MODEL(TestMaterial, "test material", "")

  ASPECT_REGISTER_POSTPROCESSOR(ConvergencePostprocessor, "error calculation", "")
}
