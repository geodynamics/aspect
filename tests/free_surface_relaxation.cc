...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    template <int dim>
    void
    ReboundBox<dim>::create_coarse_mesh(parallel::distributed::Triangulation<dim> &coarse_grid) const
    {
          ...
          if (vertex_touched[cell->vertex_index(v)] == false)
            {
              ...
              vertex[dim - 1]    = vertex[dim - 1] + std::cos(order * 2.0 * M_PI * vertex[0] / (this->get_extents()[0])) * amplitude *
                                                    vertex[dim - 1] / (this->get_extents()[dim - 1]);
    ...
    template <int dim>
    void
    ReboundBox<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Geometry model");
      {
        ...
        {
          prm.declare_entry("Order", "1", Patterns::Double(0), "Order of the perturbation");
          prm.declare_entry("Amplitude", "0.1", Patterns::Double(0), "Scaled amplitude of the perturbation");
        }
...
namespace aspect
{
  namespace GeometryModel
  {
    ASPECT_REGISTER_GEOMETRY_MODEL(ReboundBox, "rebound box", "Geometry model for benchmarking relaxation of topography.")
  }
}
