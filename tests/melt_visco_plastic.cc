...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    double
    MeltViscoPlastic<dim>::reference_darcy_coefficient() const
    {
      // 0.01 = 1% melt
    ...
    template <int dim>
    bool
    MeltViscoPlastic<dim>::is_compressible() const
    {
      return false;
    }
    ...
    template <int dim>
    double
    MeltViscoPlastic<dim>::melt_fraction(const double temperature, const double pressure) const
    {
      // anhydrous melting of peridotite after Katz, 2003
      const double T_solidus        = A1 + 273.15 + A2 * pressure + A3 * pressure * pressure;
      const double T_lherz_liquidus = B1 + 273.15 + B2 * pressure + B3 * pressure * pressure;
      const double T_liquidus       = C1 + 273.15 + C2 * pressure + C3 * pressure * pressure;
    ...
    template <int dim>
    void
    MeltViscoPlastic<dim>::melt_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
                                          std::vector<double>                           &melt_fractions,
                                          const MaterialModel::MaterialModelOutputs<dim> *) const
    {
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        melt_fractions[q] = melt_fraction(in.temperature[q], std::max(0.0, in.pressure[q]));
      return;
    }

    template <int dim>
    void
    MeltViscoPlastic<dim>::evaluate(const typename Interface<dim>::MaterialModelInputs &in,
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          out.densities[i] =
            MaterialUtilities::average_value(volume_fractions, densities, MaterialUtilities::CompositionalAveragingOperation::arithmetic);
          out.thermal_expansion_coefficients[i] =
            MaterialUtilities::average_value(volume_fractions,
                                             thermal_expansivities,
                                             MaterialUtilities::CompositionalAveragingOperation::arithmetic);
          ...
          if (in.current_cell.state() == IteratorState::valid)
            {
              ...
              fe_values[extractor_pressure].get_function_values(this->get_solution(), fluid_pressures);
                      ...
                      if (reaction_rate_out != nullptr)
                        {
                          if (c == peridotite_idx && this->get_timestep_number() > 0)
                            reaction_rate_out->reaction_rates[i][c] =
                              porosity_change / melting_time_scale * (1 - maximum_melt_fraction) / (1 - old_porosity);
          ...
          for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
            {
              ...
              const double effective_pressure =
                ((this->include_melt_transport() && this->get_melt_handler().is_melt_cell(in.current_cell)) ?
              ...
              for (unsigned int c = 0; c < volume_fractions.size(); ++c)
                {
                  ...
                  if (effective_pressure > transition_pressure || !this->include_melt_transport())
                    yield_strength_c = ((dim == 3) ? (6.0 * cohesions[c] * std::cos(phi) + 2.0 * effective_pressure * std::sin(phi)) /
              ...
              const double angle_internal_friction =
                MaterialUtilities::average_value(volume_fractions, angles_internal_friction, viscosity_averaging);
    ...
    MeltViscoPlastic<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt visco plastic");
        {
          ...
          prm.declare_entry("Specific heats",
                            "1250.",
                            Patterns::List(Patterns::Double(0)),
          ...
          prm.declare_entry("Minimum strain rate", "1.0e-20", Patterns::Double(0), "Stabilizes strain dependent viscosity. Units: $1 / s$");
          ...
          prm.declare_entry("Linear viscosities",
                            "1.e22",
                            Patterns::List(Patterns::Double(0)),
          ...
          prm.declare_entry("Angles of internal friction",
                            "0",
                            Patterns::List(Patterns::Double(0)),
          ...
          prm.declare_entry("Maximum yield stress",
                            "1e12",
                            Patterns::Double(0),
          ...
          prm.declare_entry("Host rock strength reductions",
                            "4",
                            Patterns::List(Patterns::Double(0)),
          ...
          prm.declare_entry("Melt density change",
                            "-500",
                            Patterns::Double(),
          ...
          prm.declare_entry("beta",
                            ...
                            "Units: non-dimensional.");
          prm.declare_entry("Mass fraction cpx",
                            "0.15",
                            Patterns::Double(),
          ...
          prm.declare_entry("Use melting rate function",
                            "false",
                            Patterns::Bool(),
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt visco plastic");
        {
          ...
          thermal_expansivities = Utilities::possibly_extend_from_1_to_N(
            Utilities::string_to_double(Utilities::split_string_list(prm.get("Thermal expansivities"))), n_fields, "Thermal expansivities");
          specific_heats =
            Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(prm.get("Specific heats"))),
                                                   n_fields,
                                                   "Specific heats");
          thermal_conductivities = Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(
                                                                            prm.get("Thermal conductivities"))),
                                                                          n_fields,
          ...
          linear_viscosities =
            Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(prm.get("Linear viscosities"))),
                                                   n_fields,
                                                   "Linear viscosities");

          angles_internal_friction = Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(
                                                                              prm.get("Angles of internal friction"))),
                                                                            n_fields,
                                                                            "Angles of internal friction");
          cohesions =
            Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(prm.get("Cohesions"))),
                                                   n_fields,
                                                   "Cohesions");

          maximum_yield_stress = prm.get_double("Maximum yield stress");

          strength_reductions = Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(Utilities::split_string_list(
                                                                         prm.get("Host rock strength reductions"))),
                                                                       n_fields,
          ...
          AssertThrow(melting_time_scale > 0, ExcMessage("The Melting time scale for operator splitting must be larger than 0!"));
      ...
      if (out.template has_additional_output_object<PlasticAdditionalOutputs2<dim>>() == false)
        {
          ...
          out.additional_outputs.push_back(std::make_unique<MaterialModel::PlasticAdditionalOutputs2<dim>>(n_points));
        }
    }
