...
namespace aspect
{
  namespace Benchmark
  {
    ...
    {
      Tensor<1, 2>
      velocity(const Point<2> &pos, const double /*eta*/)
      {
      ...
      double
      pressure(const Point<2> &pos, const double eta)
      {
        const double x = pos[0];
    ...
    template <>
    Tensor<1, 2>
    MyBoundary<2>::boundary_velocity(const types::boundary_id, const Point<2> &p) const
    {
    ...
    template <>
    Tensor<1, 3>
    MyBoundary<3>::boundary_velocity(const types::boundary_id, const Point<3> &p) const
    {
    ...
    template <int dim>
    bool
    MyMaterial<dim>::is_compressible() const
    {
      return false;
    }
    ...
    MyMaterial<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("Viscosity parameter", "10", Patterns::Double(0), "Viscosity the benchmark.");
      }
    ...
    MyPostprocessor<dim>::execute(TableHandler &statistics)
    {
      ...
      {
        const MyMaterial<dim> *material_model = dynamic_cast<const MyMaterial<dim> *>(&this->get_material_model());
