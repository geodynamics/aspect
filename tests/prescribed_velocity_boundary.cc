  ...
  namespace InclusionBenchmark
  {
      ...
      class FunctionInclusion : public Function<dim>
      {
        public:
          ...
          vector_value(const Point<dim> &p, Vector<double> &values) const
          {
            ...
            AnalyticSolutions::_Inclusion(pos, 0.2, eta_B_, &values[0], &values[1], &values[2]);
          }
    ...
    class InclusionBoundary : public BoundaryVelocity::Interface<dim>
    {
      public:
        ...
        boundary_velocity(const types::boundary_id boundary_id, const Point<dim> &position) const;
    ...
    template <int dim>
    Tensor<1, dim>
    InclusionBoundary<dim>::boundary_velocity(const types::boundary_id boundary_id, const Point<dim> &p) const
    {
      Assert(dim == 2, ExcNotImplemented());

      std::cout << "Boundary_velocity called with boundary_id =" << static_cast<int>(boundary_id) << std::endl;
      ...
      double         pressure;
      AnalyticSolutions::_Inclusion(pos, 0.2, eta_B, &velocity[0], &velocity[1], &pressure);
    ...
    template <int dim>
    bool
    InclusionMaterial<dim>::is_compressible() const
    {
      return false;
    }
    ...
    InclusionMaterial<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        ...
        {
          prm.declare_entry("Viscosity jump", "1e3", Patterns::Double(0), "Viscosity in the Inclusion.");
        }
    ...
    InclusionPostprocessor<dim>::execute(TableHandler &statistics)
    {
      ...
      if (dynamic_cast<const InclusionMaterial<dim> *>(&this->get_material_model()) != nullptr)
        {
          const InclusionMaterial<dim> *material_model = dynamic_cast<const InclusionMaterial<dim> *>(&this->get_material_model());

          ref_func = std::make_unique<AnalyticSolutions::FunctionInclusion<dim>>(material_model->get_eta_B());
        }
      else
        {
          AssertThrow(false, ExcMessage("Postprocessor DuretzEtAl only works with the material model SolCx, SolKz, and Inclusion."));
        }
