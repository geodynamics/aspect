      ...
      public:
        ...
        initialize() override
        {
          const std::string datadirectory =
            Utilities::expand_ASPECT_SOURCE_DIR("$ASPECT_SOURCE_DIR/data/material-model/latent-heat-enthalpy-test/");
                  ...
                  for (unsigned int p = 0; p < n_q_points; ++p)
                    {
                      ...
                      if (std::fabs(temperatures[q] - temperatures[p]) >
                          100 * std::fabs(temperatures[q]) * std::numeric_limits<double>::epsilon())
                        {
              ...
              if (this->get_adiabatic_conditions().is_initialized())
              ...
              else
                {
                  out.thermal_expansion_coefficients[i] =
                    (1 - (reference_rho + density_jump) * material_lookup->dHdp(in.temperature[i], in.pressure[i])) / in.temperature[i];
