...
Fortran, the GNU make system, the CMake build system, development versions of
the BLAS and LAPACK libraries and the zlib library, which is used for
compressing the output data. To use more than one process for your computations
you will need to install a development version of a MPI library and the
...
and PETSC for alternative solvers, but these are not strictly required.
Numdiff is used for checking \aspect{}'s test results with reasonable accuracy.
