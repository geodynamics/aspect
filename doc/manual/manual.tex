...
\begin{align}
  ...
  &\quad
  -
  \frac 1\rho T \frac{\partial\rho}{\partial T}\frac{Dp}{Dt}
  & \quad
  & \textrm{in $\Omega$},
  \notag
\end{align}
...
\item \textit{Graphical output files:} One of the postprocessors you select
  ...
  \footnote{The underlying \dealii{} package actually supports output in
    ...
    VTK, let the authors of \aspect{} know and we can add this feature.}  If
...
\note{It is currently not possible to change the number of processors involved
in the computation.}
...
\note{It is not imperative that the parameters selected in the input file are
  ...
  should happen, etc., can be different. Yet, it is
  ...
  must be the same before and after resuming a computation. Not all invalid
...
What \aspect{} computes is drive by two things:
\begin{itemize}
