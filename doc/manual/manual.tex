...
Multithreading is controlled by setting the environment variable
\texttt{DEAL\_II\_NUM\_THREADS}. If the variable is not set \aspect{} will
create exactly one thread per MPI process, i.e. multithreading is disabled.
Setting it to any other number will spawn as many threads per MPI process as
you select (a typical example is ``2''). Note that \aspect{} will limit the
number of threads to a reasonable number, i.e. if you request 8 threads per
process on an quadcore machine with hyperthreading (8 logical cores), but start
with 2~MPI processes \aspect{} will only spawn 4 threads on each process. Also
...
processes that is not a divisor of your logical threads (e.g. 3 MPI processes
for 8 logical cores).
