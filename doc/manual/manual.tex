...
of the Stokes system (for velocity and pressure). When used correctly, this potentially
improves the solver performance by about 3x and decrease the memory requirements by a factor of 10.
For more details
...
\item The GMG solver requires that the viscosity is averaged, either as a constant (for example by using harmonic averaging)
or as a Q1 averaging. Averaging other properties is optional. You can use
\lstinputlisting[language=prmfile]{cookbooks/overview/doc/gmg-average.part.prm.out}
for example. Note that Q1 averaging is a bit slower than averaging to a constant per cell, but it
...
is more important than for other parts of ASPECT to get good performance.

\item Enable vectorization optimizations. The GMG solver takes advantage of special instructions (AVX2, AVX512)
in modern CPUs and requires these do be enabled when compiling deal.II. This can be achieved by passing the
...
\end{lstlisting}
Without optimizations enabled, the output will be ``and vectorization level 1 (128 bits)''.
\end{enumerate}
