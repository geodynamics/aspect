...
\note{\aspect{}'s tracer implementation is in a preliminary state. This in particular means that it is not optimized for performance, and more than a few thousand tracers per process can slow down a model significantly. Moreover the MPI transfer of tracers is currently not handled in a very scalable way, and models with more than a hundred processes might be hindered by this limitation. Additionally, the checkpoint/restart functionality for tracers is only implemented in models with a constant number of processes before and after the checkpoint and when the selected tracer properties do not change. These limitations will be removed over time, but for current models the user should be aware of them.}
