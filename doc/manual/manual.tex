...
\lstinputlisting[language=prmfile]{cookbooks/adiabatic_boundary/adiabatic_boundary_geometry_model.prm}

The adiabatic boundary plugin uses the simple material model (see Sections~\ref{parameters:Material_20model} and  \ref{parameters:Material_20model/Simple_20model}.), the radial Earth-like gravity model (Section~\ref{parameters:Gravity_20model}), and tangential velocity boundary conditions (Section~\ref{sec:equations}).The adiabatic boundary is input in \aspect{} using the following set of input parameters:

 \lstinputlisting[language=prmfile]{cookbooks/adiabatic_boundary/adiabatic_boundary_initial_temp_model.prm}

Figure~\ref{fig:adiabatic-boundary} illustrates a thermally defined lithosphere with temperatures ranging from 273.15 K at the surface to 1673.15 K at the lithosphere-asthenosphere-boundary. The temperature of the asthenosphere ranges from 1673.15 K to 1900.15 K at the base of the model (500 km).

 \begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{cookbooks/adiabatic_boundary/initial_temperature_model.pdf}
    \caption{Initial temperature model highlighting the 1673.15 K temperature contour, which marks the lithosphere-asthenosphere thermal boundary corresponding to a sharp change in the geothermal gradient.}
