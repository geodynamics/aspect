...
some video lectures demonstrating this process at
\url{http://www.youtube.com/watch?v=3ChnUxqtt08} for the Visit visualization
program, and at
...
doing so, however, let us discuss ways in which plugins can query other information.
...
\begin{lstlisting}[frame=single,language=C++]
    ...
    protected:
      ...
      // ... many more things ...
\end{lstlisting}
This way, \texttt{SimulatorAccess} makes information available to plugins
without the need for them to understand details of the core of \aspect{}.
Rather, if the core changes, the \texttt{SimulatorAccess} class can still
provide exactly the same interface. Thus, it insulates plugins from having
to know the core. Equally importantly, since \texttt{SimulatorAccess} only
...
from the \texttt{SimulatorAccess} class, then you can write a member
...
\footnote{For complicated, technical reasons, in the code below we need to
	access elements of the \texttt{SimulatorAccess} class using the notation
...
these boundary conditions only once at the beginning of the program since they
...
\begin{itemize}
  \item \textit{Plugins that compute things from the solution in a pointwise way:}
   ...
   the \texttt{SimulatorAccess} class) but also from the deal.II class
