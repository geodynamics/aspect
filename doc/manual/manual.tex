...
\note{We do not use the visco-elasto-plastic rheology of the formulation in \cite{KMK2014} yet. In order to do this, we would have to add elastic deformation terms on the right hand side of equation \ref{eq:stokes-1-melt} and equation \ref{eq:stokes-3-melt} including the elastic and compaction stress evolution parameters $\xi_\tau$ and $\xi_p$. In addition, this would require a modification of the rheologic law using effective shear and compaction viscosities $\eta_\text{eff}$ and $\xi_\text{eff}$ combining a failure criteria and a shear and compaction visco-elasticity.}
