...
The plugin was tested against analytical solutions for the deformation gradient tensor in simple and pure shear as described in \url{benchmark/finite_strain/pure_shear.prm} and \url{benchmark/finite_strain/simple_shear.prm}.
...
It would have been feasible to run the same model with tracer particles that track the deformation gradient, as
...
maximum accumulated strain. Tracers only provide finite strain values at discrete positions, but can, if this is desired, be used in 
fewer numbers and only a part of the model domain (and are much faster in this case). If however
there needs to be a large number of tracers (possibly because they are used for other purposes as well), then they
...
(\url{www.burnman.org}) using the python example program \texttt{simple\_adiabat.py}. 
...
The viscosity in this model is computed as the product of a radial profile $\eta_r(z)$ and a 
...
\texttt{Viscosity prefactors} and a list of of depths that describe in which depth range each prefactor 
should be applied, in other words, in which depth the viscosity changes. By default, it is set
to viscosity jumps in 150\,km depth, between upper mantle and transition zone, 
...
the density in the buoyancy term (on the right-hand side of the momentum conservation equation). 
In addition, the density derivative in the mass conservation equation is taken from the adiabatic 
conditions, where it is computed as depth derivative of the provided reference density profile
(see also Section~\ref{sec:combined_formulations}).
Without these lines in the parameter file, \aspect{} will use its default implementation for 
compressible models, ``isothermal compression'' (see Section~\ref{sec:combined_formulations}), 
which uses the full density everywhere in the equations, and uses the compressibility 
to compute the density derivative term on the right-hand side of the mass conservation equation.
In principle, the \texttt{ascii reference profile} material model also supports the truncated 
anelastic liquid approximation (see Section~\ref{sec:tala}), which can be used by setting 
the formulation of the equations to \texttt{anelastic liquid approximation} and specifying
\lstinputlisting[language=prmfile]{cookbooks/burnman/tala.part.prm}
This formulation will neglect any density changes due to dynamic pressure. 
...
at \url{https://youtu.be/FS-ru4N0p-c}.
...
mineral phase to the next. We can also visualize the gravity and the adiabatic profile, to see how 
...
\begin{figure}
  ...
  \caption{\it Compressible convection in a 2d spherical shell, using a reference profile exported 
               form BurnMan, which is based on the Birch-Murnaghan equation of state. Shown is the
...
gravity, thermal expansivity, specific heat capacity and compressibility. Using BurnMan 
to generate the reference profile has the advantage that all the material property data 
is consistent, for example, the gravity profile is computed using the reference density.
\begin{figure}
  \includegraphics[width=\textwidth]{cookbooks/burnman/reference_profile.pdf}
  \caption{\it Reference profile generated using BurnMan.}
  \label{fig:burnman-reference-profile}
...
In addition, we can assign the viscosity profile in our model by providing a list of 
viscosity prefactors and a list of of depths where the viscosity changes (which is 
by default set to viscosity jumps in 150\,km depth, between upper mantle and transition zone, 
and between transition zone and lower mantle). The prefactors used here lead to a low-viscosity 
asthenosphere, and high viscosities in the lower mantle. To make sure that these viscosity jumps 
do not lead to numerical problems in our computation (see Section~\ref{sec:sinker-with-averaging}), 
we also use harmonic averaging of the material properties.
