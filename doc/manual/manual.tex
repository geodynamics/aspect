...
introduce a thermodynamic potential known as the specific Gibbs free energy
$\mathcal{G}(p, T)$ [\textit{J/kg}]. The word ``specific'' indicates that the energy is
given per unit mass, rather than volume or number of atoms/molecules. This potential is
...
\end{eqnarray}
\noindent where $S$ is the specific entropy, $C_p$ and $C_V$ are the specific isobaric
...
the heat capacity \emph{cannot} be globally constant. One way to solve this issue is to define
heat capacity at constant pressure, and then integrate~\eqref{eq:heat_capacity_p_dependence}
with respect to pressure:
\begin{equation}
  C_p(p, T) = C_p(p_{\textrm{ref}}, T) -T \int_{p_{\textrm{ref}}}^p \pddc{\left(\alpha / \rho \right)}{T}{p} dp
\end{equation}
There is no guarantee that this expression will have a simple form. 
...
we can define this in terms of partial differentials of the entropy with respect to pressure
and temperature:
\begin{eqnarray}
...
given formulation is thermodynamically inconsistent. Indeed, there are many equations
of state which exist in the thermodynamic literature which do not obey these relations! 
...
\[
\mu(y)=\frac{1}{\frac{1}{\pi} \tan^{-1} (\frac{y-y_0}{\beta} ) + 1/2 + \epsilon}
\]
where $\beta$ and $\epsilon$ are chosen by the user. 
Viscosity profiles for different values of $\beta$ and $\epsilon$ are shown in Fig.~\ref{fig:layeredflow1}.

\begin{figure}
\begin{center}
  \centering
  \includegraphics[width=0.48\textwidth]{cookbooks/benchmarks/layeredflow/viscosityA}
  \includegraphics[width=0.48\linewidth]{cookbooks/benchmarks/layeredflow/viscosityD}
  \caption{\it Viscosity profiles for various $\beta$ values and two $\epsilon$ values for $y_0=1/3$.} 
...
\begin{eqnarray}
C_1 &=& 2\pi \left[ 
 \beta  \log [\beta^2 + (1+z_0)^2]  -  2(1+z_0) \tan^{-1} \frac{1+z_0}{\beta} 
-\beta  \log [\beta^2 + (1-z_0)^2]  +  2(1-z_0) \tan^{-1} \frac{1-z_0}{\beta} + 2\pi (1+2\epsilon)   \right]^{-1} \nonumber\\
...
\begin{center}
  \centering
  \includegraphics[width=0.48\textwidth]{cookbooks/benchmarks/layeredflow/vel}
  \includegraphics[width=0.48\textwidth]{cookbooks/benchmarks/layeredflow/viscosity}
  \caption{\it Velocity and viscosity fields for $\beta=0.01$ and $\epsilon=0.05$ at uniform level 8 resolution with $y_0=1/3$.}  
