...
where
\begin{align*}
  v_\text{eff}^v = B \left(\frac{\dot{\varepsilon}}{\dot{\varepsilon}_{ref}}\right)^{-1+1/n_v}
  \exp\left(\frac{E_a +V_a \rho_m g z}{n_v R T}\right),
  \\
  v_\text{eff}^p = (\tau_0 + \gamma \rho_m g z) \left( \frac{\dot{\varepsilon}^{-1+1/n_p}}
  {\dot{\varepsilon}_{ref}^{1/n_p}} \right),
\end{align*}
where $v_\text{eff}$ is the effective viscosity, $B$ is a scaling constant, $\dot{\varepsilon}$ is
