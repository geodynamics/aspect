...
$\nabla p \approx \nabla p_\text{static} \approx \rho \textbf{g}$.
...
\begin{align}
  ...
  \\
  &\quad
  - \frac{\phi }{\rho_f} \mathbf u_s \cdot \nabla\rho_f 
  - \frac{1 - \phi }{\rho_s} \mathbf u_s \cdot \nabla\rho_s
  \notag
  \\
  &\quad
  - K_D \mathbf g \cdot \nabla \rho_f 
  ...
  \nabla \cdot \mathbf u_s + \frac{p_c}{\xi} 
  &=
  - \kappa_s \rho_s \mathbf g \cdot \mathbf u_s
\end{align}

We use the indices s to indicate properties of the solid and f for the properties of the fluid. To avoid the density gradients, we can use the same method as for the mass conservation (described in section \ref{sec:boussinesq}) and assume the change in density (for both the solid and the fluid phase) is dominated by the change in static pressure, which can be written as
$\nabla p_{s,f} \approx \nabla p_\text{static} \approx \rho_{s,f} \textbf{g}$.
This finally allows us to write
\begin{equation*}
\frac{1}{\rho_{s,f}} \nabla \rho_{s,f}
\approx \frac{1}{\rho_{s,f}} \frac{\partial \rho_{s,f}}{\partial p_{s,f}} \nabla p_{s,f}
\approx \frac{1}{\rho_{s,f}} \frac{\partial \rho_{s,f}}{\partial p_{s,f}} \nabla p_{s,f}
\approx \frac{1}{\rho_{s,f}} \frac{\partial \rho_{s,f}}{\partial p_{s,f}} \rho_{s,f} \textbf{g}
\approx \kappa_{s,f} \rho_{s,f} \textbf{g} 
\end{equation*}
so we can replace the second equation by
\begin{align*}
\nabla \cdot \mathbf u_s - \nabla \cdot K_D \nabla p_f 
  - K_D \kappa_f \rho_f \nabla p_f \cdot \textbf{g} 
  &= 
  - \nabla \cdot K_D \rho_f \mathbf g
  \\
  &\quad
  ...
  \\
  &\quad
  - (\mathbf u_s \cdot \mathbf g ) 
  \left[ \phi \kappa_f \rho_f + (1 - \phi) \kappa_s \rho_s \right]
  \\
  &\quad
  - K_D \kappa_f \rho_f^2 \mathbf g \cdot \textbf{g} 
  & \qquad
  & \textrm{in $\Omega$},
\end{align*}

 The equations are solved for the solid velocity $\mathbf{u_s}$, the fluid pressure $p_f$, and an additional variable, the compaction pressure $p_c$, which is related to the fluid and solid pressure over the relation $p_c = (1-\phi) (p_s-p_f)$. $K_D$ is the Darcy coefficient, which is defined as the quotient of the permeability and the fluid viscosity and $\Gamma$ is the melting rate. $\eta$ and $\xi$ are the shear and compaction viscosities and can depend on the porosity, temperature, pressure, strain rate and composition. However, there are various laws for these quantities and so this is implemented in the material model. Common formulations for the dependence on porosity are $\eta = (1-\phi) \eta_0 e^{-\alpha_\phi \phi}$ with $\alpha_\phi \approx 25...30$ and $\xi = \eta_0 \phi^{-n}$ with $n \approx 1$. We do not use the visco-elasto-plastic rheology of the \cite{KMK2014} formulation yet. In order to do this, we would have to add elastic deformation terms on the right hand side of equation \ref{eq:stokes-1-melt} and equation \ref{eq:stokes-3-melt} including the elastic and compaction stress evolution parameters $\xi_\tau$ and $\xi_p$. In addition, this would require a modification of the rheologic law using effective shear and compaction viscosities $\eta_\text{eff}$ and $\xi_\text{eff}$ combining a failure criteria and a shear and compaction visco-elasticity. 

Moreover, melt transport requires an advection equation for the porosity field $\phi$:
\begin{align}
  \label{eq:porosity}
  \rho_s \frac{\partial (1 - \phi)}{\partial t} + \nabla \cdot \left[ \rho_s (1 - \phi) \mathbf u_s \right]
  &=
  - \Gamma
  & \quad
  & \textrm{in $\Omega$},
...
+ \nabla \rho_s \cdot \mathbf u_s \right)
- \nabla \phi \cdot \rho_s \mathbf u_s 
\end{equation*}
Then we use the same method as for the mass conservation (described in section \ref{sec:boussinesq}) and assume the change in density is dominated by the change in static pressure, which can be written as
$\nabla p_s \approx \nabla p_\text{static} \approx \rho_s \textbf{g}$.
This finally allows us to write
\begin{equation*}
\frac{1}{\rho_s} \nabla \rho_s \cdot \mathbf{u_s} 
\approx \frac{1}{\rho_s} \frac{\partial \rho_s}{\partial p_s} \nabla p \cdot \mathbf{u_s} 
\approx \frac{1}{\rho_s} \frac{\partial \rho_s}{\partial p_s} \nabla p_s \cdot \mathbf{u_s} 
\approx \frac{1}{\rho_s} \frac{\partial \rho_s}{\partial p_s} \rho_s \textbf{g} \cdot \mathbf{u_s} 
\approx \kappa_s \rho_s \textbf{g} \cdot \mathbf{u_s} 
\end{equation*}
so we get
\begin{equation*}
\frac{\partial \phi}{\partial t} + \mathbf u_s \cdot \nabla \phi
= \frac{\Gamma}{\rho_s}
+ (1 - \phi) (\nabla \cdot \mathbf u_s + \kappa_s \rho_s \textbf{g} \cdot \mathbf{u_s} )
\end{equation*}

as a postprocess we compute:
\[
...
set Linear solver tolerance                = 1e-10 % \index[prmindex]{Linear solver tolerance} \index[prmindexfull]{Linear solver tolerance} %
set Temperature solver tolerance           = 1e-10 % \index[prmindex]{Temperature solver tolerance} \index[prmindexfull]{Temperature solver tolerance} %
