...
such a simulation is shown in Fig.~\ref{fig:simple-shell-3d}.

\begin{figure}[tb]
\centering
\includegraphics[width=0.8\textwidth]{cookbooks/shell_simple_2d/x-movie0000}
\caption{\it Convection in a spherical shell: Snapshot of
isosurfaces of the temperature field at time $t=0$ with a quarter of the
geometry cut away. The surface shows vectors indicating the flow velocity and
direction.}
\label{fig:simple-shell-3d}
\end{figure}

Compared to the input file discussed in the previous section, we only need to
change the following things (the full input file can be found in
\url{cookbooks/shell_simple_2d.prm}):
