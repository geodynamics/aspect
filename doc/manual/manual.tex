...
including their latent heat effects, are solved using a smaller time step. In the input file, 
we have to choose both the size of this time step and the rate (or characteristic time scale) of melting, 
...
The melting model we use here is the anyhdrous mantle melting model of \cite{KSL2003} for a peridotitic 
...
set up the boundary conditions in a way that they will lead to corner flow. At the top boundary, we can 
...
the bottom and right side of the model. However, what we can do here is prescribing the lithostatic 
...
\aspect{} also supports an alternative method for how to make sure that regions with melt are sufficiently 
...
refinement criterion that directly relates to the compaction length, which can be done in the mesh 
