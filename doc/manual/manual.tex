...
$\Gamma_{0,\mathbf u}\cup\Gamma_{\text{prescribed},\mathbf
  ...
  u}=\Gamma$ and
$\Gamma_{D,T}\cup\Gamma_{N,T}=\Gamma$. No boundary conditions have to be
posed for the compositional fields at those parts of the boundary where flow is either
...
To avoid the density gradients, we can use the same method as for the mass conservation (described in section \ref{sec:boussinesq}) and assume the change in density (for both the solid and the fluid phase) is dominated by the change in static pressure, which can be written as
...
\[
 \mathbf u_f =  \mathbf u_s - \frac{K_D}{\phi} (\nabla p_f - \rho_f g),
\]
...
\ref{eq:stokes-1-melt} and Equation \ref{eq:stokes-3-melt} and that include the elastic and compaction stress 
...
More details on the implementation can be found in \cite{DH2015}. A benchmark case demonstrating the propagation of solitary waves can be found in Section \ref{sec:benchmark-solitary_wave}, and an example for a model including melt migration is presented in Section ...
...
%Current latest deal.ii version is 8.2.1.
{\footnotesize
\begin{verbatim}
...
\footnote{The outer iterations are only part of the problem. As discussed in
  ...
  global refinement steps, the 30+188 outer iterations without averaging whosn
...
Assuming constant shear and compaction viscosities and using a permeability law in form of 
{%
\begin{align*}
...
%
the analytical solution for the shape of the solitary wave can be written in the implicit form as:
\begin{align*}
x(\phi) &= \pm (A + 0.5) 
\left[ -2 \sqrt{A-\phi} + \frac{1}{\sqrt{A-1}} 
\ln \frac{\sqrt{A-1} - \sqrt{A-\phi}}{\sqrt{A-1} + \sqrt{A-\phi}} \right]
\end{align*}
and the phase speed $c$, scaled back to physical units, is $c = u_0 (2A/\phi_0+1)$. 
...
And extensive discussion of the results and convergence behaviour can be found in \cite{DH2015}.
