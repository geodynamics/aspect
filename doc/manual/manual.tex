...
\begin{enumerate}
  ...
  leads to flows so slow that we need to measure time in years, and we will set
  the end time to 1.5 billion years -- enough to see a significant amount of
  motion.
  
  \item The next block (lines 7--10) describes the material that is convecting
  ...
  lines 28--46. There, we describe that we want a model where equation
  ...
  choose the initial mesh and how to adapt it (lines 59--64) and what to do at
  the end of each time step with the solution that \aspect{} computes for us.
...
difference is much smaller in reality than 3300 K, and we can expect convection
to be significantly less vigorous than in the simulation here.
