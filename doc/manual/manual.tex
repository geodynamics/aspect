...
\aspect{} installation is located):
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%]
subsection Model settings
...
through the Earth with realistic boundary conditions. Fig.~\ref{fig:gv-1} shows an example
...
\begin{figure}
 ...
 % \includegraphics[width=0.48\textwidth]{cookbooks/gplates/gplates-3d.png}
  \caption{\it Using GPlates for 2d boundary conditions: The figure shows
  the results of three independent computations on two-dimensional slices
  through Earth. The boundary conditions for each of these slices (white
  arrows) are tangential to the slices and are projections of the
  three-dimensional velocity vectors computed by GPlates into the two-dimensional plane
  occupied by the slice. For comparison, the figure also shows the full
  three-dimensional surface velocities provided by GPlates (red arrows).
  The view is centered on Eurasia's east coast with Japan being near the center of
  the figure (coastlines provided by NGU and the GPlates project).}
  \label{fig:gv-1}
\end{figure}

\marginpar{To be finished. Aspect outputs the necessary rotation information for
visualization. Compare the 2D models with cuts of a 3D model of the same setup.
3D model not yet stable, bug in implementation or model setup?}
...
GPlates plugin is able to read in multiple velocity files and interpolate
...
T \frac{\Delta S}{c_p} \frac{\partial X}{\partial y} 
+ \frac{\kappa}{v_y}
