...
of entropy $\Delta S$. Here, $\Delta S$ is positive for exothermic phase
...
\begin{gather*}
\frac{DX}{Dt} = \frac{\partial X}{\partial T} \frac{DT}{Dt} + \frac{\partial X}{\partial p} \mathbf u\cdot\nabla p
\end{gather*}

These terms apply both to solid-state phase transitions and to melting/solidification.
...
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%,numbers=left]
set Dimension                              = 2 % \index[prmindex]{Dimension} \index[prmindexfull]{Dimension} %
set Use years in output instead of seconds = true % \index[prmindex]{Use years in output instead of seconds} \index[prmindexfull]{Use years in output instead of seconds} %
set End time                               = 1.5e9 % \index[prmindex]{End time} \index[prmindexfull]{End time} %
set Output directory                       = output % \index[prmindex]{Output directory} \index[prmindexfull]{Output directory} %
...
subsection Material model
  ...
  subsection Simple model
    set Thermal expansion coefficient = 4e-5 % \index[prmindex]{Thermal expansion coefficient} \index[prmindexfull]{Material model!Simple model!Thermal expansion coefficient} %
...
subsection Geometry model
  set Model name = spherical shell % \index[prmindex]{Model name} \index[prmindexfull]{Geometry model!Model name} %

  subsection Spherical shell
    set Inner radius  = 3481000 % \index[prmindex]{Inner radius} \index[prmindexfull]{Geometry model!Spherical shell!Inner radius} %
    set Outer radius  = 6336000 % \index[prmindex]{Outer radius} \index[prmindexfull]{Geometry model!Spherical shell!Outer radius} %
    set Opening angle = 90 % \index[prmindex]{Opening angle} \index[prmindexfull]{Geometry model!Spherical shell!Opening angle} %
  end
end


subsection Model settings
  set Zero velocity boundary indicators       = 0 % \index[prmindex]{Zero velocity boundary indicators} \index[prmindexfull]{Model settings!Zero velocity boundary indicators} %
  set Tangential velocity boundary indicators = 1,2,3 % \index[prmindex]{Tangential velocity boundary indicators} \index[prmindexfull]{Model settings!Tangential velocity boundary indicators} %
  set Prescribed velocity boundary indicators = % \index[prmindex]{Prescribed velocity boundary indicators} \index[prmindexfull]{Model settings!Prescribed velocity boundary indicators} %

  set Fixed temperature boundary indicators   = 0,1 % \index[prmindex]{Fixed temperature boundary indicators} \index[prmindexfull]{Model settings!Fixed temperature boundary indicators} %

  set Include shear heating                   = true % \index[prmindex]{Include shear heating} \index[prmindexfull]{Model settings!Include shear heating} %
end


subsection Boundary temperature model
  set Model name = spherical constant % \index[prmindex]{Model name} \index[prmindexfull]{Boundary temperature model!Model name} %
  subsection Spherical constant
    set Inner temperature = 4273 % \index[prmindex]{Inner temperature} \index[prmindexfull]{Boundary temperature model!Spherical constant!Inner temperature} %
    set Outer temperature = 973 % \index[prmindex]{Outer temperature} \index[prmindexfull]{Boundary temperature model!Spherical constant!Outer temperature} %
  end
end


subsection Initial conditions
  set Model name = spherical hexagonal perturbation % \index[prmindex]{Model name} \index[prmindexfull]{Initial conditions!Model name} %
end


subsection Gravity model
  set Model name = radial earth-like % \index[prmindex]{Model name} \index[prmindexfull]{Gravity model!Model name} %
end
...
subsection Mesh refinement
  ...
  set Strategy                           = temperature % \index[prmindex]{Strategy} \index[prmindexfull]{Mesh refinement!Strategy} %
  set Time steps between mesh refinement = 15 % \index[prmindex]{Time steps between mesh refinement} \index[prmindexfull]{Mesh refinement!Time steps between mesh refinement} %
end
...
subsection Postprocess
  set List of postprocessors = visualization, velocity statistics, temperature statistics, ...
                               ... heat flux statistics, depth average % \index[prmindex]{List of postprocessors} \index[prmindexfull]{Postprocess!List of postprocessors} %

  subsection Visualization
    set Output format                 = vtu % \index[prmindex]{Output format} \index[prmindexfull]{Postprocess!Visualization!Output format} %
    set Time between graphical output = 1e6 % \index[prmindex]{Time between graphical output} \index[prmindexfull]{Postprocess!Visualization!Time between graphical output} %
    set Number of grouped files       = 0 % \index[prmindex]{Number of grouped files} \index[prmindexfull]{Postprocess!Visualization!Number of grouped files} %
  end

  subsection Depth average
    set Time between graphical output = 1e6 % \index[prmindex]{Time between graphical output} \index[prmindexfull]{Postprocess!Depth average!Time between graphical output} %
  end
end
\end{lstlisting}
...
\url{cookbooks/shell_simple_2d.prm}). Let us start from the top where everything
...
section:
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%]
 subsection Geometry model set Model name = spherical shell % \index[prmindex]{Model name} \index[prmindexfull]{Geometry model!Model name} %
...
subsection Model settings
  set Zero velocity boundary indicators       = 0 % \index[prmindex]{Zero velocity boundary indicators} \index[prmindexfull]{Model settings!Zero velocity boundary indicators} %
  set Tangential velocity boundary indicators = 1 % \index[prmindex]{Tangential velocity boundary indicators} \index[prmindexfull]{Model settings!Tangential velocity boundary indicators} %
  set Prescribed velocity boundary indicators = % \index[prmindex]{Prescribed velocity boundary indicators} \index[prmindexfull]{Model settings!Prescribed velocity boundary indicators} %
...
subsection Boundary temperature model
  set Model name = spherical constant % \index[prmindex]{Model name} \index[prmindexfull]{Boundary temperature model!Model name} %
  subsection Spherical constant
    set Inner temperature = 1973 % \index[prmindex]{Inner temperature} \index[prmindexfull]{Boundary temperature model!Spherical constant!Inner temperature} %
    set Outer temperature = 973 % \index[prmindex]{Outer temperature} \index[prmindexfull]{Boundary temperature model!Spherical constant!Outer temperature} %
  end
end
\end{lstlisting}
...
subsection Initial conditions
  set Model name = function % \index[prmindex]{Model name} \index[prmindexfull]{Initial conditions!Model name} %
  subsection Function
    set Function expression = 1473 % \index[prmindex]{Function expression} \index[prmindexfull]{Initial conditions!Function!Function expression} %
  end
end
...
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%]
subsection Mesh refinement
  set Initial global refinement          = 2 % \index[prmindex]{Initial global refinement} \index[prmindexfull]{Mesh refinement!Initial global refinement} %
  set Initial adaptive refinement        = 3 % \index[prmindex]{Initial adaptive refinement} \index[prmindexfull]{Mesh refinement!Initial adaptive refinement} %
  set Strategy                           = temperature % \index[prmindex]{Strategy} \index[prmindexfull]{Mesh refinement!Strategy} %
  set Time steps between mesh refinement = 15 % \index[prmindex]{Time steps between mesh refinement} \index[prmindexfull]{Mesh refinement!Time steps between mesh refinement} %
end
...
subsection Postprocess
  set List of postprocessors = visualization, velocity statistics, ...
              ... temperature statistics, heat flux statistics, depth average % \index[prmindex]{List of postprocessors} \index[prmindexfull]{Postprocess!List of postprocessors} %

  subsection Visualization
    set Output format                 = vtu % \index[prmindex]{Output format} \index[prmindexfull]{Postprocess!Visualization!Output format} %
    set Time between graphical output = 1e6 % \index[prmindex]{Time between graphical output} \index[prmindexfull]{Postprocess!Visualization!Time between graphical output} %
    set Number of grouped files       = 1 % \index[prmindex]{Number of grouped files} \index[prmindexfull]{Postprocess!Visualization!Number of grouped files} %
  end

  subsection Depth average
    set Time between graphical output = 1.5e6 % \index[prmindex]{Time between graphical output} \index[prmindexfull]{Postprocess!Depth average!Time between graphical output} %
    set Output format                 = vtu % \index[prmindex]{Output format} \index[prmindexfull]{Postprocess!Depth average!Output format} %
  end
end
\end{lstlisting}
...
Radioactive decay provides $7.4\cdot 10^{-12} W/kg$ times 3300 kg/m3 times
approx. $10^{12} m^3$, so roughly $2.4\cdot 10^{13} W$. Plus heat flux through
interior boundary. Compare to actual value for the earth of 44--47 TW
(\url{http://en.wikipedia.org/wiki/Earth's_internal_heat_budget}) with
estimate of 12--30 TW for primordial heat overall and 5--15 TW from core to
mantle.
...
The plate reconstruction that is used in this cookbook is included in \aspect{}'s data folders. For a new model setup however, the user eventually needs to create their own data files, therefore we will shortly discuss the process of acquiring a usable plate reconstruction and transferring it into a format usable by ASPECT.
Both the necessary software and data are provided by the GPlates project. GPlates is an open-source software for interactive visualization of plate tectonics. It is developed by the EarthByte Project in the School of Geosciences at the University of Sydney, the Division of Geological and Planetary Sciences (GPS) at CalTech and the Center for Geodynamics at the Norwegian Geological Survey (NGU). For extensive documentation and support we refer to the GPlates website (\url{http://www.gplates.org}). Apart from the software one needs the actual plate reconstruction that consists of closed polygons covering the complete model domain. For our case we will use the provided data by \cite{GTZDSMBSMB12} that is available from the GPlates website under "Download $\rightarrow$ Download GPlates-compatible data $\rightarrow$ Global reconstructions with continuously closing plates from 140 Ma to the present". The data is provided under a Creative Commons Attribution 3.0 Unported License (\url{http://creativecommons.org/licenses/by/3.0/}). After loading the data files into GPlates (*.gpml for plate polygons, *.rot for plate rotations over time) the user needs to convert the GPlates data to readable velocity information for \aspect{}.

\paragraph{Converting GPlates data to \aspect{} input.}

With loaded plate polygon and rotation information the conversion from GPlates data to \aspect{} readable velocity files is straightforward. First the user needs to generate (or import) so-called "velocity domain points", which are discrete sets of points at which GPlates will calculate individual velocity information. This is done using the "Features $\rightarrow$ Generate Velocity Domain Points $\rightarrow$ Latitude Longitude" Menu option. Because \aspect{} is using an adaptive mesh it is not possible for GPlates to generate velocity information at the precise surface node positions like for CitcomS or Terra (the other currently available interfaces). Instead GPlates will output the information on a general Latitude/Longitude grid with nodes on all crossing points. \aspect{} then internally interpolates this information to the current node locations during the model run. This poses the challenge to the user to decide for a sensible resolution of the GPlates output, which can be adjusted in the "Generate Latitude/Longitude Velocity Domain Points" dialog of GPlates. In general a resolution that resolves the important features is necessary, while a resolution that is higher than the maximal resolution for the \aspect{} model is unnecessary and only increases the computational cost and memory consumption of the model. After creating the Velocity Domain Points the user should see the created points and their velocities indicated as points and arrows in GPlates. To export the calculated velocities one would use the "Reconstruction $\rightarrow$ Export" menu. In this dialog the user may specify the time instant or range at which the velocities shall be exported. The only necessary option is to include the "Velocities" data type in the "Add Export" sub-dialog. The velocities need to be exported in the native GPlates *.gpml format, which is based on XML and can be read by \aspect{}. In case of a time-range the user need to add a pattern specifier to the name to create a series of files. The \%u flag is especially suited for the interaction with \aspect{}, since it can easily be replaced by a calculated file index (see also \ref{sec:time-dependent-gplates-velocities}).

\paragraph{Setting up the \aspect{} model.}

For this cookbook we will use the parameter file provided in \url{cookbooks/gplates_2d.prm}. The relevant section controlling the behaviour of the upper velocity boundary condition is listed below, for the other parameters we refer the reader to the actual file. \aspect{}'s GPlates plugin allows for the use of two- and threedimensional models incorporating the GPlates velocities. Since the output by GPlates is threedimensional in any case, \aspect{} internally handles the 2D model by rotating the model plane to the specified orientation and projecting the plate velocities into this plane. The user specifies the orientation of the model plane by prescribing two points that define a plane together with the coordinate origin (i.e. in the current formulation only great-circle slices are allowed). The coordinates need to be in spherical coordinates $\theta$ and $\phi$ with $\theta$ being the colatitude (0 at north pole) and $\phi$ being the longitude (0 at Greenwich meridian, positive eastwards) both given in radians.
...
\begin{figure}
 ...
 % \includegraphics[width=0.48\textwidth]{cookbooks/gplates/gplates-3d.png}
  \caption{\it Three rotated 2D models (slices) with prescribed surface velocities (white arrows) compared to 3D surface velocities (red arrows). The model surface velocities are projections of global velocities to their respective model plane. The view is centered on Eurasia's east coast with Japan being near the center of the figure (coastlines provided by NGU and GPlates project).}
  \label{fig:gv-1}
\end{figure}

To be finished. Aspect outputs the necessary rotation information for visualization. Compare the 2D models with cuts of a 3D model of the same setup. 3D model not yet stable, bug in implementation or model setup?


\paragraph{Time-dependent boundary conditions.}
\label{sec:time-dependent-gplates-velocities}
The above presented example uses a constant velocity boundary conditions that equals the present day plate movements. For a number of purposes one may want to use a prescribed velocity boundary condition that changes over time, e.g. to investigate the effect of trench migration on subduction. Therefore \aspect{}'s GPlates plugin is able to read in two velocity files and interpolate between them to the current model time. For this purpose the user needs to add a \textit{\%d} flag into the velocity file name, which represents the current velocity file index (e.g. \url{time_dependent.\%d.gpml}). This index is calculated by dividing the current model time by the user-defined time step between velocity files (see parameter file above). As the model time progresses the plugin will update the interpolation accordingly and if necessary read in new velocity files. In case it can not read the next velocity file, it assumes the last velocity file to be the constant boundary condition until the end of the model run. The user may test this behaviour with the provided data files \url{data/velocity_boundary_conditions/gplates/time_dependent.\%d.gpml} with d ranging from 0 to 3 and representing the plate movements of the last 3 million years according to the same plate reconstruction as used above.
...
\begin{gather*}
\frac{\partial T}{\partial y} = T \frac{\Delta S}{c_p} \frac{\partial X}{\partial y} + \frac{\kappa}{v_y} \frac{\partial^2 T}{\partial y^2}
\end{gather*}
...
The exact values of the parameters used for this benchmark can be found in Fig.~\ref{fig:latent-heat-benchmark}. They result in a predicted value of $T_2 = 1109.08 \, \text{K}$ for the temperature in the bottom half of the model.

To run this benchmark, we need to set up an input file that describes the situation. In principle, what we need to do is to describe the position and entropy change of the phase transition in addition to the previously outlined boundary and initial conditions. For this purpose, we use the ``latent heat'' material model that allows us to set the density change $\Delta\rho$ and Clapeyron slope $\gamma$ (which both determine the entropy change via $\Delta S = \gamma \frac{\Delta\rho}{\rho^2}$) as well as the depth of the phase transition as input parameters.

All of this setup is then described by the input file \url{cookbooks/latent-heat.prm}. The following section shows the central part of this file:
...
(For your first runs you will probably want to reduce the number of mesh refinement steps to make things run more quickly. Later on, you might also want to change the phase transition width to look how this influences the result.)
...
Using this input file, let us try to evaluate the results of the current
computations. It is to note that it takes some time for the model to reach a steady state and only then the bottom temperature is to reach the theoretical value. Therefore, we use the last output step to compare predicted and computed values.
...
using the filter ``Plot Over Line'' (Point1: 500000,0,0; Point2: 500000,1000000,0, then go to the ``Display'' tab and select ``T'' as only variable in the ``Line series'' section) or ``Calculator'' (as seen in Fig.~\ref{fig:latent-heat-benchmark}). In Fig.~\ref{fig:latent-heat-benchmark-results} (left) we can see that with increasing resolution, the value for the bottom temperature converges to a value of $T_2 = 1105.27 \, \text{K}$. However, this is not what the analytic solution predicted. The reason for this difference is the width of the phase transition. Our analytical approach is only valid if the phase transition is discontinuous and constrained to one specific depth $y=y_{tr}$. However, as the latent heat production is proportional to the spatial derivative of the phase function, we can not simulate such a phase transition in our numerical model: We had to choose a step function as phase function and its derivative would be infinite. In addition, for the Earth's mantle we also expect phase transitions that are distributed over a certain depth range. Because of this, we chose a hyperbolic tangent as our phase function. Moreover, Fig.~\ref{fig:latent-heat-benchmark-results} (right) illustrates what happens to the temperature at the bottom when we vary the width of the phase transition: The smaller the width, the closer the temperature gets to the predicted value of $T_2 = 1109.08 \, \text{K}$.
