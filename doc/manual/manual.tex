...
\lstinputlisting[language=prmfile]{cookbooks/composition-passive/doc/passive.part.prm.out}
...
\begin{figure}
  \centering
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0007.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0008.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0009.png}
  \\
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0010.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0012.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-passive/doc/visit0014.png}
...
\begin{figure}
  \centering
  \includegraphics[height=0.3\textwidth]{cookbooks/composition-passive/doc/visit0015.png}
  \hfill
  \includegraphics[height=0.3\textwidth]{cookbooks/composition-passive/doc/visit0017.png}
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{cookbooks/composition-passive/doc/mass-composition-1.png}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-passive/doc/postprocess.part.prm.out}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-active/doc/active.part.prm.out}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-active/doc/postprocess.part.prm.out}

\begin{figure}
  \centering
  \centering
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0007.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0009.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0008.png}
...
\begin{figure}
  \centering
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0000.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0001.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0002.png}
  \\[6pt]
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0003.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0004.png}
  \hfill
  \includegraphics[width=0.3\textwidth]{cookbooks/composition-active/doc/visit0006.png}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-passive-particles/doc/particles.part.prm.out}
...
\begin{figure}
  \centering
  \includegraphics[width=0.5\textwidth]{cookbooks/composition-passive-particles/doc/solution-00072.png}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-passive-particles/doc/particle-properties.part.prm.out}
...
\begin{figure}
  ...
  \phantom{.}
  \hfill
  \includegraphics[width=0.45\textwidth]{cookbooks/composition-passive-particles-properties/doc/composition-C1.png}
  \hfill
  \includegraphics[width=0.45\textwidth]{cookbooks/composition-passive-particles-properties/doc/particles-C1.png}
  \phantom{.}
  \\
  \phantom{.}
  \hfill
  \includegraphics[width=0.45\textwidth]{cookbooks/composition-passive-particles-properties/doc/initial-position-00000.png}
  \hfill
  \includegraphics[width=0.45\textwidth]{cookbooks/composition-passive-particles-properties/doc/initial-position-00199.png}
  \phantom{.}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-active-particles/doc/particles.part.prm.out}
...
\lstinputlisting[language=prmfile]{cookbooks/composition-active-particles/doc/composition.part.prm.out}
...
The structure and refinement of the mesh are determined in two subsections of the parameter file. First, because the model domain is not a square, it is necessary to subdivide the domain into sections that are equidimensional (or as close as possible): this is done using the \texttt{repetitions} parameters in the \texttt{Geometry} section. In this case, because the model domain has an aspect ratio of 5:1, we use 5 repetitions in the x direction, dividing the domain into 5 equidimensional elements each 1000 by 1000 km. Note that this counts as level 1 for refinement. 
\lstinputlisting[language=prmfile]{cookbooks/muparser-temperature-example/doc/repetitions.part.prm.out}
Second, the \texttt{Mesh refinement} subsection is where we define further refinement of the mesh. In this example, there are 4 parameters that are controlling the refinement. 
First, the \texttt{Initial global refinement} refines the full domain. In this example, this parameter is set to 3, so this brings the refinement level to 4 across the full domain. 
Second, we set the \texttt{Minimum refinement} parameter to 3. We choose this value to match the \texttt{Initial global refinement} so that when coarsening occurs, it will not coarsen beneath the initial global level. Note this is a choice, that you need to make, you may want to let mesh coarsen below the \texttt{Initial global refinement} depending on your application. 
Third, we set the \texttt{Initial adaptive refinement} to 3. This value brings the highest level of refinement up to 7. However, only the regions marked for refinement using the \texttt{Strategy} parameters will be refined to this level. In this example, the value of \texttt{Initial adaptive refinement} is specifically chosen to result in the same minimum level of refinement as set by the \texttt{Minimum refinement function} expression. 
Finally, we choose the \texttt{Strategy} parameters. In this example, we use the single strategy \texttt{minimum refinement function} to define a specific function to set the minimum level of refinement in two layers of the mesh. In this function expression the if-then-else statement  sets the minimum level of refinement to 7 at depths less than 150 km, the minimum level to 3 deeper than 150 km. Note that if we had put 8 here instead of 7, we would not get 8 levels of refinement because the other three refinement parameters discussed above only allow refinement to level 7. 

Similarly the choice of the value of 3 in the 'else' statement is chosen to match the value set for the \texttt{Minimum refinement}. If the the value for the 'else' part of the statement is less than that set with the \texttt{Minimum refinement}, then the actual level of refinement will be that set by \texttt{Minimum refinement}. However, if \texttt{Minimum refinement} is not set, then the value set in the else statement could be lead less refinement than set in the \texttt{Initial global refinement}. For example, if a value of 0 is used in the 'else' statement, then coarsening below 150 km could lead to elements that are larger than that set by the initial global refinement, but the amount of coarsening will depend on the size of the domain and space available to coarsen to large elements.
\lstinputlisting[language=prmfile]{cookbooks/muparser-temperature-example/doc/mesh-refinement.part.prm.out}
Figure \ref{lazy-expression-tempic-zoom} zooms in on the region where the two plates meet and shows the temperature on a wireframe highlighting the element size refinement. Notice that the mesh refinement algorithm automatically adjusts the element size between the region that is specified to be at level 7 and the region specified to be at level 3 to create a smooth transition in element size.
\begin{figure}
\centering
