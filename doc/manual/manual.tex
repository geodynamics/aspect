...
\note{In reality, \aspect{} has no preferred system of
  ...
  geometries, etc., must also be expressed in these units.

  That said, for convenience, output quantities are sometimes provided in
  units \textit{centimeters per year} instead of \textit{meters per second}
...
\begin{enumerate}
\item \textit{Obtain Trilinos:} We recommend Trilinos Version 10.4.2, which
  can be downloaded from
  \href{http://trilinos.sandia.gov/download/trilinos-10.4.html}{http://trilinos.sandia.gov}. For
  installation instructions see
  \href{http://www.dealii.org/developer/readme-petsc-trilinos.html}{the
    deal.II README}. Note that you have to configure with MPI by using
\begin{verbatim}
 TPL_ENABLE_MPI:BOOL=ON
\end{verbatim}
  in the call to cmake. After that, run {\tt{make install}}.

\item \textit{p4est:} Download and install p4est as described in the
  \href{http://www.dealii.org/developer/external-libs/p4est.html}{deal.II
    p4est installation instructions}. This is done using the {\tt{p4est-setup.sh}} you can find in
\begin{verbatim}
...
\end{verbatim}
  You may want to set the environment variable\footnote{for bash
    ...
    the file {\tt{\~{}/.bashrc}}} {\tt{DEAL\_DIR}} to the directory where you
  checked out \dealii.

\item \textit{Configure and compile \dealii:} Now it is time to configure
  \dealii. Remember to point the {\tt{./configure}} script to the paths where
  you installed p4est and Trilinos and make sure you use MPI
  compilers. Example line:
\begin{verbatim}
...
\item \textit{Test your installation:} Test that your installation works
...
\begin{verbatim}
  make
\end{verbatim}
on the command line. This builds the \aspect{} executable which will reside in
...
\item \textit{Graphical output files:} One of the postprocessors you select
  ...
  \texttt{output/solution-00000}. At the current time, \aspect{} generates
  this output in VTK format%
  \footnote{The output is in fact in the VTU version of the VTK file
    format. This is the XML-based version that compresses contents. Given that
    typical file sizes for 3d simulation are substantial, the compression
    saves a significant amount of disk space.}
  as that is widely used by a number of excellent
  visualization packages and also supports parallel visualization.%
  \footnote{The underlying \dealii{} package actually supports output in
    ...
    VTK, let the authors of \aspect{} know and we can add this feature.}
  If the program has been run with multiple MPI processes, then the list of
...
\item \texttt{A statistics file:} The \texttt{output/statistics} file contains
...
\marginpar{Write something here. overview}
