...
There are several postprocessors that can be used to turn the the velocity 
and pressure solution into quantities that can be compared to surface 
observations. In this cookbook we introduce two postprocessors: dynamic 
topography and the geoid. We initialize the model with a harmonic 
...
and negative values are in (down), analogous to the deformation of the upper 
...
the output field. Since mass is conserved within the Earth, the mean dynamic topography
...
surface and core mantle boundary. The geoid postprocessor subtracts the mean
dynamic topography at the top and bottom surface when evaluating its contribution
to the gravity field. 
In this model, downwellings coincide with lows in dynamic topography. That 
...
\begin{figure}
  ...
  (c) and (d) show the resulting surface and bottom topography. 
  }
  \label{fig:pp}
\end{figure}
...
refinement steps (other panels in these figures). Figure~\ref{fig:ic-1}a 
and b shows the density variation 
...
top and bottomw surface, which is shown for 2 refinement steps (\ref{fig:ic-2}a and c, respectively) 
...
higher than the resolution at the surface). The geoid postprocessor removes the
average top and bottom surface when evaluating its contribution
to the gravity field. The average geoid height is therefore zero. 
...
of all tests by saying \begin{lstlisting}[frame=single,language=ksh] ctest -R <regex>
\end{lstlisting}
