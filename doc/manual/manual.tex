...
\textit{This section contributed by Grant Euen, Tahiry Rajaonarison, and Shangxin Liu as part of the Geodynamics and ASPECT class at Virginia Tech}\\
...
The smooth model allows for much better convergence of secondary plumes, although they are still more scattered than the primary plumes.  The convergence of the secondary and tertiary plumes can be better constrained by altering the smoothing parameter.  This term controls how much of the problem is smoothed over: as the parameter is increased, the smoothed boundary grows and vice versa.  As the smoothed boundary shrinks it becomes sharper until the original discontinuous behavior is revealed.  As it grows the two layers eventually become one large transitioning layer rather than two distinct layers and a boundary.  The overall effect is that the secondary and tertiary plumes rise at different times based on these conditions.  In general, as the smoothing parameter is decreased, the smoothed boundary shrinks, and the plumes rise more quickly.  As it is increased, the boundary grows, and the plumes rise more slowly.  This trend can be used to force a much more accurate convergence from the secondary plumes. Testing different sizes of the smoothing parameter using refinement degree 7 (128$\times$128 mesh size) produces the images shown in Fig.~\ref{fig:vk-5}.  We use refinement degree 7 because the root mean square velocity converges at greater refinement degree while keeping average runtime very low.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{cookbooks/benchmarks/van-keken/smoothing-parameter.png}
	\caption{\it Van Keken Benchmark (using smoothed out interface): compositional field at time t=0 using smoothing parameter size: a) 0.002, b) 0.02, c) 0.05, d) 0.1, e) 0.2, f) 2}
	\label{fig:vk-5}
\end{figure}

The evolution in time of the resulting compositional fields (Fig.~\ref{fig:vk-6}) shows that the first peak converges as the smoothed interface decreases. There is a good agreement for the first peak for smoothing parameters lower than 0.1. This is because the first diaper is slower for thicker interfaces and therefore appears at later times. No second peak seems to appear for smoothing parameters 0.2 and 2.  For these two cases the system behaves like three layer compositional fields.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{cookbooks/benchmarks/van-keken/smoothing-parameter-velocity.png}
	\caption{\it Van Keken benchmark with smoothed initial conditions for the compositional field using refinement degree 7 for different smoothing parameters.}
	\label{fig:vk-6}
\end{figure}

Now let us further add a two-layer viscosity model to the domain. Let's assume the viscosity value of the upper heavier layer is $\eta_{t}$ and the viscosity value of the lower lighter layer is $\eta_{b}$. Based on the initial constant viscosity value 1$\times10^{2}$ Pa$\times$s, we set the viscosity proportion $\frac{\eta_{t}}{\eta_{b}}=0.1, 0.01$, meaning the viscosity of the upper, heavier layer is still 1$\times10^{2}$ Pa$\times$s, but the viscosity of the lower, lighter layer is now either 10 or 1 Pa$\times$s, respectively. The viscosity profiles of the discontinuous and smooth models are shown in Fig.~\ref{fig:vk-7}.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{cookbooks/benchmarks/van-keken/2viscosities-initial.png}
...
Using the original input parameter file, running the cases in 8 initial refinement degree, and also adding the two-layer viscosity model, the temperature fields at the end time, t=2000 s, for both benchmark cases, discontinuous and smooth, and both viscosity proportions, 0.1 and 0.01, are shown in Fig.~\ref{fig:vk-8}.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{cookbooks/benchmarks/van-keken/2viscosities-final.png}
	\caption{\it Van Keken benchmark two-layer viscosity model at final timestep, t=2000. Discontinuous cases are the left images, smooth cases are the right. The upper images are $\frac{\eta_{t}}{\eta_{b}}=0.1$, and the lower are $\frac{\eta_{t}}{\eta_{b}}=0.01$.}
	\label{fig:vk-8}
\end{figure}

Compared to the results of the constant viscosity throughout the domain, the plumes obviously rise up faster when adding the two-layer viscosity model. Also, the larger the viscosity difference is, the earlier the plumes appear and the faster their ascent. To further reveal the effect of the two-layer viscosity model, we also plot the evolution of the fluids' average velocity over time, as shown in Fig.~\ref{fig:vk-9}.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{cookbooks/benchmarks/van-keken/2viscosities-velocity.png}
	\caption{\it Evolution of the root mean square velocity as a function of time for different viscosity contrast proportions (0.1/0.01) for both discontinuous and smooth models.}
	\label{fig:vk-9}
\end{figure}

We can observe that when the two-layer viscosity model is added, there is only one apparent peak for each case. The first peaks of the 0.01 viscosity contrast tests appear earlier and are larger in magnitude than those of 0.1 viscosity contrast tests.  There are no secondary plumes and the whole system tends to reach stability after around 500 seconds.
