...
$[0,1.2]\times [0,1]$ where we keep the velocity at the left, bottom and right
boundaries fixed at zero, but allow free slip along the right boundary (in
other words, along the right boundary we have $\mathbf u \cdot \mathbf n=0$,
but the tangential component of $\mathbf u$ is unconstrained). Furthermore, we
assume that the temperature is one along the left, bottom and top boundaries,
but zero along the right boundary. This results in a situation where the cold
material along the right wall has negative buoyancy, drops down and creates a
convection roll in the box. The fact that the domain has different extents in
the $x$- and $y$-directions complicates the resulting flow slightly.
...
*** Timestep 1093:  t=0.499561 seconds
   Solving temperature system... 9 iterations.
   Solving Stokes system... 4 iterations.

   Postprocessing:
     RMS, max velocity:                  43.9 m/s, 70.9 m/s
     Temperature min/avg/max:            0 K, 0.5 K, 1 K
     Heat fluxes through boundary parts: 0.01891 W, -0.01895 W, -4.669 W, 4.669 W



+---------------------------------------------+------------+------------+
| Total wallclock time elapsed since start    |       464s |            |
|                                             |            |            |
| Section                         | no. calls |  wall time | % of total |
+---------------------------------+-----------+------------+------------+
| Assemble Stokes system          |      1094 |      20.2s |       4.4% |
| Assemble temperature system     |      1094 |       343s |        74% |
| Build Stokes preconditioner     |         3 |     0.344s |     0.074% |
| Build temperature preconditioner|      1094 |      7.21s |       1.6% |
| Solve Stokes system             |      1094 |      17.7s |       3.8% |
| Solve temperature system        |      1094 |      4.48s |      0.97% |
| Create snapshot                 |         2 |     0.279s |      0.06% |
| Initialization                  |         2 |    0.0477s |      0.01% |
| Postprocessing                  |      1094 |      48.7s |        11% |
| Setup dof systems               |         1 |     0.222s |     0.048% |
+---------------------------------+-----------+------------+------------+
\end{lstlisting}
...
\begin{lstlisting}[frame=single,language=ksh]
+---------------------------------------------+------------+------------+
| Total wallclock time elapsed since start    |      47.2s |            |
|                                             |            |            |
| Section                         | no. calls |  wall time | % of total |
+---------------------------------+-----------+------------+------------+
| Assemble Stokes system          |      1094 |      1.74s |       3.7% |
| Assemble temperature system     |      1094 |        27s |        57% |
| Build Stokes preconditioner     |         3 |    0.0682s |      0.14% |
| Build temperature preconditioner|      1094 |      5.05s |        11% |
| Solve Stokes system             |      1094 |      6.87s |        15% |
| Solve temperature system        |      1094 |      1.81s |       3.8% |
| Create snapshot                 |         2 |     0.346s |      0.73% |
| Initialization                  |         2 |   0.00904s |     0.019% |
| Postprocessing                  |      1094 |      2.54s |       5.4% |
| Setup dof systems               |         1 |    0.0262s |     0.056% |
+---------------------------------+-----------+------------+------------+
\end{lstlisting}
...
Fig.~\ref{fig:convection-box-stats} shows the results of visualizing this data.
It shows clearly that the simulation enters a steady state after about
$t\approx 0.1$ and then changes very little. This can also be observed using the
...
Secondly, the fluxes through the left and right boundary are not exactly zero
but small. Of course, we have prescribed boundary conditions of the form
...
internal workings. Fig.~\ref{fig:convection-box-stats} shows the number of
...
\hfill
\phantom.
\caption{\it Convection in a box: ...}
\label{fig:convection-box-stats}
\end{figure}
