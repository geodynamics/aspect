...
% and for gnuplot
\usepackage{listings}
\lstset{language=C++,basicstyle=\small\ttfamily,columns=fullflexible,keepspaces=true,frame=single,escapeinside={\%}{\%}}
...
% FILL: manual version
% e.g. 1.0
\raggedleft \huge \fontfamily{\sfdefault}\selectfont Version {1.1.pre}\\\large(generated \today)\\}
...
\marginpar{ fix the markup on this }
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%]
set CFL number                             = 0.1
...
the model. After creating the Velocity Domain Points the user should see the
...
subsection Model settings
  ...
  set Tangential velocity boundary indicators = 0 % \index[prmindex]{Tangential velocity boundary indicators} \index[prmindexfull]{Model settings!Tangential velocity boundary indicators} %
...
\subsubsection{Kinematic Subduction Model with Nonlinear Rheology}
\label{sec:cookbooks-subduction}
\textit{This section was contributed by Katrina Arredondo}

A simple 2D subduction model contains a subducting and overriding plate, where their velocities are explicitly defined. Possible rheologies include Newtonian (linear/diffusion creep) and nonNewtonian (nonlinear/dislocation creep) flow laws. This general, simple model may act as a starting point before constructing more complex subduction models or moving into 3D. This model is based off the reference model from Billen and Hirth, 2007.

\paragraph{Temperature Initial Condition for Two Plates}
A simple model set-up assumes a subducting plate with a ridge at the grid edge and an overriding plate of constant age. Plate temperatures with depth are defined by the half-space cooling model (Turcotte and Schubert, Geodynamics, 2002) and by distance from the ridge in the subducting plate. Under subsection "Initial conditions", set the Model name to "two plates" and define seven parameters: the ages of the two plates in terms of millions of years, the location of the trench in meters along the surface (also the length of the subducting plate, from the ridge to the trench), the surface temperature and mantle temperature below the plate (where the difference in temperature is equal to delta T, or change in temperature within the Rayleigh number drives convection, which does NOT include a possible adiabat temperature gradient), a reference thermal diffusivity (for the half-space cooling model), and the angle of the boundary between the two plates. 

 \paragraph{Viscosity Model}
The material model controls several processes that affect the viscosity: the rheology and weak zone. Under subsection "Material model", choose Model name "billen 2013." First choose the rheology of the model, where a linear model with diffusion creep will only be temperature dependent (on: "true"; off: "false"). Nonlinear models (diffusion and dislocation creep) will turn on temperature, stress and pressure dependence. If the model does not have an adiabat, turn on "Constant mantle temperature." The next section describes the weakzone (see figure). The following sections define the maximum and minimum caps on viscosity, reference values, and rheology parameters. The rheology values stem from Hirth and Kohlstedt [2003] and Bell et al. [2003]. 

 \paragraph{Kinematic Boundary Condition}
In the input paramter file, the "Boundary velocity model" defines the kinematic boundary conditions at the surface of the Earth. The trench location should be identical to that in the viscosity and initial condition model. While the overriding plate is held stationary, the velocity of the subducting plate is defined in terms of meters per year. The transition length defines the distance over which the surface velocity changes to zero at the edges of the subducting plate. Re is the radius of the Earth. 
...
\lstinputlisting[frame=single,language=prmfile]{cookbooks/benchmarks/solcx.prm}
...
\begin{lstlisting}[frame=single,language=prmfile,escapechar=\%]
subsection Material model
  set Model name = latent heat % \index[prmindex]{Model name} \index[prmindexfull]{Material model!Model name} %
  subsection Latent heat
    ...
    set Phase transition density jumps                 = 115.6 % \index[prmindex]{Phase transition density jumps} \index[prmindexfull]{Material model!Latent heat!Phase transition density jumps} %
    set Corresponding phase for density jump           = 0 % \index[prmindex]{Corresponding phase for density jump} \index[prmindexfull]{Material model!Latent heat!Corresponding phase for density jump} %
    ...
    # for T=T1. 
    set Phase transition depths                        = 500000 % \index[prmindex]{Phase transition depths} \index[prmindexfull]{Material model!Latent heat!Phase transition depths} %
    set Phase transition temperatures                  = 1000 % \index[prmindex]{Phase transition temperatures} \index[prmindexfull]{Material model!Latent heat!Phase transition temperatures} %
    set Phase transition Clapeyron slopes              = 1e7 % \index[prmindex]{Phase transition Clapeyron slopes} \index[prmindexfull]{Material model!Latent heat!Phase transition Clapeyron slopes} %
    ...
    set Phase transition widths                        = 5000 % \index[prmindex]{Phase transition widths} \index[prmindexfull]{Material model!Latent heat!Phase transition widths} %

    set Reference density                              = 3400 % \index[prmindex]{Reference density} \index[prmindexfull]{Material model!Latent heat!Reference density} %
    set Reference specific heat                        = 1000 % \index[prmindex]{Reference specific heat} \index[prmindexfull]{Material model!Latent heat!Reference specific heat} %
    set Reference temperature                          = 1000 % \index[prmindex]{Reference temperature} \index[prmindexfull]{Material model!Latent heat!Reference temperature} %
    set Thermal conductivity                           = 2.38 % \index[prmindex]{Thermal conductivity} \index[prmindexfull]{Material model!Latent heat!Thermal conductivity} %
    ...
    # and latent heating. 
    set Thermal expansion coefficient                  = 0.0 % \index[prmindex]{Thermal expansion coefficient} \index[prmindexfull]{Material model!Latent heat!Thermal expansion coefficient} %
    set Compressibility                                = 0.0 % \index[prmindex]{Compressibility} \index[prmindexfull]{Material model!Latent heat!Compressibility} %

    # Viscosity is constant. 
    set Thermal viscosity exponent                     = 0.0 % \index[prmindex]{Thermal viscosity exponent} \index[prmindexfull]{Material model!Latent heat!Thermal viscosity exponent} %
    set Viscosity                                      = 8.44e21 % \index[prmindex]{Viscosity} \index[prmindexfull]{Material model!Latent heat!Viscosity} %
    set Viscosity prefactors                           = 1.0, 1.0 % \index[prmindex]{Viscosity prefactors} \index[prmindexfull]{Material model!Latent heat!Viscosity prefactors} %
    set Composition viscosity prefactor                = 1.0 % \index[prmindex]{Composition viscosity prefactor} \index[prmindexfull]{Material model!Latent heat!Composition viscosity prefactor} %
    set Activation enthalpies                          = 3.9473e-3, 3.9473e-3  % \index[prmindex]{Activation enthalpies} \index[prmindexfull]{Material model!Latent heat!Activation enthalpies} %
  end
end
\end{lstlisting}
