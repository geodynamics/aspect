...
\subsubsection{The 2D Cylindrical Shell Benchmark}
...
\par The benchmark is run for a series of apporximations (Boussinesq, Extended Boussinesq,
...
will output all of these values directly except for the Nusselt number, which we must calculate outselves.
The Nusselt number of the top and bottom surfaces, ${Nu}_T$ and ${Nu}_B$, respectively, are given by
...
\begin{equation*}
{Nu}_{B} = \frac{fln(f)}{2{\pi}r_{min}(1-f)}\int \limits_{0}^{2\pi} \frac{\partial T}{\partial r}\, d\theta \\
\end{equation*}

where f is the ratio $\frac{r_{min}}{r_{max}}$\\

We can put this in terms of heat flux through the inner and outer surfaces as follows\\

\begin{equation*}
	q_r = -k\frac{\partial T}{\partial r}
\end{equation*}

where $q_r$ is heat flux in the radial direction. Let Q be the total heat that fluxes throug a surface.\\
...
For further details on the nondimensionalization and equations used for each approximation, refer to Davies et al. 

\paragraph{Case 1.1: BA\_Ra104\_Iso\_ZS}\mbox{}\\
\label{sec:case11_BA}

This case is run with the following conditions:

\begin{table*}[h]
 \centering
 \begin{tabular}{ l l }
    Boussinesq Approximation &  Boundary Condition: Zero-Slip   \\
    Rayleigh Number = $10^4$ &  Initial Conditions: D = 0, O = 4 \\ 
    $\eta(T) = 1$
 \end{tabular}
\end{table*}

where D and O refer to the degree and order of a spherical harmonic that describes the 
initial temperature. What is important here though is that the system evolve to four convective cells.
We can explicitly tell \aspect{} to do this by setting the parameter \texttt{angular mode} to 4. 
This is seen in \url{benchmark/davies_et_al/case-1.1.prm} \\

\lstinputlisting[language=prmfile]{cookbooks/benchmarks/davies_et_al/case-1.1.prm}


\paragraph{Case 2.1: BA\_Ra104\_Iso\_FS}\mbox{}
\label{sec:case21_BA}

\begin{table*}[h]
 \centering
 \begin{tabular}{ l l }
    Boussinesq Approximation &  Boundary Condition: Free-Slip   \\
    Rayleigh Number = $10^4$ &  Initial Conditions: D = 0, O = 4 \\ 
    $\eta(T) = 1$
 \end{tabular}
\end{table*}

Case 2.1 is identical to case 1.1 with the exception that the boundary conditions are free-slip.
Additinally, tangential rotation needs to be removed from the system. This is done by setting the 
\texttt{Model settings} to look as follows: \\

\begin{lstlisting}[language=prmfile]
...
\end{lstlisting}
\mbox{}\\
The script for case 2.1 located in \url{benchmark/davies_et_al/case-2.1.prm} \\

\paragraph{Case 2.2: BA\_Ra105\_Iso\_FS}\mbox{} 
\label{sec:case22_BA}
...
\begin{table*}[h]
 ...
 \begin{tabular}{ l l }
    Boussinesq Approximation &  Boundary Condition: Free-Slip   \\
    Rayleigh Number = $10^5$ &  Initial Conditions: FSS: BA\_Ra104\_Iso\_FS \\ 
    $\eta(T) = 1$ \\ 
 \end{tabular}
\end{table*}


For case 2.2, the system should have an increased Rayleigh number and begin with the final steady state of case 2.1.\\
 To start the model where case 2.1 left off, point case 2.2's output directory to a copy of
 case 2.1's results and increase the end time by 1 second.\\

\begin{lstlisting}[language=prmfile]
############### Global parameters
# Case 2.2 begins with the final steady state solution of Case 2.1
# Resume computiation must be set to true, and Output directory must
# point to the folder that contains the results of Case 2.1.

set CFL number                             = 10
set End time                               = 3
set Output directory                       = output-2
...
\mbox{}\\
Set the Rayleigh number to $10^5$ by increasing the magnitude of gravity:\\
...
\end{lstlisting}
\mbox{}\\
The full script for case 2.2 is located in \url{benchmark/davies_et_al/case-2.2.prm} \\

\paragraph{Case 2.3: BA\_Ra103\_vv\_FS} \mbox{}
\label{sec:case23_BA}
...
\begin{table*}[h]
 ...
 \begin{tabular}{ l l }
    Boussinesq Approximation &  Boundary Condition: Free-Slip   \\
    Rayleigh Number = $10^3$ &  Initial Conditions: FSS: BA\_Ra104\_Iso\_FS \\ 
    $\eta(T) = 1000^{-T}$
 \end{tabular}
\end{table*}

Viscosity is now a function of temperature. We create a plugin \url{benchmarks/davies_et_al/case2.3_plugin/VoT.cc}
from the \texttt{simpler} material model to allow viscosity to depend on temperature such that $\eta(T) = 1000^{-T}$.
The plugin is made by applying the following changes to \url{source/material_model/simpler.cc} :
...
\end{lstlisting}
\mbox{}\\
To set the Rayleigh number to $10^3$, adjust the magnitude of gravity to $10^8$.\\
The complete script for case 2.3 is located in \url{benchmark/davies_et_al/case-2.3.prm}

\paragraph{Results}\mbox{}\\


\begin{figure}[h]
\label{fig:2DcylinderFSS}
\subfigure[Case 1.1]{\includegraphics[width=3cm]{cookbooks/benchmarks/davies_et_al/case11_final}}
\hfill
\subfigure[Case 2.1]{\includegraphics[width=3cm]{cookbooks/benchmarks/davies_et_al/case21_final}}
\hfill
\subfigure[Case 2.2]{\includegraphics[width=3cm]{cookbooks/benchmarks/davies_et_al/case22_final}}
\hfill
\subfigure[Case 2.3]{\includegraphics[width=3cm]{cookbooks/benchmarks/davies_et_al/case23_final}}
\hfill
\caption{Final Steady State Temperatures for 2D Cylindrical Cases}
\end{figure}

\begin{figure}[h]
\label{fig:2DcylinderFSS}
\subfigure[Case 1.1]{\includegraphics[width=3.5cm]{cookbooks/benchmarks/davies_et_al/Case11Vrms}}
\hfill
\subfigure[Case 2.1]{\includegraphics[width=3.5cm]{cookbooks/benchmarks/davies_et_al/Case21Vrms}}
\hfill
\subfigure[Case 2.2]{\includegraphics[width=3.5cm]{cookbooks/benchmarks/davies_et_al/Case22Vrms}}
\hfill
\subfigure[Case 2.3]{\includegraphics[width=3.5cm]{cookbooks/benchmarks/davies_et_al/Case23Vrms}}
\hfill
\caption{Vrms for 2D Cylindrical Cases. Large jumps occur when transitioning from case 2.1 to cases 2.2 and 2.3 due to the rapid change of parameter settings.}
\end{figure}
