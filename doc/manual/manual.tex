...
framework.  Most of this is done internally, so you do not need to worry about the
details to run this cookbook.  Here we demonstrate the evolution of surface topography 
that results when blob of hot material rises in the mantle, pushing up the free
...
The bulk of the parameter file for this cookbook is similar to previous ones in this section.
The main addition is the \texttt{Free surface} subsection.  There are two
parameters that need to be set here.  First, you need to give \aspect{} a comma 
separated list of the free surface boundary indicators.  In this case, we are 
dealing with the top boundary of a box in 2D, corresponding to boundary indicator
3.  Second, we need to specify the value for the stabilization parameter ``theta''.
