...
&lsquo;function&rsquo;: Gravity is given in terms of an explicit formula that is elaborated in the parameters in section &lsquo;&lsquo;Gravity model|Function&rsquo;&rsquo;. The format of these functions follows the syntax understood by the muparser library, see Section~\ref{sec:muparser-format}.
...
&lsquo;radial linear&rsquo;: A gravity model which is radial (pointing inward if the gravity is positive) and the magnitude changes linearly with depth. The magnitude of gravity at the surface and bottom is read from the input file in a section &lsquo;&lsquo;Gravity model/Radial linear&rsquo;&rsquo;.
