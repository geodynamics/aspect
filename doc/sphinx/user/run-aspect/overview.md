...
in these files is given in {ref}`sec:parameters`8].

Running ASPECT with an input file[8] will
...
Sections&nbsp;{ref}`parameters:Geometry_20model`48] and
{ref}`parameters:Geometry_20model/Spherical_20shell`49] for details). The
...
-   *Screen output:* The file `output/log.txt` contains a copy of the output
    that is printed to the terminal when you run <span
    ASPECT.
...
-   *A listing of all run-time parameters:* The file `output/original.prm` is
    ...
    As `output/original.prm` is identical to the prm you started <span
...
-   *Graphical output files:* One of the postprocessors chosen in the
    ...
    At the current time, the default is that <span
    ASPECT generates this output in VTK format[10] as
    that is widely used by a number of excellent visualization packages and
    also supports parallel visualization.[11] If the program has been run with
    ...
    In particular, the [Visit][] and [ParaView][] programs, both widely used,
    ...
    and `.visit` files. To make it easy for you to view data, <span
    ...
    Visit.[12] Because loading an *entire* simulation is the most common use
...
-   *Output files generated by other postprocessors:* Similar to the
    ...
    the output directory. For example, <span
