...
match_aspect_includes = re.compile("# *include *<" + "|".join(aspect_includes) + ">")
...
# that are being #included, excluding "config.h" and "revision.h".
def get_aspect_includes(lines):
    aspect_include_list = []
    for line in lines:
        m = match_aspect_includes.match(line)
        if m:
            if (m.group(1) != "config") and (m.group(1) != "revision"):
                aspect_include_list.append(m.group(1))
    return aspect_include_list
