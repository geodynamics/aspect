...
m = re.search(r".*/include/aspect/(.*).h", header_file)
...
match_aspect_open = re.compile(r"^ *namespace aspect *$")
match_aspect_close = re.compile(r"^}( *// namespace aspect *)?$")
...
# inclusion guards.
match_header_include_guard = re.compile(r"^\# *(ifndef|define) _aspect_.*_h$")
...
aspect_include_list = header_to_partition_maps.get_aspect_includes(lines)
...
output.write("#include <aspect/config.h>\n\n")
...
for line in lines:
    ...
    # tries this.
    if header_to_partition_maps.match_aspect_includes.match(
        line
    ...
    # If this line contained the text `namespace aspect', then
    ...
    # statement.
    elif match_aspect_open.match(line):
        output.write(
            "\nexport module aspect : " + interface_module_partition_name + ";\n\n"
        )

        for external_project in used_external_projects:
            if (external_project != "dealii"):
                output.write("import dealii.external." + external_project + ";\n")
            else:
                output.write("import dealii;\n")

        for inc in aspect_include_list:
    ...
    elif (started_export == True) and match_aspect_close.match(line):
        output.write(line)  # Copy the previous line
