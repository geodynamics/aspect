  ...
  namespace InitialTemperature
  {
    ...
    class Manager : public Plugins::ManagerBase<Interface<dim>>, public SimulatorAccess<dim>
    {
      public:
        ...
        template <typename InitialTemperatureType,
                  typename = typename std::enable_if_t<std::is_base_of<Interface<dim>, InitialTemperatureType>::value>>
        DEAL_II_DEPRECATED const InitialTemperatureType &
        ...
        DeclException1(ExcInitialTemperatureNameNotFound,
                       std::string,
                       << "Could not find entry <" << arg1 << "> among the names of registered initial temperature objects.");
    ...
    template <int dim>
    template <typename InitialTemperatureType, typename>
    inline const InitialTemperatureType &
...
#define ASPECT_REGISTER_INITIAL_TEMPERATURE_MODEL(classname, name, description) \
  ...
  { \
    aspect::internal::Plugins::RegisterHelper<aspect::InitialTemperature::Interface<2>, classname<2>> dummy_##classname##_2d( \
      &aspect::InitialTemperature::Manager<2>::register_initial_temperature, name, description); \
    aspect::internal::Plugins::RegisterHelper<aspect::InitialTemperature::Interface<3>, classname<3>> dummy_##classname##_3d( \
      &aspect::InitialTemperature::Manager<3>::register_initial_temperature, name, description); \
  }
  }
}
