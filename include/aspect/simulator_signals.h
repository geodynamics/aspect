...
namespace aspect
{
  ...
  struct SimulatorSignals
  {
       ...
       * @p pressure_scaling.
       */
      boost::signals2::signal<double(const double pressure_scaling, const double reference_viscosity, const double length_scale)>
  ...
  extern template boost::signals2::signal<void(const unsigned int aspect_dim, ParameterHandler &prm)>
    SimulatorSignals<2>::declare_additional_parameters;
  extern template boost::signals2::signal<void(const Parameters<2> &, ParameterHandler &)> SimulatorSignals<2>::parse_additional_parameters;

  extern template struct SimulatorSignals<3>;
  extern template boost::signals2::signal<void(const unsigned int aspect_dim, ParameterHandler &prm)>
    SimulatorSignals<3>::declare_additional_parameters;
  extern template boost::signals2::signal<void(const Parameters<3> &, ParameterHandler &)> SimulatorSignals<3>::parse_additional_parameters;
  ...
  namespace internals
  {
    ...
    namespace SimulatorSignals
    {
      ...
      template <int dim>
      void
      call_connector_functions(aspect::SimulatorSignals<dim> &signals);
    }
  }
