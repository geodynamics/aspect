  ...
  namespace Utilities
  {
    ...
    class ScratchSpace
    {
      public:
         ...
         */
        void
        return_object_to_pool(T &t) const;
    ...
    template <typename T>
    std::vector<T>
    possibly_extend_from_1_to_N(const std::vector<T> &values, const unsigned int N, const std::string &id_text);
    ...
    namespace Coordinates
    {
      ...
      template <int dim>
      Tensor<1, dim>
      spherical_to_cartesian_vector(const Tensor<1, dim> &spherical_vector, const dealii::Point<dim> &position);
      ...
      template <int dim>
      std::array<double, 3>
      cartesian_to_ellipsoidal_coordinates(const dealii::Point<3> &position, const double semi_major_axis_a, const double eccentricity);
    ...
    real_spherical_harmonic(unsigned int l,     // degree
    ...
    template <typename T>
    inline std::vector<T>
    apply_permutation(const std::vector<T> &vector, const std::vector<std::size_t> &permutation_vector);
     ...
     */
    Tensor<2, 3>
    zxz_euler_angles_to_rotation_matrix(const double phi1, const double theta, const double phi2);
...
// inline implementations:
  ...
  namespace Utilities
  {
    ...
    possibly_extend_from_1_to_N(const std::vector<T> &values, const unsigned int N, const std::string &id_text)
    {
      if (values.size() == 1)
      ...
      else
        {
          ...
          AssertThrow(false,
                      ExcMessage("Length of " + id_text + " list must be " + "either one or " + Utilities::to_string(N) +
                                          ...
                                          std::vector<double>                    &composition_values_at_q_point)
    {
      ...
      Assert(composition_values_at_q_point.size() > 0, ExcInternalError());

      for (unsigned int k = 0; k < composition_values_at_q_point.size(); ++k)
        {
          Assert(composition_values[k].size() == composition_values_at_q_point.size(), ExcInternalError());
    ...
    compute_sorting_permutation(const std::vector<T> &vector)
    {
      ...
      std::sort(p.begin(), p.end(), [&](std::size_t i, std::size_t j) { return vector[i] < vector[j]; });
      return p;
    }
    ...
    apply_permutation(const std::vector<T> &vector, const std::vector<std::size_t> &permutation_vector)
    {
      ...
      std::transform(permutation_vector.begin(), permutation_vector.end(), sorted_vec.begin(), [&](std::size_t i) { return vector[i]; });
      return sorted_vec;
    }
    ...
    namespace Tensors
    {
      ...
      to_symmetric_tensor(const Iterator begin, const Iterator end)
      {
      ...
      template <int dim, class Iterator>
      inline void
      unroll_symmetric_tensor_into_array(const SymmetricTensor<2, dim> &tensor, const Iterator begin, const Iterator end)
      {
        AssertDimension(std::distance(begin, end), (SymmetricTensor<2, dim>::n_independent_components));
        (void)end;
