...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    {
      template <int dim>
      class StressFractionOutputs : public AdditionalMaterialOutputs<dim>
      {
        public:
          StressFractionOutputs(const unsigned int n_points,
                                const unsigned int number_of_maxwell_arms)
            :
            stress_fractions(n_points,std::vector<double>(number_of_maxwell_arms,
                                                          numbers::signaling_nan<double>()))
          {}

          ~StressFractionOutputs() override = default;

          /**
           * Stress fractions for each Maxwell arm at each quadrature point.
           * The first index corresponds to the quadrature point, the second
           * index corresponds to the Maxwell arm.
           */
          std::vector<std::vector<double>> stress_fractions;
      };
      ...
      class MaxwellWierchert : public ::aspect::SimulatorAccess<dim>
      {
        public:
           ...
           */
          void
          create_maxwell_wiechert_additional_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const;
        ...
        private:
          ...
          void
          calculate_stress_fraction_outputs(const typename Interface<dim>::MaterialModelInputs &in,
          ...
          /**
           * A fractional multiplier to each of the maxwell arm, which
           * determines how much of the total stress is carried by each arm.
           */
          std::vector<double> stress_fractions;
