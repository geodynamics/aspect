...
#ifndef _aspect_material_model_reaction_model_solid_state_cahn1956_site_saturated_n1_kinetics_eutectoid_decomposition_h
#define _aspect_material_model_reaction_model_solid_state_cahn1956_site_saturated_n1_kinetics_eutectoid_decomposition_h
    ...
    namespace ReactionModel
    {
      namespace SolidState
      {
        namespace Cahn1956
        {
          ...
          {
            /**
             * A reaction model that computes the rate of a polymorphic (solid-solid) phase transformation under eutectoid decomposition kinetics,
             * following the formulation of Kubo et al. (2000, 2002) for the ringwoodite -> bridgmanite + periclase (post-spinel) transformation.
             * The same functional form is applicable to any two-phase transformation for which short-range diffusion across alternating lamallae,
             * rather than interface attachment, is rate-limiting.
             *
             * The reaction rate for the forward transformation A -> B is:
             *    dX/dt = Z * (-dG) * |dG| * exp(-Ea / (R * T)) * X_A
             * where X_A is the mass (or volume) fraction of the reactant phase A, Z is a kinetic prefactor, Ea is the activation energy of diffusion,
             * and dG is the Gibbs free energy change of the transformation (dG < 0 favors the product phase B). Note the quadratic dependence on
             * the driving force dG, in contrast to the exponential dependence used in interface-controlled growth (see InterfaceControlledGrowth).
             *
             ...
             */
            template <int dim>
            class EutectoidDecomposition : public Interface<dim>
            {
              public:
                /**
