...
#ifndef _aspect_particle_integrator_rk4_38_h
#define _aspect_particle_integrator_rk4_38_h
...
namespace aspect
{
  namespace Particle
  {
    ...
    {
      /**
       * Runge Kutta fourth order integrator (3/8 rule). This scheme requires
       ...
       */
      template <int dim>
      class RK4_38 : public Interface<dim>, public SimulatorAccess<dim>
      {
        public:
          RK4_38();

          /**
           * Look up where the RK4_38 data is stored. Done once and cached to
           ...
           * at the new time (k+1).
           *
           * The RK4_38 integrator requires the solution vector at the
           ...
           *
           * The Runge-Kutta 4 integrator (3/8 rule) requires 4 tensors with dim components each.
           */
          static const unsigned int n_integrator_properties = 4*dim;

        private:
          /**
           * The current integration step, i.e. for RK4_38 a number between 0
           * and 3.
           */
          unsigned int integrator_substep;

          /**
           * The location of the 4 RK4_38 data fields stored in the particle properties.
           */
