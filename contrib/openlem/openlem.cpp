// OpenLEM Version 43 2023-08-28
//
// Copyright (C) 2012-2023 Stefan Hergarten
//
...
//    vector<Point>  neigh;	  
//    for ( int i = 0; i < 8; ++i )
//      neigh.push_back ( Point ( (p.i+shift[i].i+m)%m, (p.j+shift[i].j+n)%n ) );
...
//    vector<Point>  neigh(8);	  
//    for ( int i = 0; i < 8; ++i )
//      neigh[i] = Point ( (p.i+shift[i].i+m)%m, (p.j+shift[i].j+n)%n );
    return neigh;
  }
...
class Lake
{
  ...
  void addEqualHeights()
  {
    for ( int i = 0; i < boundary.size(); ++i )
    {
      vector<Point>  neigh = g->getNeighbors(boundary[i]);	    
      for ( vector<Point>::iterator p = neigh.begin(); p != neigh.end(); ++p )
      {
        if ( !getNode(p)->b && getNode(p)->m == 0 && getNode(p)->l == l )
        {
          getNode(p)->m = 1;
          points.push_back(*p);
  ...
  Point getLowestNeighbor()
  {
    ...
    for ( int i = 0; i < boundary.size(); ++i )
    {
      ...
      for ( vector<Point>::iterator p = neigh.begin(); p != neigh.end(); ++p )
      {
        if ( getNode(p)->m == 0 && getNode(p)->l < lmin )
        {
          lowest = *p;
          lmin = getNode(p)->l;
        }
  ...
  int isBoundary ( Point u )
  {
    vector<Point>  neigh = g->getNeighbors(u);	    
    for ( vector<Point>::iterator p = neigh.begin(); p != neigh.end(); ++p )
      if ( getNode(p)->m == 0 )  return 1;
    return 0; 
  }

  void shortenBoundary()
  {
    for ( int i = 0; i < boundary.size(); ++i )
      if ( !isBoundary(boundary[i]) )
      {
        boundary[i] = boundary.back();
        boundary.pop_back();
      }
  }
...
//      getNode(points.back())->d = outlet;
  ...
  int fill ( Point p )
  {
    ...
    points.clear();
    boundary.clear();
    if ( getNode(p)->b || getNode(p)->m )  return 0;
    points.push_back(p);
    boundary.push_back(p);
    getNode(p)->m = 1;
    n = 0;
    l = getNode(p)->l;
    while ( 0*points.size() <= 1000 )
    {    
      int v = points.size();
      addEqualHeights();
//      if ( points.size() != v )  printf ( "Captured %i points\n", points.size()-v ); 
      Point lowest = getLowestNeighbor();
      if ( getNode(lowest)->l <= l )
      {
	outlet = lowest;      
//        printf ( "Outflow %i %i %e\n", lowest.i, lowest.j, getNode(lowest)->w );
        break;
      }
      l = getNode(lowest)->l;
//printf ( "Added %e\n", w );
      points.push_back(lowest);
      boundary.push_back(lowest);
      getNode(lowest)->m = 1;  
      v = boundary.size();
      shortenBoundary();
    }
...
//      getNode(points[i])->dest = points[i];
    }
    return points.size();
  }
...
//    int msum = 0;
//    for ( int i = 0; i < g->m; ++i )
//      for ( int j = 0; j < g->n; ++j )  msum += getNode(Point(i,j))->m;
    int  osize = points.size();
//    for ( int i = 0; i < points.size(); ++i )
//      if ( points[i] == outlet )  printf("Outlet already included\n" );
    for ( int i = 0; i < points.size(); ++i )  getNode(points[i])->m = 1;
//    for ( int i = 0; i < points.size(); ++i )
//      if ( points[i] == Point(1380,2996) )  printf ( "Found\n" );
    points.clear();
    points.push_back(outlet);
    getNode(outlet)->m = 0;
    boundary.clear();
