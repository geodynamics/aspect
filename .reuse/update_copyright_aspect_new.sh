...
files="source/mesh_deformation/free_surface.cc"
...
for i in "${files[@]}" ; do
  ...
  if test ! "${first_year}" = "${last_year}" ; then
    reuse annotate --year="${first_year} - ${last_year}" --style="cpp"  --copyright="by the authors of the ASPECT code."  --license="GPL-2.0-or-later"  --copyright-prefix="spdx-string-c" --multi-line --template=aspect_new ${i}
  else
    reuse annotate --year="${first_year}" --style="cpp"  --copyright="by the authors of the ASPECT code."  --license="GPL-2.0-or-later"  --copyright-prefix="spdx-string-c" --multi-line --template=aspect_new ${i}
  fi
