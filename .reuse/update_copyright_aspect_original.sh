...
#mapfile -d '' files < <(git ls-files -z -- ':(glob)**/mesh_deformation/function.cc')
files="source/mesh_deformation/function.cc"
...
for i in "${files[@]}" ; do
  ...
  # print a status message and update the header
  echo "Processing $i: ${first_year} - ${last_year}"
  if test ! "${first_year}" = "${last_year}" ; then
    reuse annotate --year="${first_year} - ${last_year}" --style="cpp"  --copyright="by the authors of the ASPECT code."  --license="GPL-2.0-or-later"  --copyright-prefix="spdx-string-c" --multi-line --template=aspect_original ${i}
  else
    reuse annotate --year="${first_year}" --style="cpp"  --copyright="by the authors of the ASPECT code."  --license="GPL-2.0-or-later"  --copyright-prefix="spdx-string-c" --multi-line --template=aspect_original ${i}
  fi

  # delete old copyright statement if present
  perl -n -i.bak -e 'print unless m/\A \* Copyright/' ${i}
done
