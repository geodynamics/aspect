#ifndef ASPECT_SOLKZ_H
#define ASPECT_SOLKZ_H
  ...
  namespace InclusionBenchmark
  {
    ...
    namespace AnalyticSolutions
    {
        ...
        double total_stress[], double strain_rate[])
      {
        ...
        t16 = 0.2e1 * t3 * t4;
        t17 = t3 * t3;
        _aa = -0.4e1 * B * km * kn * (t3 + t4) / (t8 + t10 + t13 + t14 + t16 + t17) /
              (-t13 + t8 + t10 + t14 + t16 + t17);
    ...
    class SolKzPostprocessor : public Postprocess::Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
        ...
        execute(TableHandler &/*statistics*/)
        {
          ...
          error_file_stream.open("solkz_errors.csv", std::ios_base::out);
          error_file_stream << "V_L1" << "\t"
                            << "V_L2" << "\t"
                            << "P_L1" << "\t"
                            <<"P_L2" << std::endl;
          error_file_stream << u_l1 << "\t"
                            << u_l2 << "\t"
                            << p_l1 << "\t"
                            << p_l2 << std::endl;
