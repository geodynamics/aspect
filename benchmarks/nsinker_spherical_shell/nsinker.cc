  ...
  namespace NSinkerBenchmark
  {
    ...
    class NSinkerMaterial : public MaterialModel::Interface<dim>
    {
          ...
          for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
            {
              ...
              if (in.position[i].norm()<0.89583) // below "660km"?
                out.viscosities[i] *= transition_zone_jump;
          ...
          prm.enter_subsection("Material model");
          {
            prm.enter_subsection("NSinker");
            {
              ...
              prm.declare_entry ("Number of sinkers", "4",
                                 Patterns::Integer (0,31),
                                 "Number of sinking spheres.");
              prm.declare_entry ("Transition zone viscosity jump", "1.0",
                                 Patterns::Double (0),
                                 "The factor by which the viscosity increases below the base of the "
                                 "transition zone at a depth of 660km.");
            }
        ...
        parse_parameters (ParameterHandler &prm) override
        {
          prm.enter_subsection("Material model");
          {
            prm.enter_subsection("NSinker");
            {
              ...
              transition_zone_jump = prm.get_double("Transition zone viscosity jump");
            }
            prm.leave_subsection();
          }
          prm.leave_subsection();
      ...
      private:
        ...
        /**
         * Magnitude of the jump of viscosity when going below the transition zone.
         */
        double transition_zone_jump;
    ...
    NSinkerMaterial<dim>::NSinkerMaterial ()
    {
      ...
      // The original 75 randomly placed sinkers from the nsinker benchmark:
      centers.resize(75);
      ...
      // First rescale their location from [0,1] to [-1,1]:
      for (auto &p: centers)
        for (int d=0; d<3; ++d)
          p[d] = p[d]*2.0-1.0;

      // Now erase all points with center outside our shell. This
      // means some of them are not fully inside the domain, but this is ok.
      auto e = std::remove_if(centers.begin(), centers.end(), [](const Point<3> &p)
      {
        return (p.norm()<0.54) || (p.norm()>1.0);
      });
      centers.erase(e, centers.end());
    }
...
namespace aspect
{
  namespace NSinkerBenchmark
  {
    ASPECT_REGISTER_MATERIAL_MODEL(NSinkerMaterial,
                                   "nsinker",
                                   "A material model that corresponds to the 'Spherical Shell NSinker' benchmark.")
  }
}
