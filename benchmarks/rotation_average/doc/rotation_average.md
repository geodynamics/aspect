...
Any rotation in 3D can be represented by a 3x3 orthogonal matrix that preserves orientation, formally the group $SO(3)$. For a crystal with certain symmetries there are operations that leave the crystal orientation invariant. For example, a 180° rotation around $z$ is defined by the matrix $\mathbf{P}_z = \mathrm{diag}(-1,-1,1)$, and would leave a crystal with monoclinic symmetry around $z$ invariant. Following {cite}`man:2022` Chapter 6, one can define the set of all symmetry operations as,
```{math}
...
The choice of the initial guess for the combinatorial optimization matters for a small number of particles. Here we choose the rotation with the maximum geodesic distance from the initial mean value as the guess. For a unimodal or antipodal distribution, this is likely to be close to the actual mean. More information and justification for these choices will be given in a publication at a later point.
...
<img src="shearbox_comparison_t07.*" style="width:100.0%" />

Expected output of the shear box model at time t=0.7. White arrows are the applied velocity boundary condition. Dark red arrows are the interpolated rotation axis. Light red is the interpolated 100-axis. Dark green is the rotation axis on the particles. Light green is the 100-axis on the particles.
```
At $t=0.7$ one can nicely see how the symmetry average is able to interpolate rotations while taking into account that an orientation of $[\mathbf{v}_1, \mathbf{v}_2, \mathbf{v}_2]$ is equivalent to an orientation of $[-\mathbf{v}_1, -\mathbf{v}_2, \mathbf{v}_2]$. While the triclinic average shows a mean 100-axis above the $x$-$y$-plane, the orthorhombic average shows a mean 100-axis, which is intersecting the plane. The latter is more in line with the orientations on the particles.
...
We compare 3 methods to average a crystal orientation subject to orthorhombic symmetry.
1. We output the rotation as Euler angles and use a cell average.
2. Using a general rotation average (triclinic average/Markley average).
3. Using the new average for orthorhombic crystal symmetry.
At this point, the only implemented rotation average was the Euler-angle average (possibly with linear least squares). The triclinic average is a well-established method in aviation and computer graphics, and the orthorhombic average is a new experimental feature described in the section above.
