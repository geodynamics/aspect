...
This benchmark is carried out in {cite:t}`deubelbeiss:kaus:2008, gerya:2010, thieulot:2011`
 and is based on the analytical solution by {cite:t}`ramberg:1968`, which consists of a gravitationally unstable two-layer system. Free
...
<img src="setup.*" style="width:40.0%" />
 ...
 Setup of the Rayleigh-Taylor instability benchmark (taken from {cite}`thieulot:2011`)
```
...
```
We set
$L_x=L_y=\pu{512 km}$, $h_1=h_2=\pu{256 km}$,
$|\boldsymbol{g}|=\pu{10 m//s2}$, $\Delta=\pu{3 km}$,
$\rho_1=\pu{3300 kg//m3}$, $\rho_2=\pu{3000 kg//m3}$,
$\eta_1=\pu{1e21 Pa s}$. $\eta_2$ is varied between $10^{20}$ and $10^{23}$
and 3 values of $\lambda$ ($\pu{64 km}$, $\pu{128 km}$, and $\pu{256 km}$) are used. Adaptive mesh
refinement based on density is used to capture the interface between the two
fluids, as shown in {numref}`fig:RTi_grid1`. This translates as follows in the input
file:
