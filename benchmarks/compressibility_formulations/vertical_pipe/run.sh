...
for repetitions in 2 4 8 16 32 64 128; do
  ...
  for temperature in sub_adiabatic adiabatic; do
    for formulation in ala isentropic hydrostatic projected_density; do
      output_folder=output-vertical_pipe_repetitions_${repetitions}_${formulation}_${temperature}
      echo "set Output directory = ${output_folder}" > output.prm
      cat vertical_pipe.prm ${temperature}.prm ${formulation}.prm repetitions.prm output.prm | mpirun -np 4 ../plugins/aspect --
    done
  done
done
