  ...
  namespace SchmelingBenchmark
  {
    ...
    class SchmelingMeltMaterial : public MaterialModel::MeltInterface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
        ...
        {
          std::vector<double> old_melt_fractions(in.position.size());

          // we want to get the porosity field from the old solution here
          // to calculate the difference between the actual and the equilibrium melt fraction
          // we can't take the solution fot that because it gets updated in the iterated IMPES steps
          if(this->include_melt_transport() && in.cell != this->get_dof_handler().end())
            {
              ...
              // only get porosity field from the old the solution
          ...
          for (unsigned int i=0; i<in.position.size(); ++i)
            {
              ...
              // calculate the solidus for the computation of melt fraction and latent heat
              // the solidus temperature changes when melt migrates away from its origin, being expressed
              // by the difference between the porosity and depletion field
              const double delta_T_solidus = (this->include_melt_transport()
                                              ?
              ...
              // parametrization of the melting function
              ...
              // the reaction terms include melting and solidification rates of the material
              // they are calculated as the difference between the present (old) melt fraction and
              // the (new) equilibrium melt fraction
              for (unsigned int c=0;c<in.composition[i].size();++c)
                {
                  if (this->introspection().name_for_compositional_index(c) == "porosity")
                    {
                      // in case of melt migration the reaction terms are mass reaction rates instead
                      // of volume reaction terms
                      if (this->include_melt_transport() && this->get_timestep_number() > 0)
                        out.reaction_terms[i][c] = (melt_fraction - old_melt_fractions[i])
                                                 * reference_rho_s / this->get_timestep();
                      else
                        out.reaction_terms[i][c] = melt_fraction - in.composition[i][c];
                    }
                  // the reaction term of the depletion field has to be the same as for the porosity field
              ...
              // them back to the dimensions we use
    ...
    SchmelingMeltMaterial<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Schmeling melt");
        {
          ...
          prm.declare_entry ("Reference permeability", "8e4",
                             Patterns::Double(),
     ...
     */
    template <int dim>
    class SchmelingHeating : public HeatingModel::Interface<dim>, public ::aspect::SimulatorAccess<dim>
    {
      public:
        /**
                               ...
                               const Point<dim> &position) const
        {
          ...
          if(this->introspection().compositional_name_exists("porosity"))
            {
              const double porosity_idx = this->introspection().compositional_index_for_name("porosity");
              porosity = composition[porosity_idx];
            }
          ...
          if(this->introspection().compositional_name_exists("depletion"))
            {
              const double depletion_idx = this->introspection().compositional_index_for_name("depletion");
              depletion = composition[depletion_idx];
            }

          const double density = reference_rho_s
                                 + density_scaling * (- thermal_expansivity * (temperature - reference_T)
                                 + depletion * (1.0 - porosity) * (rho_depletion - reference_rho_s));
          ...
          const double T_solidus = A1 + A2 * (1.0 - position(1)) + delta_T_solidus;
          const double c_melt = ((temperature > T_solidus) && (temperature < T_solidus + Delta_T_liquidus_solidus))
                                ?
                                1.0
                                :
                                0.0;
          ...
          // parametrization of the melting function
          double melt_fraction = 0.0;
          if ((temperature > T_solidus) && (temperature < T_solidus + Delta_T_liquidus_solidus))
            melt_fraction = (temperature - T_solidus) / Delta_T_liquidus_solidus;
          else if (temperature > T_solidus + Delta_T_liquidus_solidus)
            melt_fraction = 1.0;

          const double porosity_change = (melt_fraction - depletion);
          return - porosity_change * latent_heat / density;
        }
    ...
    template <int dim>
    void
    SchmelingHeating<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Schmeling melt");
    ...
    template <int dim>
    void
    SchmelingHeating<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Schmeling melt");
...
namespace aspect
{
  namespace SchmelingBenchmark
  {
   ...
   ASPECT_REGISTER_HEATING_MODEL(SchmelingHeating,
                                 "schmeling heating",
                                 "A model for the latent heat of melt that corresponds to the melt "
                                 "transport benchmark defined in Schmeling et al., 2015.")
  }
}
