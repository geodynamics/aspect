...
namespace aspect
{
  namespace Assemblers
  {
                                                                       ...
                                                                       internal::Assembly::CopyData::VolumeOfFluidSystem<dim> &data) const
    {
      ...
      const LinearAlgebra::BlockVector &vof_solution_vector = (update_from_old) ? this->get_old_solution() : this->get_solution();
      ...
      std::fill(data.face_contributions_mask.begin(), data.face_contributions_mask.end(), false);

      scratch.finite_element_values[solution_field].get_function_values(vof_solution_vector, scratch.old_field_values);

      scratch.finite_element_values[volume_of_fluidN_n].get_function_values(vof_solution_vector, scratch.cell_i_n_values);

      scratch.finite_element_values[volume_of_fluidN_d].get_function_values(vof_solution_vector, scratch.cell_i_d_values);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          for (unsigned int k = 0; k < volume_of_fluid_dofs_per_cell; ++k)
            scratch.phi_field[k] =
              scratch.finite_element_values[solution_field].value(main_fe.component_to_system_index(solution_component, k), q);

          for (unsigned int i = 0; i < volume_of_fluid_dofs_per_cell; ++i)
            {
              data.local_rhs[i] += scratch.old_field_values[q] * JxW;
              for (unsigned int j = 0; j < volume_of_fluid_dofs_per_cell; ++j)
                data.local_matrix(i, j) += scratch.phi_field[i] * scratch.phi_field[j] * JxW;
            }
          scratch.volume += JxW;
        }
    ...
    template <int dim>
    void
    VolumeOfFluidAssembler<dim>::local_assemble_boundary_face_volume_of_fluid_system(
      ...
      internal::Assembly::CopyData::VolumeOfFluidSystem<dim> &data) const
    {
      ...
      scratch.face_finite_element_values[this->introspection().extractors.velocities].get_function_values(
        this->get_current_linearization_point(), scratch.face_current_velocity_values);

      scratch.face_finite_element_values[this->introspection().extractors.velocities].get_function_values(this->get_old_solution(),
      ...
      if (cell->has_periodic_neighbor(face_no))
      ...
      else
        {
              ...
              for (unsigned int q = 0; q < n_f_q_points; ++q)
                {
                  ...
                  if (old_velocity_available)
                    current_u += 0.5 * (scratch.face_old_velocity_values[q] - scratch.face_current_velocity_values[q]);


                  face_flux += this->get_timestep() * current_u * scratch.face_finite_element_values.normal_vector(q) *
                  ...
                  boundary_fluid_flux +=
                    ...
                    this->get_timestep() * current_u * scratch.face_finite_element_values.normal_vector(q) *
                    scratch.face_finite_element_values.JxW(q);
                }
            }
          else
          ...
          if (std::abs(face_flux) < volume_fraction_threshold * cell_volume)
            ...
            {
              flux_volume_of_fluid =
                VolumeOfFluid::Utilities::calculate_volume_flux<dim>(f_dim, face_level_set_time_grad, cell_i_normal, face_level_set_d);
            }
    ...
    template <int dim>
    void
    VolumeOfFluidAssembler<dim>::local_assemble_internal_face_volume_of_fluid_system(
      const VolumeOfFluidField<dim>                          &field,
      const bool                                              update_from_old,
      const typename DoFHandler<dim>::active_cell_iterator   &cell,
      const unsigned int                                      face_no,
      ...
      internal::Assembly::CopyData::VolumeOfFluidSystem<dim> &data) const
    {
      ...
      if ((!face->at_boundary() && !face->has_children()) || (face->at_boundary() && neighbor->is_active()))
        {
          if (neighbor->level() == cell->level() && neighbor->is_active() &&
               ...
               ((!neighbor->is_locally_owned()) && (cell->subdomain_id() < neighbor->subdomain_id()))))
            {
              ...
              scratch.neighbor_finite_element_values[solution_field].get_function_values(vof_solution_vector, scratch.neighbor_old_values);
              scratch.neighbor_finite_element_values[volume_of_fluidN_n].get_function_values(vof_solution_vector,
                                                                                             scratch.neighbor_i_n_values);
              scratch.neighbor_finite_element_values[volume_of_fluidN_d].get_function_values(vof_solution_vector,
              ...
              for (unsigned int i = 0; i < volume_of_fluid_dofs_per_cell; ++i)
                data.neighbor_dof_indices[f_rhs_ind][i] = neighbor_dof_indices[main_fe.component_to_system_index(solution_component, i)];
          ...
          for (unsigned int subface_no = 0; subface_no < face->n_children(); ++subface_no)
            {
              ...
              scratch.subface_finite_element_values[this->introspection().extractors.velocities].get_function_values(
                this->get_current_linearization_point(), scratch.face_current_velocity_values);

              scratch.subface_finite_element_values[this->introspection().extractors.velocities].get_function_values(
                this->get_old_solution(), scratch.face_old_velocity_values);
              ...
              for (unsigned int q = 0; q < n_f_q_points; ++q)
                {
                  ...
                  face_flux += this->get_timestep() * current_u * scratch.subface_finite_element_values.normal_vector(q) *
...
namespace aspect
{
  namespace Assemblers
  {
#define INSTANTIATE(dim) template class VolumeOfFluidAssembler<dim>;
