...
namespace aspect
{
  ...
  VolumeOfFluidHandler<2>::update_volume_of_fluid_normals(const VolumeOfFluidField<2> &field, LinearAlgebra::BlockVector &solution)
  {
    ...
    // Boundary reference
    typename DoFHandler<dim>::active_cell_iterator endc = this->get_dof_handler().end();
    ...
    FEValues<dim> fevalues(this->get_mapping(), system_fe, quadrature, update_JxW_values);
    ...
    const unsigned int     volume_of_fluid_ind     = this->get_fe().component_to_system_index(volume_of_fluid_c_index, 0);
    ...
    for (auto cell : this->get_dof_handler().active_cell_iterators())
      {
        ...
        if (cell_volume_of_fluid > 1.0 - volume_fraction_threshold)
        ...
        else
          {
                ...
                if (i == 0 || i == 2)
                  {
                    ...
                    if ((face->at_boundary() && !cell->has_periodic_neighbor(neighbor_no)) || face->has_children())
                      cen = endc;
                    else
                      {
                        const typename DoFHandler<dim>::cell_iterator neighbor = cell->neighbor_or_periodic_neighbor(neighbor_no);
                        ...
                        if (j == 0 || j == 2)
                          {
                            ...
                            if ((face->at_boundary() && !cen->has_periodic_neighbor(neighbor_no)) || face->has_children())
                              curr = endc;
                            else
                              {
                                const typename DoFHandler<dim>::cell_iterator neighbor = cen->neighbor_or_periodic_neighbor(neighbor_no);
                                if (neighbor->level() == cell->level() && neighbor->is_active())
                                  curr = neighbor;
                                else
                                  curr = endc;
                    ...
                    if (curr != endc)
                      {
                        ...
                        stencil_unit_cell_centers[3 * j + i] = Point<dim>(-1.0 + i, -1.0 + j);
                      }
                    else
                      {
                        ...
                        stencil_unit_cell_centers[3 * j + i] = Point<dim>(0.0, 0.0);
                      }
            ...
            for (unsigned int i = 0; i < dim; ++i)
              normals[6][i] = solution(local_dof_indices[system_fe.component_to_system_index(volume_of_fluidN_c_index + i, 0)]);
              ...
              for (unsigned int nind = 0; nind < n_candidate_normals; ++nind)
                {
                  ...
                  if (normal_norm > volume_fraction_threshold) // If candidate normal too small set error to maximum
                    {
                      d_vals[nind] = VolumeOfFluid::Utilities::compute_interface_location_newton<dim>(max_degree,
                                                                                                      normals[nind],
                    ...
                    if (normal_norm > volume_fraction_threshold) // If candidate normal too small skip as set to max already
                      {
                        ...
                        const double n_volume_of_fluid = VolumeOfFluid::Utilities::compute_fluid_volume<dim>(max_degree,
                                                                                                             normals[nind],
                                                                                                             d_vals[nind] - dot,
                                                                                                             quadrature.get_points(),
                                                                                                             weights) /
        ...
        for (unsigned int i = 0; i < dim; ++i)
          initial_solution(local_dof_indices[system_fe.component_to_system_index(volume_of_fluidN_c_index + i, 0)]) = normal[i];

        initial_solution(local_dof_indices[system_fe.component_to_system_index(volume_of_fluidN_c_index + dim, 0)]) = d;

        for (unsigned int i = 0; i < n_volume_of_fluidLS_dofs; ++i)
          {
            // Recenter unit cell on origin
            Tensor<1, dim, double> recentered_support_point =
              volume_of_fluidLS_var.fe->unit_support_point(i) - reconstruction_stencil_unit_cell_center;
            initial_solution(local_dof_indices[system_fe.component_to_system_index(volume_of_fluidLS_c_index, i)]) =
                                                              ...
                                                              LinearAlgebra::BlockVector                  &solution)
  {
    ...
    const unsigned int     volume_of_fluid_c_index = volume_of_fluid_var.first_component_index;
    const unsigned int     volume_of_fluid_ind     = system_fe.component_to_system_index(volume_of_fluid_c_index, 0);
    ...
    for (auto cell : this->get_dof_handler().active_cell_iterators())
      {
        ...
        for (unsigned int i = 0; i < dim; ++i)
          normal[i] = solution(local_dof_indices[system_fe.component_to_system_index(volume_of_fluidN_c_index + i, 0)]);
        ...
        double correction_factor =
          (normal_l1_norm < volume_fraction_threshold) ? 0.0 : 2.0 * (0.5 - std::abs(cell_volume_of_fluid - 0.5)) / normal_l1_norm;
        for (unsigned int i = 0; i < system_fe.base_element(base_element).dofs_per_cell; ++i)
          {
            const unsigned int system_local_dof = system_fe.component_to_system_index(composition_field.component_index(sim.introspection),
