...
namespace aspect
{
  ...
  template <int dim>
  void
  VolumeOfFluidHandler<dim>::initialize_from_composition_field(const VolumeOfFluidField<dim> &field)
  {
    ...
    FEValues<dim, dim>   fe_init(this->get_mapping(), this->get_fe(), quadrature, update_JxW_values | update_quadrature_points);
    ...
    const unsigned int     volume_of_fluid_ind = this->get_fe().component_to_system_index(component_index, 0);
    ...
    for (auto cell : this->get_dof_handler().active_cell_iterators())
      {
        ...
        for (unsigned int q = 0; q < fe_init.n_quadrature_points; ++q)
          {
            const double fraction_at_point =
              this->get_initial_composition_manager().initial_composition(fe_init.quadrature_point(q), field.composition_index);
  ...
  template <int dim>
  void
  VolumeOfFluidHandler<dim>::initialize_from_level_set(const VolumeOfFluidField<dim> &field)
  {
    LinearAlgebra::BlockVector initial_solution;
    ...
    for (auto cell : this->get_dof_handler().active_cell_iterators())
      {
        ...
        const double d_func    = this->get_initial_composition_manager().initial_composition(cell->barycenter(), field.composition_index);
        fe_init.reinit(cell);
                ...
                for (unsigned int di = 0; di < dim; ++di)
                  {
                    ...
                    xL[di] -= 0.5 * h;
                    const double dH =
                      this->get_initial_composition_manager().initial_composition(cell->intermediate_point(xH), field.composition_index);
                    const double dL =
                      this->get_initial_composition_manager().initial_composition(cell->intermediate_point(xL), field.composition_index);
                    grad[di] = (dL - dH);
                    d += (0.5 / dim) * (dH + dL);
                  }
