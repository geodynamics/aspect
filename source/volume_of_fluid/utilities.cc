...
namespace aspect
{
  namespace VolumeOfFluid
  {
    namespace Utilities
    {
      ...
      double
      compute_interface_location(const Tensor<1, 2> normal, const double vol)
      {
        const int dim = 2;
      ...
      compute_fluid_fraction(const Tensor<1, 3> normal, const double d)
      {
        ...
        const double v1 =
          (6.0 * nnormal[1] * nnormal[2] > eps) ? nnormal[0] * nnormal[0] / (eps) : nnormal[0] * nnormal[0] / (6 * nnormal[1] * nnormal[2]);
      ...
      double
      compute_interface_location(const Tensor<1, 3, double> normal, const double vol)
      {
        // Calculations done by Scardovelli and Zaleski in
        // doi:10.1006/jcph.2000.6567,
        // modified to fit chosen convention on normal interface
        ...
        const double v1 = (6.0 * m2 * m3 > eps) ? m1 * m1 / (eps) : m1 * m1 / (6 * m2 * m3);
        ...
        // Case 3 of Scardovelli and Zaleski
        double v3 = (m12 < m3) ? m3 * m3 * (3 * m12 - m3) / (6.0 * mprod) + m1 * m1 * (m1 - 3 * m3) / (6 * mprod) +
                     ...
                     std::vector<double>         &values)
      {
        ...
        const double sign_n_x = (((normal[0]) > 0) - ((normal[0]) < 0)), sign_n_y = (((normal[1]) > 0) - ((normal[1]) < 0));
        ...
        AssertThrow(degree <= max_degree, ExcMessage("Cannot generate xFEM polynomials. Only implemented for degrees<2."));
        ...
        if (d < -0.5 * norm1)
        ...
        else if (d < triangle_break && n_yp < n_xp)
          {
            ...
            coeffs[2] =
              0.25 * (12.0 * dealii::Utilities::fixed_power<2>(d) - 3.0 * n_xp * n_xp + n_yp * n_yp) / (n_xp * n_xp) * sign_n_x; // 2*x - 1
        ...
        else
          {
            ...
            coeffs[3]         = 1.5L * (d_nn * d_nn) * sign_n_x * sign_n_y *
                        (-(d_nn * d_nn) + 2 * d_nn * n_xp + 2 * d_nn * n_yp - 3 * n_xp * n_yp) /
                        ((n_xp * n_xp) * (n_yp * n_yp)); // (2*x - 1)*(2*y - 1)
          }
        ...
        for (unsigned int i = 0; i < points.size(); ++i)
          {
            ...
            if (degree >= 1)
              {
                values[i] += coeffs[1] * (2.0 * y - 1.0) + coeffs[2] * (2.0 * x - 1.0) + coeffs[3] * (2.0 * x - 1) * (2.0 * y - 1.0);
              }
          }
      }
                                  ...
                                  std::vector<double>         &values)
      {
        ...
        AssertThrow(degree <= max_degree, ExcMessage("Cannot generate xFEM polynomials are only functional for degrees<2."));
        ...
        if (d < -0.5 * norm1)
        ...
        else if (d <= -triangle_break)
          {
            ...
            coeffs[3]        = 3 * d_n * sign_n_x * sign_n_y * (2 * (d_n * d_n) - 3 * d_n * n_xp - 3 * d_n * n_yp + 3 * n_xp * n_yp) /
                        ((n_xp * n_xp) * (n_yp * n_yp)); // (2*x - 1)*(2*y - 1)
          }
        else if (d < triangle_break && n_xp < n_yp)
          {
        ...
        else
          {
            ...
            coeffs[3]         = 3 * d_nn * sign_n_x * sign_n_y * (2 * (d_nn * d_nn) - 3 * d_nn * n_xp - 3 * d_nn * n_yp + 3 * n_xp * n_yp) /
                        ((n_xp * n_xp) * (n_yp * n_yp)); // (2*x - 1)*(2*y - 1)
          }
        ...
        for (unsigned int i = 0; i < points.size(); ++i)
          {
            ...
            if (degree >= 1)
              {
                values[i] += coeffs[1] * (2 * y - 1.0) + coeffs[2] * (2 * x - 1.0) + coeffs[3] * (2 * x - 1) * (2 * y - 1);
              }
          }
      }
