...
namespace aspect
{
  ...
  namespace Utilities
  {
                               ...
                               const bool allow_missing_keys)
    {
      ...
      // Allow the case where only one value is given to a key
            ...
            if (check_structure)
              {
                ...
                // Mark the key which has only one value in inputs
      ...
      for (const std::string &field_name: field_names)
        {
          ...
          if (only_one_value_fields[field_index])
            {
              // Fix the return values where a key has only one value.
            ...
            {
              // Otherwise, the return values consist of all the entries of a key
