...
namespace aspect
{
  namespace BoundaryVelocity
  {
    ...
    template <int dim>
    Tensor<1, dim>
    Manager<dim>::boundary_velocity(const types::boundary_id boundary_indicator, const Point<dim> &position) const
    {
      ...
      for (const auto &plugin : this->plugin_objects)
        {
          if (boundary_indicators[i] == boundary_indicator)
            {
              found_plugin                         = true;
              const Tensor<1, dim> plugin_velocity = plugin->boundary_velocity(boundary_indicator, position);
    ...
    template <int dim>
    DEAL_II_DEPRECATED const std::map<types::boundary_id, std::pair<std::string, std::vector<std::string>>>                          &
    ...
    template <int dim>
    DEAL_II_DEPRECATED const std::map<types::boundary_id, std::vector<std::unique_ptr<BoundaryVelocity::Interface<dim>>>>                          &
    Manager<dim>::get_active_boundary_velocity_conditions() const
    {
      AssertThrow(false,
                  ExcMessage("This function has been removed. Use the function "
    ...
    Manager<dim>::get_component_mask(const types::boundary_id boundary_id) const
    {
      Assert(prescribed_velocity_boundary_indicators.find(boundary_id) != prescribed_velocity_boundary_indicators.end(),
             ExcMessage("The boundary velocity manager class was asked for the "
                        ...
                        Utilities::int_to_string(boundary_id) + "> with symbolic name <" +
        ...
        try
          {
            const std::vector<types::boundary_id> x_zero_velocity_boundary_indicators =
              this->get_geometry_model().translate_symbolic_boundary_names_to_ids(
                Utilities::split_string_list(prm.get("Zero velocity boundary indicators")));
            zero_velocity_boundary_indicators =
              std::set<types::boundary_id>(x_zero_velocity_boundary_indicators.begin(), x_zero_velocity_boundary_indicators.end());
          }
        catch (const std::string &error)
          {
            AssertThrow(false,
                        ExcMessage("While parsing the entry <Boundary velocity model/Zero velocity "
        ...
        try
          {
            const std::vector<types::boundary_id> x_tangential_velocity_boundary_indicators =
              this->get_geometry_model().translate_symbolic_boundary_names_to_ids(
                Utilities::split_string_list(prm.get("Tangential velocity boundary indicators")));
            tangential_velocity_boundary_indicators = std::set<types::boundary_id>(x_tangential_velocity_boundary_indicators.begin(),
                                                                                   x_tangential_velocity_boundary_indicators.end());
          }
        catch (const std::string &error)
          {
            AssertThrow(false,
                        ExcMessage("While parsing the entry <Boundary velocity model/Tangential velocity "
                                   "boundary indicators>, there was an error. Specifically, "
                                   "the conversion function complained as follows:\n\n" +
                                   error));
          }
        ...
        for (const auto &p : x_boundary_velocity_indicators)
          {
            ...
            while ((key_and_comp.size() > 0) &&
                   ((key_and_comp[key_and_comp.size() - 1] == 'x') || (key_and_comp[key_and_comp.size() - 1] == 'y') ||
            ...
            try
              ...
              {
                AssertThrow(false,
                            ExcMessage("While parsing the entry <Boundary velocity model/Prescribed "
            ...
            if (boundary_velocity_indicators.find(boundary_id) != boundary_velocity_indicators.end())
              {
                Assert(boundary_velocity_indicators[boundary_id].first == comp,
                       ExcMessage("Different velocity plugins for the same boundary have to have the same component selector. "
                                  "This was not the case for boundary: " +
                                  key_and_comp + ", for plugin: " + value + ", with component selector: " + comp));
            ...
            ComponentMask component_mask(this->introspection().n_components, false);
      ...
      for (const auto &plugin_name : this->plugin_names)
        {
          ...
          this->plugin_objects.push_back(std::get<dim>(registered_plugins).create_plugin(plugin_name, "Boundary velocity::Model names"));
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Boundary velocity interface", out);
    }
  }
}
