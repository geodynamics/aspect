  ...
  namespace Particle
  {
    namespace Property
    {
      ...
      ElasticStress<dim>::update_particles(typename Particle::World<dim> &particle_world) const
      {
        ...
        // As we need the strain rate as well as the pressure, temperature and composition to
        // compute the viscosity and stress, the update_flags include both gradients and values.
      ...
      ElasticStress<dim>::get_property_information() const
      {
        ...
        if (this->introspection().compositional_name_exists("ve_stress_xx_old"))
          property_information.emplace_back("ve_stress_xx_old",1);

        if (this->introspection().compositional_name_exists("ve_stress_yy_old"))
          property_information.emplace_back("ve_stress_yy_old",1);

        if (dim == 2)
          {
            if (this->introspection().compositional_name_exists("ve_stress_xy_old"))
              property_information.emplace_back("ve_stress_xy_old",1);
          }
        else if (dim == 3)
          {
            if (this->introspection().compositional_name_exists("ve_stress_zz_old"))
              property_information.emplace_back("ve_stress_zz_old",1);

            if (this->introspection().compositional_name_exists("ve_stress_xy_old"))
              property_information.emplace_back("ve_stress_xy_old",1);

            if (this->introspection().compositional_name_exists("ve_stress_xz_old"))
              property_information.emplace_back("ve_stress_xz_old",1);

            if (this->introspection().compositional_name_exists("ve_stress_yz_old"))
