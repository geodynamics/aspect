...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
      ...
      {
        AssertThrow(
          (Plugins::plugin_type_matches<const MaterialModel::ViscoPlastic<dim>>(this->get_material_model()) ||
          ...
          ExcMessage(
            "This particle property only makes sense in combination with the viscoelastic, visco_plastic, or compositing material model."));
        ...
        AssertThrow(
          (stress_field_indices.size() == 2 * SymmetricTensor<2, dim>::n_independent_components),
          ExcMessage(
            "The number of stress tensor element fields in the 'elastic stress' plugin does not equal twice the number of independent components."));
        ...
        std::set_difference(all_field_indices.begin(),
                            all_field_indices.end(),
                            stress_field_indices.begin(),
        ...
        this->get_signals().post_restore_particles.connect(
          [&](typename Particle::Manager<dim> &particle_manager) { this->update_particles(particle_manager); });
      }
      ...
      ElasticStress<dim>::update_particles(typename Particle::Manager<dim> &particle_manager) const
      {
        ...
        const unsigned int data_position =
          particle_manager.get_property_manager().get_data_info().get_position_by_field_name("ve_stress_xx");
        ...
        // solution.
        std::unique_ptr<SolutionEvaluator<dim>> evaluators = construct_solution_evaluator(*this, mapping_flags);
          ...
          if (cell->is_locally_owned())
            {
              ...
              typename ParticleHandler<dim>::particle_iterator_range particles_in_cell = particle_handler.particles_in_cell(cell);
                  ...
                  if (evaluation_flags_union & (EvaluationFlags::values | EvaluationFlags::gradients))
                    {
                      ...
                      evaluators->evaluate({old_solution_values.data(), old_solution_values.size()}, evaluation_flags);
                    }

                  // To store the old solutions
                  std::vector<small_vector<double, 50>> old_solution(n_particles_in_cell,
                                                                     small_vector<double, 50>(evaluators->n_components(),
                                                                                              numbers::signaling_nan<double>()));

                  // To store the old gradients
                  std::vector<small_vector<Tensor<1, dim>, 50>> old_gradients(
                    n_particles_in_cell,
                    small_vector<Tensor<1, dim>, 50>(evaluators->n_components(), numbers::signaling_nan<Tensor<1, dim>>()));
                  ...
                  for (unsigned int i = 0; particle != particles_in_cell.end(); ++particle, ++i)
                    {
                      ...
                      for (const unsigned int &n : non_stress_field_indices)
                        material_inputs_cell.composition[i][n] =
                          old_solution[i][this->introspection().component_indices.compositional_fields[n]];
                      ...
                      for (unsigned int n = 0; n < 2 * SymmetricTensor<2, dim>::n_independent_components; ++n)
                        {
                          ...
                          const double field_stress_value =
                            old_solution[i][this->introspection().component_indices.compositional_fields[stress_field_indices[n]]];
                  ...
                  for (unsigned int i = 0; particle != particles_in_cell.end(); ++particle, ++i)
                    {
                      ...
                      for (unsigned int p = 0; p < 2 * SymmetricTensor<2, dim>::n_independent_components; ++p)
                        particle->get_properties()[data_position + p] =
                          material_inputs_cell.composition[i][stress_field_indices[p]] +
                          reaction_rate_outputs->reaction_rates[i][stress_field_indices[p]] * this->get_timestep();
                    }
                }
            }
      }
      ...
      template <int dim>
      void
      ElasticStress<dim>::initialize_one_particle_property(const Point<dim> &position, std::vector<double> &data) const
      {
        ...
        data.push_back(
          this->get_initial_composition_manager().initial_composition(position,
                                                                      this->introspection().compositional_index_for_name("ve_stress_xx")));

        data.push_back(
          this->get_initial_composition_manager().initial_composition(position,
                                                                      this->introspection().compositional_index_for_name("ve_stress_yy")));

        if (dim == 2)
          {
            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xy")));
          }
        else if (dim == 3)
          {
            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_zz")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xy")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xz")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_yz")));
          }

        data.push_back(this->get_initial_composition_manager().initial_composition(
          position, this->introspection().compositional_index_for_name("ve_stress_xx_old")));

        data.push_back(this->get_initial_composition_manager().initial_composition(
          position, this->introspection().compositional_index_for_name("ve_stress_yy_old")));

        if (dim == 2)
          {
            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xy_old")));
          }
        else if (dim == 3)
          {
            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_zz_old")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xy_old")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_xz_old")));

            data.push_back(this->get_initial_composition_manager().initial_composition(
              position, this->introspection().compositional_index_for_name("ve_stress_yz_old")));
          }
      }
      ...
      ElasticStress<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        {
          prm.declare_entry("Particle stress value weight",
                            "1.0",
                            Patterns::Double(0.),
