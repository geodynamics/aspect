...
namespace aspect
{
  namespace Particle
  {
    namespace Property
    {
      ...
      ParticlePropertyInformation::ParticlePropertyInformation(
        const std::vector<std::vector<std::pair<std::string, unsigned int>>> &properties)
      {
      ...
      {
        const std::vector<std::string>::const_iterator field = std::find(field_names.begin(), field_names.end(), name);

        AssertThrow(field != field_names.end(),
                    ExcMessage("The particle property manager was asked for a property "
                               "field with the name <" +
                               name +
                               ">, but no such field could "
                               "be found."));
      ...
      ParticlePropertyInformation::get_field_name_by_index(const unsigned int field_index) const
      {
        Assert(field_index < field_names.size(),
               ExcMessage("The number of field names (" + std::to_string(field_names.size()) +
                          ") is smaller than the requested field index (" + std::to_string(field_index) + ")."));
      ...
      template <int dim>
      void
      Interface<dim>::initialize_one_particle_property(const Point<dim> &, std::vector<double> &) const
      {}
        ...
        for (auto particle = particles.begin(); particle != particles.end(); ++particle)
          {
            ...
            update_particle_property(this->data_position, solution, gradient, particle);
      ...
      template <int dim>
      void
      IntegratorProperties<dim>::initialize_one_particle_property(const Point<dim> & /*position*/, std::vector<double> &data) const
      {
      ...
      Manager<dim>::initialize_one_particle(typename ParticleHandler<dim>::particle_iterator &particle) const
      {
        ...
        for (const auto &p : this->plugin_objects)
          {
            p->initialize_one_particle_property(particle->get_location(), particle_properties);
          }
            ...
            switch (p->late_initialization_mode())
              {
                case aspect::Particle::Property::initialize_to_zero:
                  {
                    for (unsigned int property_component = 0;
                         property_component < property_information.get_components_by_plugin_index(property_index);
                         ++property_component)
                ...
                case aspect::Particle::Property::initialize:
                  {
                    p->initialize_one_particle_property(particle_location, particle_properties);
                    break;
                  }
                ...
                case aspect::Particle::Property::interpolate:
                  {
                    ...
                    if (cell == typename parallel::distributed::Triangulation<dim>::active_cell_iterator())
                      {
                        found_cell =
                          (GridTools::find_active_cell_around_point<>(this->get_mapping(), this->get_triangulation(), particle_location))
                            .first;
                      }
                    else
                      found_cell = cell;
                    ...
                    try
                      {
                        interpolated_properties =
                          interpolator.properties_at_points(particle_handler,
                    ...
                    // generated, catch it, print an error message, and abort the program.
                    catch (std::exception &exc)
                      {
                        ...
                        std::cerr << "Exception on MPI process <" << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
                                  << "> while generating new particle properties: " << std::endl
                                  << exc.what() << std::endl
                                  << "Aborting!" << std::endl
                                  << "----------------------------------------------------" << std::endl;
                    ...
                    for (unsigned int property_component = 0;
                      ...
                      particle_properties.push_back(
                        interpolated_properties[0][property_information.get_position_by_plugin_index(property_index) + property_component]);
                    break;
                  }
                ...
                case aspect::Particle::Property::interpolate_respect_boundary:
                  {
                    ...
                    // Otherwise use the value of the boundary condition of the corresponding compositional field
                    else
                      {
                        ...
                        for (unsigned int particle_property_component = 0; particle_property_component < n_property_components;
                             ++particle_property_component)
                          {
                            ...
                            Assert(field_to_use.compositional_variable < this->n_compositional_fields(),
                                   ExcMessage("The composition index specified in the function"
                                              ...
                                              "<" +
                                              property_information.get_field_name_by_index(property_index) +
                                              "> is "
                                              "larger than the number of compositional fields. This is not possible "
                                              "and likely a bug in the particle property plugin."));

                            const double field_boundary_value =
                              manager.boundary_composition(boundary_id, particle_location, field_to_use.compositional_variable);
      ...
      Manager<dim>::get_update_flags() const
      {
        ...
        for (unsigned int i = 0; i < n_components; ++i)
          {
            Assert((update[i] & ~(update_gradients | update_values)) == false, ExcNotImplemented());
          }
      ...
      {
        AssertThrow(first != second, ExcMessage("The first and second string are the same, so can not check the order."));
        AssertThrow(plugin_name_exists(first), ExcMessage("Could not find a plugin with the name <" + first + ">."));
        AssertThrow(plugin_name_exists(second), ExcMessage("Could not find a plugin with the name <" + second + ">."));
      ...
      {
        const std::vector<std::string>::const_iterator plugin = std::find(this->plugin_names.begin(), this->plugin_names.end(), name);

        AssertThrow(plugin != this->plugin_names.end(),
                    ExcMessage("The particle property manager was asked for a plugin "
                               "with the name <" +
                               name +
                               ">, but no such plugin could "
                               "be found."));
      ...
      namespace
      {
        std::tuple<aspect::internal::Plugins::UnusablePluginList,
                   ...
                   aspect::internal::Plugins::PluginList<Property::Interface<3>>>
          registered_plugins;
      }
      ...
      {
        Assert(std::get<dim>(registered_plugins).plugins != nullptr, ExcMessage("No postprocessors registered!?"));
        ...
        if (std::find(this->plugin_names.begin(), this->plugin_names.end(), "all") != this->plugin_names.end())
          {
        ...
        for (auto &plugin_name : this->plugin_names)
          {
            this->plugin_objects.emplace_back(std::get<dim>(registered_plugins).create_plugin(plugin_name, "Particle property plugins"));
      ...
      template <int dim>
      void
      Manager<dim>::register_particle_property(const std::string &name,
      ...
      {
        std::get<dim>(registered_plugins).write_plugin_graph("Particle property interface", out);
      }
