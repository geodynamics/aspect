    ...
    namespace Property
    {
      namespace Utilities
      {
                                  ...
                                  const SymmetricTensor<2, 3> &voigt_stiffness_tensor)
        {
          ...
          const std::array<std::pair<double, Tensor<1, 3, double>>, 3> voigt_eigenvectors_a =
            eigenvectors(voigt_stiffness_tensor, SymmetricTensorEigenvectorMethod::jacobi);
          const std::array<std::pair<double, Tensor<1, 3, double>>, 3> dilatation_eigenvectors_a =
            eigenvectors(dilatation_stiffness_tensor, SymmetricTensorEigenvectorMethod::jacobi);
          ...
          for (unsigned int i1 = 0; i1 < 3; i1++)
            {
              ...
              unpermutated_SCCS[i1] =
                0.5 * (dilatation_eigenvectors_a[i1].second +
                       static_cast<double>(vector_index_signed) * voigt_eigenvectors_a[std::abs(vector_index_signed)].second);
              unpermutated_SCCS[i1] /= unpermutated_SCCS[i1].norm();
            }

          return Tensor<2, 3>({{unpermutated_SCCS[0][0], unpermutated_SCCS[0][1], unpermutated_SCCS[0][2]},
          ...
          for (unsigned int permutation_i = 0; permutation_i < 3; permutation_i++)
            {
              ...
              rotated_elastic_matrix[permutation_i] =
                aspect::Utilities::Tensors::rotate_voigt_stiffness_matrix((permutated[permutation_i]), elastic_matrix);

              const Tensor<1, 21> full_elastic_vector_rotated =
                aspect::Utilities::Tensors::to_voigt_stiffness_vector(rotated_elastic_matrix[permutation_i]);
              ...
              // The monoclinic vector is the monoclinic and higher symmetry axes vector minux the orthoclinic and higher symmetry axes
              // vector.
      ...
      ElasticTensorDecomposition<dim>::initialize()
      {
        ...
        AssertThrow(manager.plugin_name_exists("crystal preferred orientation"), ExcMessage("No cpo property plugin found."));
        AssertThrow(manager.plugin_name_exists("cpo elastic tensor"), ExcMessage("No cpo elastic tensor property plugin found."));
        ...
        AssertThrow(
          manager.check_plugin_order("cpo elastic tensor", "elastic tensor decomposition"),
          ExcMessage(
            "To use the elastic tensor decomposition plugin, the cpo elastic tensor plugin needs to be defined before this plugin."));

        cpo_elastic_tensor_data_position =
          manager.get_data_info().get_position_by_plugin_index(manager.get_plugin_index_by_name("cpo elastic tensor"));
      }
      ...
      template <int dim>
      void
      ElasticTensorDecomposition<dim>::initialize_one_particle_property(const Point<dim> &, std::vector<double> &data) const
      {
        const SymmetricTensor<2, 6> elastic_matrix =
          Particle::Property::CpoElasticTensor<dim>::get_elastic_tensor(cpo_elastic_tensor_data_position, data);

        const SymmetricTensor<2, 3> dilatation_stiffness_tensor_full =
          Property::Utilities::compute_dilatation_stiffness_tensor(elastic_matrix);
        const SymmetricTensor<2, 3> voigt_stiffness_tensor_full = Property::Utilities::compute_voigt_stiffness_tensor(elastic_matrix);
        const Tensor<2, 3>          SCCS_full =
          Property::Utilities::compute_unpermutated_SCCS(dilatation_stiffness_tensor_full, voigt_stiffness_tensor_full);

        const std::array<std::array<double, 3>, 7> norms =
          Property::Utilities::compute_elastic_tensor_SCCS_decompositions(SCCS_full, elastic_matrix);
        ...
        for (auto &particle : particles)
          {
            ...
            const Tensor<2, 3>          SCCS_full =
              Utilities::compute_unpermutated_SCCS(dilatation_stiffness_tensor_full, voigt_stiffness_tensor_full);

            const std::array<std::array<double, 3>, 7> norms =
              Utilities::compute_elastic_tensor_SCCS_decompositions(SCCS_full, elastic_matrix);
      ...
      {
        std::vector<std::pair<std::string, unsigned int>> property_information = {{"cpo elastic axis e1", 3},
