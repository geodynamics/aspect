...
namespace aspect
{
  namespace Particle
  {
    namespace Interpolator
    {
      ...
      LinearLeastSquares<dim>::properties_at_points(
                    ...
                    if (cell->at_boundary(face_id))
                      {
                        ...
                        if (opposing_cell.state() == IteratorState::IteratorStates::valid && opposing_cell->is_active() &&
                            opposing_cell->is_artificial() == false)
                          {
      ...
      LinearLeastSquares<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Interpolator");
        {
          ...
          {
            prm.declare_entry("Use linear least squares limiter",
                              "true",
      ...
      LinearLeastSquares<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        prm.enter_subsection("Interpolator");
        {
          prm.enter_subsection("Linear least squares");
          {
            ...
            std::vector<std::string> linear_least_squares_limiter_split =
              Utilities::split_string_list(prm.get("Use linear least squares limiter"));
            std::vector<bool> linear_least_squares_limiter_parsed;
            if (linear_least_squares_limiter_split.size() == 1)
              {
                linear_least_squares_limiter_parsed = std::vector<bool>(n_property_components - n_internal_components,
                                                                        Utilities::string_to_bool(linear_least_squares_limiter_split[0]));
              }
            ...
            else
              {
                AssertThrow(false,
                            ExcMessage(
                              "The size of 'Use linear least squares limiter' should either be 1 or the number of particle properties"));
              }
