  ...
  namespace Particle
  {
    namespace Generator
    {
      ...
      UniformBox<dim>::generate_particles(Particles::ParticleHandler<dim> &particle_handler)
      {
        ...
        for (unsigned int i = 0; i < dim; ++i)
          {
            n_particles_per_direction[i] =
              static_cast<unsigned int>(std::round(std::pow(n_particles * Utilities::pow(P_diff[i], dim) / volume, 1. / dim)));
                  ...
                  for (unsigned int k = 0; k < n_particles_per_direction[2]; ++k)
                    {
                      const Point<dim> particle_position =
                        Point<dim>(P_min[0] + i * spacing[0], P_min[1] + j * spacing[1], P_min[2] + k * spacing[2]);
                      this->insert_particle_at_position(particle_position, particle_index, particle_handler);
                      ++particle_index;
                    }
              }
      ...
      UniformBox<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Generator");
        {
          prm.enter_subsection("Uniform box");
          {
            ...
            prm.declare_entry("Minimum x", "0.", Patterns::Double(), "Minimum x coordinate for the region of particles.");
            prm.declare_entry("Maximum x", "1.", Patterns::Double(), "Maximum x coordinate for the region of particles.");
            prm.declare_entry("Minimum y", "0.", Patterns::Double(), "Minimum y coordinate for the region of particles.");
            prm.declare_entry("Maximum y", "1.", Patterns::Double(), "Maximum y coordinate for the region of particles.");
            prm.declare_entry("Minimum z", "0.", Patterns::Double(), "Minimum z coordinate for the region of particles.");
            prm.declare_entry("Maximum z", "1.", Patterns::Double(), "Maximum z coordinate for the region of particles.");
          }
