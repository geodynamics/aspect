...
namespace aspect
{
  namespace Particle
  {
    namespace Integrator
    {
             ...
             it != end_particle; ++it, ++velocity, ++old_velocity)
          {
            ...
            if (integrator_substep == 0)
            ...
            else if (integrator_substep == 3)
              {
                ...
                Point<dim> new_location = old_location + (k[0] + 3.0*k[1] + 3.0*k[2] + k[3])/8.0;
              ...
              {
                Assert(false,
                       ExcMessage("The RK4_38 integrator should never continue after four integration stages."));
              }
          }
      }
      ...
      template <int dim>
      bool
      RK4_38<dim>::new_integration_step()
      {
      ...
      template <int dim>
      std::array<bool, 3>
      RK4_38<dim>::required_solution_vectors() const
      {
        switch (integrator_substep)
          {
            ...
            default:
              Assert(false,
                     ExcMessage("The RK4_38 integrator should never continue after four integration steps."));
          }
...
namespace aspect
{
  namespace Particle
  {
    namespace Integrator
    {
      ASPECT_REGISTER_PARTICLE_INTEGRATOR(RK4_38,
                                          "rk4_38",
                                          "Runge Kutta fourth order integrator 3/8 rule, where "
                                          "$y_{n+1} = y_n + \\frac{1}{8} k_1 + \\frac{3}{8} k_2 "
                                          "+ \\frac{3}{8} k_3 + \\frac{1}{8} k_4$ "
