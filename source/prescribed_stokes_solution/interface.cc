...
namespace aspect
{
  namespace PrescribedStokesSolution
  {
    ...
    create_prescribed_stokes_solution(ParameterHandler &prm)
    {
      ...
      std::unique_ptr<Interface<dim>> plugin =
        std::get<dim>(registered_plugins).create_plugin(model_name, "Prescribed Stokes solution::Model name");
      return plugin;
    }
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Prescribed Stokes solution interface", out);
    }
  }
}
...
\
  template void register_prescribed_stokes_solution_model<dim>(const std::string &, \
                                                               const std::string &, \
...
\
  template std::unique_ptr<Interface<dim>> create_prescribed_stokes_solution<dim>(ParameterHandler & prm);
