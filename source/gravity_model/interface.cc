...
namespace aspect
{
  namespace GravityModel
  {
    ...
    create_gravity_model(ParameterHandler &prm)
    {
      ...
      return std::get<dim>(registered_plugins).create_plugin(model_name, "Gravity model::Model name");
    }
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Gravity model interface", out);
    }
  }
}
...
\
  template void register_gravity_model<dim>(const std::string &, \
                                            const std::string &, \
...
\
  template std::unique_ptr<Interface<dim>> create_gravity_model<dim>(ParameterHandler & prm);
