      ...
      if ((this->get_timestep_number() == 0) && (this->get_pre_refinement_step() == std::numeric_limits<unsigned int>::max()))
        {
          ...
          const double temperature_contrast =
          ...
          const double Ra = (thermal_diffusivity != 0) ?
                              out.densities[0] * gravity * out.thermal_expansion_coefficients[0] * temperature_contrast *
                                Utilities::fixed_power<3>(model_depth) / (thermal_diffusivity * out.viscosities[0]) :
                              std::numeric_limits<double>::infinity();

          this->get_pcout() << std::endl;
          this->get_pcout() << "     Model domain depth (m):                        " << model_depth << std::endl;
          this->get_pcout() << "     Temperature contrast across model domain (K):  " << temperature_contrast << std::endl;
          this->get_pcout() << "     Reference depth (m):                           " << reference_depth << std::endl;
          this->get_pcout() << "     Reference temperature (K):                     " << temperature << std::endl;
          this->get_pcout() << "     Reference pressure (Pa):                       " << pressure << std::endl;
          this->get_pcout() << "     Reference gravity (m/s^2):                     " << gravity << std::endl;
          this->get_pcout() << "     Reference density (kg/m^3):                    " << out.densities[0] << std::endl;
          this->get_pcout() << "     Reference thermal expansion coefficient (1/K): " << out.thermal_expansion_coefficients[0] << std::endl;
          this->get_pcout() << "     Reference specific heat capacity (J/(K*kg)):   " << out.specific_heat[0] << std::endl;
          this->get_pcout() << "     Reference thermal conductivity (W/(m*K)):      " << out.thermal_conductivities[0] << std::endl;
          this->get_pcout() << "     Reference viscosity (Pa*s):                    " << out.viscosities[0] << std::endl;
          this->get_pcout() << "     Reference thermal diffusivity (m^2/s):         " << thermal_diffusivity << std::endl;
          this->get_pcout() << "     Rayleigh number:                               " << Ra << std::endl;
