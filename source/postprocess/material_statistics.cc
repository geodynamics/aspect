...
namespace aspect
{
  namespace Postprocess
  {
    ...
    MaterialStatistics<dim>::execute(TableHandler &statistics)
    {
      ...
      output << average_density << " kg/m^3, " << average_viscosity << " Pa s, " << global_mass << " kg";

      return std::pair<std::string, std::string>("Average density / Average viscosity / Total mass: ", output.str());
    }
  }
}
