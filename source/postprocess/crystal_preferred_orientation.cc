...
namespace aspect
{
  namespace Postprocess
  {
    ...
    CrystalPreferredOrientation<dim>::execute (TableHandler &statistics)
    {
      ...
      const Particle::Property::Manager<dim> &manager = this->get_particle_manager(0).get_property_manager();
      const Particle::Property::ParticleHandler<dim> &particle_handler = this->get_particle_manager(0).get_particle_handler();
      ...
      // get particle data
      const Particle::Property::ParticlePropertyInformation &property_information = this->get_particle_manager(0).get_property_manager().get_data_info();
