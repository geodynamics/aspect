...
namespace aspect
{
  namespace Postprocess
  {
    ...
    void
    CrystalPreferredOrientation<dim>::writer(const std::string &filename,
    ...
    CrystalPreferredOrientation<dim>::execute(TableHandler &statistics)
    {
      ...
      for (unsigned int i = 0; i < this->n_particle_managers(); ++i)
        {
          if (this->get_particle_manager(i)
                .get_property_manager()
                .template has_matching_active_plugin<Particle::Property::CrystalPreferredOrientation<dim>>())
            {
      ...
      const Particle::Property::ParticleHandler<dim> &particle_handler =
        this->get_particle_manager(particle_manager_index).get_particle_handler();
      ...
      std::string particle_file_prefix_main =
        this->get_output_directory() + "particles_cpo/particles-" + Utilities::int_to_string(output_file_number, 5);
      std::string particle_file_prefix_content_raw =
        this->get_output_directory() + "particles_cpo/CPO-" + Utilities::int_to_string(output_file_number, 5);
      std::string particle_file_prefix_content_draw_volume_weighting =
        this->get_output_directory() + "particles_cpo/weighted_CPO-" + Utilities::int_to_string(output_file_number, 5);
      ...
      string_stream_main << "id x y" << (dim == 3 ? " z" : "") << " olivine_deformation_type"
                         << (cpo_elastic_decomposition_plugin_exists ?
                               (std::string(" full_norm_square ") +
                                ...
                                "hexagonal_norm_square_p1 hexagonal_norm_square_p2 hexagonal_norm_square_p3 " + "isotropic_norm_square") :
                               "")
                         << std::endl;

      const unsigned int cpo_data_position =
        property_information.n_fields() == 0 ? 0 : property_information.get_position_by_field_name("cpo mineral 0 type");
          ...
          for (unsigned int property_i = 0; property_i < write_draw_volume_weighted_cpo.size(); ++property_i)
            {
              switch (write_draw_volume_weighted_cpo[property_i].second)
                {
                  case Output::VolumeFraction:
                    string_stream_content_draw_volume_weighting << "mineral_" << write_draw_volume_weighted_cpo[property_i].first
                                                                << "_volume_fraction"
                                                                << " ";
                    break;
                  ...
                  case Output::EulerAngles:
                    string_stream_content_draw_volume_weighting
                      << "mineral_" << write_draw_volume_weighted_cpo[property_i].first << "_EA_phi"
                      << " "
              ...
              for (unsigned int i_grain = 0; i_grain < n_grains; ++i_grain)
                {
                  rotation_matrices[mineral][i_grain] =
                    cpo_particle_property.get_rotation_matrix_grains(cpo_data_position, properties, mineral, i_grain);
                }
            }

          const unsigned int lpo_hex_data_position =
            property_information.n_fields() == 0 || cpo_elastic_decomposition_plugin_exists == false ?
              0 :
              ...
              for (unsigned int mineral = 0; mineral < n_minerals; ++mineral)
                {
                  ...
                  for (unsigned int i_grain = 0; i_grain < n_grains; ++i_grain)
                    {
                      euler_angles[mineral][i_grain] =
                        Utilities::zxz_euler_angles_from_rotation_matrix(rotation_matrices[mineral][i_grain]);
                    }
                }
            }
                  ...
                  for (unsigned int property_i = 0; property_i < write_raw_cpo.size(); ++property_i)
                    {
                      switch (write_raw_cpo[property_i].second)
                        {
                          case Output::VolumeFraction:
                            string_stream_content_raw << cpo_particle_property.get_volume_fractions_grains(cpo_data_position,
                                                                                                           properties,
                  ...
                  for (unsigned int i_grain = 0; i_grain < n_grains; ++i_grain)
                    {
                      ...
                      volume_fractions_grains[mineral][i_grain] =
                        cpo_particle_property.get_volume_fractions_grains(cpo_data_position, properties, mineral, i_grain);
                    }
                  weighted_rotation_matrices[mineral] = Utilities::rotation_matrices_random_draw_volume_weighting(
                    volume_fractions_grains[mineral], rotation_matrices[mineral], n_grains, this->random_number_generator);

                  Assert(weighted_rotation_matrices[mineral].size() == euler_angles[mineral].size(),
                         ExcMessage(
                           "Weighted rotation matrices vector (size = " + std::to_string(weighted_rotation_matrices[mineral].size()) +
                           ") has different size from input angles (size = " + std::to_string(euler_angles[mineral].size()) + ")."));

                  for (unsigned int i_grain = 0; i_grain < n_grains; ++i_grain)
                    {
                      weighted_euler_angles[mineral][i_grain] =
                        Utilities::zxz_euler_angles_from_rotation_matrix(weighted_rotation_matrices[mineral][i_grain]);
                    }
                }
              string_stream_content_draw_volume_weighting << std::endl;
                      ...
                      switch (write_draw_volume_weighted_cpo[property_i].second)
                        {
                          case Output::VolumeFraction:
                            string_stream_content_draw_volume_weighting
                              << volume_fractions_grains[write_draw_volume_weighted_cpo[property_i].first][grain] << " ";
                            break;

                          case Output::RotationMatrix:
                            string_stream_content_draw_volume_weighting
                              << weighted_rotation_matrices[write_draw_volume_weighted_cpo[property_i].first][grain] << " ";
                            break;
                          ...
                          case Output::EulerAngles:
                            Assert(compute_raw_euler_angles == true,
                                   ExcMessage("Internal error: writing out raw Euler angles, without them being computed."));
                            string_stream_content_draw_volume_weighting
                              << weighted_euler_angles[write_draw_volume_weighted_cpo[property_i].first][grain][0] << " "
      ...
      std::string filename_main = particle_file_prefix_main + "." +
                                  Utilities::int_to_string(dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()), 4) +
                                  ".dat";
      std::string filename_raw = particle_file_prefix_content_raw + "." +
                                 Utilities::int_to_string(dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()), 4) +
                                 ".dat";
      std::string filename_draw_volume_weighting =
        particle_file_prefix_content_draw_volume_weighting + "." +
        Utilities::int_to_string(dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()), 4) + ".dat";
      ...
      std::unique_ptr<std::string> file_contents_draw_volume_weighting =
        std::make_unique<std::string>(string_stream_content_draw_volume_weighting.str());
      ...
      if (write_in_background_thread)
        {
          ...
          background_thread_main = std::thread([my_filename                  = std::move(filename_main),
          ...
          if (write_raw_cpo.size() != 0)
            {
              ...
              background_thread_content_raw = std::thread([my_filename                  = std::move(filename_raw),
                                                           my_temporary_output_location = temporary_output_location,
      ...
      else
        {
          ...
          if (write_draw_volume_weighted_cpo.size() != 0)
            writer(filename_draw_volume_weighting,
    ...
    {
      // olivine volume fraction, olivine rotation matrix, olivine Euler angles, enstatite volume fraction, enstatite rotation matrix,
      // enstatite Euler angles
    ...
    {
      ar &last_output_time &output_file_number &times_and_pvtu_file_names &output_file_names_by_timestep &xdmf_entries;
    }
    ...
    CrystalPreferredOrientation<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Time between data output",
                            "1e8",
                            Patterns::Double(0),
          ...
          prm.declare_entry("Write out draw volume weighted cpo data",
                            ...
                            "and if entries are entered multiple times, they will be written "
                            "out multiple times.");
          prm.declare_entry("Compress cpo data files",
                            "true",
                            Patterns::Bool(),
    ...
    CrystalPreferredOrientation<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Crystal Preferred Orientation");
        {
          ...
          aspect::Utilities::create_directory(this->get_output_directory() + "particles_cpo/", this->get_mpi_communicator(), true);
          ...
          for (unsigned int i = 0; i < write_raw_cpo_list.size(); ++i)
            {
              ...
              AssertThrow(
                split_raw_cpo_instructions.size() == 2,
                ExcMessage(
                  "Value \"" + write_raw_cpo_list[i] + "\", set in \"Write out raw cpo data\", is not a correct option " +
              ...
              AssertThrow(mineral_instructions[0] == "mineral",
                          ExcMessage("Value \"" + write_raw_cpo_list[i] +
                                     "\", set in \"Write out raw cpo data\", is not a correct option " +
              ...
              Assert(mineral_number >= 0,
                     ExcMessage("Internal error: mineral_number is negative: " + std::to_string(mineral_number) + "."));

              AssertThrow((unsigned int)mineral_number < n_minerals,
                          ExcMessage("Value \"" + write_raw_cpo_list[i] +
                                     "\", set in \"Write out raw cpo data\", is not a correct option " + "because the mineral number (" +
                                     std::to_string(mineral_number) + ") is larger than the number of minerals " +
          ...
          std::vector<std::string> write_draw_volume_weighted_cpo_list =
            Utilities::split_string_list(prm.get("Write out draw volume weighted cpo data"));
          ...
          for (unsigned int i = 0; i < write_draw_volume_weighted_cpo_list.size(); ++i)
            {
              std::vector<std::string> split_draw_volume_weighted_cpo_instructions =
                Utilities::split_string_list(write_draw_volume_weighted_cpo_list[i], ':');

              AssertThrow(
                split_draw_volume_weighted_cpo_instructions.size() == 2,
                ExcMessage(
                  "Value \"" + write_draw_volume_weighted_cpo_list[i] +
                  "\", set in \"Write out draw volume weighted cpo data\", is not a correct option " +
                  "because it should contain a mineral identification and a output specifier separated by a colon (:). This entry " +
                  "does not follow those rules."));

              // get mineral number
              std::vector<std::string> mineral_instructions =
                Utilities::split_string_list(split_draw_volume_weighted_cpo_instructions[0], ' ');
              AssertThrow(mineral_instructions.size() == 2,
                          ExcMessage(
                            "Value \"" + write_draw_volume_weighted_cpo_list[i] +
                            "\", set in \"Write out draw volume weighted cpo data\", is not a correct option " +
              ...
              AssertThrow(mineral_instructions[0] == "mineral",
                          ExcMessage("Value \"" + write_draw_volume_weighted_cpo_list[i] +
                                     "\", set in \"Write out draw volume weighted cpo data\", is not a correct option " +
                                     "because the mineral identification part should start with the word mineral and it starts with \"" +
                                     mineral_instructions[0] + "\"."));
