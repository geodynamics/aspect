...
namespace aspect
{
  namespace Postprocess
  {
    namespace internal
    {
      ...
      compute_dirichlet_boundary_heat_flux_solution_vector(const SimulatorAccess<dim> &simulator_access)
      {
        ...
        const unsigned int quadrature_degree =
          simulator_access.get_parameters().temperature_degree + (simulator_access.get_parameters().stokes_velocity_degree + 1) / 2;
        ...
        FEFaceValues<dim> fe_face_values(simulator_access.get_mapping(),
                                         ...
                                         update_JxW_values | update_values | update_gradients | update_normal_vectors |
        ...
        typename MaterialModel::Interface<dim>::MaterialModelInputs  in(fe_volume_values.n_quadrature_points,
                                                                       simulator_access.n_compositional_fields());
        typename MaterialModel::Interface<dim>::MaterialModelOutputs out(fe_volume_values.n_quadrature_points,
                                                                         simulator_access.n_compositional_fields());

        HeatingModel::HeatingModelOutputs heating_out(fe_volume_values.n_quadrature_points, simulator_access.n_compositional_fields());

        typename MaterialModel::Interface<dim>::MaterialModelInputs  face_in(fe_face_values.n_quadrature_points,
                                                                            simulator_access.n_compositional_fields());
        typename MaterialModel::Interface<dim>::MaterialModelOutputs face_out(fe_face_values.n_quadrature_points,
                                                                              simulator_access.n_compositional_fields());
          ...
          if (cell->is_locally_owned() && cell->at_boundary())
            {
              ...
              fe_volume_values[simulator_access.introspection().extractors.temperature].get_function_gradients(
                simulator_access.get_solution(), temperature_gradients);
              fe_volume_values[simulator_access.introspection().extractors.temperature].get_function_values(
                simulator_access.get_old_solution(), old_temperatures);
              fe_volume_values[simulator_access.introspection().extractors.temperature].get_function_values(
                simulator_access.get_old_old_solution(), old_old_temperatures);
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  ...
                  if (simulator_access.get_timestep_number() > 1)
                    {
                      ...
                      temperature_time_derivative =
                        (1.0 / time_step) *
                        (in.temperature[q] * (2 * time_step + old_time_step) / (time_step + old_time_step) -
                         old_temperatures[q] * (1 + time_step / old_time_step) +
                         old_old_temperatures[q] * (time_step * time_step) / (old_time_step * (time_step + old_time_step)));
                    }
                  else if (simulator_access.get_timestep_number() == 1)
                    {
                      ...
                      temperature_time_derivative = (in.temperature[q] - old_temperatures[q]) / time_step;
                    }
                  else
                  ...
                  // The SUPG parameter tau does not have the physical dimensions of a thermal conductivity and as such should not be
                  // included in heat flux calculations By default, ASPECT includes the artificial viscosity in the thermal conductivity
                  // when calculating boundary heat flux.
                  const double diffusion_constant = (simulator_access.get_parameters().advection_stabilization_method ==
                                                     Parameters<dim>::AdvectionStabilizationMethod::supg) ?
                                                      ...
                                                      std::max(out.thermal_conductivities[q], artificial_viscosity_cell);
                  ...
                  // Compute heat flux through Neumann and Robin boundary by integrating the heat flux
                           ...
                           robin_temperature_boundaries.find(boundary_id) != robin_temperature_boundaries.end())
                    {
                      ...
                      if (fixed_heat_flux_boundaries.find(boundary_id) != fixed_heat_flux_boundaries.end())
                        heat_flux = simulator_access.get_boundary_heat_flux().heat_flux(boundary_id,
                                                                                        face_in,
                                                                                        face_out,
                        ...
                        heat_flux = simulator_access.get_boundary_convective_heating_manager().heat_flux(
                          boundary_id, face_in, face_out, fe_face_values.get_normal_vectors());
                ...
                if (cell->at_boundary(f))
                  {
                    ...
                    if (heat_flux_and_area[cell->active_cell_index()].size() == 0)
                      heat_flux_and_area[cell->active_cell_index()].resize(cell->n_faces(), std::pair<double, double>(0.0, 0.0));
                    else
                      Assert(heat_flux_and_area[cell->active_cell_index()].size() == cell->n_faces(), ExcInternalError());

                    // Determine the type of boundary
                    const unsigned int boundary_id = cell->face(f)->boundary_id();
                    const bool         prescribed_temperature =
                      fixed_temperature_boundaries.find(boundary_id) != fixed_temperature_boundaries.end();
                    ...
                    if (prescribed_temperature || robin_temperature)
                      {
                        fe_face_values[simulator_access.introspection().extractors.temperature].get_function_values(heat_flux_vector,
                                                                                                                    heat_flux_values);

                        for (unsigned int q = 0; q < n_face_q_points; ++q)
                          heat_flux_and_area[cell->active_cell_index()][f].first += heat_flux_values[q] * fe_face_values.JxW(q);
                      }
                        ...
                        for (unsigned int q = 0; q < n_face_q_points; ++q)
                          {
                            heat_flux_and_area[cell->active_cell_index()][f].first +=
                              face_out.densities[q] * face_out.specific_heat[q] * face_in.temperature[q] * face_in.velocity[q] *
    ...
    {
      std::vector<std::vector<std::pair<double, double>>> heat_flux_and_area = internal::compute_heat_flux_through_boundary_faces(*this);
      ...
      return std::pair<std::string, std::string>("Writing heat flux map", placeholder_name);
    }
                                     ...
                                     const std::vector<std::vector<std::pair<double, double>>> &heat_flux_and_area)
    {
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          output << "# " << ((dim == 2) ? "x y" : "x y z") << " heat_flux_" << boundary_name << std::endl;
        }
            ...
            if (cell->at_boundary(f) && (cell->face(f)->boundary_id() == boundary_id))
              {
                ...
                const double flux_density =
                  heat_flux_and_area[cell->active_cell_index()][f].first / heat_flux_and_area[cell->active_cell_index()][f].second;

                output << std::setprecision(10) << midpoint_at_surface << ' ' << std::setprecision(10) << flux_density << std::endl;
              }
      ...
      std::string filename = this->get_output_directory() + "heat_flux_map/heat_flux_" + boundary_name + "." +
