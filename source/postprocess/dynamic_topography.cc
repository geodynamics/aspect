...
namespace aspect
{
  namespace Postprocess
  {
    ...
    DynamicTopography<dim>::execute(TableHandler &)
    {
      ...
      LinearAlgebra::BlockVector distributed_topo_vector(this->introspection().index_sets.system_partitioning,
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  {
                    ...
                    local_vector(i) +=
                      2.0 * eta *
                      (epsilon_phi_u[i] * in_volume.strain_rate[q] - (is_compressible ? 1. / 3. * div_phi_u[i] * div_solution[q] : 0.0)) *
                      JxW;
                ...
                for (unsigned int i = 0; i < dofs_per_cell; ++i)
                  {
                    ...
                    local_mass_matrix(i) += phi_u * phi_u * JxW;
                  }
              }
      ...
      std::vector<Point<dim - 1>> face_support_points =
      ...
      FEFaceValues<dim>   fe_support_values(this->get_mapping(),
                                          this->get_fe(),
                                          support_quadrature,
                                          update_values | update_normal_vectors | update_gradients | update_quadrature_points);
                    ...
                    if (at_upper_surface)
                      {
                        ...
                        dynamic_topography = (-stress_support_values[support_index] * normal - surface_pressure) / delta_rho / gravity_norm;
                      }
                    else
                      {
                        ...
                        dynamic_topography = (-stress_support_values[support_index] * normal - bottom_pressure) / delta_rho / gravity_norm;
                      }
                ...
                if (at_upper_surface)
                  {
                    ...
                    dynamic_topography += (-stress_output_values[q] * normal - surface_pressure) / delta_rho / gravity_norm * JxW;
                  }
                else
                  {
                    const double delta_rho = out_output.densities[q] - density_below;

                    dynamic_topography += (-stress_output_values[q] * normal - bottom_pressure) / delta_rho / gravity_norm * JxW;
                  }
                face_area += JxW;
              }
    ...
    DynamicTopography<dim>::topography_vector() const
    {
      return topo_vector;
    }
    ...
    template <int dim>
    double
    DynamicTopography<dim>::get_density_above() const
    {
    ...
    template <int dim>
    double
    DynamicTopography<dim>::get_density_below() const
    {
    ...
    template <int dim>
    const Vector<float> &
    DynamicTopography<dim>::cellwise_topography() const
    {
                                           ...
                                           const std::vector<std::pair<Point<dim>, double>> &position_and_topography)
    {
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          output << "# " << ((dim == 2) ? "x y " : "x y z ") << "dynamic_topography_" << boundary_name << std::endl;
        }

      for (unsigned int i = 0; i < position_and_topography.size(); ++i)
        {
          output << std::setprecision(10) << position_and_topography[i].first << ' ' << std::setprecision(10)
                 << position_and_topography[i].second << std::endl;
        }
      ...
      std::string filename = this->get_output_directory() + "dynamic_topography/dynamic_topography_" + boundary_name + "." +
    ...
    DynamicTopography<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Density above",
                            "0.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Density below",
                            "9900.",
                            Patterns::Double(0.),
                            "Dynamic topography is calculated as the excess or lack of mass that is supported by mantle flow. "
