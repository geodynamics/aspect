...
namespace aspect
{
  namespace Postprocess
  {
    ...
    MobilityStatistics<dim>::execute(TableHandler &statistics)
    {
      ...
      const Quadrature<dim - 1> &quadrature_formula_face = this->introspection().face_quadratures.velocities;
      ...
      const double global_rms_vel = std::sqrt(global_velocity_square_integral) / std::sqrt(this->get_volume());

      const double global_top_velocity_square_integral =
        Utilities::MPI::sum(local_velocity_square_integral_top_boundary, this->get_mpi_communicator());
      ...
      statistics.add_value(name_mobility, mobility);
      ...
      return std::pair<std::string, std::string>("Mobility:", output.str());
    }
  }
}
