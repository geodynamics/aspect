...
// The ice height data is now only loaded once from the first time step. It is not constant over time. Implement an option to load data for
// current timestep (as in boundary traction). Implement iterative structure with solvers.
...
namespace aspect
{
  namespace Postprocess
  {
    ...
    SeaLevel<3>::compute_nonuniform_sea_level_change(const Point<3> &position) const
    {
      ...
      const Postprocess::Geoid<dim> &geoid = this->get_postprocess_manager().template get_matching_active_plugin<Postprocess::Geoid<dim>>();
            ...
            for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
              {
                ...
                const std::array<double, dim> spherical_position =
                  aspect::Utilities::Coordinates::cartesian_to_spherical_coordinates(current_position);
                for (unsigned int d = 1; d < dim; ++d)
                  long_colat[d - 1] = spherical_position[d];
    ...
    SeaLevel<3>::compute_total_surface_pressure(const Point<3> &position) const
    {
      ...
      const double total_surface_pressure =
        surface_gravity * ((nonuniform_sea_level_change + sea_level_offset) * density_water + ice_height * density_ice);
    ...
    SeaLevel<3>::execute(TableHandler &statistics)
    {
                    ...
                    if (write_to_file)
                      {
                        ...
                        output_file << scoord[0] << ' ' << scoord[1] << ' ' << scoord[2] << ' ' << nonuniform_sea_level_change << ' '
                                    << sea_level_offset << ' ' << geoid_displacement << ' ' << topography << ' ' << ice_height << ' '
                                    << ocean_mask << std::endl;
                      }
      ...
      statistics.add_value("Minimum non-uniform sea_level change (m)", min_nonuniform_sea_level_change);
      statistics.add_value("Maximum non-uniform sea_level change (m)", max_nonuniform_sea_level_change);
      const char *columns[] = {"Minimum non-uniform sea_level change (m)", "Maximum non-uniform sea_level change (m)"};
      ...
      output_stats << min_nonuniform_sea_level_change << " m, " << max_nonuniform_sea_level_change << " m";
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          Utilities::create_directory(this->get_output_directory() + "sea_level/",
                                      ...
                                      /* silent=*/true);
          std::string filename = this->get_output_directory() + "sea_level/nonuniform_sea_level_change." +
          ...
          for (unsigned int p = 1; p < Utilities::MPI::n_mpi_processes(this->get_mpi_communicator()); ++p)
            {
              Utilities::MPI::Future<std::string> data = Utilities::MPI::irecv<std::string>(this->get_mpi_communicator(), p, mpi_tag);
              file << data.get();
            }
        }
      else
    ...
    SeaLevel<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Ice density", "931", Patterns::Double(0.), "The density of ice [kg/m3]");
          prm.declare_entry("Water density", "1000", Patterns::Double(0.), "The density of water [kg/m3]");
          ...
          prm.declare_entry("Data directory topography",
                            ...
                            "of ASPECT.");
          prm.declare_entry("Data file name topography",
          ...
          prm.declare_entry("Data directory ice height",
                            ...
                            "for example, to reference files located in the `data/' subdirectory "
                            "of ASPECT.");
          prm.declare_entry("Data file name ice height",
    ...
    {
      ar &last_output_time &sea_level_offset;
    }
