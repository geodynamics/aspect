...
namespace aspect
{
  namespace Postprocess
  {
    ...
    PointValues<dim>::execute(TableHandler &)
    {
      ...
      std::vector<Vector<double>> current_point_values(evaluation_points_cartesian.size(),
      ...
      for (unsigned int p = 0; p < evaluation_points_cartesian.size(); ++p)
        {
          ...
          AssertThrow(n_procs > 0,
                      ExcMessage(
                        "While trying to evaluate the solution at point " + Utilities::to_string(evaluation_points_cartesian[p][0]) + ", " +
                        Utilities::to_string(evaluation_points_cartesian[p][1]) +
                        (dim == 3 ? ", " + Utilities::to_string(evaluation_points_cartesian[p][2]) : "") + "), " +
                        "no processors reported that the point lies inside the " +
                        "set of cells they own. Are you trying to evaluate the " + "solution at a point that lies outside of the domain?"));

          // Reduce all collected values into local Vector
          Utilities::MPI::sum(current_point_values[p], this->get_mpi_communicator(), current_point_values[p]);
      ...
      const std::string filename = (this->get_output_directory() + "point_values.txt");
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          ...
          f << ("# <time> "
            ...
            << ("<velocity_x> "
                "<velocity_y> ")
            << (dim == 3 ? "<velocity_z> " : "") << "<pressure> <temperature>";
          ...
          for (const auto &time_point : point_values)
            {
              Assert(time_point.second.size() == evaluation_points_cartesian.size(), ExcInternalError());
                  ...
                  for (unsigned int c = 0; c < time_point.second[i].size(); ++c)
                    {
                      ...
                      // requested
                      if ((this->introspection().component_masks.velocities[c] == false) || (this->convert_output_to_years() == false))
      ...
      return std::make_pair(std::string("Writing point values:"), filename);
    }
    ...
    PointValues<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Time between point values output",
                            "0.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Evaluation points",
                            ...
                            Patterns::List(Patterns::List(Patterns::Double(), dim, dim, ","), 0, Patterns::List::max_int_value, ";"),
          ...
          prm.declare_entry("Use natural coordinates",
                            "false",
                            Patterns::Bool(),
    ...
    PointValues<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Point values");
        {
          ...
          const std::vector<std::string> point_list = Utilities::split_string_list(prm.get("Evaluation points"), ';');

          std::vector<std::array<double, dim>> evaluation_points;

          for (const auto &p : point_list)
            {
              const std::vector<std::string> coordinates = Utilities::split_string_list(p, ',');
    ...
    {
      ar &evaluation_points_cartesian &point_values &last_output_time;
    }
