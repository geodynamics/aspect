  ...
  namespace Postprocess
  {
    ...
    GravityPointValues<dim>::initialize()
    {
      ...
      if (Plugins::plugin_type_matches<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()))
        {
          model_outer_radius =
            Plugins::get_plugin_as_type<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()).outer_radius();
          model_inner_radius =
            Plugins::get_plugin_as_type<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()).inner_radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(this->get_geometry_model()))
        {
          model_outer_radius = Plugins::get_plugin_as_type<const GeometryModel::Chunk<dim>>(this->get_geometry_model()).outer_radius();
          model_inner_radius = Plugins::get_plugin_as_type<const GeometryModel::Chunk<dim>>(this->get_geometry_model()).inner_radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::Sphere<dim>>(this->get_geometry_model()))
        {
          model_outer_radius = Plugins::get_plugin_as_type<const GeometryModel::Sphere<dim>>(this->get_geometry_model()).radius();
                  ...
                  for (unsigned int j = 0; j < n_points_latitude; ++j)
                    {
                      if (n_points_radius > 1)
                        satellite_positions_spherical[p][0] =
                          minimum_radius + ((maximum_radius - minimum_radius) / (n_points_radius - 1)) * h;
                      else
                        satellite_positions_spherical[p][0] = minimum_radius;
                      if (n_points_longitude > 1)
                        satellite_positions_spherical[p][1] =
                          (minimum_colongitude + ((maximum_colongitude - minimum_colongitude) / (n_points_longitude - 1)) * i) *
                          constants::degree_to_radians;
                      else
                        satellite_positions_spherical[p][1] = minimum_colongitude * constants::degree_to_radians;
                      if (n_points_latitude > 1)
                        satellite_positions_spherical[p][2] =
                          (minimum_colatitude + ((maximum_colatitude - minimum_colatitude) / (n_points_latitude - 1)) * j) *
                          constants::degree_to_radians;
                      else
    ...
    GravityPointValues<3>::execute(TableHandler &statistics)
    {
      ...
      const std::string filename    = (this->get_output_directory() + "gravity_point_values/" + file_prefix);

      // Get quadrature formula and increase the degree of quadrature over the velocity
      // element degree.
      const unsigned int degree =
        this->get_fe().base_element(this->introspection().base_elements.velocities).degree + quadrature_degree_increase;
        ...
        if (cell->is_locally_owned())
          {
            ...
            for (unsigned int q = 0; q < n_quadrature_points_per_cell; ++q)
              {
                G_times_density_times_JxW[q]         = G * out.densities[q] * fe_values.JxW(q);
                G_times_density_anomaly_times_JxW[q] = G * (out.densities[q] - reference_density) * fe_values.JxW(q);
              }
                ...
                for (unsigned int q = 0; q < n_quadrature_points_per_cell; ++q)
                  {
                    ...
                    for (unsigned int e = 0; e < dim; ++e)
                      for (unsigned int f = e; f < dim; ++f)
                        local_g_gradient[p][e][f] += grad_KK * (3.0 * r_vector[e] * r_vector[f] - (e == f ? r_squared : 0));
                  }
              }
          }
      ...
      Utilities::MPI::sum(make_const_array_view(local_g_potential), this->get_mpi_communicator(), make_array_view(g_potential));

      const auto tensor_sum = [](const auto &v1, const auto &v2) {
      ...
      const std::vector<Tensor<1, dim>> g =
        Utilities::MPI::all_reduce<decltype(local_g)>(local_g, this->get_mpi_communicator(), tensor_sum);

      const std::vector<Tensor<1, dim>> g_anomaly =
        Utilities::MPI::all_reduce<decltype(local_g_anomaly)>(local_g_anomaly, this->get_mpi_communicator(), tensor_sum);

      const std::vector<SymmetricTensor<2, dim>> g_gradient =
        Utilities::MPI::all_reduce<decltype(local_g_gradient)>(local_g_gradient, this->get_mpi_communicator(), tensor_sum);
        ...
        if (p % n_ranks == my_rank)
          {
            ...
            if (satellite_positions_spherical[p][0] <= model_inner_radius)
                     ...
                     (satellite_positions_spherical[p][0] < model_outer_radius))
              {
                ...
                g_theory[p] =
                  G * numbers::PI * 4. / 3. * reference_density *
                  (satellite_positions_spherical[p][0] - ((model_inner_radius * model_inner_radius * model_inner_radius) /
                ...
                g_potential_theory[p] =
                  ...
                  G * numbers::PI * 2.0 * reference_density * (model_outer_radius * model_outer_radius);
              }
            else
              {
                const double common_factor = G * numbers::PI * 4. / 3. * reference_density *
                                             ((model_outer_radius * model_outer_radius * model_outer_radius) -
                ...
                g_gradient_theory[p][0][0] = g_gradient_theory[p][1][1] = g_gradient_theory[p][2][2] = -1. / (r * r * r);

                // Then do the off-diagonal elements:
                for (unsigned int e = 0; e < dim; ++e)
                  for (unsigned int f = e; f < dim; ++f)
                    g_gradient_theory[p][e][f] += -(-3.0 * satellite_position[e] * satellite_position[f]) / Utilities::fixed_power<5>(r);
                g_gradient_theory[p] *= common_factor;
              }
          }

      g_theory = Utilities::MPI::all_reduce<decltype(g_theory)>(g_theory, this->get_mpi_communicator(), tensor_sum);

      g_potential_theory =
        Utilities::MPI::all_reduce<decltype(g_potential_theory)>(g_potential_theory, this->get_mpi_communicator(), tensor_sum);

      g_gradient_theory =
        Utilities::MPI::all_reduce<decltype(g_gradient_theory)>(g_gradient_theory, this->get_mpi_communicator(), tensor_sum);
          ...
          for (unsigned int p = 0; p < n_satellites; ++p)
            {
              ...
              output << satellite_positions_spherical[p][0] << ' ' << satellite_positions_spherical[p][1] * constants::radians_to_degree
                     << ' ' << satellite_positions_spherical[p][2] * constants::radians_to_degree << ' ' << satellite_position[0] << ' '
                     << satellite_position[1] << ' ' << satellite_position[2] << ' ' << std::setprecision(precision) << g[p] << ' '
                     << g[p].norm() << ' ' << g_theory[p] << ' ' << g_potential[p] << ' ' << g_potential_theory[p] << ' ' << g_anomaly[p]
                     << ' ' << g_anomaly[p].norm() << ' ' << g_gradient[p][0][0] * 1e9 << ' ' << g_gradient[p][1][1] * 1e9 << ' '
                     << g_gradient[p][2][2] * 1e9 << ' ' << g_gradient[p][0][1] * 1e9 << ' ' << g_gradient[p][0][2] * 1e9 << ' '
                     << g_gradient[p][1][2] * 1e9 << ' ' << g_gradient_theory[p][0][0] * 1e9 << ' ' << g_gradient_theory[p][1][1] * 1e9
                     << ' ' << g_gradient_theory[p][2][2] * 1e9 << ' ' << g_gradient_theory[p][0][1] * 1e9 << ' '
                     << g_gradient_theory[p][0][2] * 1e9 << ' ' << g_gradient_theory[p][1][2] * 1e9 << ' ' << '\n';
            }

          // Now push the entire buffer into the output file in one swoop fell:
          std::ofstream output_file(filename);
          AssertThrow(output_file, ExcMessage("Unable to open file for writing: " + filename + "."));
    ...
    GravityPointValues<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Sampling scheme",
                            "map",
                            ...
                            "maximum radius.");
          prm.declare_entry("Number points fibonacci spiral",
                            "200",
                            Patterns::Integer(0),
          ...
          prm.declare_entry("Quadrature degree increase",
                            "0",
                            Patterns::Integer(-1),
          ...
          prm.declare_entry("Number points radius",
                            "1",
                            Patterns::Integer(0),
                            ...
                            "maximum radius.");
          prm.declare_entry("Number points longitude",
                            "1",
                            Patterns::Integer(0),
                            "Parameter for the map sampling scheme: "
                            "This specifies the number of points along "
                            ...
                            "maximum longitude.");
          prm.declare_entry("Number points latitude",
                            "1",
                            Patterns::Integer(0),
                            "Parameter for the map sampling scheme: "
                            "This specifies the number of points along "
                            ...
                            "maximum latitude.");
          prm.declare_entry("Minimum radius",
                            "0.",
                            Patterns::Double(0.0),
                            "Parameter for the map sampling scheme: "
                            ...
                            "specific height.");
          prm.declare_entry("Maximum radius",
                            "0.",
                            Patterns::Double(0.0),
                            "Parameter for the map sampling scheme: "
                            "Maximum radius can be defined in or outside the model.");
          prm.declare_entry("Minimum longitude",
                            "-180.",
          ...
          prm.declare_entry("Minimum latitude",
                            "-90.",
                            Patterns::Double(-90.0, 90.0),
                            "Parameter for the uniform distribution sampling scheme: "
                            "Gravity may be calculated for a sets of points along "
                            "the latitude between a minimum and maximum latitude.");
          prm.declare_entry("Maximum longitude",
                            "180.",
                            Patterns::Double(-180.0, 180.0),
                            "Parameter for the uniform distribution sampling scheme: "
                            "Gravity may be calculated for a sets of points along "
                            "the longitude between a minimum and maximum longitude.");
          prm.declare_entry("Maximum latitude",
                            "90",
                            Patterns::Double(-90.0, 90.0),
                            "Parameter for the uniform distribution sampling scheme: "
                            "Gravity may be calculated for a sets of points along "
          ...
          prm.declare_entry("Precision in gravity output",
                            "12",
                            Patterns::Integer(1),
          ...
          prm.declare_entry("Time between gravity output",
                            "1e8",
                            Patterns::Double(0.0),
                            ...
                            "\\si{\\second} otherwise.");
          prm.declare_entry("Time steps between gravity output",
    ...
    GravityPointValues<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      Utilities::create_directory(gravity_subdirectory, this->get_mpi_communicator(), true);
      ...
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Gravity calculation");
        {
          ...
          AssertThrow(dim == 3, ExcMessage("This postprocessor was only tested for 3d models."));
    ...
    GravityPointValues<dim>::serialize(Archive &ar, const unsigned int)
    {
      ...
      ar &last_output_time &last_output_timestep &output_file_number;
    }
