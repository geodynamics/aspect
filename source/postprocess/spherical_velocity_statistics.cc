...
namespace aspect
{
  namespace Postprocess
  {
    ...
    SphericalVelocityStatistics<dim>::execute(TableHandler &statistics)
    {
      ...
      if (this->convert_output_to_years() == true)
        {
          ...
          const char *columns[] = {"Radial RMS velocity (m/yr)", "Tangential RMS velocity (m/yr)", "Total RMS velocity (m/yr)"};
          statistics.add_value(columns[0], rad_vrms * year_in_seconds);
          statistics.add_value(columns[1], tan_vrms * year_in_seconds);
          statistics.add_value(columns[2], vrms * year_in_seconds);
      ...
      else
        {
          // make sure that the columns filled by this object
          // all show up with sufficient accuracy and in scientific notation
          const char *columns[] = {"Radial RMS velocity (m/s)", "Tangential RMS velocity (m/s)", "Total RMS velocity (m/s)"};
      ...
      output.precision(3);
      if (this->convert_output_to_years() == true)
        output << rad_vrms * year_in_seconds << " m/yr, " << tan_vrms * year_in_seconds << " m/yr, " << vrms * year_in_seconds << " m/yr";
      else
        output << rad_vrms << " m/s, " << tan_vrms << " m/s, " << vrms << " m/s";

      return std::pair<std::string, std::string>("Radial RMS, tangential RMS, total RMS velocity:", output.str());
    }
  }
}
