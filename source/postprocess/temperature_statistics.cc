...
namespace aspect
{
  namespace Postprocess
  {
    ...
    TemperatureStatistics<dim>::execute(TableHandler &statistics)
    {
      ...
      {
        const char *columns[] = {"Minimal temperature (K)", "Average temperature (K)", "Maximal temperature (K)"};
      ...
      output.precision(4);
      output << global_min_temperature << " K, " << global_temperature_integral / this->get_volume() << " K, " << global_max_temperature
             << " K";

      return std::pair<std::string, std::string>("Temperature min/avg/max:", output.str());
    }
  }
}
