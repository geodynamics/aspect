...
namespace aspect
{
  namespace Postprocess
  {
    ...
    VelocityBoundaryStatistics<dim>::execute(TableHandler &statistics)
    {
            ...
            if (cell->face(f)->at_boundary())
              {
                ...
                for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                  {
                    ...
                    local_max            = std::max(vel_mag, local_max);
                    local_min            = std::min(vel_mag, local_min);
                ...
                local_max_vel[boundary_indicator] = std::max(local_max, local_max_vel[boundary_indicator]);
                local_min_vel[boundary_indicator] = std::min(local_min, local_min_vel[boundary_indicator]);
      ...
      for (std::map<types::boundary_id, double>::const_iterator max_velocity = global_max_vel.begin(),
           ...
           ++max_velocity, ++min_velocity, ++rms, ++index)
        {
          const std::string name_max =
            "Maximum velocity magnitude on boundary with indicator " + Utilities::int_to_string(max_velocity->first) +
          ...
          const std::string name_min =
            "Minimum velocity magnitude on boundary with indicator " + Utilities::int_to_string(min_velocity->first) +
          ...
          const std::string name_rms =
            "RMS velocity on boundary with indicator " + Utilities::int_to_string(rms->first) +
      ...
      return std::pair<std::string, std::string>("Max, min, and rms velocity along boundary parts:", screen_text.str());
    }
  }
}
