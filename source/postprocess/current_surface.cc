...
namespace aspect
{
  namespace Postprocess
  {
    ...
    CurrentSurface<dim>::execute(TableHandler &)
    {
      ...
      AssertThrow(dim == 2, ExcMessage("Depth with mesh deformation currently only works with a 2D model."));
      ...
      std::vector<std::vector<double>> temp_surface = Utilities::MPI::compute_set_union(local_surface_height, this->get_mpi_communicator());
      ...
      const auto compareRows = [](const std::vector<double> &row1, const std::vector<double> &row2) { return row1 == row2; };
      ...
      return std::make_pair("Storing deformed surface topography: ", "Done");
    }
    ...
    CurrentSurface<dim>::depth_including_mesh_deformation(const Point<dim> &position) const
    {
      ...
      AssertThrow(dim == 2, ExcMessage("Depth with mesh deformation currently only works with a 2D geometry model."));
