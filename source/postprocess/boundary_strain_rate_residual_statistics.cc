...
namespace aspect
{
  namespace Postprocess
  {
    ...
    BoundaryStrainRateResidualStatistics<dim>::execute(TableHandler &statistics)
    {
              ...
              if (cell->face(f)->at_boundary() && cell->face(f)->boundary_id() == current_boundary_id)
                {
                  ...
                  for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                    {
                      ...
                      const double e_ii = std::sqrt(std::fabs(Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(
                      ...
                      local_max = std::max(strain_invariant_residual, local_max);
                      local_min = std::min(strain_invariant_residual, local_min);
                  ...
                  local_max_eii[current_boundary_id] = std::max(local_max, local_max_eii[current_boundary_id]);
                  local_min_eii[current_boundary_id] = std::min(local_min, local_min_eii[current_boundary_id]);
      ...
      for (std::map<types::boundary_id, double>::const_iterator max_eii = global_max_eii.begin(),
                                                                min_eii = global_min_eii.begin(),
                                                                rms     = global_rms_eii.begin();
           ...
           ++max_eii, ++min_eii, ++rms, ++index)
        {
          const std::string name_max =
            "Maximum strain rate invariant residual magnitude on boundary with indicator " + Utilities::int_to_string(max_eii->first) +
            aspect::Utilities::parenthesize_if_nonempty(this->get_geometry_model().translate_id_to_symbol_name(max_eii->first)) + " (" +
            units + ")";
          statistics.add_value(name_max, max_eii->second * unit_scale_factor);
          const std::string name_min =
            "Minimum strain rate invariant residual magnitude on boundary with indicator " + Utilities::int_to_string(min_eii->first) +
            aspect::Utilities::parenthesize_if_nonempty(this->get_geometry_model().translate_id_to_symbol_name(min_eii->first)) + " (" +
            units + ")";
          statistics.add_value(name_min, min_eii->second * unit_scale_factor);
          const std::string name_rms =
            "RMS strain rate invariant residual on boundary with indicator " + Utilities::int_to_string(rms->first) +
          ...
          screen_text << max_eii->second * unit_scale_factor << " " << units << ", " << min_eii->second * unit_scale_factor << " " << units
                      << ", " << rms->second * unit_scale_factor << " " << units << (index == global_max_eii.size() - 1 ? "" : ", ");
        }
