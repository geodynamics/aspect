...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      EntropyAverage<dim>::EntropyAverage()
        : DataPostprocessorScalar<dim>("entropy_average", update_values | update_quadrature_points)
      ...
      template <int dim>
      void
      EntropyAverage<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
                                                 std::vector<Vector<double>>                &computed_quantities) const
      {
        ...
        const std::vector<unsigned int> &entropy_field_idx =
        ...
        for (unsigned int q = 0; q < n_quadrature_points; ++q)
          {
            ...
            double entropy =
              background_chemical_composition * input_data.solution_values[q][composition_component_indices[entropy_field_idx[0]]];
            ...
            for (unsigned int i = 0; i < chemical_composition_idx.size(); ++i)
              {
                ...
                entropy +=
                  input_data.solution_values[q][composition_component_index] * input_data.solution_values[q][entropy_component_index];
              }
