...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      SeismicVsAnomaly<dim>::SeismicVsAnomaly()
      ...
      SeismicVsAnomaly<dim>::execute() const
      {
        std::pair<std::string, std::unique_ptr<Vector<float>>> return_value(
          "Vs_anomaly", std::make_unique<Vector<float>>(this->get_triangulation().n_active_cells()));
                  ...
                  if (cell->is_locally_owned())
                    {
                      ...
                      out.additional_outputs.push_back(std::make_unique<MaterialModel::SeismicAdditionalOutputs<dim>>(n_q_points));
                  ...
                  if (cell->is_locally_owned())
                    {
                      ...
                      // Compute the percentage deviation from the average
                      const double Vs_average =
                        (1. - fractional_slice) * padded_Vs_depth_average[idx] + fractional_slice * padded_Vs_depth_average[idx + 1];
      ...
      template <int dim>
      SeismicVpAnomaly<dim>::SeismicVpAnomaly()
        : CellDataVectorCreator<dim>("") // no physical units
      {}
      ...
      {
        std::pair<std::string, std::unique_ptr<Vector<float>>> return_value(
          "Vp_anomaly", std::make_unique<Vector<float>>(this->get_triangulation().n_active_cells()));
                  ...
                  if (cell->is_locally_owned())
                    {
                      ...
                      // Compute the percentage deviation from the average
                      const double Vp_average =
                        (1. - fractional_slice) * padded_Vp_depth_average[idx] + fractional_slice * padded_Vp_depth_average[idx + 1];
      ...
      SeismicVsAnomaly<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Visualization");
          {
            ...
            {
              prm.declare_entry("Average velocity scheme",
                                "reference profile",
