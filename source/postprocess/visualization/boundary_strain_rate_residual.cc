...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      BoundaryStrainRateResidual<dim>::BoundaryStrainRateResidual()
        : DataPostprocessorScalar<dim>("boundary_strain_rate_residual", update_quadrature_points | update_gradients)
      ...
      template <int dim>
      std::string
      BoundaryStrainRateResidual<dim>::get_physical_units() const
      {
      ...
      template <int dim>
      void
      BoundaryStrainRateResidual<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
          ...
          for (unsigned int q = 0; q < computed_quantities.size(); ++q)
            {
              ...
              if (data_surface_strain_rate < 1e300 || !std::isnan(data_surface_strain_rate))
                computed_quantities[q](0) =
                  data_surface_strain_rate - std::sqrt(std::fabs(Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(
