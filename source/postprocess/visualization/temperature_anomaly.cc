...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      TemperatureAnomaly<dim>::TemperatureAnomaly()
        : DataPostprocessorScalar<dim>("temperature_anomaly", update_values | update_quadrature_points)
        , Interface<dim>("K")
      {}
      ...
      TemperatureAnomaly<dim>::update()
      {
        ...
        if (extrapolate_bottom)
          {
            padded_temperature_depth_average[n_slices + 1] =
              2. * temperature_depth_average[n_slices - 1] - temperature_depth_average[n_slices - 2];
          }
        else
          {
      ...
      template <int dim>
      void
      TemperatureAnomaly<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
        ...
        for (unsigned int q = 0; q < n_quadrature_points; ++q)
          {
            ...
            const double depth_average_temperature = (1. - fractional_slice) * padded_temperature_depth_average[idx] +
                                                     fractional_slice * padded_temperature_depth_average[idx + 1];
      ...
      TemperatureAnomaly<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Visualization");
          {
            ...
            {
              prm.declare_entry("Number of depth slices",
                                "20",
                                Patterns::Integer(1),
                                "Number of depth slices used to define "
                                "average temperature.");
              prm.declare_entry("Use maximal temperature for bottom",
                                "true",
                                Patterns::Bool(),
              ...
              prm.declare_entry("Use minimal temperature for surface",
                                "true",
                                Patterns::Bool(),
