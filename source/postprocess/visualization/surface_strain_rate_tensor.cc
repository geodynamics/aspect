...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      SurfaceStrainRateTensor<dim>::SurfaceStrainRateTensor()
        : DataPostprocessorTensor<dim>("surface_strain_rate_tensor", update_gradients | update_quadrature_points)
        , Interface<dim>("1/s")
      {}
      ...
      template <int dim>
      void
      SurfaceStrainRateTensor<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
