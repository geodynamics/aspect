...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      ShearStress<dim>::ShearStress()
        : DataPostprocessorTensor<dim>("shear_stress", update_values | update_gradients | update_quadrature_points)
        , Interface<dim>("Pa")
      {}
      ...
      template <int dim>
      void
      ShearStress<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
        ...
        for (unsigned int q = 0; q < n_quadrature_points; ++q)
          {
            ...
            if (this->get_parameters().enable_elasticity == true)
              {
                ...
                Assert(elastic_additional_out != nullptr,
                       ExcMessage(
                         "Elastic Additional Outputs are needed for the 'shear stress' postprocessor, but they have not been created."));
            ...
            for (unsigned int d = 0; d < dim; ++d)
              for (unsigned int e = 0; e < dim; ++e)
                computed_quantities[q][Tensor<2, dim>::component_to_unrolled_index(TableIndices<2>(d, e))] = shear_stress[d][e];
          }
