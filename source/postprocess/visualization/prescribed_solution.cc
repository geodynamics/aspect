...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
       ...
       */
      template <int dim>
      PrescribedSolutionPostprocessor<dim>::PrescribedSolutionPostprocessor()
      ...
      PrescribedSolutionPostprocessor<dim>::get_names() const
      {
      ...
      PrescribedSolutionPostprocessor<dim>::get_data_component_interpretation() const
      {
      ...
      template <int dim>
      UpdateFlags
      PrescribedSolutionPostprocessor<dim>::get_needed_update_flags() const
      {
      ...
      template <int dim>
      void
      PrescribedSolutionPostprocessor<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
            ...
            for (auto &it : plugin_objects)
              {
                ...
                it->constrain_solution(
                  cell, input_data.evaluation_points, component_indices, component_is_constrained, constrained_component_value);
              }

            for (unsigned int q = 0; q < n_evaluation_points; ++q)
              {
                computed_quantities[q][2 * component_index] =
                  std::max(computed_quantities[q][2 * component_index], static_cast<double>(component_is_constrained[q]));
