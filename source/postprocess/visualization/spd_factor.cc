...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      SPD_Factor<dim>::SPD_Factor()
        : DataPostprocessorScalar<dim>("spd_factor", update_values | update_gradients | update_quadrature_points)
      ...
      template <int dim>
      void
      SPD_Factor<dim>::evaluate_vector_field(const DataPostprocessorInputs::Vector<dim> &input_data,
                                             std::vector<Vector<double>>                &computed_quantities) const
      {
        ...
        out.additional_outputs.push_back(std::make_unique<MaterialModel::MaterialModelDerivatives<dim>>(n_quadrature_points));
