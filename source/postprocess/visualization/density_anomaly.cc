...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      DensityAnomaly<dim>::DensityAnomaly()
      ...
      DensityAnomaly<dim>::execute() const
      {
        std::pair<std::string, std::unique_ptr<Vector<float>>> return_value(
          "density_anomaly", std::make_unique<Vector<float>>(this->get_triangulation().n_active_cells()));
                  ...
                  case lateral_average:
                    {
                      ...
                      const double depth_average_density = (1. - fractional_slice) * padded_density_depth_average[idx] +
                                                           fractional_slice * padded_density_depth_average[idx + 1];
      ...
      DensityAnomaly<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.enter_subsection("Postprocess");
        {
          prm.enter_subsection("Visualization");
          {
            ...
            {
              prm.declare_entry("Average density scheme",
                                "reference profile",
