...
namespace aspect
{
  namespace Postprocess
  {
    ...
    {
      template <int dim>
      GrainLagAngle<dim>::GrainLagAngle()
      ...
      GrainLagAngle<dim>::execute() const
      {
        std::pair<std::string, std::unique_ptr<Vector<float>>> return_value(
          "grain_lag_angle", std::make_unique<Vector<float>>(this->get_triangulation().n_active_cells()));
          ...
          if (cell->is_locally_owned() && this->get_timestep_number() > 0)
            {
              ...
              std::vector<Tensor<2, dim>> velocity_gradient(n_q_points, Tensor<2, dim>());
              fe_values[this->introspection().extractors.velocities].get_function_gradients(this->get_solution(), velocity_gradient);
              ...
              const std::array<double, dim> strain_rate_eigenvalues = eigenvalues(strain_rate);

              const double lambda1 = std::max(std::abs(strain_rate_eigenvalues[0]), std::abs(strain_rate_eigenvalues[dim - 1]));
              ...
              for (unsigned int i = 1; i <= maxorder; ++i) // loop and calculate terms of sum
                {
                  ...
                  for (unsigned int j = 0; j < i; ++j)
                    {
                      multiplier = multiplier * Tensor<2, dim>(velocity_gradient[0]);
                      factor *= tmax / (j + 1);
                    }
