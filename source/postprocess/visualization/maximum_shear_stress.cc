...
#include <aspect/postprocess/visualization/maximum_shear_stress.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
        ...
        for (unsigned int q=0; q<n_quadrature_points; ++q)
          {
            ...
            // negative stress
            SymmetricTensor<2,dim> compressive_stress = 2*eta*compressible_strain_rate;
            //compressive_stress[1][1] += 1000;
            ...
            const Tensor<1,dim> tau = (tau_direction/tau_direction.norm()) * tau_magnitude;


            for (unsigned int i=0; i<dim; ++i)
              computed_quantities[q](i) = tau[i];
          }
      }
      ...
      MaximumShearStress<dim>::get_names () const
      {
        return std::vector<std::string> (dim, "maximum_shear_stress");
      }


      template <int dim>
      std::vector<DataComponentInterpretation::DataComponentInterpretation>
      MaximumShearStress<dim>::get_data_component_interpretation () const
      {
        return
          std::vector<DataComponentInterpretation::DataComponentInterpretation>
          (dim,
      ...
      template <int dim>
      UpdateFlags
      MaximumShearStress<dim>::get_needed_update_flags () const
      {
