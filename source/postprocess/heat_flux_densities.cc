...
namespace aspect
{
  namespace Postprocess
  {
    ...
    HeatFluxDensities<dim>::execute(TableHandler &statistics)
    {
      ...
      unsigned int       index = 0;
      for (std::map<types::boundary_id, double>::const_iterator p = global_boundary_flux_densities.begin();
           p != global_boundary_flux_densities.end();
           ++p, ++index)
        {
          const std::string name =
            "Outward heat flux density for boundary with id " + Utilities::int_to_string(p->first) +
          ...
          screen_text << p->second << ' ' << unit << (index == global_boundary_flux_densities.size() - 1 ? "" : ", ");
        }

      return std::pair<std::string, std::string>("Heat flux densities for boundaries:", screen_text.str());
    }
  }
}
