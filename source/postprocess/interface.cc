...
namespace aspect
{
  namespace Postprocess
  {
          ...
          try
            {
              ...
              std::pair<std::string, std::string> output = p->execute(statistics);
          ...
          // and abort the program
          catch (std::exception &exc)
            {
              ...
              std::cerr << "Exception on MPI process <" << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
                        << "> while running postprocessor <" << typeid(*p).name() << ">: " << std::endl
                        << exc.what() << std::endl
                        << "Aborting!" << std::endl
                        << "----------------------------------------------------" << std::endl;
    ...
    Manager<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Postprocess");
      {
        this->plugin_names = Utilities::split_string_list(prm.get("List of postprocessors"));
      ...
      // difference.
      std::vector<std::string>::iterator new_end = std::remove(this->plugin_names.begin(), this->plugin_names.end(), "global statistics");
      ...
      for (unsigned int name = 0; name < this->plugin_names.size(); ++name)
        {
          this->plugin_objects.emplace_back(
            std::get<dim>(registered_plugins).create_plugin(this->plugin_names[name], "Postprocessor plugins"));
          ...
          const std::list<std::string> additional_postprocessors = this->plugin_objects.back()->required_other_postprocessors();

          for (const auto &p : additional_postprocessors)
            {
              AssertThrow(Patterns::Selection(std::get<dim>(registered_plugins).get_pattern_of_names()).match(p) == true,
                          ExcMessage("Postprocessor <" + this->plugin_names[name] + "> states that it depends on another postprocessor, <" +
                                     p + ">, but the latter is not a valid name."));
      ...
      Assert(this->plugin_names.size() == this->plugin_objects.size(), ExcInternalError());
      ...
      while (sorted_names.size() < this->plugin_objects.size())
        {
          ...
          {
            typename std::list<std::unique_ptr<Interface<dim>>>::iterator pp = this->plugin_objects.begin();
              ...
              if (already_assigned[i] == false)
                {
                  ...
                  for (const auto &p : deps)
                    if (std::find(sorted_names.begin(), sorted_names.end(), p) == sorted_names.end())
                      {
          ...
          if (at_least_one_element_added == false)
            {
              ...
              typename std::list<std::unique_ptr<Interface<dim>>>::const_iterator pp = this->plugin_objects.begin();
              for (unsigned int i = 0; i < this->plugin_names.size(); ++i, ++pp)
                if (already_assigned[i] == false)
                  {
      ...
      Assert(this->plugin_names.size() == sorted_names.size(), ExcInternalError());
      Assert(sorted_postprocessors.size() == sorted_names.size(), ExcInternalError());
      Assert(std::find(already_assigned.begin(), already_assigned.end(), false) == already_assigned.end(), ExcInternalError());
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Postprocessor interface", out);
    }
