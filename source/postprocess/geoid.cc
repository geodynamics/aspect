...
namespace aspect
{
  namespace Postprocess
  {
              ...
              for (unsigned int ds_num = 0; ds_num < spherical_function.size(); ++ds_num)
                {
                  ...
                  const std::pair<double, double> sph_harm_vals = aspect::Utilities::real_spherical_harmonic(
                    ideg, iord, spherical_function.at(ds_num).at(0), spherical_function.at(ds_num).at(1));
                    ...
                    for (unsigned int q = 0; q < quadrature_formula.size(); ++q)
                      {
                        ...
                        const std::pair<double, double> sph_harm_vals =
                          aspect::Utilities::real_spherical_harmonic(ideg, iord, scoord[2], scoord[1]);
                        const double cos_component = sph_harm_vals.first;  // real / cos part
                        const double sin_component = sph_harm_vals.second; // imaginary / sin part
                        ...
                        integrated_density_cos_component +=
                          density * (1. / r_q) * Utilities::pow(r_q / outer_radius, ideg + 1) * cos_component * JxW;
                        integrated_density_sin_component +=
                          density * (1. / r_q) * Utilities::pow(r_q / outer_radius, ideg + 1) * sin_component * JxW;
                      }
                  }
    ...
    Geoid<dim>::topography_contribution(const double & /*outer_radius*/, const double & /*inner_radius*/) const
    {
      Assert(false, ExcNotImplemented());
    ...
    std::pair<std::pair<double, std::pair<std::vector<double>, std::vector<double>>>,
              std::pair<double, std::pair<std::vector<double>, std::vector<double>>>>
    Geoid<3>::topography_contribution(const double &outer_radius, const double &inner_radius) const
    {
      ...
      // Vectors to store the location, infinitesimal area, and topography associated with each quadrature point of each surface and bottom
      // cell respectively.
                ...
                if (include_surface_topo_contribution == true)
                  {
                    if (use_free_surface_topography == true)
                    ...
                    else
                      {
                        ...
                        for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                          {
                            surface_stored_values.emplace_back(fe_face_values.quadrature_point(q),
                                                               std::make_pair(fe_face_values.JxW(q), topo_values[q]));
                          }
                      }
                  }
                else
                    ...
                    if (at_upper_surface)
                      {
                        for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                          {
                            surface_stored_values.emplace_back(fe_face_values.quadrature_point(q),
                                                               std::make_pair(fe_face_values.JxW(q), 0.0));
                          }
                      }
                  }
              }
                ...
                if (include_CMB_topo_contribution == true)
                  {
                    if (use_free_CMB_topography == true)
                        ...
                        for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                          {
                            ...
                            const double   topography =
                              this->get_geometry_model().height_above_reference_surface(current_position) + (outer_radius - inner_radius);
                    ...
                    else
                      {
                        ...
                        for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                          {
                            CMB_stored_values.emplace_back(fe_face_values.quadrature_point(q),
                                                           std::make_pair(fe_face_values.JxW(q), topo_values[q]));
                          }
                      }
                  }
      ...
      for (const auto &surface_stored_value : surface_stored_values)
        {
          const std::array<double, 3> scoord =
            aspect::Utilities::Coordinates::cartesian_to_spherical_coordinates(surface_stored_value.first);
          ...
          surface_topo_spherical_function.emplace_back(
            std::vector<double>{scoord[2], scoord[1], infinitesimal, surface_stored_value.second.second});
        }
      ...
      for (const auto &CMB_stored_value : CMB_stored_values)
        {
          ...
          CMB_topo_spherical_function.emplace_back(
            std::vector<double>{scoord[2], scoord[1], infinitesimal, CMB_stored_value.second.second});
        }
    ...
    Geoid<dim>::execute(TableHandler &)
    {
          ...
          for (unsigned int iord = 0; iord < ideg + 1; ++iord)
            {
              ...
              if (include_surface_topo_contribution == true || include_CMB_topo_contribution == true)
                {
                  const double coecos_surface_topo = (4 * numbers::PI * G / (surface_gravity * (2 * ideg + 1))) * surface_delta_rho *
                                                     SH_surface_topo_coes.second.first.at(ind) * outer_radius;
                  const double coesin_surface_topo = (4 * numbers::PI * G / (surface_gravity * (2 * ideg + 1))) * surface_delta_rho *
                                                     SH_surface_topo_coes.second.second.at(ind) * outer_radius;
                  ...
                  const double coecos_CMB_topo = (4 * numbers::PI * G / (surface_gravity * (2 * ideg + 1))) * CMB_delta_rho *
                                                 SH_CMB_topo_coes.second.first.at(ind) * inner_radius *
                                                 Utilities::pow(inner_radius / outer_radius, ideg + 1);
                  const double coesin_CMB_topo = (4 * numbers::PI * G / (surface_gravity * (2 * ideg + 1))) * CMB_delta_rho *
                                                 SH_CMB_topo_coes.second.second.at(ind) * inner_radius *
                                                 Utilities::pow(inner_radius / outer_radius, ideg + 1);
          ...
          for (unsigned int iord = 0; iord < ideg + 1; ++iord)
            {
              if (include_surface_topo_contribution == true || include_CMB_topo_contribution == true)
                {
                  geoid_coecos.push_back(density_anomaly_contribution_coecos.at(ind) + surface_topo_contribution_coecos.at(ind) +
                                         CMB_topo_contribution_coecos.at(ind));
                  geoid_coesin.push_back(density_anomaly_contribution_coesin.at(ind) + surface_topo_contribution_coesin.at(ind) +
      ...
      for (unsigned int i = 0; i < surface_cell_locations.size(); ++i)
        {
          const std::array<double, dim> scoord =
            aspect::Utilities::Coordinates::cartesian_to_spherical_coordinates(surface_cell_locations.at(i));
              ...
              for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                {
                  // Normalization after Dahlen and Tromp (1986) Appendix B.6.
                  const std::pair<double, double> sph_harm_vals = aspect::Utilities::real_spherical_harmonic(
                    ideg, iord, surface_cell_spherical_coordinate.first, surface_cell_spherical_coordinate.second);
                  const double cos_component = sph_harm_vals.first;  // real / cos part
                  const double sin_component = sph_harm_vals.second; // imaginary / sin part
          ...
          for (unsigned int ideg = min_degree; ideg < max_degree + 1; ++ideg)
            {
              for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                {
                  output_density_anomaly_contribution_SH_coes << ideg << ' ' << iord << ' '
                                                              << density_anomaly_contribution_coecos.at(SH_coes_ind) << ' '
                                                              << density_anomaly_contribution_coesin.at(SH_coes_ind) << std::endl;
                  ++SH_coes_ind;
                }
            }

          const std::string density_anomaly_contribution_SH_coes_filename =
            this->get_output_directory() + "geoid/density_anomaly_contribution_SH_coefficients." +
            dealii::Utilities::int_to_string(this->get_timestep_number(), 5);

          // Because each processor already held all the SH coefficients from density anomaly contribution, we only need to stop by the
          // processor 0 to get the data. On processor 0, collect all the data and put them into the output density anomaly contribution SH
          // coefficients file.
          if (dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
            {
              ...
              density_anomaly_contribution_SH_coes_file << "# "
                                                        << "degree order cosine_coefficient sine_coefficient" << std::endl;
          ...
          for (unsigned int ideg = min_degree; ideg < max_degree + 1; ++ideg)
            {
              for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                {
                  output_surface_topo_contribution_SH_coes << ideg << ' ' << iord << ' ' << surface_topo_contribution_coecos.at(SH_coes_ind)
                                                           << ' ' << surface_topo_contribution_coesin.at(SH_coes_ind) << std::endl;
                  ++SH_coes_ind;
                }
            }
          ...
          if (dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
            {
              ...
              surface_topo_contribution_SH_coes_file << "surface density contrast(kg/m^3): " << output_surface_delta_rho.str() << std::endl;
              // Write out the data on processor 0
      ...
      if (output_CMB_topo_contribution_SH_coes == true)
        {
          ...
          for (unsigned int ideg = min_degree; ideg < max_degree + 1; ++ideg)
            {
              for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                {
                  output_CMB_topo_contribution_SH_coes << ideg << ' ' << iord << ' ' << CMB_topo_contribution_coecos.at(SH_coes_ind) << ' '
                                                       << CMB_topo_contribution_coesin.at(SH_coes_ind) << std::endl;
                  ++SH_coes_ind;
                }
            }
          ...
          // Because each processor already held all the SH coefficients from CMB topography contribution, we only need to stop by the
          // processor 0 to get the data. On processor 0, collect all the data and put them into the output CMB topography contribution SH
          // coefficients file.
          if (dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
            {
              ...
              CMB_topo_contribution_SH_coes_file << "CMB density contrast(kg/m^3): " << output_CMB_delta_rho.str() << std::endl;
              // Write out the data on processor 0
          ...
          for (unsigned int ideg = min_degree; ideg < max_degree + 1; ++ideg)
            {
              for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                {
                  output_geoid_anomaly_SH_coes << ideg << ' ' << iord << ' ' << geoid_coecos.at(SH_coes_ind) << ' '
                                               << geoid_coesin.at(SH_coes_ind) << std::endl;
                  ++SH_coes_ind;
                }
            }

          const std::string geoid_anomaly_SH_coes_filename = this->get_output_directory() + "geoid/geoid_anomaly_SH_coefficients." +
                                                             dealii::Utilities::int_to_string(this->get_timestep_number(), 5);

          // Because each processor already held all the geoid anomaly SH coefficients, we only need to stop by the processor 0 to get the
          // data. On processor 0, collect all the data and put them into the output geoid anomaly SH coefficients file.
          if (dealii::Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
            {
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          output << "# " << ((output_in_lat_lon == true) ? "longitude latitude" : "x y z") << " geoid_anomaly" << std::endl;
        }
          ...
          for (unsigned int i = 0; i < surface_cell_spherical_coordinates.size(); ++i)
            {
              ...
              output << lon << ' ' << lat << ' ' << geoid_anomaly.at(i) << std::endl;
            }
        }
      else
        {
          for (unsigned int i = 0; i < surface_cell_locations.size(); ++i)
            {
              // Write the solution to the stream output.
              output << surface_cell_locations.at(i) << ' ' << geoid_anomaly.at(i) << std::endl;
            }
        }

      const std::string filename =
        this->get_output_directory() + "geoid/geoid_anomaly." + dealii::Utilities::int_to_string(this->get_timestep_number(), 5);
      ...
      if (output_gravity_anomaly == true)
        {
          ...
          if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
            {
              output_gravity_anomaly << "# " << ((output_in_lat_lon == true) ? "longitude latitude" : "x y z") << " gravity_anomaly"
                                     << std::endl;
            }
                  ...
                  for (unsigned int iord = 0; iord < ideg + 1; ++iord)
                    {
                      ...
                      gravity_value += (geoid_coecos.at(ind) * cos_component + geoid_coesin.at(ind) * sin_component) * (ideg - 1) *
              ...
              for (unsigned int i = 0; i < surface_cell_spherical_coordinates.size(); ++i)
                {
                  ...
                  // Write the solution to the stream output.
                  output_gravity_anomaly << lon << ' ' << lat << ' ' << gravity_anomaly.at(i) << std::endl;
                }
            }
          else
            {
              for (unsigned int i = 0; i < surface_cell_locations.size(); ++i)
                {
                  // Write the solution to the stream output.
                  output_gravity_anomaly << surface_cell_locations.at(i) << ' ' << gravity_anomaly.at(i) << std::endl;
                }
            }

          const std::string filename =
            this->get_output_directory() + "geoid/gravity_anomaly." + dealii::Utilities::int_to_string(this->get_timestep_number(), 5);

          Utilities::collect_and_write_file_content(filename, output_gravity_anomaly.str(), this->get_mpi_communicator());
        }

      return std::pair<std::string, std::string>("Writing geoid anomaly:", filename);
    }
    ...
    Geoid<dim>::required_other_postprocessors() const
    {
      ...
      if ((include_surface_topo_contribution == true && use_free_surface_topography == false) ||
          (include_CMB_topo_contribution == true && use_free_CMB_topography == false))
        ...
        for (unsigned int iord = 0; iord < ideg + 1; ++iord, ++k)
          {
            ...
            value += geoid_coecos[k] * val.first + geoid_coesin[k] * val.second;
          }
      return value;
    }
    ...
    Geoid<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Geoid");
        {
          ...
          prm.declare_entry("Minimum degree",
                            "2",
                            Patterns::Integer(0),
          ...
          prm.declare_entry("Density above", "0.", Patterns::Double(0.), "The density value above the surface boundary.");
          prm.declare_entry("Density below", "9900.", Patterns::Double(0.), "The density value below the CMB boundary.");
          prm.declare_entry("Output geoid anomaly coefficients",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Output surface topography contribution coefficients",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Output CMB topography contribution coefficients",
                            "false",
                            Patterns::Bool(),
                            "Option to output the spherical harmonic coefficients of the CMB topography contribution "
                            "to the maximum degree. The default is false. ");
          prm.declare_entry("Output density anomaly contribution coefficients",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Output gravity anomaly",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_alias("Output surface topography contribution coefficients",
                            "Also output the spherical harmonic coefficients of surface dynamic topography contribution");
          prm.declare_alias("Output CMB topography contribution coefficients",
                            "Also output the spherical harmonic coefficients of CMB dynamic topography contribution");
          prm.declare_alias("Output density anomaly contribution coefficients",
          ...
          if (this->get_parameters().mesh_deformation_enabled == true)
            {
              ...
              const std::set<types::boundary_id> mesh_deformation_boundaries =
