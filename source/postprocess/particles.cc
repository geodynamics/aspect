  ...
  namespace Postprocess
  {
    namespace internal
    {
                                         ...
                                         const bool                                                     only_group_3d_vectors)
      {
        ...
        if (std::find(exclude_output_properties.begin(), exclude_output_properties.end(), "all") == exclude_output_properties.end())
          {
            ...
            for (unsigned int field_index = 0; field_index < property_information.n_fields(); ++field_index)
              {
                ...
                const unsigned int field_position =
                  property_information.n_fields() == 0 ? 0 : property_information.get_position_by_field_index(field_index);
                ...
                const bool field_is_vector = (only_group_3d_vectors == false ? n_components == dim : dim == 3 && n_components == 3);
    ...
    template <int dim>
    void
    Particles<dim>::writer(const std::string &filename, const std::string &temporary_output_location, const std::string &file_contents)
    {
                                            ...
                                            const std::vector<std::string>      &filenames)
    {
      ...
      std::ofstream     pvtu_file(this->get_output_directory() + solution_file_directory + pvtu_filename);
      ...
      const std::string visit_filename = (this->get_output_directory() + solution_file_directory + solution_file_prefix + ".visit");
      ...
      std::ofstream global_visit_file(this->get_output_directory() + description_file_prefix + ".visit");
      ...
      for (unsigned int particle_manager = 0; particle_manager < this->n_particle_managers(); ++particle_manager)
        {
          ...
          const std::string statistics_column_name = (particle_manager == 0 ? "Number of advected particles" :
                                                                              "Number of advected particles (Particle system " +
                                                                                Utilities::int_to_string(particle_manager + 1) + ")");
      ...
      for (unsigned int particle_manager = 0; particle_manager < this->n_particle_managers(); ++particle_manager)
        {
          ...
          const bool output_hdf5 = std::find(output_formats[particle_manager].begin(), output_formats[particle_manager].end(), "hdf5") !=
                                   output_formats[particle_manager].end();
          ...
          std::string particle_file_prefix =
            particles_output_base_name + "-" + Utilities::int_to_string(output_file_number[particle_manager], 5);
              ...
              if (output_format == "hdf5")
                {
                  ...
                  const XDMFEntry new_xdmf_entry =
                    data_out.create_xdmf_entry(data_filter, particle_file_name, time_in_years_or_seconds, this->get_mpi_communicator());
                  xdmf_entries[particles_output_base_name].push_back(new_xdmf_entry);
                  data_out.write_xdmf_file(xdmf_entries[particles_output_base_name],
                  ...
                  if (my_id == 0)
                    {
                      ...
                      write_description_files(
                        data_out, particles_output_base_name, particles_output_base_name + "/", particle_file_prefix, filenames);
                    }

                  const unsigned int n_processes = Utilities::MPI::n_mpi_processes(this->get_mpi_communicator());

                  const unsigned int my_file_id = (group_files == 0 ? my_id : my_id % group_files);
                  const std::string  filename   = this->get_output_directory() + particles_output_base_name + "/" + particle_file_prefix +
                      ...
                      if (write_in_background_thread)
                        {
                          ...
                          background_thread = std::thread([my_filename = filename, // filename is const, so we can not move from it
          ...
          const std::string statistics_column_name =
            (particle_manager == 0 ? "Particle file name" : "Particle file name (" + Utilities::int_to_string(particle_manager + 1) + ")");
          statistics.add_value(statistics_column_name, particle_output);
        }
    ...
    template <int dim>
    void
    Particles<dim>::set_last_output_time(const unsigned int particle_manager, const double current_time)
    {
      ...
      if (output_interval[particle_manager] > 0)
        {
          ...
          last_output_time[particle_manager] =
            last_output_time[particle_manager] +
            std::floor((current_time - last_output_time[particle_manager]) / output_interval[particle_manager] * magic) *
              output_interval[particle_manager] / magic;
        }
    }
        ...
        for (unsigned int particle_manager = 0; particle_manager < number_of_particle_managers; ++particle_manager)
          {
              ...
              }
            {
              ...
              prm.declare_entry("Data output format",
                                "vtu",
        ...
        for (unsigned int particle_manager = 0; particle_manager < this->get_parameters().n_particle_managers; ++particle_manager)
          {
              ...
              }
            {
              ...
              AssertThrow((std::find(tmp_output_formats.begin(), tmp_output_formats.end(), "none") == tmp_output_formats.end()) ||
              ...
              if (std::find(tmp_output_formats.begin(), tmp_output_formats.end(), "none") == tmp_output_formats.end())
                {
              ...
              std::vector<std::string>::iterator output_format = std::find(tmp_output_formats.begin(), tmp_output_formats.end(), "ascii");
...
namespace aspect
{
  namespace Postprocess
  {
    namespace internal
    {
#define INSTANTIATE(dim) template class ParticleOutput<dim>;
