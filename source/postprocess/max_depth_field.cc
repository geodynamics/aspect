...
namespace aspect
{
  namespace Postprocess
  {
    ...
    MaxDepthField<dim>::execute(TableHandler &statistics)
    {
      ...
      AssertThrow(this->n_compositional_fields() > 0, ExcMessage("This postprocessor cannot be used without compositional fields."));
      const Quadrature<dim> &quadrature_formula = this->introspection().quadratures.compositional_field_max;

      const unsigned int n_q_points = quadrature_formula.size();

      FEValues<dim> fe_values(this->get_mapping(), this->get_fe(), quadrature_formula, update_values | update_quadrature_points);
      ...
      Utilities::MPI::max(local_max_depth, this->get_mpi_communicator(), global_max_depth);
      ...
      return std::pair<std::string, std::string>("Compositions max depth [m]:", output.str());
    }
  }
}
