...
namespace aspect
{
  namespace Postprocess
  {
    namespace
    {
              ...
              for (unsigned int i = 0; i < computed_quantities[q].size(); ++i)
                {
                  ...
                  if (this->introspection().component_masks.velocities[i] ||
                      (this->include_melt_transport() && this->introspection().variable("fluid velocity").component_mask[i]))
       ...
       */
      template <int dim>
      class SurfaceBaseVariablePostprocessor : public VisualizationPostprocessors::SurfaceOnlyVisualization<dim>,
                                               public BaseVariablePostprocessor<dim>
      {
        public:
          std::vector<std::string>
          get_names() const override
      ...
      {
        public:
          MeshDeformationPostprocessor(const std::string &name, bool is_velocity)
            : DataPostprocessorVector<dim>(name, update_values)
                                ...
                                std::vector<Vector<double>>                &computed_quantities) const override
          {
            ...
            Assert(computed_quantities[0].size() == dim, ExcMessage("Unexpected dimension in mesh velocity postprocessor"));
            const double       velocity_scaling_factor = (is_velocity && this->convert_output_to_years()) ? year_in_seconds : 1.0;
            const unsigned int n_q_points              = input_data.solution_values.size();
            for (unsigned int q = 0; q < n_q_points; ++q)
              for (unsigned int i = 0; i < dim; ++i)
                computed_quantities[q][i] = input_data.solution_values[q][i] * velocity_scaling_factor;
    ...
    {
      ar &last_mesh_file_name &times_and_pvtu_names &output_file_names_by_timestep &xdmf_entries;
    ...
    {
      static_assert(std::is_same<DataOutType, DataOut<dim>>::value || std::is_same<DataOutType, DataOutFaces<dim>>::value,
      ...
      const double time_in_years_or_seconds = (this->convert_output_to_years() ? this->get_time() / year_in_seconds : this->get_time());
      const std::string pvtu_filename       = (solution_file_prefix + ".pvtu");
      std::ofstream     pvtu_file(this->get_output_directory() + "solution/" + pvtu_filename);
      ...
      if (this->get_parameters().run_postprocessors_on_nonlinear_iterations)
        {
          ...
          if (this->get_nonlinear_iteration() == 0)
            ...
            output_history.times_and_pvtu_names.back() = (std::make_pair(time_in_years_or_seconds, "solution/" + pvtu_filename));
        }
      else
        output_history.times_and_pvtu_names.emplace_back(time_in_years_or_seconds, "solution/" + pvtu_filename);

      const std::string pvd_filename = (this->get_output_directory() + (is_cell_data_output ? "solution.pvd" : "solution_surface.pvd"));
      ...
      const std::string visit_filename = (this->get_output_directory() + "solution/" + solution_file_prefix + ".visit");
      ...
      std::ofstream global_visit_file(this->get_output_directory() + (is_cell_data_output ? "solution.visit" : "solution_surface.visit"));
      ...
      for (unsigned int timestep = 0; timestep < output_history.times_and_pvtu_names.size(); ++timestep)
        times_and_output_file_names.push_back(
          std::make_pair(output_history.times_and_pvtu_names[timestep].first, output_history.output_file_names_by_timestep[timestep]));
                                            ...
                                            const std::map<std::string, std::string> &visualization_field_names_and_units) const
    {
      ...
      std::string solution_file_prefix =
        (is_cell_data_output ? "solution-" : "solution_surface-") + Utilities::int_to_string(output_file_number, 5);
      if (this->get_parameters().run_postprocessors_on_nonlinear_iterations)
      ...
      if (output_format == "hdf5")
        {
          ...
          const std::string mesh_file_prefix =
            (is_cell_data_output ? "mesh-" : "mesh_surface-") + Utilities::int_to_string(output_file_number, 5);
          ...
          new_xdmf_entry = data_out.create_xdmf_entry(
            data_filter, output_history.last_mesh_file_name, h5_solution_file_name, time_in_years_or_seconds, this->get_mpi_communicator());
          output_history.xdmf_entries.push_back(new_xdmf_entry);
          data_out.write_xdmf_file(output_history.xdmf_entries, this->get_output_directory() + xdmf_filename, this->get_mpi_communicator());
      ...
      else if (output_format == "vtu")
        {
          ...
          if (my_id == 0)
            {
              ...
              for (unsigned int i = 0; i < n_files; ++i)
                filenames.push_back(solution_file_prefix + "." + Utilities::int_to_string(i, 4) + ".vtu");
          ...
          const std::string  filename =
            this->get_output_directory() + "solution/" + solution_file_prefix + "." + Utilities::int_to_string(my_file_id, 4) + ".vtu";
          ...
          if ((group_files == 0) || (group_files >= n_processes))
              ...
              if (write_in_background_thread)
                {
                  ...
                  output_history.background_thread = std::thread([my_filename = filename, // filename is const, so we can not move from it
            ...
            if (group_files == 1)
              {
                data_out.write_vtu_in_parallel(filename, this->get_mpi_communicator());
              }
        ...
        {
          const std::string filename = this->get_output_directory() + "solution/" + solution_file_prefix + ".pd2";
        ...
        // the model setup and the network file system type.
        {
          ...
          const std::string  filename = this->get_output_directory() + "solution/" + solution_file_prefix + "." +
                                       Utilities::int_to_string(myid, 4) +
                                       DataOutBase::default_suffix(DataOutBase::parse_output_format(output_format));
          std::ofstream out(filename);
          AssertThrow(out, ExcMessage("Unable to open file for writing: " + filename + "."));
                                         ...
                                         std::map<std::string, std::string> &output_data_names_and_units)
      {
        ...
        for (const auto &name : output_data_names)
          AssertThrow(output_data_names_and_units.find(name) == output_data_names_and_units.end(),
                      ExcMessage("The output variable <" + name +
                                 "> already exists in the list of output "
        ...
        for (unsigned int i = 0; i < output_data_names.size(); ++i)
          {
            AssertThrow(
              ...
              (output_data_names_and_units.find(output_data_names[i]) == output_data_names_and_units.end()) ||
              ...
              ExcMessage("The output variable <" + output_data_names[i] + "> had previously been added with physical units <" +
                         output_data_names_and_units[output_data_names[i]] + "> but is now added again with units <" + output_units[i] +
                         ">."));
        ...
        if (output_units.find(',') != output_units.npos)
          {
            ...
            track_output_field_names_and_units(output_data_names, split_units, output_data_names_and_units);
          }
        else
    ...
    Visualization<dim>::execute(TableHandler &statistics)
    {
      ...
      if ((this->get_time() < last_output_time + output_interval) &&
          (this->get_timestep_number() < last_output_timestep + maximum_timesteps_between_outputs) && (this->get_timestep_number() != 0))
        return {"", ""};
      ...
      const bool increase_file_number =
        (this->get_nonlinear_iteration() == 0) || (!this->get_parameters().run_postprocessors_on_nonlinear_iterations);
      ...
      data_out.add_data_vector(this->get_solution(), base_variables);
      ...
      if (output_base_variables_on_mesh_surface)
        data_out_faces.add_data_vector(this->get_solution(), surface_base_variables);
      ...
      if (this->get_parameters().mesh_deformation_enabled && output_mesh_velocity)
        {
          ...
          data_out.add_data_vector(this->get_mesh_velocity(), *mesh_deformation_velocity);
        }
      ...
      for (const auto &p : postprocessors)
        {
          try
            {
              ...
              if (const DataPostprocessor<dim> *viz_postprocessor = dynamic_cast<const DataPostprocessor<dim> *>(&*p))
                {
                  track_output_field_names_and_units(viz_postprocessor->get_names(),
                                                     p->get_physical_units(),
                                                     visualization_field_names_and_units);

                  if (dynamic_cast<const VisualizationPostprocessors::SurfaceOnlyVisualization<dim> *>(&*p) == nullptr)
                    data_out.add_data_vector(this->get_solution(), *viz_postprocessor);
                  else
                    {
                      data_out_faces.add_data_vector(this->get_solution(), *viz_postprocessor);
                      // Apart from the base variables, other postprocessors
                      // output on the mesh surface.
                      have_face_viz_postprocessors = true;
                    }
                }
              else if (const VisualizationPostprocessors::CellDataVectorCreator<dim> *cell_data_creator =
                         dynamic_cast<const VisualizationPostprocessors::CellDataVectorCreator<dim> *>(&*p))
                {
                  // get the data produced here
                  std::pair<std::string, std::unique_ptr<Vector<float>>> cell_data = cell_data_creator->execute();
                  Assert(cell_data.second->size() == this->get_triangulation().n_active_cells(),
                  ...
                  if (dynamic_cast<const VisualizationPostprocessors::SurfaceOnlyVisualization<dim> *>(&*p) == nullptr)
                    data_out.add_data_vector(*cell_data_vectors.back(), cell_data.first, DataOut<dim>::type_cell_data);
                  else
                    {
                      data_out_faces.add_data_vector(*cell_data_vectors.back(), cell_data.first, DataOutFaces<dim>::type_cell_data);
                      // Apart from the base variables, other postprocessors
                      // output on the mesh surface.
                      have_face_viz_postprocessors = true;
                    }
          ...
          // and abort the program
          catch (std::exception &exc)
            {
              ...
              std::cerr << "An exception happened on MPI process <" << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
                        << "> while running the visualization postprocessor <" << typeid(*p).name() << ">: " << std::endl
                        << exc.what() << std::endl
                        << "Aborting!" << std::endl
                        << "----------------------------------------------------" << std::endl;
      ...
      const unsigned int subdivisions = interpolate_output ? this->get_stokes_velocity_degree() : 0;
      ...
      std::string solution_file_prefix;
      {
        data_out.build_patches(mapping,
                               subdivisions,
                               this->get_geometry_model().has_curved_elements() ? DataOut<dim>::curved_inner_cells :
                                                                                  DataOut<dim>::no_curved_cells);

        solution_file_prefix = write_data_out_data(data_out, cell_output_history, visualization_field_names_and_units);
        statistics.add_value("Visualization file name", this->get_output_directory() + "solution/" + solution_file_prefix);
      }
      ...
      if (output_base_variables_on_mesh_surface || have_face_viz_postprocessors)
        {
          data_out_faces.build_patches(mapping, subdivisions);

          const std::string face_solution_file_prefix =
            write_data_out_data(data_out_faces, face_output_history, visualization_field_names_and_units);
          statistics.add_value("Surface visualization file name",
                               this->get_output_directory() + "solution_surface/" + face_solution_file_prefix);
        }
      ...
      // update.
      return std::make_pair(std::string("Writing graphical output:"), this->get_output_directory() + "solution/" + solution_file_prefix);
    }


    template <int dim>
    void
    Visualization<dim>::writer(const std::string &filename, const std::string &temporary_output_location, const std::string &file_contents)
    {
      ...
      if (temporary_output_location != "")
        {
          ...
          if (tmp_file_desc == -1)
            {
              const std::string x = ("***** WARNING: could not create temporary file <" + tmp_filename +
      ...
      AssertThrow(out, ExcMessage(std::string("Trying to write to file <") + filename + ">, but the file can't be opened!"));
      ...
      if (tmp_filename != filename)
        {
          ...
          AssertThrow(error == 0,
                      ExcMessage("Could not move " + tmp_filename + " to " + filename + ". On processor " +
                                 Utilities::int_to_string(Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)) + "."));
        }
    }
    ...
    namespace
    {
      std::tuple<aspect::internal::Plugins::UnusablePluginList,
                 ...
                 aspect::internal::Plugins::PluginList<VisualizationPostprocessors::Interface<3>>>
        registered_visualization_plugins;
    }
    ...
    Visualization<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Postprocess");
      {
        ...
        {
          prm.declare_entry("Time between graphical output",
                            "1e8",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Output format",
                            "vtu",
                            Patterns::Selection(DataOutBase::get_output_format_names() + "|parallel deal.II intermediate"),
          ...
          prm.declare_entry("Number of grouped files",
                            "16",
                            Patterns::Integer(0),
          ...
          prm.declare_entry("Write in background thread",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Interpolate output",
                            "true",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Point-wise stress and strain",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Write higher order output",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Filter output",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Output mesh velocity",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Output mesh displacement",
                            "false",
                            Patterns::Bool(),
                            "For computations with deforming meshes, ASPECT uses an Arbitrary-Lagrangian-"
          ...
          prm.declare_entry("Output undeformed mesh",
                            "false",
                            Patterns::Bool(),
                            "For computations with deforming meshes, ASPECT uses an Arbitrary-Lagrangian-"
          ...
          prm.declare_entry("Output base variables on mesh surface",
                            "false",
                            Patterns::Bool(),
          ...
          const std::string pattern_of_names = std::get<dim>(registered_visualization_plugins).get_pattern_of_names();
    ...
    {
      Assert(std::get<dim>(registered_visualization_plugins).plugins != nullptr, ExcMessage("No postprocessors registered!?"));
      std::vector<std::string> viz_names;

      const std::string visualization_subdirectory = this->get_output_directory() + "solution/";
      Utilities::create_directory(visualization_subdirectory, this->get_mpi_communicator(), true);
      ...
      prm.enter_subsection("Postprocess");
      {
        prm.enter_subsection("Visualization");
        {
          ...
          if (std::find(viz_names.begin(), viz_names.end(), "all") != viz_names.end())
            {
              viz_names.clear();
      ...
      for (const auto &viz_name : viz_names)
        {
          std::unique_ptr<VisualizationPostprocessors::Interface<dim>> viz_postprocessor =
            std::get<dim>(registered_visualization_plugins).create_plugin(viz_name, "Visualization plugins");

          // make sure that the postprocessor is indeed of type
          // dealii::DataPostprocessor or of type
          // VisualizationPostprocessors::CellDataVectorCreator
          Assert((dynamic_cast<DataPostprocessor<dim> *>(viz_postprocessor.get()) != nullptr) ||
                   (dynamic_cast<VisualizationPostprocessors::CellDataVectorCreator<dim> *>(viz_postprocessor.get()) != nullptr),
      ...
      cell_output_history.mesh_changed = true;
      face_output_history.mesh_changed = true;
      this->get_triangulation().signals.post_refinement.connect([&]() { this->mesh_changed_signal(); });
    }
    ...
    {
      ar &last_output_time &last_output_timestep &output_file_number &cell_output_history &face_output_history;
    }
      ...
      if (output_interval > 0)
        {
          ...
          last_output_time =
            last_output_time + std::floor((current_time - last_output_time) / output_interval * magic) * output_interval / magic;
        }
    }
    ...
    template <int dim>
    void
    Visualization<dim>::register_visualization_postprocessor(
      const std::string &name,
      const std::string &description,
      void (*declare_parameters_function)(ParameterHandler &),
      std::unique_ptr<VisualizationPostprocessors::Interface<dim>> (*factory_function)())
    {
      std::get<dim>(registered_visualization_plugins).register_plugin(name, description, declare_parameters_function, factory_function);
    }
      ...
      for (const auto &p : postprocessors)
        {
          ...
          requirements.insert(requirements.end(), this_requirements.begin(), this_requirements.end());
        }
    ...
    Visualization<dim>::write_plugin_graph(std::ostream &out)
    {
      ...
      std::get<dim>(registered_visualization_plugins)
        .write_plugin_graph("Visualization postprocessor interface", out, typeid(Visualization<dim>).name());
    }
