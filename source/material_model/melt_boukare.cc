  ...
  namespace MaterialModel
  {
    ...
    MeltBoukare<dim>::initialize()
    {
      ...
      for (unsigned int i = 0; i < n_endmembers; ++i)
        {
          tait_parameters_a[i] =
            (1. + bulk_modulus_pressure_derivatives[i]) /
    ...
    template <int dim>
    double
    MeltBoukare<dim>::reference_darcy_coefficient() const
    {
      // 0.01 = 1% melt
    ...
    template <int dim>
    bool
    MeltBoukare<dim>::is_compressible() const
    {
      return true;
    }
    ...
    template <int dim>
    MeltBoukare<dim>::EndmemberProperties::EndmemberProperties(const unsigned int n_endmembers)
    ...
    template <int dim>
    void
    MeltBoukare<dim>::fill_endmember_properties(const typename Interface<dim>::MaterialModelInputs &in,
      ...
      for (unsigned int i = 0; i < n_endmembers; ++i)
        {
          ...
          const double heat_capacity_ratio =
            endmember_molar_heat_capacity(in.temperature[q], i) / endmember_molar_heat_capacity(reference_temperature, i);
          ...
          if (pressure != reference_pressure && pressure > 0.0)
            {
              intVdP = reference_volumes[i] *
                       ((pressure - reference_pressure) * (1. - a) +
                        (a * (std::pow((1. + b * (reference_pressure - Pth)), 1. - c) - std::pow((1. + b * (pressure - Pth)), 1. - c)) /
                         (b * (c - 1.))));

              const double prefactor =
                reference_volumes[i] * reference_thermal_expansivities[i] * reference_bulk_moduli[i] * a * heat_capacity_ratio;
          ...
          properties.bulk_moduli[i] =
            reference_bulk_moduli[i] * (1. + b * (pressure - Pth)) * (a + (1. - a) * std::pow(1. + b * (pressure - Pth), c));
          ...
          properties.thermal_expansivities[i] = reference_thermal_expansivities[i] * (C_V / C_V0) * 1. /
                                                ((1. + b * (pressure - Pth)) * (a + (1. - a) * std::pow(1 + b * (pressure - Pth), c)));
          ...
          const long double dSdT0 = reference_volumes[i] * reference_bulk_moduli[i] *
                                    Utilities::fixed_power<2>(heat_capacity_ratio * reference_thermal_expansivities[i]) *
    ...
    template <int dim>
    double
    MeltBoukare<dim>::endmember_thermal_energy(const double temperature, const unsigned int endmember_index) const
    {
      AssertThrow(temperature > 0.0,
                  ExcMessage("The temperature has to be larger than 0, but it is " + std::to_string(temperature) + " for endmember " +
                             std::to_string(endmember_index) + "."));
    ...
    template <int dim>
    double
    MeltBoukare<dim>::endmember_molar_heat_capacity(const double temperature, const unsigned int endmember_index) const
    {
      AssertThrow(temperature > 0.0, ExcMessage("The temperature has to be larger than 0!"));
    ...
    template <int dim>
    double
    MeltBoukare<dim>::endmember_thermal_pressure(const double temperature, const unsigned int endmember_index) const
    {
      ...
      const double thermal_pressure =
        reference_thermal_expansivities[endmember_index] * reference_bulk_moduli[endmember_index] / heat_capacity * thermal_energy;
    ...
    template <int dim>
    double
    MeltBoukare<dim>::endmember_enthalpy_thermal_addition(const double temperature, const unsigned int i) const
    {
      const double addition =
        reference_specific_heats[i] * temperature + 0.5 * specific_heat_linear_coefficients[i] * Utilities::fixed_power<2>(temperature) -
        specific_heat_second_coefficients[i] / temperature + 2. * specific_heat_third_coefficients[i] * std::sqrt(temperature) -
    ...
    template <int dim>
    double
    MeltBoukare<dim>::endmember_entropy_thermal_addition(const double temperature, const unsigned int i) const
    {
      const double addition =
        reference_specific_heats[i] * std::log(temperature) + specific_heat_linear_coefficients[i] * temperature -
        ...
        (reference_specific_heats[i] * std::log(reference_temperature) + specific_heat_linear_coefficients[i] * reference_temperature -
    ...
    template <int dim>
    void
    MeltBoukare<dim>::convert_composition_to_fraction_of_endmembers(const double               temperature,
                                                                    ...
                                                                    double                    &molar_bridgmanite_in_solid) const
    {
      ...
      const double x_SiO2 =
        molar_Fe_in_solid * molar_SiO2_in_Fe_mantle_endmember + (1. - molar_Fe_in_solid) * molar_SiO2_in_Mg_mantle_endmember;
      ...
      endmember_mole_fractions_per_phase[febdg_idx] =
        (-1. + molar_fraction_FeO - (molar_fraction_FeO * partition_coefficient) + molar_fraction_SiO2 -
         (molar_fraction_SiO2 * partition_coefficient) + std::sqrt(num_to_sqrt)) /
        (2. * molar_fraction_SiO2 * (1. - partition_coefficient));

      endmember_mole_fractions_per_phase[wus_idx] =
        endmember_mole_fractions_per_phase[febdg_idx] /
        (((1. - endmember_mole_fractions_per_phase[febdg_idx]) * partition_coefficient) + endmember_mole_fractions_per_phase[febdg_idx]);
    ...
    template <int dim>
    double
    MeltBoukare<dim>::compute_melt_molar_fraction(const double               porosity,
      ...
      for (unsigned int i = 0; i < endmember_names.size(); ++i)
        {
          ...
          const double endmember_phase_fraction_in_solid =
            i < 2 ? bridgmanite_molar_fraction_in_solid : 1.0 - bridgmanite_molar_fraction_in_solid;
    ...
    template <int dim>
    double
    MeltBoukare<dim>::assert_update_is_within_0_and_1(const double old_value, const double change_of_value) const
    {
    ...
    template <int dim>
    double
    MeltBoukare<dim>::melt_fraction(const double temperature,
                                    ...
                                    double      &new_molar_composition_of_melt) const
    {
      ...
      const double molar_volatiles_in_bulk = 1.e-4;
      {
        ...
        const double dG_Fe_mantle =
          (Fe_mantle_melting_temperature - T) * Fe_mantle_melting_entropy + (P - melting_reference_pressure) * Fe_mantle_melting_volume;
        const double dG_Mg_mantle =
          (Mg_mantle_melting_temperature - T) * Mg_mantle_melting_entropy + (P - melting_reference_pressure) * Mg_mantle_melting_volume;
        ...
        const double q1 = c_Fe_endmember * (1. - 1. / c_Mg_endmember) -
                          molar_composition_of_bulk * (c_Fe_endmember - c_Mg_endmember) / c_Mg_endmember +
                          molar_volatiles_in_bulk * (1. - c_Fe_endmember);
        ...
        if (molar_composition_of_bulk < std::numeric_limits<double>::min())
        ...
        else if (molar_composition_of_melt <= molar_composition_of_bulk && std::min(T_Fe_mantle, T_Mg_mantle) < 0.0) // above the liquidus
          {
        ...
        else
          {
            ...
            if (std::abs(molar_composition_of_melt - molar_composition_of_solid) > std::numeric_limits<double>::min())
              melt_molar_fraction = std::clamp((molar_composition_of_bulk - molar_composition_of_solid) /
                                                 (molar_composition_of_melt - molar_composition_of_solid),
                                               0.0,
                                               1.0);
            ...
            if (molar_volatiles_in_bulk > 0.0 &&
                std::abs(molar_composition_of_bulk - molar_composition_of_solid) > std::numeric_limits<double>::min())
              molar_volatiles_in_melt = molar_volatiles_in_bulk * (molar_composition_of_melt - molar_composition_of_solid) /
                                        (molar_composition_of_bulk - molar_composition_of_solid);
          }
    ...
    template <int dim>
    void
    MeltBoukare<dim>::melt_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
      ...
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        {
          ...
          const double solid_molar_volume =
    ...
    template <int dim>
    void
    MeltBoukare<dim>::evaluate(const typename Interface<dim>::MaterialModelInputs &in,
                               typename Interface<dim>::MaterialModelOutputs      &out) const
    {
                  ...
                  for (unsigned int q = 0; q < n_points; ++q)
                    {
                      ...
                      melt_out->permeabilities[q] =
                        reference_permeability * Utilities::fixed_power<3>(porosity) * Utilities::fixed_power<2>(1.0 - porosity);
                    }
                }
              return;
            }
      ...
      if (this->simulator_is_past_initialization())
        {
          const unsigned int number_of_reaction_steps =
            std::max(static_cast<unsigned int>(this->get_timestep() / this->get_parameters().reaction_time_step),
      ...
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        {
          ...
          for (unsigned int i = 0; i < n_endmembers; ++i)
            {
              out.thermal_expansion_coefficients[q] +=
                (endmember_mole_fractions_in_composite[i] * endmembers.volumes[i] * endmembers.thermal_expansivities[i]) / total_volume;
              ...
              if (endmember_states[i] == EndmemberState::solid && solid_molar_volume > 0.0)
                {
                  out.compressibilities[q] +=
                    (endmember_mole_fractions_in_composite[i] * endmembers.volumes[i]) / (solid_molar_volume * endmembers.bulk_moduli[i]);
                }
            }
          ...
          if (melt_out != nullptr)
            {
              ...
              for (unsigned int i = 0; i < n_endmembers; ++i)
                if (endmember_states[i] == EndmemberState::melt && melt_molar_volume > 0.0)
                  melt_compressiblity +=
                    (endmember_mole_fractions_in_composite[i] * endmembers.volumes[i]) / (melt_molar_volume * endmembers.bulk_moduli[i]);
              ...
              melt_out->fluid_density_gradients[q] = melt_out->fluid_densities[q] * melt_out->fluid_densities[q] * melt_compressiblity *
          ...
          if (this->include_melt_transport())
            {
              ...
              const double change_of_melt_composition =
                reaction_fraction * assert_update_is_within_0_and_1(old_melt_composition, melt_composition - old_melt_composition);
              const double change_of_melt_fraction =
                reaction_fraction * assert_update_is_within_0_and_1(old_melt_molar_fraction, melt_molar_fraction - old_melt_molar_fraction);
              ...
              if (melt_molar_fraction > 0.0 && melt_molar_fraction < 1.0)
                solid_composition = (bulk_composition - melt_molar_fraction * melt_composition) / (1.0 - melt_molar_fraction);
              ...
              if (enthalpy_out != nullptr)
                {
                  ...
                  const double Fe_enthalpy_of_fusion =
                    Fe_mantle_melting_temperature * Fe_mantle_melting_entropy +
                    (this->get_adiabatic_conditions().pressure(in.position[q]) - melting_reference_pressure) * Fe_mantle_melting_volume;
                  const double Mg_enthalpy_of_fusion =
                    Mg_mantle_melting_temperature * Mg_mantle_melting_entropy +
          ...
          const double visc_temperature_dependence =
            std::clamp(std::exp(-thermal_viscosity_exponent * delta_temp / this->get_adiabatic_conditions().temperature(in.position[q])),
                       1. / viscosity_bound,
                       viscosity_bound);
          ...
          for (unsigned int q = 0; q < n_points; ++q)
            {
              ...
              const double porosity_threshold =
                0.01 * std::pow(this->get_melt_handler().melt_parameters.melt_scaling_factor_threshold, 1. / 3.);
              ...
              const double visc_temperature_dependence = std::clamp(std::exp(-thermal_bulk_viscosity_exponent * delta_temp /
                                                                             this->get_adiabatic_conditions().temperature(in.position[q])),
                                                                    1. / compaction_viscosity_bound,
                                                                    compaction_viscosity_bound);
    ...
    MeltBoukare<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt boukare");
        {
          ...
          prm.declare_entry("Thermal bulk viscosity exponent",
                            "0.0",
                            Patterns::Double(0),
          ...
          prm.declare_entry("Reference permeability",
                            "1e-8",
                            Patterns::Double(),
                            "Reference permeability of the solid host rock."
                            "Units: \\si{\\meter\\squared}.");
          prm.declare_entry("Include melting and freezing",
                            "true",
                            Patterns::Bool(),
                            ...
                            "false).");
          prm.declare_entry("Melting time scale for operator splitting",
                            "1e3",
                            Patterns::Double(0),
          ...
          prm.declare_entry("Fe mantle melting temperature",
                            "3424.5",
                            Patterns::Double(),
                            "The melting temperature of one of the components in the melting "
                            "model, the Fe mantle endmember."
                            "Units: \\si{\\kelvin}.");
          prm.declare_entry("Mg mantle melting temperature",
                            "4821.2",
                            Patterns::Double(),
                            "The melting temperature of one of the components in the melting "
                            "model, the Mg mantle endmember."
                            "Units: \\si{\\kelvin}.");
          prm.declare_entry("Fe number of moles",
                            "0.48",
                            Patterns::Double(),
                            ...
                            "Units: none.");
          prm.declare_entry("Mg number of moles",
                            "0.62",
                            Patterns::Double(),
          ...
          prm.declare_entry("Reference temperature",
                            ...
                            "Units: \\si{\\kelvin}.");
          prm.declare_entry("Reference pressure",
                            "1e11",
                            Patterns::Double(),
          ...
          prm.declare_entry("Endmember names",
                            ...
                            Patterns::List(Patterns::MultipleSelection(
                              "MgSiO3_bridgmanite|FeSiO3_bridgmanite|MgO_periclase|FeO_periclase|MgO_melt|FeO_melt|SiO2_melt")),
                            ...
                            "are given in. "
                            "Units: none.");
          prm.declare_entry("Endmember states",
          ...
          prm.declare_entry("Molar masses",
          ...
          prm.declare_entry("Reference thermal expansivities",
                            ...
                            "Units: \\si{\\per\\kelvin}.");
          prm.declare_entry("Reference bulk moduli",
                            ...
                            "and reference pressure."
                            "Units: \\si{\\pascal}.");
          prm.declare_entry("First derivatives of the bulk modulus",
                            ...
                            "Units: none.");
          prm.declare_entry("Second derivatives of the bulk modulus",
          ...
          prm.declare_entry("Reference entropies",
          ...
          prm.declare_entry("Reference specific heat capacities",
          ...
          prm.declare_entry("Third coefficients for specific heat polynomial",
    ...
    MeltBoukare<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt boukare");
        {
          ...
          AssertThrow(melting_time_scale > 0.0, ExcMessage("The Melting time scale for operator splitting must be larger than 0!"));
          ...
          bulk_modulus_second_pressure_derivatives =
            Utilities::MapParsing::parse_map_to_double_array(prm.get(options.property_name), options);
    ...
    MeltBoukare<dim>::create_additional_named_outputs(MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      if (out.template has_additional_output_object<BoukareOutputs<dim>>() == false)
        {
          out.additional_outputs.push_back(std::make_unique<MaterialModel::BoukareOutputs<dim>>(out.n_evaluation_points()));
        }
    }
  }
}
