...
namespace aspect
{
  namespace MaterialModel
  {
    namespace ReactionModel
    {
      ...
      template <int dim>
      double
      Tian2019Solubility<dim>::melt_fraction(const MaterialModel::MaterialModelInputs<dim> &in,
                                             ...
                                             unsigned int                                   q) const
      {
        ...
        double average_eq_bound_water_content =
          MaterialUtilities::average_value(tracked_rock_mass_fractions, tian_eq_bound_water_content, MaterialUtilities::arithmetic);
      ...
      template <int dim>
      std::vector<double>
      Tian2019Solubility<dim>::tian_equilibrium_bound_water_content(const MaterialModel::MaterialModelInputs<dim> &in, unsigned int q) const
      {
        ...
        for (unsigned int i = 0; i < devolatilization_enthalpy_changes.size(); ++i)
          {
            ...
            for (unsigned int j = 0; j < devolatilization_enthalpy_changes[i].size(); ++j)
              {
                LR_values[i] += devolatilization_enthalpy_changes[i][j] *
                                Utilities::pow(inverse_pressure_for_reaction, devolatilization_enthalpy_changes[i].size() - 1 - j);
              }

            for (unsigned int j = 0; j < water_mass_fractions[i].size(); ++j)
              {
                csat_values[i] += i == 3 ? water_mass_fractions[i][j] * Utilities::pow(std::log10(pressure_for_reaction_in_GPa),
                                                                                       water_mass_fractions[i].size() - 1 - j) :
                                           water_mass_fractions[i][j] *
                                             Utilities::pow(pressure_for_reaction_in_GPa, water_mass_fractions[i].size() - 1 - j);
              }

            for (unsigned int j = 0; j < devolatilization_onset_temperatures[i].size(); ++j)
              {
                Td_values[i] += devolatilization_onset_temperatures[i][j] *
                                Utilities::pow(pressure_for_reaction_in_GPa, devolatilization_onset_temperatures[i].size() - 1 - j);
              }
          }
        ...
        std::vector<double> max_bound_water_content = {tian_max_peridotite_water,
        ...
        for (unsigned int k = 0; k < LR_values.size(); ++k)
          {
            eq_bound_water_content[k] =
              (std::min(std::exp(csat_values[k]) * std::exp(std::exp(LR_values[k]) * (1 / in.temperature[q] - 1 / Td_values[k])),
                        max_bound_water_content[k]) /
               100.0);
          }
      ...
      {
        prm.declare_entry("Use adiabatic pressure for reactions",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Maximum weight percent water in sediment",
                          "3",
                          Patterns::Double(0),
                          "The maximum allowed weight percent that the sediment composition can hold.");
        prm.declare_entry("Maximum weight percent water in MORB",
                          "2",
                          Patterns::Double(0),
                          "The maximum allowed weight percent that the sediment composition can hold.");
        prm.declare_entry("Maximum weight percent water in gabbro",
                          "1",
                          Patterns::Double(0),
                          "The maximum allowed weight percent that the sediment composition can hold.");
        prm.declare_entry("Maximum weight percent water in peridotite",
                          "8",
                          Patterns::Double(0),
                          "The maximum allowed weight percent that the sediment composition can hold.");
      }
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace ReactionModel
    {
#define INSTANTIATE(dim) template class Tian2019Solubility<dim>;
