...
namespace aspect
{
  namespace MaterialModel
  {
    namespace ReactionModel
    {
      template <int dim>
      double
      Katz2003MantleMelting<dim>::reference_darcy_coefficient() const
      {
        // 0.01 = 1% melt
      ...
      template <int dim>
      double
      Katz2003MantleMelting<dim>::melt_fraction(const double temperature, const double pressure) const
      {
      ...
      template <int dim>
      double
      Katz2003MantleMelting<dim>::entropy_change(const double                          temperature,
        ...
        if (temperature > T_solidus && temperature < T_liquidus && pressure < 1.3e10)
          {
            ...
            double melt_fraction_derivative_temperature =
              beta * std::pow((temperature - T_solidus) / (T_lherz_liquidus - T_solidus), beta - 1) / (T_lherz_liquidus - T_solidus);

            double melt_fraction_derivative_pressure =
              beta * std::pow((temperature - T_solidus) / (T_lherz_liquidus - T_solidus), beta - 1) *
              (dT_solidus_dp * (temperature - T_lherz_liquidus) + dT_lherz_liquidus_dp * (T_solidus - temperature)) /
            ...
            if (melt_fraction(temperature, pressure) > F_max)
              {
                ...
                melt_fraction_derivative_temperature =
                  (1.0 - F_max) * beta * std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) / (T_liquidus - T_max);

                melt_fraction_derivative_pressure =
                  dF_max_dp - dF_max_dp * std::pow((temperature - T_max) / (T_liquidus - T_max), beta) +
                  (1.0 - F_max) * beta * std::pow((temperature - T_max) / (T_liquidus - T_max), beta - 1) *
                    (dT_max_dp * (T_max - T_liquidus) - (dT_liquidus_dp - dT_max_dp) * (temperature - T_max)) /
                    Utilities::fixed_power<2>(T_liquidus - T_max);
              }
      ...
      template <int dim>
      void
      Katz2003MantleMelting<dim>::calculate_reaction_rate_outputs(const typename Interface<dim>::MaterialModelInputs &in,
                                                                  typename Interface<dim>::MaterialModelOutputs      &out) const
      {
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          {
            const double maximum_melt_fraction =
              this->include_melt_transport() ? in.composition[i][this->introspection().compositional_index_for_name("peridotite")] : 0.0;
            ...
            if (this->include_melt_transport() && in.requests_property(MaterialProperties::reaction_rates))
              {
                ...
                if (fractional_melting)
                  {
                    ...
                    const double eq_melt_fraction =
                      melt_fraction(in.temperature[i] - solidus_change, this->get_adiabatic_conditions().pressure(in.position[i]));
                ...
                else
                  {
                    ...
                    const double eq_melt_fraction =
                      melt_fraction(in.temperature[i], this->get_adiabatic_conditions().pressure(in.position[i]));
                ...
                if (this->get_geometry_model().depth(in.position[i]) < extraction_depth)
                  porosity_change = -old_porosity * (in.position[i](1) - (this->get_geometry_model().maximal_depth() - extraction_depth)) /
                                    extraction_depth;
            ...
            if (in.requests_property(MaterialProperties::entropy_derivative_pressure))
              out.entropy_derivative_pressure[i] = entropy_change(in.temperature[i],
            ...
            if (in.requests_property(MaterialProperties::entropy_derivative_temperature))
              out.entropy_derivative_temperature[i] = entropy_change(in.temperature[i],
                                                                     this->get_adiabatic_conditions().pressure(in.position[i]),
                                                                     maximum_melt_fraction,
      ...
      template <int dim>
      void
      Katz2003MantleMelting<dim>::calculate_fluid_outputs(const typename Interface<dim>::MaterialModelInputs &in,
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                ...
                const double fluid_compressibility =
                  melt_compressibility / (1.0 + in.pressure[i] * melt_bulk_modulus_derivative * melt_compressibility);

                melt_out->fluid_densities[i] = reference_rho_fluid *
                                               std::exp(fluid_compressibility * (in.pressure[i] - this->get_surface_pressure())) *
                                               temperature_dependence;

                melt_out->fluid_density_gradients[i] = melt_out->fluid_densities[i] * melt_out->fluid_densities[i] * fluid_compressibility *
                ...
                if (this->include_adiabatic_heating())
                  {
                    ...
                    visc_temperature_dependence = std::clamp(std::exp(-thermal_bulk_viscosity_exponent * delta_temp /
                ...
                else
                  {
                    ...
                    const double T_dependence =
                      (thermal_bulk_viscosity_exponent == 0.0 ? 0.0 : thermal_bulk_viscosity_exponent * delta_temp / reference_T);
      ...
      Katz2003MantleMelting<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Mass fraction cpx",
                          ...
                          "Units: non-dimensional.");
        prm.declare_entry("Peridotite melting entropy change",
                          "-300.",
                          Patterns::Double(),
                          "The entropy change for the phase transition "
                          "from solid to melt of peridotite. "
                          "Units: \\si{\\joule\\per\\kelvin\\per\\kilogram}.");
        prm.declare_entry("Reference melt density",
                          "2500.",
                          Patterns::Double(0.),
        ...
        prm.declare_entry("Melt extraction depth",
                          "1000.0",
                          Patterns::Double(0.),
                          ...
                          "porosity field. "
                          "Units: \\si{\\meter}.");
        prm.declare_entry("Melt compressibility",
                          "0.0",
                          Patterns::Double(0.),
                          "The value of the compressibility of the melt. "
                          "Units: \\si{\\per\\pascal}.");
        prm.declare_entry("Melt bulk modulus derivative",
                          "0.0",
                          Patterns::Double(0.),
                          "The value of the pressure derivative of the melt bulk "
                          "modulus. "
                          "Units: None.");
        prm.declare_entry("Use fractional melting",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Freezing rate",
                          "0.0",
                          Patterns::Double(0.),
        ...
        prm.declare_entry("Depletion solidus change",
                          "200.0",
                          Patterns::Double(0.),
