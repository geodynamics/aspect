...
namespace aspect
{
  namespace MaterialModel
  {
    namespace ReactionModel
    {
      template <int dim>
      double
      EutectoidDecomposition<dim>::net_forward_reaction_rate(const double temperature,
                                                             ...
                                                             const unsigned int reaction_index) const
      {
        ...
        AssertThrow(activation_energy >= 0.0,
                    ExcMessage("Activation energy must be non-negative, "
                               "but received " +
                               std::to_string(activation_energy) + " J/mol."));
      ...
      EutectoidDecomposition<dim>::parse_parameters(ParameterHandler &prm, const unsigned int n_reactions)
      {
        prm.enter_subsection("Eutectoid decomposition");
        {
          ...
          AssertThrow(kinetic_prefactors.size() == n_reactions && activation_energies.size() == n_reactions,
                      ExcMessage("'Eutectoid decomposition' parameter lists must each have exactly " + std::to_string(n_reactions) +
