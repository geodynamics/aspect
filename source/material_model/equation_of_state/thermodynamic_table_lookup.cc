  ...
  namespace MaterialModel
  {
    namespace EquationOfState
    {
      ...
      ThermodynamicTableLookup<dim>::initialize()
      {
        ...
        for (unsigned i = 0; i < n_material_lookups; ++i)
          {
            if (material_file_format == perplex)
              material_lookup.push_back(std::make_unique<MaterialModel::MaterialUtilities::Lookup::PerplexReader>(
                data_directory + material_file_names[i], use_bilinear_interpolation, this->get_mpi_communicator()));
            else if (material_file_format == hefesto)
              material_lookup.push_back(
                std::make_unique<MaterialModel::MaterialUtilities::Lookup::HeFESToReader>(data_directory + material_file_names[i],
            ...
            for (const auto &phase_volume_column_name : phase_volume_column_names)
              {
                ...
                std::vector<std::string>::iterator it =
                  std::find(unique_phase_names.begin(), unique_phase_names.end(), phase_volume_column_name);
            ...
            for (const auto &phase_name : phase_names_one_lookup)
              {
                std::vector<std::string>::iterator it =
                  std::find(list_of_dominant_phases.begin(), list_of_dominant_phases.end(), phase_name);
        ...
        const std::string filename = (this->get_output_directory() + "thermodynamic_lookup_table_phases.txt");

        if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0 && list_of_dominant_phases.size() > 0)
          {
            std::ofstream file;
            file.open(filename);
            ...
            for (unsigned int p = 0; p < list_of_dominant_phases.size(); ++p)
              {
                file << p << ' ' << list_of_dominant_phases[p] << std::endl;
              }
            AssertThrow(file,
                        ExcMessage("Writing data to <" + filename +
      ...
      template <int dim>
      bool
      ThermodynamicTableLookup<dim>::is_compressible() const
      {
        return true;
      }
      ...
      template <int dim>
      void
      ThermodynamicTableLookup<dim>::fill_seismic_velocities(const MaterialModel::MaterialModelInputs<dim> &in,
            ...
            if (material_lookup.size() == 1)
            ...
            else
              {
                ...
                for (unsigned int j = 0; j < material_lookup.size(); ++j)
                  {
                    const double mu = material_lookup[j]->density(in.temperature[i], in.pressure[i]) *
                                      Utilities::fixed_power<2>(material_lookup[j]->seismic_Vs(in.temperature[i], in.pressure[i]));
                    const double k = material_lookup[j]->density(in.temperature[i], in.pressure[i]) *
                                       Utilities::fixed_power<2>(material_lookup[j]->seismic_Vp(in.temperature[i], in.pressure[i])) -
                                     4. / 3. * mu;
      ...
      template <int dim>
      void
      ThermodynamicTableLookup<dim>::fill_phase_volume_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
                                                                 const std::vector<std::vector<double>>        &volume_fractions,
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          for (unsigned j = 0; j < material_lookup.size(); ++j)
            for (unsigned int k = 0; k < unique_phase_indices[j].size(); ++k)
              phase_volume_fractions[unique_phase_indices[j][k]][i] +=
                volume_fractions[i][j] * material_lookup[j]->phase_volume_fraction(k, in.temperature[i], in.pressure[i]);
      ...
      template <int dim>
      void
      ThermodynamicTableLookup<dim>::fill_dominant_phases(const MaterialModel::MaterialModelInputs<dim> &in,
                                                          const std::vector<std::vector<double>>        &volume_fractions,
                                                          PhaseOutputs<dim>                             &dominant_phases_out) const
      {
        ...
        // the index i corresponds to the ith evaluation point
        // the index j corresponds to the jth compositional field
        std::vector<std::vector<double>> dominant_phase_indices(1,
                                                                std::vector<double>(in.n_evaluation_points(),
                                                                                    std::numeric_limits<double>::quiet_NaN()));
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          {
            const unsigned int dominant_material_index =
              std::distance(volume_fractions[i].begin(), std::max_element(volume_fractions[i].begin(), volume_fractions[i].end()));
            const unsigned int dominant_phase_in_material =
              material_lookup[dominant_material_index]->dominant_phase(in.temperature[i], in.pressure[i]);
      ...
      ThermodynamicTableLookup<dim>::enthalpy_derivatives(const typename Interface<dim>::MaterialModelInputs &in) const
      {
        ...
        fe_values[this->introspection().extractors.temperature].get_function_values(this->get_current_linearization_point(), temperatures);
        fe_values[this->introspection().extractors.pressure].get_function_values(this->get_current_linearization_point(), pressures);
      ...
      template <int dim>
      void
      ThermodynamicTableLookup<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim>           &in,
      ...
      template <int dim>
      void
      ThermodynamicTableLookup<dim>::evaluate_thermal_enthalpy_derivatives(
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          {
            ...
            if ((in.current_cell.state() == IteratorState::valid) && (dH[0].second > 0) && (dH[1].second > 0))
              {
              ...
              {
                eos_outputs[i].thermal_expansion_coefficients[0] =
                  (1 - eos_outputs[i].densities[0] * material_lookup[0]->dHdp(in.temperature[i], pressure)) / in.temperature[i];
                                                             ...
                                                             MaterialModel::MaterialModelOutputs<dim>      &out) const
      {
        ...
        if (const std::shared_ptr<PhaseOutputs<dim>> dominant_phases_out = out.template get_additional_output_object<PhaseOutputs<dim>>())
      ...
      ThermodynamicTableLookup<dim>::create_additional_named_outputs(MaterialModel::MaterialModelOutputs<dim> &out) const
      {
        ...
        if (out.template has_additional_output_object<PhaseOutputs<dim>>() == false && material_lookup[0]->has_dominant_phase())
          {
            const unsigned int n_points = out.n_evaluation_points();
            out.additional_outputs.push_back(std::make_unique<MaterialModel::PhaseOutputs<dim>>(n_points));
          }
      }
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
#define INSTANTIATE(dim) template class ThermodynamicTableLookup<dim>;
