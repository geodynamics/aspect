...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
      template <int dim>
      void
      LinearizedIncompressible<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in,
                                              ...
                                              MaterialModel::EquationOfStateOutputs<dim>    &eos_outputs) const
      {
        Assert(maximum_number_of_compositions + 1 >= eos_outputs.densities.size(),
               ExcMessage("Error: You are trying to evaluate the equation of state with " +
                          Utilities::to_string(eos_outputs.densities.size() - 1) + " compositional fields, which is larger than " +
      ...
      template <int dim>
      bool
      LinearizedIncompressible<dim>::is_compressible() const
      {
        return false;
      }
      ...
      template <int dim>
      void
      LinearizedIncompressible<dim>::declare_parameters(ParameterHandler &prm, const unsigned int n_compositions)
      {
        ...
        if (n_compositions > 1)
          prm.declare_entry("Density differential for compositional field 2",
                            "0.",
                            Patterns::Double(),
                            "If compositional fields are used, then one would frequently want "
                            "to make the density depend on these fields. In this simple material "
      ...
      template <int dim>
      void
      LinearizedIncompressible<dim>::parse_parameters(ParameterHandler &prm, const unsigned int n_compositions)
      {
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
#define INSTANTIATE(dim) template class LinearizedIncompressible<dim>;
