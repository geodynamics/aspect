...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
      template <int dim>
      void
      MulticomponentIncompressible<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in,
                                                  ...
                                                  MaterialModel::EquationOfStateOutputs<dim>    &eos_outputs) const
      {
        ...
        const double reference_temperature =
          (this->include_adiabatic_heating() ? this->get_adiabatic_conditions().temperature(in.position[input_index]) : reference_T);

        for (unsigned int c = 0; c < eos_outputs.densities.size(); ++c)
          {
            eos_outputs.densities[c] =
              densities[c] * (1 - thermal_expansivities[c] * (in.temperature[input_index] - reference_temperature));
      ...
      template <int dim>
      bool
      MulticomponentIncompressible<dim>::is_compressible() const
      {
        return false;
      }
      ...
      template <int dim>
      void
      MulticomponentIncompressible<dim>::declare_parameters(ParameterHandler &prm, const double default_thermal_expansion)
      {
        prm.declare_entry("Reference temperature", "293.", Patterns::Double(0.), "The reference temperature $T_0$. Units: \\si{\\kelvin}.");
        ...
        prm.declare_entry("Heat capacities",
                          "1250.",
                          Patterns::Anything(),
      ...
      template <int dim>
      void
      MulticomponentIncompressible<dim>::parse_parameters(
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
#define INSTANTIATE(dim) template class MulticomponentIncompressible<dim>;
