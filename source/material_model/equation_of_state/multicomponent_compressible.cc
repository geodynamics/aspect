...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
        ...
        for (unsigned int c = 0; c < eos_outputs.densities.size(); ++c)
          {
            ...
            const double f  = (1. + (pressure_for_density - ak * (temperature - reference_temperatures[c])) *
                                     isothermal_bulk_modulus_pressure_derivatives[c] * reference_isothermal_compressibilities[c]);
            ...
            eos_outputs.specific_heat_capacities[c] =
              (isochoric_specific_heats[c] +
               (temperature * reference_thermal_expansivities[c] * ak *
                std::pow(f, -1. - (1. / isothermal_bulk_modulus_pressure_derivatives[c])) / reference_densities[c]));
      ...
      {
        prm.declare_entry("Reference temperatures",
                          "298.15",
                          Patterns::Anything(),
                          ...
                          "Units: \\si{\\kelvin}.");
        prm.declare_entry("Reference densities",
                          "3300.",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Reference isothermal compressibilities",
                          "4e-12",
                          Patterns::Anything(),
                          ...
                          "If only one value is given, then all fields/phases use the same value. "
                          "Units: \\si{\\per\\pascal}.");
        prm.declare_entry("Isothermal bulk modulus pressure derivatives",
                          "4.",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Reference thermal expansivities",
                          ...
                          "If only one value is given, then all fields/phases use the same value. "
                          "Units: \\si{\\per\\kelvin}.");
        prm.declare_entry("Isochoric specific heats",
                          "1250.",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Enable phase transitions",
                          "false",
                          Patterns::Bool(),
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace EquationOfState
    {
#define INSTANTIATE(dim) template class MulticomponentCompressible<dim>;
