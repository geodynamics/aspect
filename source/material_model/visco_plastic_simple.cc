...
namespace aspect
{
  namespace MaterialModel
  {
                     ...
                     const std::vector<double> &values) const
    {
      ...
      switch (operation)
        {
          case MaterialAveraging::arithmetic_average:
          {
            double sum = 0;
            ...
            for (unsigned int i=0; i<N; ++i)
              {
                Assert (values[i] >= 0,
                        ExcMessage ("Computing the geometric average "
                                    ...
                                    "quantities."));
                average *= std::pow (values[i], 1./N);
              }

            return average;
          }

          case MaterialAveraging::pick_largest:
          {
    ...
    ViscoPlasticSimple<dim>::
    calculate_viscosity_and_plastic_strain_rate(const unsigned int j,
                                                const double raw_strain_rate,
                                                ...
                                                const double plastic_strain) const
    {
      ...
      const double strain_rate = std::max(raw_strain_rate, min_strain_rate);
              ...
              if (iteration == 0)
                {
                  ...
                  if (std::fabs(initial_residual) < std::numeric_limits<double>::min())
                    break;
                }
      ...
      return std::make_pair(viscosity, plastic_strain_rate);
    }
      ...
      if (in.requests_property(MaterialProperties::viscosity) || in.requests_property(MaterialProperties::additional_outputs))
        {
          ...
          for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
            {
              ...
              if (use_reference_strain_rate)
                ...
                {
                  // Calculate the second moment invariant of the deviatoric strain rate tensor.
                  edot_ii_sqr = std::max(-second_invariant(in.strain_rate[i]), 0.);
                }
              else
                {
              ...
              const double JxW = in.JxW[i];

              average_strain_rate    += JxW * edot_ii_sqr;
              average_pressure       += JxW * in.pressure[i];
              average_temperature    += JxW * in.temperature[i];
              average_plastic_strain += JxW * e_p;

              cell_measure += JxW;
            }

          average_strain_rate = std::sqrt(average_strain_rate / cell_measure);
          average_pressure /= cell_measure;
          average_temperature /= cell_measure;
          average_plastic_strain /= cell_measure;

          // Calculate the viscosity and plastic strain rate for each chemical field.
          const unsigned int n_chemical_fields = volume_fractions[0].size();
          std::vector<double> composition_viscosities(n_chemical_fields);
          std::vector<double> composition_plastic_strain_rates(n_chemical_fields);
          std::vector<bool> chemical_field_exists(n_chemical_fields);
          for (unsigned int j = 0; j < n_chemical_fields; ++j)
            {
              // Ignore this field if its volume fraction is zero
              double max_fraction = 0.;
              for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
                max_fraction = std::max(max_fraction, volume_fractions[i][j]);

              chemical_field_exists[j] = (max_fraction > 1.e-8);
              if (chemical_field_exists[j] == false)
                {
                  composition_viscosities[j] = max_viscosity;
                  composition_plastic_strain_rates[j] = 0.;
                  continue;
                }

              const std::pair<double, double> eta_and_ep = calculate_viscosity_and_plastic_strain_rate(
                j, average_strain_rate, average_pressure, average_temperature, average_plastic_strain);

              composition_viscosities[j]          = eta_and_ep.first;
              composition_plastic_strain_rates[j] = eta_and_ep.second;
            }

          // Calculate the average values of viscosity and plastic strain rate
          // at each evaluation point
          std::vector<double> viscosities(in.n_evaluation_points());
          for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
            {
              viscosities[i] = MaterialUtilities::average_value(volume_fractions[i], 
          ...
          // Perform cellwise averaging to viscosity
          const double viscosity = cellwise_average(cellwise_viscosity_averaging, viscosities);
          std::fill(out.viscosities.begin(), out.viscosities.end(), viscosity);
          ...
          if (MaterialModel::MaterialModelDerivatives<dim> *derivatives =
              out.template get_additional_output<MaterialModel::MaterialModelDerivatives<dim>>())
            {
              ...
              double viscosity_forward = numbers::signaling_nan<double>();
              ...
              for (unsigned int j = 0; j < n_chemical_fields; ++j)
                if (chemical_field_exists[j])
                  composition_viscosities[j] = calculate_viscosity_and_plastic_strain_rate(
                    j, strain_rate_forward, average_pressure, average_temperature, average_plastic_strain).first;
              ...
              viscosity_forward = cellwise_average(cellwise_viscosity_averaging, viscosities);

              derivatives->viscosity_derivative_wrt_average_strain_rate = 
                (viscosity_forward - viscosity) / (average_strain_rate * finite_difference_accuracy);
              ...
              for (unsigned int j = 0; j < n_chemical_fields; ++j)
                if (chemical_field_exists[j])
                  composition_viscosities[j] = calculate_viscosity_and_plastic_strain_rate(
                    j, average_strain_rate, pressure_forward, average_temperature, average_plastic_strain).first;
              ...
              const double pressure_difference = average_pressure * finite_difference_accuracy;
              derivatives->viscosity_derivative_wrt_average_pressure =
                (std::fabs(pressure_difference) > std::numeric_limits<double>::min() 
                 ?
                 (viscosity_forward - viscosity) / pressure_difference
                 :
                 0.0);
            }
        }
...
prm.declare_entry("Thermal diffusivities", "0.8e-6", Patterns::List(Patterns::Double(0.)),
          ...
          const std::string cellwise_viscosity_averaging_schemes =
            "arithmetic average|harmonic average|geometric average|pick largest";
          prm.declare_entry("Cellwise viscosity averaging scheme", "harmonic average",
                            Patterns::Selection(cellwise_viscosity_averaging_schemes),
                            "The 'visco plastic simple' model requires the viscosity to be "
                            "averaged in each cell."
                            "\n\n"
                            "Possible choices: " + cellwise_viscosity_averaging_schemes + ".");
