...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    bool
    PerpleXLookup<dim>::is_compressible() const
    {
      return true;
    }
    ...
    template <int dim>
    void
    PerpleXLookup<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      const double average_temperature =
        std::min(max_temperature, std::max(min_temperature, (accumulate(in.temperature.begin(), in.temperature.end(), 0.0) / n_quad)));
      const double average_pressure =
        std::min(max_pressure, std::max(min_pressure, (accumulate(in.pressure.begin(), in.pressure.end(), 0.0) / n_quad)));
      ...
      AssertThrow(!isnan(sysprop[9]) && !isnan(sysprop[11]) && !isnan(sysprop[12]) && !isnan(sysprop[13]),
                  ExcMessage("PerpleX returned NaN for at least one material property at " + std::to_string(average_pressure) + " bar, " +
    ...
    PerpleXLookup<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        ...
        {
          prm.declare_entry("PerpleX input file name",
                            "rock.dat",
                            Patterns::Anything(),
          ...
          prm.declare_entry("Minimum material temperature",
                            "0.",
                            Patterns::Double(0.),
                            "The value of the minimum temperature used to query PerpleX. "
                            "Units: \\si{\\kelvin}.");
          prm.declare_entry("Maximum material temperature",
                            "6000.",
                            Patterns::Double(0.),
                            "The value of the maximum temperature used to query PerpleX. "
                            "Units: \\si{\\kelvin}.");
          prm.declare_entry("Minimum material pressure",
                            "1.e5",
                            Patterns::Double(0.),
                            "The value of the minimum pressure used to query PerpleX. "
                            "Units: \\si{\\pascal}.");
          prm.declare_entry("Maximum material pressure",
                            "1.e12",
                            Patterns::Double(0.),
