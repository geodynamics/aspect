...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    void
    LatentHeat<dim>::evaluate(const MaterialModelInputs<dim> &in, MaterialModelOutputs<dim> &out) const
    {
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
            ...
            if ((composition_viscosity_prefactor != 1.0) && (composition.size() > 0))
              {
                ...
                out.viscosities[i] =
                  (std::pow(10.,
                            ((1 - composition[0]) * std::log10(eta * visc_temperature_dependence) +
          ...
          const double pressure_depth_derivative =
            (depth > 0.0) ? adiabatic_pressure / depth : this->get_gravity_model().gravity_vector(in.position[i]).norm() * reference_rho;
          MaterialUtilities::PhaseFunctionInputs<dim> phase_in(temperature, pressure, depth, pressure_depth_derivative, 0);
          ...
          // and phase dependence of viscosity
          {
            ...
            const double density_temperature_dependence =
              1.0 - (temperature - this->get_adiabatic_conditions().temperature(position)) * thermal_alpha;

            // second, calculate composition dependence of density
            // constant density difference between peridotite and eclogite
            const double density_composition_dependence = composition.size() > 0 ? compositional_delta_rho * composition[0] : 0.0;
            ...
            for (unsigned int transition_index = 0; transition_index < phase_function.n_phase_transitions(); ++transition_index)
              {
                ...
                if (transition_index + 1 < phase_function.n_phases_for_each_composition()[0]) // 1st compositional field
                  ...
                  viscosity_phase_prefactors[1] =
                    std::pow(10.,
                             phaseFunction * composition[0] *
                                 std::log10(viscosity_phase_prefactors[1] * phase_prefactors[transition_index + 2]) +
            ...
            out.densities[i] =
              (reference_rho + density_composition_dependence + pressure_dependence + phase_dependence) * density_temperature_dependence;
            ...
            if (composition.size() == 0)
              ...
              out.viscosities[i] = (std::pow(10.,
                                             ((1 - composition[0]) * std::log10(out.viscosities[i] * viscosity_phase_prefactors[0]) +
    ...
    template <int dim>
    bool
    LatentHeat<dim>::is_compressible() const
    {
    ...
    LatentHeat<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Latent heat");
        {
          ...
          density_jumps     = Utilities::string_to_double(Utilities::split_string_list(prm.get("Phase transition density jumps")));
          transition_phases = Utilities::string_to_int(Utilities::split_string_list(prm.get("Corresponding phase for density jump")));
          ...
          if (density_jumps.size() != n_transitions || transition_phases.size() != n_transitions)
            AssertThrow(false,
                        ExcMessage("Error: At least one list that provides input parameters for phase "
                                   "transitions has the wrong size. The phase function object reports that "
                                   "there are " +
                                   std::to_string(n_transitions) +
                                   " transitions, "
                                   "therefore the material model expects " +
                                   std::to_string(n_transitions) + " density jumps, corresponding phases, and viscosity prefactors."));
