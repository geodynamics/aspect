...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    void
    CompositionReaction<dim>::evaluate(const MaterialModelInputs<dim> &in, MaterialModelOutputs<dim> &out) const
    {
          ...
          switch (composition.size())
            {
              ...
              default:
                out.viscosities[i] =
                  (std::pow(10,
                            ((1 - 0.5 * composition[0] - 0.5 * composition[1]) * std::log10(eta * temperature_dependence) +
              ...
              if (this->get_parameters().use_operator_splitting)
                {
                  if (reaction_rate_out != nullptr)
                    reaction_rate_out->reaction_rates[i][c] =
                      (this->get_timestep_number() > 0 ? out.reaction_terms[i][c] / this->get_timestep() : 0.0);
    ...
    template <int dim>
    bool
    CompositionReaction<dim>::is_compressible() const
    {
      return false;
    }
    ...
    CompositionReaction<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Composition reaction model");
        {
          ...
          prm.declare_entry("Composition viscosity prefactor 1",
                            "1.0",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Composition viscosity prefactor 2",
                            "1.0",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Reaction depth",
                            "0.",
                            Patterns::Double(0.),
    ...
    CompositionReaction<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Composition reaction model");
        {
          ...
          if (thermal_viscosity_exponent != 0.0 && reference_T == 0.0)
            AssertThrow(
              false, ExcMessage("Error: Material model composition reaction with Thermal viscosity exponent can not have reference_T=0."));
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
      ...
      if ((composition_viscosity_prefactor_1 != 1.0) || (composition_viscosity_prefactor_2 != 1.0))
