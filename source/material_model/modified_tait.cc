...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    void
    ModifiedTait<dim>::evaluate(const MaterialModelInputs<dim> &in, MaterialModelOutputs<dim> &out) const
    {
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          AssertThrow(temperature > 100.,
                      ExcMessage("The current temperature is <100 K. "
          ...
          AssertThrow(x_einstein < 35.,
                      ExcMessage("The current temperature is less than 1/35th "
                                 "of the Einstein temperature. This is not "
                                 "expected for geological materials."));
          AssertThrow(x_einstein0 < 35.,
                      ExcMessage("The reference temperature of the "
          ...
          const double dintVdpdT = ((reference_thermal_expansivity * reference_isothermal_bulk_modulus / reference_rho * tait_a * xi) *
                                    (std::pow((1. + tait_b * psubpth), -tait_c) - std::pow((1. - tait_b * Pth_rel), -tait_c)));

          const double dSdT =
            (reference_isothermal_bulk_modulus / reference_rho * Utilities::fixed_power<2>((xi * reference_thermal_expansivity)) *
               (std::pow((1. + tait_b * psubpth), -1. - tait_c) - std::pow((1. - tait_b * Pth_rel), -1. - tait_c)) +
             dintVdpdT * ((1 - 2. / x_einstein + 2. / (std::exp(x_einstein) - 1.)) * einstein_temperature / (temperature * temperature)));

          const double rho                        = reference_rho / x;
          const double isothermal_compressibility = 1. / (reference_isothermal_bulk_modulus * (1. + tait_b * psubpth) *
                                                          (tait_a + (1. - tait_a) * std::pow((1. + tait_b * psubpth), tait_c)));
          const double alpha                      = reference_thermal_expansivity * xi * 1. /
                               ((1. + tait_b * psubpth) * (tait_a + (1. - tait_a) * std::pow((1 + tait_b * psubpth), tait_c)));
    ...
    template <int dim>
    bool
    ModifiedTait<dim>::is_compressible() const
    {
      return true;
    }
    ...
    ModifiedTait<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        ...
        {
          prm.declare_entry("Reference pressure", "1e5", Patterns::Double(0.), "Reference pressure $P_0$. Units: \\si{\\pascal}.");
          prm.declare_entry("Reference temperature", "298.15", Patterns::Double(0.), "Reference temperature $T_0$. Units: \\si{\\kelvin}.");
          prm.declare_entry("Reference density",
                            "3300.",
                            Patterns::Double(0.),
                            "The density at the reference pressure and temperature. "
                            "Units: \\si{\\kilogram\\per\\meter\\cubed}.");
          prm.declare_entry("Reference isothermal bulk modulus",
                            "125e9",
                            Patterns::Double(0.),
                            "The isothermal bulk modulus at the reference pressure and temperature. "
                            "Units: \\si{\\pascal}.");
          prm.declare_entry("Reference bulk modulus derivative",
                            "4.",
                            Patterns::Double(0.),
                            "The value of the first pressure derivative of the isothermal bulk modulus "
                            "at the reference pressure and temperature. "
                            "Units: None.");
          prm.declare_entry("Reference thermal expansivity",
                            "2e-5",
                            Patterns::Double(0.),
                            "The thermal expansion coefficient at the reference pressure and temperature. "
                            "Units: \\si{\\per\\kelvin}.");
          prm.declare_entry("Einstein temperature",
                            "600.",
                            Patterns::Double(0.),
