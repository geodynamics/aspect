...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    double
    MeltGlobal<dim>::reference_darcy_coefficient() const
    {
      // 0.01 = 1% melt
    ...
    template <int dim>
    bool
    MeltGlobal<dim>::is_compressible() const
    {
      return false;
    }
    ...
    template <int dim>
    double
    MeltGlobal<dim>::melt_fraction(const double temperature, const double pressure, const double depletion) const
    {
      const double T_solidus =
        surface_solidus + pressure_solidus_change * pressure + std::max(depletion_solidus_change * depletion, -200.0);
    ...
    template <int dim>
    void
    MeltGlobal<dim>::melt_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
    ...
    template <int dim>
    void
    MeltGlobal<dim>::evaluate(const typename Interface<dim>::MaterialModelInputs &in,
                              typename Interface<dim>::MaterialModelOutputs      &out) const
    {
      ...
      if (this->include_melt_transport() && in.current_cell.state() == IteratorState::valid && this->get_timestep_number() > 0 &&
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          if (this->include_adiabatic_heating())
            temperature_dependence -=
              (in.temperature[i] - this->get_adiabatic_conditions().temperature(in.position[i])) * thermal_expansivity;
          else
          ...
          if (this->include_melt_transport())
            {
              ...
              if (include_melting_and_freezing &&
                  (in.requests_property(MaterialProperties::reaction_terms) || in.requests_property(MaterialProperties::reaction_rates) ||
                   in.requests_property(MaterialProperties::viscosity)))
                {
                  ...
                  const double trace_strain_rate =
                    (this->get_timestep_number() > 1) ? (trace(in.strain_rate[i])) : numbers::signaling_nan<double>();
                  ...
                  for (unsigned int c = 0; c < in.composition[i].size(); ++c)
                    {
                      if (c == peridotite_idx && this->get_timestep_number() > 1)
                        out.reaction_terms[i][c] =
                          porosity_change - in.composition[i][peridotite_idx] * trace_strain_rate * this->get_timestep();
                          ...
                          if (reaction_rate_out != nullptr)
                            {
                              if (c == peridotite_idx && this->get_timestep_number() > 0)
                                reaction_rate_out->reaction_rates[i][c] =
                                  porosity_change / melting_time_scale - in.composition[i][peridotite_idx] * trace(in.strain_rate[i]);
          ...
          if (this->include_adiabatic_heating())
            {
              ...
              visc_temperature_dependence = std::clamp(std::exp(-thermal_viscosity_exponent * delta_temp /
          ...
          for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
            {
              ...
              melt_out->fluid_densities[i] =
                reference_rho_f * temperature_dependence * std::exp(melt_compressibility * (in.pressure[i] - this->get_surface_pressure()));
              ...
              if (this->include_adiabatic_heating())
              ...
              else if (thermal_viscosity_exponent != 0.0)
                {
                  ...
                  visc_temperature_dependence =
                    std::clamp(std::exp(-thermal_bulk_viscosity_exponent * delta_temp / reference_T), 1e-4, 1e4);
                }
    ...
    MeltGlobal<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        ...
        {
          prm.declare_entry("Reference solid density",
                            "3000.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Depletion density change",
                            "0.0",
                            Patterns::Double(),
                            ...
                            "exist in the model. "
                            "Units: \\si{\\kilogram\\per\\meter\\cubed}.");
          prm.declare_entry("Surface solidus",
                            "1300.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Exponential depletion strengthening factor",
                            "0.0",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Maximum Depletion viscosity change",
                            "1.0e3",
                            Patterns::Double(0.),
