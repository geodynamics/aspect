...
namespace aspect
{
  namespace MaterialModel
  {
                 ...
                 const SymmetricTensor<2,dim> &strain_rate) const
    {
      ...
      const std::vector<double> volume_fractions = MaterialUtilities::compute_field_fractions(composition, rheology->get_volumetric_composition_mask());
    ...
    is_yielding(const MaterialModelInputs<dim> &in) const
    {
      ...
      const std::vector<double> volume_fractions = MaterialUtilities::compute_field_fractions(in.composition[0], rheology->get_volumetric_composition_mask());
      ...
      for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
        {
          ...
          const std::vector<double> volume_fractions = MaterialUtilities::compute_field_fractions(in.composition[i], volumetric_compositions);
