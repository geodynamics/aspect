...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    bool
    ViscoPlastic<dim>::is_yielding(const MaterialModelInputs<dim> &in) const
    {
       ...
       */
      const IsostrainViscosities isostrain_viscosities = rheology->calculate_isostrain_viscosities(
        in, 0, volume_fractions, phase_function_values, phase_function.n_phase_transitions_for_each_composition());
    ...
    template <int dim>
    void
    ViscoPlastic<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      std::vector<double> phase_function_discrete_values =
        (use_dominant_phase_for_viscosity ? std::vector<double>(phase_function_discrete->n_phase_transitions(), 0.0) :
                                            std::vector<double>());
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          out.specific_heat[i] =
            MaterialUtilities::average_value(volume_fractions, eos_outputs.specific_heat_capacities, MaterialUtilities::arithmetic);
          ...
          if (define_conductivities == false)
          ...
          else
            {
              ...
              // option was selected.
              out.thermal_conductivities[i] =
                MaterialUtilities::average_value(volume_fractions, thermal_conductivities, MaterialUtilities::arithmetic);
            }
          ...
          out.entropy_derivative_pressure[i] =
            MaterialUtilities::average_value(volume_fractions, eos_outputs.entropy_derivative_pressure, MaterialUtilities::arithmetic);
          out.entropy_derivative_temperature[i] =
            MaterialUtilities::average_value(volume_fractions, eos_outputs.entropy_derivative_temperature, MaterialUtilities::arithmetic);
          ...
          if (in.requests_property(MaterialProperties::viscosity) || in.requests_property(MaterialProperties::additional_outputs) ||
              (this->get_parameters().enable_elasticity && in.requests_property(MaterialProperties::reaction_rates)))
            {
              ...
              if (use_dominant_phase_for_viscosity)
                ...
                {
                  isostrain_viscosities = rheology->calculate_isostrain_viscosities(
                    in, i, volume_fractions, phase_function_values, n_phase_transitions_for_each_chemical_composition);
                }
              ...
              if (const std::shared_ptr<MaterialModel::MaterialModelDerivatives<dim>> derivatives =
                ...
                rheology->compute_viscosity_derivatives(i,
                                                        volume_fractions,
                                                        isostrain_viscosities,
                                                        in,
                                                        out,
          ...
          if (const std::shared_ptr<PrescribedPlasticDilation<dim>> plastic_dilation =
            ...
            {
              const double dilation_lhs_term =
                MaterialUtilities::average_value(volume_fractions, isostrain_viscosities.dilation_lhs_terms, MaterialUtilities::arithmetic);
              const double dilation_rhs_term =
                MaterialUtilities::average_value(volume_fractions, isostrain_viscosities.dilation_rhs_terms, MaterialUtilities::arithmetic);
    ...
    template <int dim>
    bool
    ViscoPlastic<dim>::is_compressible() const
    {
    ...
    template <int dim>
    double
    ViscoPlastic<dim>::get_min_strain_rate() const
    {
    ...
    ViscoPlastic<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        ...
        {
          prm.declare_entry("Use dominant phase for viscosity",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Thermal diffusivities",
                            "0.8e-6",
          ...
          prm.declare_entry("Define thermal conductivities",
                            "false",
                            Patterns::Bool(),
    ...
    ViscoPlastic<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Visco Plastic");
        {
          ...
          equation_of_state.parse_parameters(prm, std::make_unique<std::vector<unsigned int>>(n_phases_for_each_chemical_composition));
          ...
          if (use_dominant_phase_for_viscosity)
            {
              ...
              rheology->parse_parameters(
                prm, std::make_unique<std::vector<unsigned int>>(phase_function_discrete->n_phases_for_each_chemical_composition()));
            }
          else
            {
          ...
          reaction_progress_mapping = Utilities::string_to_unsigned_int(Utilities::split_string_list(prm.get("Reaction progress mapping")));
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
