...
namespace aspect
{
  namespace MaterialModel
  {
    namespace MaterialUtilities
    {
      ...
      {
        double
        MaterialLookup::specific_heat(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, specific_heat_values, interpolation);
        }

        double
        MaterialLookup::density(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, density_values, interpolation);
        }

        double
        MaterialLookup::thermal_expansivity(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, thermal_expansivity_values, interpolation);
        }

        double
        MaterialLookup::seismic_Vp(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, vp_values, false);
        }

        double
        MaterialLookup::seismic_Vs(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, vs_values, false);
        }

        double
        MaterialLookup::enthalpy(const double temperature, const double pressure) const
        {
          return value(temperature, pressure, enthalpy_values, true);
        }

        double
        MaterialLookup::dHdT(const double temperature, const double pressure) const
        {
        ...
        double
        MaterialLookup::dHdp(const double temperature, const double pressure) const
        {
          const double h  = value(temperature, pressure, enthalpy_values, interpolation);
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              for (unsigned int p = 0; p < n_q_points; ++p)
                {
                  if (std::fabs(temperatures[q] - temperatures[p]) >
                      100.0 * std::numeric_limits<double>::epsilon() * std::fabs(temperatures[q] + std::numeric_limits<double>::epsilon()))
                    {
                      for (unsigned int substep = 0; substep < n_substeps; ++substep)
                        {
                          ...
                          const double current_pressure = pressures[q] + step_ratio * (pressures[p] - pressures[q]);
                          const double T1_substep       = temperatures[q] + step_ratio * (temperatures[p] - temperatures[q]);
                          const double T2_substep       = temperatures[q] + step_ratio_next * (temperatures[p] - temperatures[q]);
                  ...
                  if (std::fabs(pressures[q] - pressures[p]) >
                      100.0 * std::numeric_limits<double>::epsilon() * std::fabs(pressures[q] + std::numeric_limits<double>::epsilon()))
                    {
                      for (unsigned int substep = 0; substep < n_substeps; ++substep)
                        {
                          const double step_ratio          = static_cast<double>(substep) / static_cast<double>(n_substeps);
                          const double step_ratio_next     = static_cast<double>(substep + 1) / static_cast<double>(n_substeps);
                          const double current_temperature = temperatures[q] + step_ratio * (temperatures[p] - temperatures[q]);
                          const double p1_substep          = pressures[q] + step_ratio * (pressures[p] - pressures[q]);
                          const double p2_substep          = pressures[q] + step_ratio_next * (pressures[p] - pressures[q]);
        ...
        double
        MaterialLookup::dRhodp(const double temperature, const double pressure) const
        {
        ...
        unsigned int
        MaterialLookup::dominant_phase(const double temperature, const double pressure) const
        {
        ...
        double
        MaterialLookup::phase_volume_fraction(const int phase_id, const double temperature, const double pressure) const
        {
        ...
        double
        MaterialLookup::value(const double temperature, const double pressure, const Table<2, double> &values, const bool interpol) const
        {
          ...
          Assert(inT < values.n_rows(),
                 ExcMessage("Attempting to look up a temperature value with index greater than the number of rows."));
          Assert(inp < values.n_cols(),
                 ExcMessage("Attempting to look up a pressure value with index greater than the number of columns."));
          ...
          if (!interpol)
          ...
          else
            {
              ...
              return ((1 - xi) * (1 - eta) * values[inT][inp] + xi * (1 - eta) * values[inT + 1][inp] +
                      (1 - xi) * eta * values[inT][inp + 1] + xi * eta * values[inT + 1][inp + 1]);
            }
        }

        unsigned int
        MaterialLookup::value(const double temperature, const double pressure, const Table<2, unsigned int> &values) const
        {
          const double       nT  = get_nT(temperature);
          const unsigned int inT = static_cast<unsigned int>(nT);
                                     ...
                                     const MPI_Comm     comm)
        {
          ...
          // Read material data
          {
            ...
            Assert(i == n_temperature * n_pressure, ExcMessage("Material table size not consistent."));
        ...
        PerplexReader::PerplexReader(const std::string &filename, const bool interpol, const MPI_Comm comm)
        {
          /* Initializing variables */
          interpolation             = interpol;
          delta_press               = numbers::signaling_nan<double>();
          ...
          if (column_name == "P(bar)")
            {
              ...
              in >> column_name;
              AssertThrow(column_name == "T(K)",
                          ExcMessage("The second column name in PerpleX lookup file " + filename + " should be T(K)."));
            }
          else if (column_name == "T(K)")
            {
              first_natural_variable = column_name;
              in >> column_name;
              AssertThrow(column_name == "P(bar)",
                          ExcMessage("The second column name in PerpleX lookup file " + filename + " should be P(bar)."));
            }
          else
            {
              ...
              if (column_name == "rho,kg/m3")
              ...
              else if (column_name.length() > 3)
                {
                  if (column_name.substr(0, 13).compare("vol_fraction_") == 0)
                    {
                      if (std::find(phase_column_names.begin(), phase_column_names.end(), column_name) != phase_column_names.end())
                        {
                          AssertThrow(false,
                                      ExcMessage("The PerpleX lookup file " + filename +
          ...
          AssertThrow(std::all_of(prp_indices.begin(), prp_indices.end(), [](int i) { return i >= 0; }),
                      ExcMessage("The PerpleX lookup file " + filename +
                                 " must contain columns with names "
        ...
        void
        EntropyReader::initialize(const MPI_Comm comm, const std::string &data_directory, const std::string &material_file_name)
        {
          ...
          material_lookup->load_file(data_directory + material_file_name, comm);
        }



        double
        EntropyReader::specific_heat(const double entropy, const double pressure) const
        {
        ...
        double
        EntropyReader::density(const double entropy, const double pressure) const
        {
        ...
        double
        EntropyReader::thermal_expansivity(const double entropy, const double pressure) const
        {
        ...
        double
        EntropyReader::temperature(const double entropy, const double pressure) const
        {
        ...
        double
        EntropyReader::seismic_vp(const double entropy, const double pressure) const
        {
        ...
        double
        EntropyReader::seismic_vs(const double entropy, const double pressure) const
        {
        ...
        Tensor<1, 2>
        EntropyReader::density_gradient(const double entropy, const double pressure) const
        {
      ...
      {
        AssertThrow(minimum_fraction <= 1.0, ExcMessage("The minimum composition fraction must not be greater than one."));
      ...
      std::vector<double>
      compute_volumes_from_masses(const std::vector<double> &masses, const std::vector<double> &densities, const bool return_as_fraction)
      {
        Assert(masses.size() == densities.size(),
               ExcMessage("The mass fractions and densities vectors used for computing "
                          ...
                          "You have provided " +
                          Utilities::int_to_string(masses.size()) + " mass fractions and " + Utilities::int_to_string(densities.size()) +
                          " densities."));
      ...
      parse_compositional_averaging_operation(const std::string &parameter_name, const ParameterHandler &prm)
      {
            ...
            case harmonic:
              {
                for (unsigned int i = 0; i < volume_fractions.size(); ++i)
                  {
                    AssertThrow(parameter_values[i] > 0, ExcMessage("All parameter values must be greater than 0 for harmonic averaging!"));
                    averaged_parameter += volume_fractions[i] / (parameter_values[i]);
                  }
            ...
            case maximum_composition:
              {
                const unsigned int idx =
                  static_cast<unsigned int>(std::max_element(volume_fractions.begin(), volume_fractions.end()) - volume_fractions.begin());
                                              ...
                                              MaterialModelOutputs<dim>         &out)
      {
        ...
        out.compressibilities[i] =
          MaterialUtilities::average_value(volume_fractions, eos_outputs.compressibilities, MaterialUtilities::arithmetic);
        out.thermal_expansion_coefficients[i] =
          MaterialUtilities::average_value(volume_fractions, eos_outputs.thermal_expansion_coefficients, MaterialUtilities::arithmetic);
        out.entropy_derivative_pressure[i] =
          MaterialUtilities::average_value(mass_fractions, eos_outputs.entropy_derivative_pressure, MaterialUtilities::arithmetic);
        out.entropy_derivative_temperature[i] =
          MaterialUtilities::average_value(mass_fractions, eos_outputs.entropy_derivative_temperature, MaterialUtilities::arithmetic);
        out.specific_heat[i] =
          MaterialUtilities::average_value(mass_fractions, eos_outputs.specific_heat_capacities, MaterialUtilities::arithmetic);
      }
            ...
            for (unsigned int i = 0; i < n_phase_transitions_per_composition[composition_index]; ++i)
              {
                ...
                for (unsigned int previous_phase_index = start_phase_index; previous_phase_index <= phase_index; previous_phase_index++)
                  {
                    auto [reaction_progress_active, reaction_progress_value] =
                      get_reaction_progress_for_phase_transition(reaction_progress_values, reaction_progress_mapping, previous_phase_index);
      ...
      double
      phase_average_value(const std::vector<double>                    &phase_function_values,
                          const std::vector<unsigned int>              &n_phase_transitions_per_composition,
                          const std::vector<double>                    &parameter_values,
                          const unsigned int                            composition_index,
                          const PhaseUtilities::PhaseAveragingOperation operation)
                ...
                if (operation == PhaseUtilities::logarithmic)
                  {
                    ...
                    averaged_parameter += phase_function_values[phase_index - composition_index] *
                                          (std::log(parameter_values[phase_index + 1]) - averaged_parameter);
                  }
                else if (operation == PhaseUtilities::arithmetic)
                  averaged_parameter +=
                    phase_function_values[phase_index - composition_index] * (parameter_values[phase_index + 1] - averaged_parameter);
      ...
      {
        AssertThrow(
          this->introspection().get_number_of_fields_of_type(CompositionalFieldDescription::chemical_composition) + 1 ==
            material_file_names.size(),
          ExcMessage(
            " The 'discrete phase function' requires that the number of compositional fields of type chemical composition plus one (for a background field) "
        ...
        for (unsigned int i = 0; i < material_file_names.size(); ++i)
          {
            material_lookup.push_back(std::make_unique<Utilities::StructuredDataLookup<2>>(8, 1.0));

            material_lookup[i]->load_file(data_directory + material_file_names[i], this->get_mpi_communicator());

            Assert(
              material_lookup[i]->has_equidistant_coordinates(),
              ExcMessage(
                "The loaded lookup tables do not use equidistant coordinates. The class 'PhaseFunctionDiscrete' cannot currently handle non equidistant coordinates."));
            ...
            interval_temperature[i] =
              (maximum_temperature[i] - minimum_temperature[i]) / (material_lookup[i]->get_number_of_coordinates(0) - 1);
      ...
      PhaseFunctionDiscrete<dim>::compute_value(const PhaseFunctionInputs<dim> &in) const
      {
        ...
        for (unsigned int n_relevant_fields = 0; n_relevant_fields < this->introspection().n_chemical_composition_fields() + 1;
             n_relevant_fields++)
          {
        ...
        const unsigned int i_T =
          static_cast<unsigned int>(std::round((in.temperature - minimum_temperature[n_comp]) / interval_temperature[n_comp]));
        const unsigned int i_p =
          static_cast<unsigned int>(std::round((pressure_in_bar - minimum_pressure[n_comp]) / interval_pressure[n_comp]));

        Point<2> temperature_pressure(temperature_points[i_T], pressure_points[i_p]);

        // determine the dominant phase index
        const unsigned int dominant_phase_index =
          static_cast<unsigned int>(std::round(material_lookup[n_comp]->get_data(temperature_pressure, 7)));
        ...
        if ((matched_phase_transition_index != numbers::invalid_unsigned_int) &&
            in.phase_transition_index <= matched_phase_transition_index)
          function_value = 1.0;
        else
          function_value = 0.0;
      ...
      template <int dim>
      unsigned int
      PhaseFunctionDiscrete<dim>::n_phase_transitions() const
      {
      ...
      template <int dim>
      unsigned int
      PhaseFunctionDiscrete<dim>::n_phases() const
      {
      ...
      template <int dim>
      unsigned int
      PhaseFunctionDiscrete<dim>::n_phases_over_all_chemical_compositions() const
      {
      ...
      PhaseFunctionDiscrete<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Material file names",
                          "material_table_temperature_pressure_small.txt",
      ...
      PhaseFunctionDiscrete<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        const std::vector<double> transition_indicators_double =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Phase transition indicators"), options);
        ...
        else
          {
            if (use_depth_instead_of_pressure)
            ...
            else
              {
                ...
                const double pressure_deviation =
                  in.pressure - transition_pressures[in.phase_transition_index] -
      ...
      PhaseFunction<dim>::compute_derivative(const PhaseFunctionInputs<dim> &in) const
      {
        ...
        if (use_depth_instead_of_pressure)
          {
            const Point<dim, double> transition_point =
              this->get_geometry_model().representative_point(transition_depths[in.phase_transition_index]);
            const Point<dim, double> transition_plus_width = this->get_geometry_model().representative_point(
              transition_depths[in.phase_transition_index] + transition_widths[in.phase_transition_index]);
            const Point<dim, double> transition_minus_width = this->get_geometry_model().representative_point(
              transition_depths[in.phase_transition_index] - transition_widths[in.phase_transition_index]);
        ...
        const double pressure_deviation =
          in.pressure - transition_pressure -
          transition_slopes[in.phase_transition_index] * (in.temperature - transition_temperatures[in.phase_transition_index]);
        ...
        if ((in.temperature < transition_temperature_lower_limits[in.phase_transition_index]) ||
          ...
          return 0.5 / pressure_width *
                 (1.0 - std::tanh(pressure_deviation / pressure_width) * std::tanh(pressure_deviation / pressure_width));
      }
      ...
      template <int dim>
      unsigned int
      PhaseFunction<dim>::n_phase_transitions() const
      {
        if (use_depth_instead_of_pressure)
      ...
      template <int dim>
      unsigned int
      PhaseFunction<dim>::n_phases() const
      {
        return n_phases_total;
      }
      ...
      template <int dim>
      unsigned int
      PhaseFunction<dim>::n_phases_over_all_chemical_compositions() const
      {
        return n_phases_total_chemical_compositions;
      }
      ...
      template <int dim>
      double
      PhaseFunction<dim>::get_transition_slope(const unsigned int phase_transition_index) const
      {
      ...
      template <int dim>
      double
      PhaseFunction<dim>::get_transition_depth(const unsigned int phase_transition_index) const
      {
      ...
      PhaseFunction<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Phase transition pressure widths",
                          ...
                          "Units: \\si{\\pascal}.");
        prm.declare_entry("Define transition by depth instead of pressure",
                          "true",
                          Patterns::Bool(),
