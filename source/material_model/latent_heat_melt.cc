...
namespace aspect
{
  namespace MaterialModel
  {
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          const double reference_temperature =
            (this->include_adiabatic_heating() ? this->get_adiabatic_conditions().temperature(in.position[i]) : reference_T);

          const double delta_temp = in.temperature[i] - reference_temperature;
          const double T_dependence =
            (thermal_viscosity_exponent == 0.0 ? 0.0 : thermal_viscosity_exponent * delta_temp / reference_temperature);
          ...
          if ((composition_viscosity_prefactor != 1.0) && (composition.size() > 0))
            {
              ...
              out.viscosities[i] =
                (std::pow(10,
                          ((1 - composition[0]) * std::log10(eta * temperature_dependence) +
          ...
          composition_dependence = composition.size() > 0 ? compositional_delta_rho * composition[0] : 0.0;
          ...
          double melt_dependence =
            (1.0 - relative_melt_density) * melt_fraction(in.temperature[i], in.pressure[i], in.composition[i], in.position[i]);

          // in the end, all the influences are added up
          out.densities[i] =
            (reference_rho + composition_dependence + pressure_dependence) * temperature_dependence * (1.0 - melt_dependence);


          out.entropy_derivative_pressure[i] = entropy_derivative(
            in.temperature[i], in.pressure[i], composition, in.position[i], NonlinearDependence::pressure); // for pressure dependence

          out.entropy_derivative_temperature[i] = entropy_derivative(
            in.temperature[i], in.pressure[i], composition, in.position[i], NonlinearDependence::temperature); // for temperature dependence
        }
    }
    ...
    template <int dim>
    double
    LatentHeatMelt<dim>::entropy_derivative(const double                          temperature,
                                            ...
                                            const NonlinearDependence::Dependence dependence) const
    {
      ...
      const double peridotite_fraction = (this->n_compositional_fields() > 0 ? 1.0 - compositional_fields[0] : 1.0);
      ...
      if (this->n_compositional_fields() > 0)
        {
          ...
          const double T_melting     = D1 + 273.15 + D2 * pressure + D3 * pressure * pressure;
    ...
    template <int dim>
    void
    LatentHeatMelt<dim>::melt_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
                                        std::vector<double>                           &melt_fractions,
                                        const MaterialModel::MaterialModelOutputs<dim> *) const
    {
      for (unsigned int q = 0; q < in.n_evaluation_points(); ++q)
        melt_fractions[q] = melt_fraction(in.temperature[q], std::max(0.0, in.pressure[q]), in.composition[q], in.position[q]);
    }

    template <int dim>
    double
    LatentHeatMelt<dim>::peridotite_melt_fraction(const double temperature,
                                                  const double pressure,
    ...
    template <int dim>
    double
    LatentHeatMelt<dim>::pyroxenite_melt_fraction(const double temperature,
                                                  const double pressure,
                                                  const std::vector<double> &,
                                                  const Point<dim> &) const
    {
    ...
    template <int dim>
    double
    LatentHeatMelt<dim>::melt_fraction(const double               temperature,
                                       const double               pressure,
                                       const std::vector<double> &composition, /*composition*/
                                       const Point<dim>          &position) const
    {
      return (this->n_compositional_fields() > 0 ?
                pyroxenite_melt_fraction(temperature, pressure, composition, position) * composition[0] +
                  peridotite_melt_fraction(temperature, pressure, composition, position) * (1.0 - composition[0]) :
    ...
    template <int dim>
    bool
    LatentHeatMelt<dim>::is_compressible() const
    {
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Latent heat melt");
        {
          ...
          prm.declare_entry("Thermal expansion coefficient",
                            ...
                            "Units: \\si{\\per\\kelvin}.");
          prm.declare_entry("Thermal expansion coefficient of melt",
                            "6.8e-5",
                            Patterns::Double(0.),
                            "The value of the thermal expansion coefficient $\\alpha_f$. "
                            "Units: \\si{\\per\\kelvin}.");
          prm.declare_entry("Compressibility",
                            "5.124e-12",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("E2",
                            ...
                            "the melt fraction of pyroxenite. "
                            "Units: $\\frac{^{\\circ}C}{\\text{Pa}^2}$.");
          prm.declare_entry("Pyroxenite melting entropy change",
                            "-400.",
                            Patterns::Double(),
                            "The entropy change for the phase transition "
                            "from solid to melt of pyroxenite. "
                            "Units: \\si{\\joule\\per\\kelvin\\per\\kilogram}.");
          prm.declare_entry("Maximum pyroxenite melt fraction",
                            "0.5429",
                            Patterns::Double(),
                            "Maximum melt fraction of pyroxenite "
                            "in this parameterization. At higher "
                            "temperatures peridotite begins to melt.");
          prm.declare_entry("Relative density of melt",
                            "0.9",
                            Patterns::Double(),
    ...
    LatentHeatMelt<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Latent heat melt");
        {
          ...
          pyroxenite_melting_entropy_change = prm.get_double("Pyroxenite melting entropy change");
