...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    double
    MeltSimple<dim>::reference_darcy_coefficient() const
    {
      // 0.01 = 1% melt
    ...
    template <int dim>
    bool
    MeltSimple<dim>::is_compressible() const
    {
    ...
    template <int dim>
    void
    MeltSimple<dim>::melt_fractions(const MaterialModel::MaterialModelInputs<dim> &in,
    ...
    template <int dim>
    void
    MeltSimple<dim>::evaluate(const typename Interface<dim>::MaterialModelInputs &in,
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          const double delta_rho =
            this->introspection().compositional_name_exists("peridotite") ?
              ...
              0.0;
          out.densities[i] = (reference_rho_solid + delta_rho) * temperature_dependence *
          ...
          if (this->include_adiabatic_heating())
          ...
          else
            {
              ...
              const double T_dependence = (thermal_viscosity_exponent == 0.0 ? 0.0 : thermal_viscosity_exponent * delta_temp / reference_T);
    ...
    MeltSimple<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Melt simple");
        {
          ...
          prm.declare_entry("Use full compressibility",
                            "false",
                            Patterns::Bool(),
