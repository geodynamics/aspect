...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    void
    Viscoelastic<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
          ...
          for (unsigned int j = 0; j < volume_fractions.size(); ++j)
            {
              ...
              viscoelastic_viscosities[j] =
                elastic_rheology.calculate_viscoelastic_viscosity(viscosities[j], average_elastic_shear_moduli[i]);
            }
    ...
    template <int dim>
    bool
    Viscoelastic<dim>::is_compressible() const
    {
