      ...
      {
        {
          ...
          std::string temp;
          std::ifstream in("data/table/tabeldatastruct.txt", std::ios::in);
          AssertThrow (in,
                       ExcMessage (std::string("Couldn't open file <") +
                                   "data/table/tabeldatastruct.txt>."));
...
//      if (!IsCompressible) return reference_specific_heat; TODO
      static internal::P_T_LookupFunction cp("data/table/cp_bin");
    ...
    density (const double temperature,
             ...
             const Point<dim> &position) const
    {
      static internal::P_T_LookupFunction rho("data/table/rho_bin");
