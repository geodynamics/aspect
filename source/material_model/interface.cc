...
namespace aspect
{
  namespace MaterialModel
  {
    namespace NonlinearDependence
    {
      ...
      {
        Assert(dependence != uninitialized, ExcMessage("You cannot call this function on an uninitialized dependence value!"));
        return ((dependence == temperature) || (dependence == pressure) || (dependence == strain_rate) ||
    ...
    Interface<dim>::get_model_dependence() const
    {
    ...
    template <int dim>
    void
    Interface<dim>::create_additional_named_outputs(MaterialModelOutputs & /*outputs*/) const
    {
    ...
    template <int dim>
    void
    Interface<dim>::fill_additional_material_model_inputs(MaterialModel::MaterialModelInputs<dim> &input,
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Material model interface", out);
    }


    template <int dim>
    MaterialModelInputs<dim>::MaterialModelInputs(const unsigned int n_points, const unsigned int n_comp)
    ...
    MaterialModelInputs<dim>::MaterialModelInputs(const DataPostprocessorInputs::Vector<dim> &input_data,
      ...
      , composition(input_data.solution_values.size(),
                    std::vector<double>(introspection.n_compositional_fields, numbers::signaling_nan<double>()))
      ...
      , requested_properties(MaterialProperties::all_properties)
    {
      AssertThrow(compute_strain_rate == true, ExcMessage("The option to not compute the strain rate is no longer supported."));
    ...
    MaterialModelInputs<dim>::MaterialModelInputs(const FEValuesBase<dim, dim>                         &fe_values,
      ...
      , composition(fe_values.n_quadrature_points,
                    std::vector<double>(introspection.n_compositional_fields, numbers::signaling_nan<double>()))
    ...
    template <int dim>
    void
    MaterialModelInputs<dim>::resize(const unsigned int n_points, const unsigned int n_comp)
    {
    ...
    template <int dim>
    MaterialModelOutputs<dim>::MaterialModelOutputs(const unsigned int n_points, const unsigned int n_comp)
    {
    ...
    template <int dim>
    void
    MaterialModelOutputs<dim>::resize(const unsigned int n_points, const unsigned int n_comp, const bool remove_additional_outputs)
    {
...
#  endif
        ...
        Assert((P == 0) || (/*dim=2*/ P == 4) || (/*dim=3*/ P == 8), ExcInternalError());
        Assert(((operation == project_to_Q1) && (projection_matrix.m() == P) && (projection_matrix.n() == N) &&
                (expansion_matrix.m() == N) && (expansion_matrix.n() == P)) ||
                 ((projection_matrix.m() == 0) && (projection_matrix.n() == 0)),
               ExcInternalError());
...
#  endif
                                ...
                                FullMatrix<double>                                   &expansion_matrix)
      {
        ...
        FEValues<dim>          fe_values(mapping, fe, quadrature_formula, update_values | update_JxW_values);
        ...
        for (unsigned int i = 0; i < P; ++i)
          for (unsigned int q = 0; q < N; ++q)
            F(i, q) = fe_values.shape_value(i, q) * fe_values.JxW(q);

        for (unsigned int i = 0; i < P; ++i)
          for (unsigned int j = 0; j < P; ++j)
            for (unsigned int q = 0; q < N; ++q)
              M(i, j) += fe_values.shape_value(i, q) * fe_values.shape_value(j, q) * fe_values.JxW(q);
        ...
        switch (operation)
          {
            ...
            case harmonic_average_only_viscosity:
              return Utilities::fixed_power<2, double>(average_viscosity / viscosity_before_averaging) * one_over_Nq;

            case geometric_average:
            case geometric_average_only_viscosity:
            case log_average:
              return (average_viscosity / viscosity_before_averaging) * one_over_Nq;
              ...
              MaterialModelOutputs<dim>                            &values_out)
      {
        ...
        if (operation == project_to_Q1 || operation == project_to_Q1_only_viscosity)
          {
            ...
            projection_matrix.reinit(quadrature_formula.size(), quadrature_formula.size());
            compute_projection_matrix(cell, quadrature_formula, mapping, projection_matrix, expansion_matrix);
          }
        ...
        if (average_viscosity)
          {
            if (operation == harmonic_average_only_viscosity)
              average_property(harmonic_average, projection_matrix, expansion_matrix, values_out.viscosities);

            else if (operation == geometric_average_only_viscosity)
              average_property(geometric_average, projection_matrix, expansion_matrix, values_out.viscosities);

            else if (operation == project_to_Q1_only_viscosity)
              average_property(project_to_Q1, projection_matrix, expansion_matrix, values_out.viscosities);
            ...
            if (derivatives != nullptr)
              {
                for (unsigned int q = 0; q < values_out.n_evaluation_points(); ++q)
                  derivatives->viscosity_derivative_averaging_weights[q] = compute_viscosity_derivative_averaging_weight(
                    operation, values_out.viscosities[q], viscosity_before_averaging[q], 1. / values_out.n_evaluation_points());
              }
          }

        if (operation == harmonic_average_only_viscosity || operation == geometric_average_only_viscosity ||
            operation == project_to_Q1_only_viscosity)
          return;

        average_property(operation, projection_matrix, expansion_matrix, values_out.densities);
        average_property(operation, projection_matrix, expansion_matrix, values_out.thermal_expansion_coefficients);
        average_property(operation, projection_matrix, expansion_matrix, values_out.specific_heat);
        average_property(operation, projection_matrix, expansion_matrix, values_out.thermal_conductivities);
        average_property(operation, projection_matrix, expansion_matrix, values_out.compressibilities);
        average_property(operation, projection_matrix, expansion_matrix, values_out.entropy_derivative_pressure);
        average_property(operation, projection_matrix, expansion_matrix, values_out.entropy_derivative_temperature);
    ...
    template <int dim>
    NamedAdditionalMaterialOutputs<dim>::~NamedAdditionalMaterialOutputs() = default;
    ...
    NamedAdditionalMaterialOutputs<dim>::get_nth_output(const unsigned int idx) const
    {
      ...
      Assert(output_values.size() > idx, ExcMessage("The requested output index is out of range for output_values."));
      Assert(output_values[idx].size() > 0, ExcMessage("There must be one or more points for the nth output."));
    ...
    template <int dim>
    ReactionRateOutputs<dim>::ReactionRateOutputs(const unsigned int n_points, const unsigned int n_comp)
    ...
    template <int dim>
    PrescribedFieldOutputs<dim>::PrescribedFieldOutputs(const unsigned int n_points, const unsigned int n_comp)
...
\
  template void register_material_model<dim>(const std::string &, \
                                             const std::string &, \
...
\
  template std::unique_ptr<Interface<dim>> create_material_model<dim>(const std::string &model_name); \
\
  template void write_plugin_graph<dim>(std::ostream &); \
\
  template std::unique_ptr<Interface<dim>> create_material_model<dim>(ParameterHandler & prm); \
\
