...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      ...
      template <int dim>
      GrainBoundarySliding<dim>::GrainBoundarySliding() = default;
                                                          ...
                                                          const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        if (phase_function_values == std::vector<double>())
        ...
        else
          {
            ...
            slide_parameters.activation_energy   = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       activation_energies,
                                                                                                       composition);
            slide_parameters.activation_volume   = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       activation_volumes,
                                                                                                       composition);
            slide_parameters.stress_exponent     = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                     n_phase_transitions_per_composition,
                                                                                                     stress_exponents,
                                                                                                     composition);
            slide_parameters.grain_size_exponent = MaterialModel::MaterialUtilities::phase_average_value(
                                                   ...
                                                   const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        return compute_viscosity(
          strain_rate, pressure, temperature, fixed_grain_size, composition, phase_function_values, n_phase_transitions_per_composition);
      }
      ...
      GrainBoundarySliding<dim>::compute_viscosity(const double                     strain_rate,
                                                   ...
                                                   const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        const GrainBoundarySlidingParameters p =
          compute_slide_parameters(composition, phase_function_values, n_phase_transitions_per_composition);
        ...
        double viscosity_grainboundarysliding =
          0.5 * std::pow(p.prefactor, -1.0 / p.stress_exponent) * std::pow(grain_size, p.grain_size_exponent / p.stress_exponent) *
      ...
      GrainBoundarySliding<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        {
          prm.declare_entry(
            "Prefactors for grain boundary sliding",
            "6.2e-14",
            Patterns::Anything(),
          ...
          prm.declare_entry("Stress exponents for grain boundary sliding",
                            "1.8",
                            Patterns::List(Patterns::Double(0.)),
                            "Here we use the default values for ice as given in Goldsby & Kohlstedt, 2001. Units: None.");
          prm.declare_entry("Grain size exponents for grain boundary sliding",
                            "1.4.",
                            Patterns::Anything(),
                            "Here we use the default values for ice as given in Goldsby & Kohlstedt, 2001. Units: None.");
          prm.declare_entry("Activation energies for grain boundary sliding",
                            "49e3",
                            Patterns::Anything(),
                            "Here we use the default values for ice at T < 255 K as given in Goldsby & Kohlstedt, 2001. "
                            "Units: \\si{\\joule\\per\\mole}.");
          prm.declare_entry("Activation volumes for grain boundary sliding",
                            "-13e-6",
                            Patterns::Anything(),
                            "Here we use the default values for ice as given in Goldsby & Kohlstedt, 2001. "
                                                  ...
                                                  const std::unique_ptr<std::vector<unsigned int>> &expected_n_phases_per_composition)
      {
        prm.enter_subsection("Grain boundary sliding");
        {
          ...
          prefactors = Utilities::MapParsing::parse_map_to_double_array(prm.get("Prefactors for grain boundary sliding"), options);

          options.property_name = "Stress exponents for grain boundary sliding";
          stress_exponents =
            Utilities::MapParsing::parse_map_to_double_array(prm.get("Stress exponents for grain boundary sliding"), options);

          options.property_name = "Grain size exponents for grain boundary sliding";
          grain_size_exponents =
            Utilities::MapParsing::parse_map_to_double_array(prm.get("Grain size exponents for grain boundary sliding"), options);

          options.property_name = "Activation energies for grain boundary sliding";
          activation_energies =
            Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation energies for grain boundary sliding"), options);

          options.property_name = "Activation volumes for grain boundary sliding";
          activation_volumes =
            Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation volumes for grain boundary sliding"), options);
          ...
          for (const double prefactor : prefactors)
            AssertThrow(prefactor > 0., ExcMessage("The grain boundary sliding prefactor should be larger than zero."));
          prm.leave_subsection();
        }
      }
    }
