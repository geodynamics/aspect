    ...
    namespace Rheology
    {
      ...
      StrainDependent<dim>::declare_parameters(ParameterHandler &prm)
      {
        prm.declare_entry(
          ...
          "default",
          Patterns::Selection(
            "none|finite strain tensor|total strain|plastic weakening with plastic strain only|plastic weakening with total strain only|plastic weakening with plastic strain and viscous weakening with viscous strain|viscous weakening with viscous strain only|default"),
        ...
        prm.declare_entry("Start plasticity strain weakening intervals",
                          "0.",
        ...
        prm.declare_entry("End plasticity strain weakening intervals",
                          "1.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Cohesion strain weakening factors",
                          "1.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Friction strain weakening factors",
                          "1.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Start prefactor strain weakening intervals",
                          "0.",
                          Patterns::List(Patterns::Double(0.)),
                          "List of strain weakening interval initial strains "
        ...
        prm.declare_entry("End prefactor strain weakening intervals",
                          "1.",
                          Patterns::List(Patterns::Double(0.)),
                          "List of strain weakening interval final strains "
                          "for the diffusion and dislocation prefactor parameters of the "
        ...
        prm.declare_entry("Prefactor strain weakening factors",
                          "1.",
        ...
        prm.declare_entry("Use temperature activated strain softening",
                          "false",
                          Patterns::Bool(),
                          "Whether viscous strain softening factor depends on temperature");

        prm.declare_entry("Lower temperature for onset of strain weakening",
                          "823.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Lower temperature for maximum strain weakening",
                          "923.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Upper temperature for maximum strain weakening",
                          "1023.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Upper temperature for onset of strain weakening",
                          "1123.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Strain healing mechanism",
                          "no healing",
        ...
        prm.declare_entry("Strain healing temperature dependent recovery rate",
                          "1.e-15",
                          Patterns::Double(0),
                          "Recovery rate prefactor for temperature dependent "
                          "strain healing. Units: \\si{\\per\\second}");

        prm.declare_entry("Strain healing temperature dependent prefactor",
                          "15.",
                          Patterns::Double(0),
      ...
      StrainDependent<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        if (weakening_mechanism == finite_strain_tensor)
          {
            AssertThrow(
              this->n_compositional_fields() >= s,
              ExcMessage(
                "There must be enough compositional fields to track all components of the finite strain tensor (4 in 2d, 9 in 3d). "));
            ...
            if (dim == 2)
              {
                AssertThrow(n_s11 < n_s12 && n_s12 < n_s21 && n_s21 < n_s22,
                            ExcMessage("A material model with strain weakening using the full strain tensor only works if there are "
                                       ...
                                       "s11, s12, s21, s22."));
                AssertThrow(n_s22 == n_s11 + s - 1,
                            ExcMessage("The strain tensor components should be represented by consecutive fields."));
              }
            if (dim == 3)
              {
                ...
                AssertThrow(n_s11 < n_s12 && n_s12 < n_s13 && n_s13 < n_s21 && n_s21 < n_s22 && n_s22 < n_s23 && n_s23 < n_s31 &&
                              n_s31 < n_s32 && n_s32 < n_s33,
                            ExcMessage("A material model with strain weakening using the full strain tensor only works if there are "
                                       "compositional fields called sij, with i=1,..,dim and j=1,...,dim listed in the following order: "
                                       "s11, s12, s13, s21, s22, s23, s31, s32, s33."));
                AssertThrow(n_s33 == n_s11 + s - 1,
                            ExcMessage("The strain tensor components should be represented by consecutive fields."));
              }
          }
        ...
        if (weakening_mechanism != none)
          {
            AssertThrow(
              (this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::single_Advection_single_Stokes ||
               this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::single_Advection_iterated_Stokes ||
               this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::iterated_Advection_and_Stokes ||
               this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::single_Advection_iterated_Newton_Stokes ||
               this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::iterated_Advection_and_Newton_Stokes ||
               ...
               this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::no_Advection_no_Stokes),
        ...
        start_viscous_strain_weakening_intervals =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Start prefactor strain weakening intervals"), options);

        options.property_name = "End prefactor strain weakening intervals";
        end_viscous_strain_weakening_intervals =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("End prefactor strain weakening intervals"), options);

        options.property_name = "Prefactor strain weakening factors";
        viscous_strain_weakening_factors =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Prefactor strain weakening factors"), options);

        options.property_name = "Lower temperature for onset of strain weakening";
        viscous_strain_weakening_T0 =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Lower temperature for onset of strain weakening"), options);

        options.property_name = "Lower temperature for maximum strain weakening";
        viscous_strain_weakening_T1 =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Lower temperature for maximum strain weakening"), options);

        options.property_name = "Upper temperature for maximum strain weakening";
        viscous_strain_weakening_T2 =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Upper temperature for maximum strain weakening"), options);

        options.property_name = "Upper temperature for onset of strain weakening";
        viscous_strain_weakening_T3 =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Upper temperature for onset of strain weakening"), options);
            ...
            for (unsigned int i = 0; i < this->n_particle_managers(); ++i)
              {
                ...
                AssertThrow(particle_property_manager.plugin_name_exists("viscoplastic strain invariants") == false,
                            ExcMessage("This healing mechanism currently does not work if the strain is tracked on particles."));
              }
          }
      ...
      template <int dim>
      std::array<double, 3>
      StrainDependent<dim>::compute_strain_weakening_factors(const std::vector<double> &composition, const unsigned int j) const
      {
            ...
            case finite_strain_tensor:
              {
                ...
                const Tensor<2, dim> strain(make_array_view(&composition[0], &composition[0] + Tensor<2, dim>::n_independent_components));
      ...
      template <int dim>
      std::array<double, 3>
      StrainDependent<dim>::compute_strain_weakening_factors(const unsigned int j, const std::vector<double> &composition) const
      {
      ...
      template <int dim>
      std::array<double, 3>
      StrainDependent<dim>::apply_temperature_dependence_to_strain_weakening_factors(const std::array<double, 3> &weakening_factors,
      ...
      template <int dim>
      double
      StrainDependent<dim>::calculate_strain_healing(const MaterialModel::MaterialModelInputs<dim> &in, const unsigned int j) const
      {
      ...
      StrainDependent<dim>::calculate_plastic_weakening(const double strain_ii, const unsigned int j) const
      {
        // Constrain the second strain invariant of the previous timestep by the strain interval
        const double cut_off_strain_ii =
          std::clamp(strain_ii, start_plastic_strain_weakening_intervals[j], end_plastic_strain_weakening_intervals[j]);
      ...
      template <int dim>
      double
      StrainDependent<dim>::calculate_viscous_weakening(const double strain_ii, const unsigned int j) const
      {
        // Constrain the second strain invariant of the previous timestep by the strain interval
        const double cut_off_strain_ii =
          std::clamp(strain_ii, start_viscous_strain_weakening_intervals[j], end_viscous_strain_weakening_intervals[j]);
      ...
      template <int dim>
      void
      StrainDependent<dim>::fill_reaction_outputs(const MaterialModel::MaterialModelInputs<dim> &in,
                                                  const int                                      i,
                                                  ...
                                                  MaterialModel::MaterialModelOutputs<dim>      &out) const
      {
        ...
        if (this->simulator_is_past_initialization() && this->get_timestep_number() > 0 &&
            in.requests_property(MaterialProperties::reaction_terms))
          {
            ...
            const double edot_ii    = std::max(std::sqrt(std::max(-Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(
            ...
            std::vector<Point<dim>> quadrature_positions(1,
                                                         this->get_mapping().transform_real_to_unit_cell(in.current_cell, in.position[i]));
            ...
            in.current_cell->get_dof_values(this->get_old_solution(), old_solution_values.begin(), old_solution_values.end());
            ...
            if (weakening_mechanism == plastic_weakening_with_plastic_strain_only)
              {
                ...
                if (!composition_evaluators[strain_index])
                  composition_evaluators[strain_index] = std::make_unique<FEPointEvaluation<1, dim>>(this->get_mapping(),
                                                                                                     this->get_fe(),
                                                                                                     update_values,
                ...
                out.reaction_terms[i][strain_index] = std::max(delta_e_ii_plastic, -composition_evaluators[strain_index]->get_value(0));
              }
            if (weakening_mechanism == viscous_weakening_with_viscous_strain_only)
              {
                ...
                composition_evaluators[strain_index]->evaluate({old_solution_values.data(), old_solution_values.size()},
                                                               EvaluationFlags::values);
                out.reaction_terms[i][strain_index] = std::max(delta_e_ii_viscous, -composition_evaluators[strain_index]->get_value(0));
              }
            if (weakening_mechanism == total_strain || weakening_mechanism == plastic_weakening_with_total_strain_only)
              {
                ...
                composition_evaluators[strain_index]->evaluate({old_solution_values.data(), old_solution_values.size()},
                                                               EvaluationFlags::values);
                out.reaction_terms[i][strain_index] = std::max(delta_e_ii, -composition_evaluators[strain_index]->get_value(0));
              }
            if (weakening_mechanism == plastic_weakening_with_plastic_strain_and_viscous_weakening_with_viscous_strain)
              {
                ...
                if (!composition_evaluators[plastic_strain_index])
                  composition_evaluators[plastic_strain_index] = std::make_unique<FEPointEvaluation<1, dim>>(
                    this->get_mapping(), this->get_fe(), update_values, component_indices[plastic_strain_index]);
                ...
                composition_evaluators[plastic_strain_index]->evaluate({old_solution_values.data(), old_solution_values.size()},
                                                                       EvaluationFlags::values);
                out.reaction_terms[i][plastic_strain_index] =
                  std::max(delta_e_ii_plastic, -composition_evaluators[plastic_strain_index]->get_value(0));
                ...
                if (!composition_evaluators[viscous_strain_index])
                  composition_evaluators[viscous_strain_index] = std::make_unique<FEPointEvaluation<1, dim>>(
                    this->get_mapping(), this->get_fe(), update_values, component_indices[viscous_strain_index]);
                ...
                composition_evaluators[viscous_strain_index]->evaluate({old_solution_values.data(), old_solution_values.size()},
                                                                       EvaluationFlags::values);
                out.reaction_terms[i][viscous_strain_index] =
                  std::max(delta_e_ii_viscous, -composition_evaluators[viscous_strain_index]->get_value(0));
              }
      ...
      template <int dim>
      void
      StrainDependent<dim>::compute_finite_strain_reaction_terms(const MaterialModel::MaterialModelInputs<dim> &in,
                                                                 MaterialModel::MaterialModelOutputs<dim>      &out) const
      {
        if (in.current_cell.state() == IteratorState::valid && this->get_timestep_number() > 0 &&
            in.requests_property(MaterialProperties::reaction_terms) && weakening_mechanism == finite_strain_tensor)
          {
            ...
            in.current_cell->get_dof_values(this->get_solution(), solution_values.begin(), solution_values.end());
      ...
      template <int dim>
      ComponentMask
      StrainDependent<dim>::get_strain_composition_mask() const
      {
      ...
      template <int dim>
      WeakeningMechanism
      StrainDependent<dim>::get_weakening_mechanism() const
      {
      ...
      template <int dim>
      HealingMechanism
      StrainDependent<dim>::get_healing_mechanism() const
      {
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class StrainDependent<dim>;
