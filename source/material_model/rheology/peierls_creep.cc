...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      ...
      template <int dim>
      PeierlsCreep<dim>::PeierlsCreep() = default;
                                                  ...
                                                  const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        if (phase_function_values == std::vector<double>())
        ...
        else
          {
            ...
            creep_parameters.stress_exponent   = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                     n_phase_transitions_per_composition,
                                                                                                     stress_exponents,
                                                                                                     composition);
            creep_parameters.activation_energy = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       activation_energies,
                                                                                                       composition);
            creep_parameters.activation_volume = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       activation_volumes,
                                                                                                       composition);
            creep_parameters.peierls_stress    = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                    n_phase_transitions_per_composition,
                                                                                                    peierls_stresses,
                                                                                                    composition);
            creep_parameters.glide_parameter_p = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       glide_parameters_p,
                                                                                                       composition);
            creep_parameters.glide_parameter_q = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       glide_parameters_q,
                                                                                                       composition);
            creep_parameters.fitting_parameter = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                       n_phase_transitions_per_composition,
                                                                                                       fitting_parameters,
                                                                                                       composition);
            creep_parameters.stress_cutoff     = MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                                                   n_phase_transitions_per_composition,
                                                                                                   stress_cutoffs,
                                                                                                   composition);
          }
                                                 ...
                                                 const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        if (apply_strict_cutoff)
          {
            const std::pair<double, double> log_edot_and_deriv =
              compute_exact_log_strain_rate_and_derivative(std::log(p.stress_cutoff), pressure, temperature, p);
        ...
        const std::pair<double, double> log_edot_and_deriv =
          compute_exact_log_strain_rate_and_derivative(log_stress_ii, pressure, temperature, p);
        ...
        while (std::abs(strain_rate_residual) > strain_rate_residual_threshold && stress_iteration < stress_max_iteration_number)
          {
            ...
            const bool abort_newton_iteration = !numbers::is_finite(log_stress_ii) || !numbers::is_finite(strain_rate_residual) ||
            ...
            AssertThrow(!abort_newton_iteration,
                        ExcMessage("No convergence has been reached in the loop that determines "
                                   ...
                                   "Residual is " +
                                   Utilities::to_string(strain_rate_residual) + " after " + Utilities::to_string(stress_iteration) +
                                   " iterations. "
        ...
        switch (peierls_creep_flow_law)
          {
            case viscosity_approximation:
              {
                viscosity = compute_approximate_viscosity(
                  strain_rate, pressure, temperature, composition, phase_function_values, n_phase_transitions_per_composition);
                break;
              }
            case exact:
              {
                viscosity = compute_exact_viscosity(
                  strain_rate, pressure, temperature, composition, phase_function_values, n_phase_transitions_per_composition);
                break;
              }
            default:
                                                                        ...
                                                                        const PeierlsCreepParameters creep_parameters) const
      {
        ...
        const double edot_ii =
          p.prefactor * std::pow(stress, s + p.stress_exponent) * std::pow(p.fitting_parameter * p.peierls_stress, -s) * arrhenius;
                                                                            ...
                                                                            const PeierlsCreepParameters creep_parameters) const
      {
        ...
        const double log_strain_rate =
          std::log(p.prefactor) + total_stress_exponent * log_stress - s * std::log(p.fitting_parameter * p.peierls_stress) + log_arrhenius;
                                                                      ...
                                                                      const PeierlsCreepParameters creep_parameters) const
      {
        ...
        if (stress < p.stress_cutoff)
        ...
        else
          {
            ...
            const double deriv_log =
              p.stress_exponent + p.glide_parameter_p * p.glide_parameter_q * b * c * std::pow(1 - c, p.glide_parameter_q - 1);
      ...
      {
        prm.declare_entry("Peierls creep flow law",
        ...
        prm.declare_entry("Peierls strain rate residual tolerance",
                          "1e-10",
                          Patterns::Double(0.),
        ...
        prm.declare_entry("Maximum Peierls strain rate iterations",
                          "40",
                          Patterns::Integer(0),
        ...
        prm.declare_entry("Prefactors for Peierls creep",
                          "1.4e-19",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Stress exponents for Peierls creep",
                          "2.0",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Activation energies for Peierls creep",
                          "320e3",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Activation volumes for Peierls creep",
                          "1.4e-5",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Peierls stresses",
                          "5.e9",
                          Patterns::Anything(),
                          ...
                          "If only one value is given, then all use the same value. "
                          "Units: \\si{\\pascal}");
        prm.declare_entry("Peierls fitting parameters",
                          "0.17",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Peierls glide parameters p",
                          "0.5",
                          Patterns::Anything(),
                          ...
                          "those corresponding to chemical compositions. "
                          "If only one value is given, then all use the same value. Units: none");
        prm.declare_entry("Peierls glide parameters q",
                          "1.0",
                          Patterns::Anything(),
                          ...
                          "those corresponding to chemical compositions. "
                          "If only one value is given, then all use the same value. Units: none");
        prm.declare_entry("Cutoff stresses for Peierls creep",
                          "0.0",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Apply strict stress cutoff for Peierls creep",
                          "false",
                          Patterns::Bool(),
                                          ...
                                          const std::unique_ptr<std::vector<unsigned int>> &expected_n_phases_per_composition)
      {
        ...
        prefactors = Utilities::MapParsing::parse_map_to_double_array(prm.get("Prefactors for Peierls creep"), options);

        options.property_name = "Stress exponents for Peierls creep";
        stress_exponents      = Utilities::MapParsing::parse_map_to_double_array(prm.get("Stress exponents for Peierls creep"), options);

        options.property_name = "Activation energies for Peierls creep";
        activation_energies   = Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation energies for Peierls creep"), options);

        options.property_name = "Activation volumes for Peierls creep";
        activation_volumes    = Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation volumes for Peierls creep"), options);

        options.property_name = "Peierls stresses";
        peierls_stresses      = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls stresses"), options);

        options.property_name = "Peierls fitting parameters";
        fitting_parameters    = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls fitting parameters"), options);

        options.property_name = "Peierls glide parameters p";
        glide_parameters_p    = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls glide parameters p"), options);

        options.property_name = "Peierls glide parameters q";
        glide_parameters_q    = Utilities::MapParsing::parse_map_to_double_array(prm.get("Peierls glide parameters q"), options);

        options.property_name = "Cutoff stresses for Peierls creep";
        stress_cutoffs        = Utilities::MapParsing::parse_map_to_double_array(prm.get("Cutoff stresses for Peierls creep"), options);
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class PeierlsCreep<dim>;
