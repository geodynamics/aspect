...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    namespace
    {
      template <int dim>
      std::vector<std::string>
      make_viscosity_additional_outputs_names(const std::vector<typename ViscosityAdditionalOutputs<dim>::Property> &active_properties)
      {
        std::vector<std::string> names;
    ...
    template <int dim>
    ViscosityAdditionalOutputs<dim>::ViscosityAdditionalOutputs(const unsigned int n_points, const std::vector<Property> &active_properties)
    ...
    namespace Rheology
    {
      ...
      template <int dim>
      ViscoPlastic<dim>::ViscoPlastic() = default;



      template <int dim>
      IsostrainViscosities
      ViscoPlastic<dim>::calculate_isostrain_viscosities(const MaterialModel::MaterialModelInputs<dim> &in,
                                                         ...
                                                         const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        if (this->get_parameters().enable_elasticity)
          {
            ...
            const std::vector<unsigned int> &stress_field_indices =
              this->introspection().get_indices_for_fields_of_type(CompositionalFieldDescription::stress);
            stress_0_advected = (Utilities::Tensors::to_symmetric_tensor<dim>(&in.composition[i][stress_field_indices[0]],
                                                                              &in.composition[i][stress_field_indices[0]] +
            ...
            stress_old = (Utilities::Tensors::to_symmetric_tensor<dim>(
              &in.composition[i][stress_field_indices[SymmetricTensor<2, dim>::n_independent_components]],
              &in.composition[i][stress_field_indices[SymmetricTensor<2, dim>::n_independent_components]] +
                SymmetricTensor<2, dim>::n_independent_components));
        ...
        const bool use_reference_strainrate =
          this->simulator_is_past_initialization() == false || (this->get_timestep_number() == 0 && this->get_nonlinear_iteration() == 0);
        ...
        if (use_reference_strainrate)
          ...
          edot_ii = std::max(std::sqrt(std::max(-Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(
        ...
        for (unsigned int j = 0; j < volume_fractions.size(); ++j)
          {
            ...
            AssertThrow(temperature_for_viscosity != 0,
                        ExcMessage("The temperature used in the calculation of the visco-plastic rheology is zero. "
                                   ...
                                   Utilities::to_string(in.temperature[i]) + "), adiabatic_temperature_gradient_for_viscosity (" +
                                   ...
                                   Utilities::to_string(in.pressure[i]) + ")."));
            {
              ...
              const double viscosity_diffusion =
                (viscous_flow_law != dislocation ?
                   diffusion_creep.compute_viscosity(
                     pressure_for_creep, temperature_for_viscosity, j, phase_function_values, n_phase_transitions_per_composition) :
                   numbers::signaling_nan<double>());

              // Step 1b: compute viscosity from dislocation creep law
              const double viscosity_dislocation =
                (viscous_flow_law != diffusion ? dislocation_creep.compute_viscosity(edot_ii,
                                                                                     pressure_for_creep,
              ...
              switch (viscous_flow_law)
                {
                  case diffusion:
                    {
                      non_yielding_viscosity = compositional_viscosity_prefactors.compute_viscosity(
                        in, viscosity_diffusion, j, i, CompositionalViscosityPrefactors<dim>::ModifiedFlowLaws::diffusion);
                  ...
                  case dislocation:
                    {
                      non_yielding_viscosity = compositional_viscosity_prefactors.compute_viscosity(
                        in, viscosity_dislocation, j, i, CompositionalViscosityPrefactors<dim>::ModifiedFlowLaws::dislocation);
                  ...
                  case frank_kamenetskii:
                    {
                      non_yielding_viscosity =
                        frank_kamenetskii_rheology->compute_viscosity(in.temperature[i],
                                                                      j,
                                                                      pressure_for_creep,
                                                                      this->get_adiabatic_conditions().density(
                  ...
                  case composite:
                    {
                      const double scaled_viscosity_diffusion = compositional_viscosity_prefactors.compute_viscosity(
                        in, viscosity_diffusion, j, i, CompositionalViscosityPrefactors<dim>::ModifiedFlowLaws::diffusion);
                      const double scaled_viscosity_dislocation = compositional_viscosity_prefactors.compute_viscosity(
                        in, viscosity_dislocation, j, i, CompositionalViscosityPrefactors<dim>::ModifiedFlowLaws::dislocation);
              ...
              if (use_peierls_creep)
                {
                  const double viscosity_peierls = peierls_creep->compute_viscosity(
                    edot_ii, pressure_for_creep, temperature_for_viscosity, j, phase_function_values, n_phase_transitions_per_composition);
              ...
              if (use_grain_boundary_sliding)
                {
                  const double viscosity_grain_boundary_sliding = grain_boundary_sliding_rheology->compute_viscosity(
                    edot_ii, pressure_for_creep, temperature_for_viscosity, j, phase_function_values, n_phase_transitions_per_composition);
                  non_yielding_viscosity = (non_yielding_viscosity * viscosity_grain_boundary_sliding) /
                                           (non_yielding_viscosity + viscosity_grain_boundary_sliding);
                }
            }
            ...
            if (strain_rheology.use_temperature_activated_strain_softening)
              weakening_factors =
                strain_rheology.apply_temperature_dependence_to_strain_weakening_factors(weakening_factors, temperature_for_viscosity, j);
                ...
                if (use_reference_strainrate == false)
                  {
                    ...
                    effective_edot_ii = std::max(
                      std::sqrt(std::max(-Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(effective_strain_rate), 0.)),
                      min_strain_rate);
                  }
              }
            ...
            output_parameters.drucker_prager_parameters[j] =
              drucker_prager_plasticity.compute_drucker_prager_parameters(j, phase_function_values, n_phase_transitions_per_composition);
            ...
            output_parameters.drucker_prager_parameters[j].angle_internal_friction = friction_models.compute_friction_angle(
              effective_edot_ii, j, output_parameters.drucker_prager_parameters[j].angle_internal_friction, in.position[i]);
            ...
            const double yield_stress =
              drucker_prager_plasticity.compute_yield_stress(pressure_for_plasticity, output_parameters.drucker_prager_parameters[j]);
                ...
                case stress_limiter:
                  {
                    ...
                    const double viscosity_limiter =
                      yield_stress / (2.0 * ref_strain_rate) *
                      std::pow((effective_edot_ii / ref_strain_rate), 1. / exponents_stress_limiter[j] - 1.0);
            ...
            const double maximum_viscosity_for_composition =
              MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                    n_phase_transitions_per_composition,
                                                                    maximum_viscosity,
                                                                    j,
                                                                    MaterialModel::MaterialUtilities::PhaseUtilities::logarithmic);
            const double minimum_viscosity_for_composition =
              MaterialModel::MaterialUtilities::phase_average_value(phase_function_values,
                                                                    n_phase_transitions_per_composition,
                                                                    minimum_viscosity,
                                                                    j,
            ...
            output_parameters.composition_viscosities[j] =
              std::clamp(effective_viscosity, minimum_viscosity_for_composition, maximum_viscosity_for_composition);
            ...
            if (this->get_parameters().enable_prescribed_dilation == true)
              {
                ...
                const std::pair<double, double> dilation_terms =
                  drucker_prager_plasticity.compute_dilation_terms_for_stokes_system(output_parameters.drucker_prager_parameters[j],
                                                                                     non_yielding_viscosity,
                                                                                     effective_edot_ii);
      ...
      template <int dim>
      void
      ViscoPlastic<dim>::compute_viscosity_derivatives(const unsigned int                             i,
                                                       const std::vector<double>                     &volume_fractions,
            ...
            for (unsigned int component = 0; component < SymmetricTensor<2, dim>::n_independent_components; ++component)
              {
                ...
                const double strain_rate_difference =
                  std::max(std::fabs(current_strain_rate[strain_rate_indices]), min_strain_rate) * finite_difference_accuracy;
                const SymmetricTensor<2, dim> forward_strain_rate =
                  current_strain_rate + strain_rate_difference * Utilities::nth_basis_for_symmetric_tensors<dim>(component);

                in_derivatives.strain_rate[i] = forward_strain_rate;

                const IsostrainViscosities forward_isostrain_values = calculate_isostrain_viscosities(
                  in_derivatives, i, volume_fractions, phase_function_values, n_phase_transitions_per_composition);
            ...
            for (unsigned int composition_index = 0; composition_index < n_compositions; ++composition_index)
              {
                composition_viscosity_derivatives_wrt_pressure[composition_index] =
                  pressure_difference > 0.0 ? (forward_isostrain_values.composition_viscosities[composition_index] -
                                               current_isostrain_values.composition_viscosities[composition_index]) /
                                                pressure_difference :
                                              0.0;
                ...
                if (enable_dilation)
                  {
                    composition_dilation_derivatives_wrt_pressure[composition_index] =
                      pressure_difference > 0.0 ? ((forward_isostrain_values.dilation_rhs_terms[composition_index] -
                                                    current_isostrain_values.dilation_rhs_terms[composition_index]) -
                                                   (forward_isostrain_values.dilation_lhs_terms[composition_index] -
                                                    current_isostrain_values.dilation_lhs_terms[composition_index]) *
                                                     in.pressure[i]) /
      ...
      template <int dim>
      ComponentMask
      ViscoPlastic<dim>::get_volumetric_composition_mask() const
      {
      ...
      ViscoPlastic<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Reference strain rate",
                          "1.0e-15",
                          Patterns::Double(0.),
        ...
        prm.declare_entry("Viscous flow law",
                          "composite",
        ...
        prm.declare_entry("Yield mechanism",
                          "drucker",
        ...
        prm.declare_entry("Allow negative pressures in plasticity",
                          "false",
                          Patterns::Bool(),
                          ...
                          "be set to zero.");
        prm.declare_entry("Use adiabatic pressure in creep viscosity",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Use adiabatic pressure in plasticity",
                          "false",
                          Patterns::Bool(),
                          "Whether to use the adiabatic pressure instead of the full "
        ...
        prm.declare_entry("Include Grain Boundary Sliding",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Include Peierls creep",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Stress limiter exponents",
                          "1.0",
        ...
        prm.declare_entry("Adiabat temperature gradient for viscosity",
                          "0.0",
                          Patterns::Double(0.),
                                          ...
                                          const std::unique_ptr<std::vector<unsigned int>> &expected_n_phases_per_composition)
      {
        ...
        AssertThrow(ref_strain_rate >= min_strain_rate,
                    ExcMessage(
                      "The reference strain rate for the viscoplastic material model should be larger than the minimum strain rate."));
        ...
        minimum_viscosity = Utilities::MapParsing::parse_map_to_double_array(prm.get("Minimum viscosity"), options);

        options.property_name = "Maximum viscosity";
        maximum_viscosity     = Utilities::MapParsing::parse_map_to_double_array(prm.get("Maximum viscosity"), options);
        ...
        for (auto p1 = maximum_viscosity.begin(), p2 = minimum_viscosity.begin(); p1 != maximum_viscosity.end(); ++p1, ++p2)
        ...
        if (this->get_parameters().enable_prescribed_dilation)
          AssertThrow(allow_negative_pressures_in_plasticity == true && use_adiabatic_pressure_in_plasticity == false,
        ...
        exponents_stress_limiter = Utilities::MapParsing::parse_map_to_double_array(prm.get("Stress limiter exponents"), options);
      ...
      template <int dim>
      void
      ViscoPlastic<dim>::fill_plastic_outputs(const unsigned int                             i,
                                              const std::vector<double>                     &volume_fractions,
            ...
            for (unsigned int j = 0; j < volume_fractions.size(); ++j)
              {
                ...
                plastic_out->friction_angles[i] +=
                  constants::radians_to_degree * volume_fractions[j] * drucker_prager_parameters.angle_internal_friction;

                plastic_out->yield_stresses[i] +=
                  volume_fractions[j] * drucker_prager_plasticity.compute_yield_stress(pressure_for_plasticity, drucker_prager_parameters);
              }
          }
      }
        ...
        if (out.template has_additional_output_object<ViscosityAdditionalOutputs<dim>>() == false)
          {
            ...
            const bool compute_dislocation_viscosity = (viscous_flow_law != diffusion && viscous_flow_law != frank_kamenetskii);

            const bool compute_diffusion_viscosity = (viscous_flow_law != dislocation && viscous_flow_law != frank_kamenetskii);

            if (compute_diffusion_viscosity)
              active_properties.emplace_back(ViscosityAdditionalOutputs<dim>::Property::diffusion_viscosity);

            if (compute_dislocation_viscosity)
              active_properties.emplace_back(ViscosityAdditionalOutputs<dim>::Property::dislocation_viscosity);

            out.additional_outputs.push_back(std::make_unique<ViscosityAdditionalOutputs<dim>>(n_points, active_properties));
          }
      }
      ...
      template <int dim>
      void
      Rheology::ViscoPlastic<dim>::fill_viscosity_outputs(const unsigned int                        i,
                                                          const std::vector<double>                &volume_fractions,
                                                          MaterialModel::MaterialModelOutputs<dim> &out,
                                                          const IsostrainViscosities               &isostrain_viscosities) const
      {
        if (const std::shared_ptr<ViscosityAdditionalOutputs<dim>> viscosity_out =
              out.template get_additional_output_object<ViscosityAdditionalOutputs<dim>>())
          {
            viscosity_out->dislocation_viscosities[i] =
              MaterialUtilities::average_value(volume_fractions, isostrain_viscosities.dislocation_viscosities, viscosity_averaging);

            viscosity_out->diffusion_viscosities[i] =
              MaterialUtilities::average_value(volume_fractions, isostrain_viscosities.diffusion_viscosities, viscosity_averaging);
          }
      }
    }
  }
