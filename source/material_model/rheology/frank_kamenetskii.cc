...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    {
      template <int dim>
      FrankKamenetskii<dim>::FrankKamenetskii() = default;
                                               ...
                                               const double       gravity) const
      {
        ...
        const double viscosity_frank_kamenetskii =
          prefactors_frank_kamenetskii[composition] *
          std::exp(viscosity_ratios_frank_kamenetskii[composition] * 0.5 * (1.0 - temperature / reference_temperatures[composition]) +
                   pressure_prefactors_frank_kamenetskii[composition] * (pressure - reference_pressures[composition]) /
                     (density * gravity * max_depth));
      ...
      {
        prm.declare_entry("Viscosity ratios for Frank Kamenetskii",
                          "15.",
                          ...
                          "Units: None");
        prm.declare_entry("Prefactors for Frank Kamenetskii",
                          "1.e21",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Pressure prefactors for Frank Kamenetskii",
                          "0.0",
                          Patterns::List(Patterns::Double(0.)),
      ...
      FrankKamenetskii<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        viscosity_ratios_frank_kamenetskii =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Viscosity ratios for Frank Kamenetskii"), options);

        options.property_name = "Prefactors for Frank Kamenetskii";
        prefactors_frank_kamenetskii =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Prefactors for Frank Kamenetskii"), options);

        options.property_name = "Pressure prefactors for Frank Kamenetskii";
        pressure_prefactors_frank_kamenetskii =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Pressure prefactors for Frank Kamenetskii"), options);

        options.property_name = "Reference temperatures for Frank Kamenetskii";
        reference_temperatures =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Reference temperatures for Frank Kamenetskii"), options);

        options.property_name = "Reference pressures for Frank Kamenetskii";
        reference_pressures =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Reference pressures for Frank Kamenetskii"), options);
        ...
        for (unsigned int j = 0; j < reference_temperatures.size(); ++j)
          {
            if (reference_temperatures[j] == std::numeric_limits<double>::max())
              reference_temperatures[j] = this->get_adiabatic_surface_temperature();
            if (reference_pressures[j] == std::numeric_limits<double>::max())
              reference_pressures[j] = this->get_surface_pressure();
          }
      }
    }
  }
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class FrankKamenetskii<dim>;
