...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    {
      template <int dim>
      DiffusionDislocation<dim>::DiffusionDislocation() = default;

      template <int dim>
      std::vector<double>
      DiffusionDislocation<dim>::calculate_isostrain_viscosities(const double                   pressure,
        ...
        for (unsigned int j = 0; j < n_chemical_composition_fields; ++j)
          {
            ...
            const double prefactor_stress_diffusion =
              diffusion_creep_parameters.prefactor * std::pow(grain_size, -diffusion_creep_parameters.grain_size_exponent) *
              std::exp(
                -(std::max(diffusion_creep_parameters.activation_energy + pressure * diffusion_creep_parameters.activation_volume, 0.0)) /
            ...
            double stress_ii =
              (prefactor_stress_diffusion > (0.5 / maximum_viscosity) ? edot_ii / prefactor_stress_diffusion : 0.5 / maximum_viscosity);
            ...
            while (std::abs(log_strain_rate_residual) > log_strain_rate_residual_threshold &&
                   stress_iteration < stress_max_iteration_number)
              {
                const std::pair<double, double> log_diff_edot_and_deriv =
                  diffusion_creep.compute_log_strain_rate_and_derivative(log_stress_ii, pressure, temperature, diffusion_creep_parameters);
                const std::pair<double, double> log_disl_edot_and_deriv = dislocation_creep.compute_log_strain_rate_and_derivative(
                  log_stress_ii, pressure, temperature, dislocation_creep_parameters);
                ...
                log_strain_rate_deriv =
                  (strain_rate_diffusion * log_diff_edot_and_deriv.second + strain_rate_dislocation * log_disl_edot_and_deriv.second) /
                ...
                // iteration instead.
                const bool abort_newton_iteration =
                  !numbers::is_finite(stress_ii) || !numbers::is_finite(log_strain_rate_residual) ||
                  !numbers::is_finite(log_strain_rate_deriv) || log_strain_rate_deriv < std::numeric_limits<double>::min() ||
                    ...
                    do
                      {
                        ...
                        const double diffusion_prefactor =
                          0.5 * std::pow(diffusion_creep_parameters.prefactor, -1.0 / diffusion_creep_parameters.stress_exponent);
                        const double diffusion_grain_size_dependence =
                          std::pow(grain_size, diffusion_creep_parameters.grain_size_exponent / diffusion_creep_parameters.stress_exponent);
                        const double diffusion_strain_rate_dependence =
                          std::pow(diffusion_strain_rate,
                                   (1. - diffusion_creep_parameters.stress_exponent) / diffusion_creep_parameters.stress_exponent);
                        const double diffusion_T_and_P_dependence = std::exp(
                          std::max(diffusion_creep_parameters.activation_energy + pressure * diffusion_creep_parameters.activation_volume,
                                   0.0) /
                          (constants::gas_constant * temperature));
                        ...
                        const double dislocation_prefactor =
                          0.5 * std::pow(dislocation_creep_parameters.prefactor, -1.0 / dislocation_creep_parameters.stress_exponent);
                        const double dislocation_strain_rate_dependence =
                          std::pow(dislocation_strain_rate,
                                   (1. - dislocation_creep_parameters.stress_exponent) / dislocation_creep_parameters.stress_exponent);
                        const double dislocation_T_and_P_dependence =
                          std::exp(std::max(dislocation_creep_parameters.activation_energy +
                                              pressure * dislocation_creep_parameters.activation_volume,
                                            0.0) /
                                   (dislocation_creep_parameters.stress_exponent * constants::gas_constant * temperature));

                        const double dislocation_viscosity =
                          std::clamp(dislocation_prefactor * dislocation_strain_rate_dependence * dislocation_T_and_P_dependence,
                                     minimum_viscosity,
                                     maximum_viscosity);
                        ...
                        AssertThrow(stress_iteration < stress_max_iteration_number,
                                    ExcMessage("No convergence has been reached in the loop that determines "
                                               ...
                                               "Residual is " +
                                               Utilities::to_string(log_strain_rate_residual) + " after " +
                                               Utilities::to_string(stress_iteration) +
                                               " iterations. "
                                                   ...
                                                   const SymmetricTensor<2, dim> &strain_rate) const
      {
        ...
        const std::vector<double> composition_viscosities = calculate_isostrain_viscosities(pressure, temperature, strain_rate);
      ...
      DiffusionDislocation<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Maximum strain rate ratio iterations",
                          "40",
                          Patterns::Integer(0),
        ...
        prm.declare_entry("Thermal diffusivity", "0.8e-6", Patterns::Double(0.), "Units: \\si{\\meter\\squared\\per\\second}.");
        prm.declare_entry("Heat capacity",
                          "1.25e3",
                          Patterns::Double(0.),
        ...
        prm.declare_entry("Prefactors for diffusion creep",
                          "1.5e-15",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Stress exponents for diffusion creep",
                          "1.",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Grain size exponents for diffusion creep",
                          "3.",
                          Patterns::List(Patterns::Double(0.)),
                          ...
                          "those corresponding to chemical compositions. "
                          "If only one value is given, then all use the same value.  Units: None.");
        prm.declare_entry("Activation energies for diffusion creep",
                          "375e3",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Activation volumes for diffusion creep",
                          "6e-6",
                          Patterns::List(Patterns::Double(0.)),
        ...
        prm.declare_entry("Prefactors for dislocation creep",
                          "1.1e-16",
                          Patterns::List(Patterns::Double(0.)),
                          "List of viscosity prefactors, $A$, for background mantle and compositional fields, "
                          "for a total of N+1 values, where N is the number of all compositional fields or only "
        ...
        prm.declare_entry("Stress exponents for dislocation creep",
                          "3.5",
                          Patterns::List(Patterns::Double(0.)),
                          ...
                          "those corresponding to chemical compositions. "
                          "If only one value is given, then all use the same value.  Units: None.");
        prm.declare_entry("Activation energies for dislocation creep",
                          "530e3",
                          Patterns::List(Patterns::Double(0.)),
                          "List of activation energies, $E_a$, for background mantle and compositional fields, "
                          "for a total of N+1 values, where N is the number of all compositional fields or only "
                          ...
                          "If only one value is given, then all use the same value. "
                          "Units: \\si{\\joule\\per\\mole}.");
        prm.declare_entry("Activation volumes for dislocation creep",
                          "1.4e-5",
                          Patterns::List(Patterns::Double(0.)),
                          "List of activation volumes, $V_a$, for background mantle and compositional fields, "
                          "for a total of N+1 values, where N is the number of all compositional fields or only "
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class DiffusionDislocation<dim>;
