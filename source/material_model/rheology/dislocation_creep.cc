...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      ...
      template <int dim>
      DislocationCreep<dim>::DislocationCreep() = default;
      ...
      DislocationCreep<dim>::compute_viscosity(const double                     strain_rate,
                                               ...
                                               const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        const DislocationCreepParameters p =
        ...
        double viscosity =
          0.5 * std::pow(p.prefactor, -1 / p.stress_exponent) *
          std::exp((p.activation_energy + pressure * p.activation_volume) / (constants::gas_constant * temperature * p.stress_exponent)) *
                                                                ...
                                                                const DislocationCreepParameters creep_parameters) const
      {
        ...
        // mechanism, stress: deviatoric stress, E: activation energy, P: pressure, V; activation volume, n: stress exponent, R: gas
        // constant, T: temperature.
        const double strain_rate = creep_parameters.prefactor * std::pow(stress, creep_parameters.stress_exponent) *
                                   std::exp(-(creep_parameters.activation_energy + pressure * creep_parameters.activation_volume) /
                                            (constants::gas_constant * temperature));

        const double dstrain_rate_dstress = creep_parameters.prefactor * creep_parameters.stress_exponent *
                                                                    ...
                                                                    const DislocationCreepParameters creep_parameters) const
      {
        ...
        const double log_strain_rate =
          std::log(creep_parameters.prefactor) + creep_parameters.stress_exponent * log_stress -
                                              ...
                                              const std::unique_ptr<std::vector<unsigned int>> &expected_n_phases_per_composition)
      {
        ...
        prefactors = Utilities::MapParsing::parse_map_to_double_array(prm.get("Prefactors for dislocation creep"), options);

        options.property_name = "Stress exponents for dislocation creep";
        stress_exponents = Utilities::MapParsing::parse_map_to_double_array(prm.get("Stress exponents for dislocation creep"), options);

        options.property_name = "Activation energies for dislocation creep";
        activation_energies =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation energies for dislocation creep"), options);

        options.property_name = "Activation volumes for dislocation creep";
        activation_volumes = Utilities::MapParsing::parse_map_to_double_array(prm.get("Activation volumes for dislocation creep"), options);
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class DislocationCreep<dim>;
