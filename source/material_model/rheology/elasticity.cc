            ...
            for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
              {
                ...
                // In the absence of the elastic damper, this expression is identical to the one found in
                // Moresi et al. (2003, J. Comp. Phys., equation 29).
                const double damped_elastic_viscosity = calculate_elastic_viscosity(average_elastic_shear_moduli[i]);

                // stress_0 is the combination of the elastic stress stored at the end of the last time step and the stress change generated by local rotation
                ...
                // stress new is the (new) viscoelastic stress
