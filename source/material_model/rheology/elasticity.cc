...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    namespace Rheology
    {
      ...
      {
        prm.declare_entry("Elastic shear moduli",
                          "75.0e9",
        ...
        prm.declare_entry("Use fixed elastic time step",
                          "unspecified",
        ...
        prm.declare_entry("Fixed elastic time step",
                          "1.e3",
                          Patterns::Double(0.),
                          ...
                          "\\si{\\second} otherwise.");
        prm.declare_entry("Stabilization time scale factor",
                          "1.",
                          Patterns::Double(1.),
        ...
        prm.declare_entry("Elastic damper viscosity",
                          "0.0",
                          Patterns::Double(0.),
      ...
      Elasticity<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        AssertThrow(fixed_elastic_time_step > 0, ExcMessage("The fixed elastic time step must be greater than zero"));
        ...
        // If the operator splitting scheme is used, make sure to use its fixed step solver, as we know the update and it should be applied
        // in one step.
        ...
        if ((this->get_parameters().mapped_particle_properties).count(this->introspection().compositional_index_for_name("ve_stress_xx")))
          AssertThrow(!this->get_parameters().use_operator_splitting,
                      ExcMessage(
                        "If stresses are tracked on particles, the stress update is applied by the particle property 'elastic stress' "
                        "and operator splitting should not be turned on. "));

        // Check that 3+3 in 2D or 6+6 in 3D stress fields exist.
        AssertThrow((this->introspection().get_number_of_fields_of_type(CompositionalFieldDescription::stress) ==
                     2 * SymmetricTensor<2, dim>::n_independent_components),
        ...
        std::vector<std::string> stress_field_names =
          this->introspection().get_names_for_fields_of_type(CompositionalFieldDescription::stress);
        ...
        AssertThrow(
          ((stress_field_indices[2 * n_independent_components - 1] - stress_field_indices[0]) == (2 * n_independent_components - 1)),
          ExcMessage(
            "Rheology model Elasticity requires that the compositional fields representing stress tensor components are listed in consecutive order."));
        ...
        AssertThrow((this->get_parameters().nonlinear_solver == Parameters<dim>::NonlinearSolver::iterated_Advection_and_Stokes ||
      ...
      Elasticity<dim>::create_elastic_additional_outputs(MaterialModel::MaterialModelOutputs<dim> &out) const
      {
        ...
        if (out.template has_additional_output_object<ElasticAdditionalOutputs<dim>>() == false)
          {
            ...
            out.additional_outputs.push_back(std::make_unique<ElasticAdditionalOutputs<dim>>(n_points));
          }
        ...
        if (out.template has_additional_output_object<HeatingModel::PrescribedShearHeatingOutputs<dim>>() == false)
          {
            const unsigned int n_points = out.n_evaluation_points();
            out.additional_outputs.push_back(std::make_unique<HeatingModel::PrescribedShearHeatingOutputs<dim>>(n_points));
          }
                                            ...
                                            MaterialModel::MaterialModelOutputs<dim>      &out) const
      {
        ...
        const std::shared_ptr<MaterialModel::ElasticOutputs<dim>> elastic_out =
        ...
        const std::shared_ptr<HeatingModel::PrescribedShearHeatingOutputs<dim>> heating_out =
          out.template get_additional_output_object<HeatingModel::PrescribedShearHeatingOutputs<dim>>();
        ...
        if (in.requests_property(MaterialProperties::additional_outputs))
          {
            ...
            if (this->get_parameters().material_averaging != MaterialAveraging::none)
              {
                MaterialModelOutputs<dim> out_copy(out.n_evaluation_points(), this->introspection().n_compositional_fields);
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                ...
                const SymmetricTensor<2, dim> stress_old(
                  Utilities::Tensors::to_symmetric_tensor<dim>(&in.composition[i][stress_start_index + n_independent_components],
                                                               &in.composition[i][stress_start_index + n_independent_components] +
                ...
                if (elastic_out != nullptr)
                  {
                    elastic_out->elastic_force[i] =
                      -1. * (viscosity_ratio * stress_0_advected + (1. - timestep_ratio) * (1. - viscosity_ratio) * stress_old);
                ...
                const SymmetricTensor<2, dim> stress = 2. * effective_creep_viscosity * deviatoric_strain_rate +
                                                       viscosity_ratio * stress_0_advected +
                ...
                const SymmetricTensor<2, dim> visco_plastic_strain_rate =
                  deviatoric_strain_rate - ((stress - stress_0_advected) / (2. * dtc * average_elastic_shear_moduli[i]));
                // If compressible,
        ...
        for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
          {
            ...
            elastic_additional_out->deviatoric_stress[i] = 2. * eta * deviatoric_strain_rate + eta / elastic_viscosity * stress_0_advected +
                                                           (1. - timestep_ratio) * (1. - eta / elastic_viscosity) * stress_old;
        ...
        if (in.current_cell.state() == IteratorState::valid && in.requests_property(MaterialProperties::reaction_terms))
          {
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                Assert(out.reaction_terms[i].size() == this->n_compositional_fields(),
                       ExcMessage("Out reaction terms i not equal to n fields."));
                ...
                // const SymmetricTensor<2, dim> stress_change = this->get_timestep() * (symmetrize(rotation * Tensor<2, dim>(stress_t[i]))
                // - symmetrize(Tensor<2, dim>(stress_t[i]) * rotation));
                const SymmetricTensor<2, dim> stress_change =
                  this->get_timestep() * symmetrize(rotation * Tensor<2, dim>(stress_t[i]) - Tensor<2, dim>(stress_t[i]) * rotation);
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                ...
                const SymmetricTensor<2, dim> stress_0_t(
                  Utilities::Tensors::to_symmetric_tensor<dim>(&in.composition[i][stress_start_index],
                                                               &in.composition[i][stress_start_index] + n_independent_components));
      ...
      Elasticity<dim>::elastic_timestep() const
      {
        ...
        if (use_fixed_elastic_time_step && this->get_timestep_number() > 0 && this->simulator_is_past_initialization())
          AssertThrow(fixed_elastic_time_step >= this->get_timestep(),
                      ExcMessage("The elastic timestep has to be equal to or bigger than the numerical timestep"));

        const double dte =
          ((this->get_timestep_number() > 0 && this->simulator_is_past_initialization() && use_fixed_elastic_time_step == false) ?
      ...
      Elasticity<dim>::calculate_timestep_ratio() const
      {
        ...
        if (!this->simulator_is_past_initialization() || (this->get_timestep_number() == 0 && this->get_timestep() == 0))
          dtc = std::min(std::min(this->get_parameters().maximum_time_step, this->get_parameters().maximum_first_time_step),
                         elastic_timestep());
      ...
      template <int dim>
      double
      Elasticity<dim>::calculate_elastic_viscosity(const double shear_modulus) const
      {
      ...
      template <int dim>
      double
      Elasticity<dim>::calculate_viscoelastic_viscosity(const double viscosity, const double shear_modulus) const
      {
      ...
      Elasticity<dim>::calculate_viscoelastic_strain_rate(const SymmetricTensor<2, dim> &strain_rate,
                                                          const SymmetricTensor<2, dim> &stress_0_advected,
                                                          const SymmetricTensor<2, dim> &stress_old,
                                                          ...
                                                          const double                   shear_modulus) const
      {
        ...
        const SymmetricTensor<2, dim> edot =
          strain_rate + 0.5 * stress_0_advected / elastic_viscosity +
      ...
      Elasticity<dim>::retrieve_stress_previous_timestep(const MaterialModel::MaterialModelInputs<dim> &in,
        ...
        if (this->get_parameters().compositional_field_methods[stress_start_index] == Parameters<dim>::AdvectionFieldMethod::fem_field)
          {
            ...
            if (!evaluator_composition)
              evaluator_composition.reset(new FEPointEvaluation<n_independent_components, dim>(
                this->get_mapping(),
                this->get_fe(),
                update_values,
            ...
            evaluator_composition->evaluate(old_solution_values, EvaluationFlags::values);
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                ...
                stress_t[i] =
                  Utilities::Tensors::to_symmetric_tensor<dim>(&composition_values[0], &composition_values[0] + n_independent_components);
              }
          }
        else
          {
