...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      template <int dim>
      double
      FrictionModels<dim>::compute_friction_angle(const double       current_edot_ii,
            ...
            case dynamic_friction:
              {
                ...
                const double mu =
                  (std::tan(dynamic_angles_of_internal_friction[volume_fraction_index]) +
                   (std::tan(static_friction_angle) - std::tan(dynamic_angles_of_internal_friction[volume_fraction_index])) /
                     (1. + std::pow((current_edot_ii / dynamic_characteristic_strain_rate), dynamic_friction_smoothness_exponent)));
      ...
      template <int dim>
      FrictionMechanism
      FrictionModels<dim>::get_friction_mechanism() const
      {
      ...
      {
        prm.declare_entry("Friction mechanism",
                          "none",
        ...
        prm.declare_entry("Dynamic characteristic strain rate",
                          "1e-12",
                          Patterns::Double(0),
        ...
        prm.declare_entry("Dynamic angles of internal friction",
                          "2",
        ...
        prm.declare_entry("Dynamic friction smoothness exponent",
                          "1",
                          Patterns::Double(0),
      ...
      FrictionModels<dim>::parse_parameters(ParameterHandler &prm)
      {
        ...
        dynamic_angles_of_internal_friction =
          Utilities::MapParsing::parse_map_to_double_array(prm.get("Dynamic angles of internal friction"), options);

        // Convert angles from degrees to radians
        for (double &angle : dynamic_angles_of_internal_friction)
          {
            AssertThrow(angle <= 90, ExcMessage("Dynamic angles of friction must be <= 90 degrees"));
        ...
        if (friction_mechanism == function)
          {
            prm.enter_subsection("Friction function");
            {
              ...
              try
                {
                  friction_function = std::make_unique<Functions::ParsedFunction<dim>>(chemical_field_names.size());
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class FrictionModels<dim>;
