...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      ...
      template <int dim>
      CompositeViscoPlastic<dim>::CompositeViscoPlastic() = default;
                                                              ...
                                                              const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
                ...
                if (use_diffusion_creep)
                  {
                    diffusion_creep_parameters.push_back(
                      diffusion_creep->compute_creep_parameters(composition, phase_function_values, n_phase_transitions_per_composition));
                    viscoplastic_viscosity_guess_c = std::min(
                      diffusion_creep->compute_viscosity(
                        pressure, temperature, grain_size, composition, phase_function_values, n_phase_transitions_per_composition),
                      viscoplastic_viscosity_guess_c);
                  }

                if (use_dislocation_creep)
                  {
                    dislocation_creep_parameters.push_back(
                      dislocation_creep->compute_creep_parameters(composition, phase_function_values, n_phase_transitions_per_composition));
                    viscoplastic_viscosity_guess_c =
                      std::min(dislocation_creep->compute_viscosity(
                                 edot_ii, pressure, temperature, composition, phase_function_values, n_phase_transitions_per_composition),
                               viscoplastic_viscosity_guess_c);
                  }
                ...
                if (use_peierls_creep)
                  {
                    ...
                    viscoplastic_viscosity_guess_c =
                      std::min(peierls_creep->compute_approximate_viscosity(edot_ii, pressure, temperature, composition),
                               viscoplastic_viscosity_guess_c);
                  }

                if (use_drucker_prager)
                  {
                    Rheology::DruckerPragerParameters drucker_prager_parameters_c =
                      drucker_prager->compute_drucker_prager_parameters(composition,
                                                                        phase_function_values,
                                                                        n_phase_transitions_per_composition);
                    drucker_prager_parameters.push_back(drucker_prager_parameters_c);
                    viscoplastic_viscosity_guess_c =
                      std::min(drucker_prager->compute_viscosity(drucker_prager_parameters_c.cohesion,
                                                                 drucker_prager_parameters_c.angle_internal_friction,
                                                                 pressure,
                                                                 edot_ii,
                                                                 drucker_prager_parameters_c.max_yield_stress),
                               viscoplastic_viscosity_guess_c);
                  }
        ...
        for (unsigned int i = 0; i < active_compositions.size(); ++i)
          {
            ...
            if (use_diffusion_creep)
              {
                log_edot_and_deriv[i][0] = diffusion_creep->compute_log_strain_rate_and_derivative(
                  log_viscoplastic_stress, pressure, temperature, grain_size, diffusion_creep_parameters[i]);
                log_edot_and_deriv[i][0].first += log_volume_fraction;
              }
            if (use_dislocation_creep)
              {
                log_edot_and_deriv[i][1] = dislocation_creep->compute_log_strain_rate_and_derivative(log_viscoplastic_stress,
                                                                                                     pressure,
                                                                                                     temperature,
                                                                                                     dislocation_creep_parameters[i]);
                log_edot_and_deriv[i][1].first += log_volume_fraction;
              }
            if (use_peierls_creep)
              {
                log_edot_and_deriv[i][2] = peierls_creep->compute_approximate_log_strain_rate_and_derivative(log_viscoplastic_stress,
                                                                                                             pressure,
                                                                                                             temperature,
                                                                                                             peierls_creep_parameters[i]);
                log_edot_and_deriv[i][2].first += log_volume_fraction;
              }
            if (use_drucker_prager)
              {
                log_edot_and_deriv[i][3] =
                  drucker_prager->compute_log_strain_rate_and_derivative(log_viscoplastic_stress, pressure, drucker_prager_parameters[i]);
        ...
        std::pair<double, double> log_edot_ii_and_deriv_iterate =
          calculate_isostress_log_strain_rate_and_derivative(log_edot_and_deriv, viscoplastic_stress, partial_strain_rates);
        ...
        while (std::abs(log_strain_rate_residual) > log_strain_rate_residual_threshold && stress_iteration < stress_max_iteration_number)
          {
            ...
            AssertThrow(!abort_newton_iteration,
                        ExcMessage("No convergence has been reached in the loop that determines "
                                   ...
                                   "Residual is " +
                                   Utilities::to_string(log_strain_rate_residual) + " after " + Utilities::to_string(stress_iteration) +
                                   " iterations. "
      ...
      CompositeViscoPlastic<dim>::calculate_isostress_log_strain_rate_and_derivative(
        ...
        std::vector<double>                                         &partial_strain_rates) const
      {
        ...
        const double log_strain_rate_derivative =
          (strain_rate_scaling_factor * viscoplastic_strain_rate_sum * log_viscoplastic_strain_rate_derivative + f) / strain_rate;
            ...
            if (volume_fractions[composition] > 2. * std::numeric_limits<double>::epsilon())
              {
                ...
                viscosity += (volume_fractions[composition] * compute_composition_viscosity(pressure,
                                                                                            temperature,
                                                                                            grain_size,
                                                                                            composition,
                                                                                            strain_rate,
                                                                ...
                                                                const std::vector<unsigned int> &n_phase_transitions_per_composition) const
      {
        ...
        if (use_diffusion_creep)
          {
            ...
            viscoplastic_viscosity_guess =
              std::min(diffusion_creep->compute_viscosity(
                         pressure, temperature, grain_size, composition, phase_function_values, n_phase_transitions_per_composition),
                       viscoplastic_viscosity_guess);
          }
        ...
        if (use_drucker_prager)
          {
            drucker_prager_parameters =
              drucker_prager->compute_drucker_prager_parameters(composition, phase_function_values, n_phase_transitions_per_composition);
        ...
        if (use_diffusion_creep)
          log_edot_and_deriv[0] = diffusion_creep->compute_log_strain_rate_and_derivative(
            log_viscoplastic_stress, pressure, temperature, grain_size, diffusion_creep_parameters);

        if (use_dislocation_creep)
          log_edot_and_deriv[1] = dislocation_creep->compute_log_strain_rate_and_derivative(log_viscoplastic_stress,
                                                                                            pressure,
                                                                                            temperature,
                                                                                            dislocation_creep_parameters);

        if (use_peierls_creep)
          log_edot_and_deriv[2] = peierls_creep->compute_approximate_log_strain_rate_and_derivative(log_viscoplastic_stress,
                                                                                                    pressure,
                                                                                                    temperature,
                                                                                                    peierls_creep_parameters);

        if (use_drucker_prager)
          log_edot_and_deriv[3] =
            drucker_prager->compute_log_strain_rate_and_derivative(log_viscoplastic_stress, pressure, drucker_prager_parameters);
        ...
        // accommodate strain at that creep stress.
        std::pair<double, double> log_edot_ii_and_deriv_iterate =
          calculate_composition_log_strain_rate_and_derivative(log_edot_and_deriv, viscoplastic_stress, partial_strain_rates);
        double log_strain_rate_residual = log_edot_ii_and_deriv_iterate.first - log_edot_ii;
      ...
      template <int dim>
      std::pair<double, double>
      CompositeViscoPlastic<dim>::calculate_composition_log_strain_rate_and_derivative(
      ...
      CompositeViscoPlastic<dim>::declare_parameters(ParameterHandler &prm)
      {
        ...
        prm.declare_entry("Include diffusion creep in composite rheology",
                          "true",
                          Patterns::Bool(),
                          "Whether to include diffusion creep in the composite rheology formulation.");

        prm.declare_entry("Include dislocation creep in composite rheology",
                          "true",
                          Patterns::Bool(),
                          "Whether to include dislocation creep in the composite rheology formulation.");

        prm.declare_entry("Include Peierls creep in composite rheology",
                          "true",
                          Patterns::Bool(),
                          "Whether to include Peierls creep in the composite rheology formulation.");

        prm.declare_entry("Include Drucker Prager plasticity in composite rheology",
                          "true",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Strain rate residual tolerance",
                          "1e-10",
                          Patterns::Double(0.),
                          "Tolerance for correct log strain rate residual.");
        prm.declare_entry("Maximum creep strain rate iterations",
                          "40",
                          Patterns::Integer(0),
      ...
      {
        viscosity_averaging_scheme = ViscosityAveraging::parse("Viscosity averaging scheme", prm);
        ...
        AssertThrow(minimum_viscosity > 0, ExcMessage("Minimum viscosity needs to be larger than zero."));
        ...
        AssertThrow(active_flow_mechanisms.size() > 0, ExcMessage("You need to include at least one deformation mechanism."));
      }
    }
  }
}
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
#define INSTANTIATE(dim) template class CompositeViscoPlastic<dim>;
