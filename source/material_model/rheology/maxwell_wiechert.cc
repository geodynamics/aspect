...
#include <aspect/material_model/rheology/maxwell_wiechert.h>
...
namespace aspect
{
  namespace MaterialModel
  {
    namespace Rheology
    {
      template <int dim>
      void
      MaxwellWierchert<dim>::declare_parameters (ParameterHandler &prm)
      {
        ...
        prm.declare_entry ("Viscosities", "1.e21",
                           ...
                           "Units: $\\text{Pa}\\text{s}$.");
        prm.declare_entry ("Stress fractions", "1.0",
                           Patterns::List(Patterns::Double (0.)),
                           "List of stress fractions for each of the maxwell arms. "
                           "This is the constant multiplier to the stress in each maxwell arm. "
                           "We ensure that the sum is equal to one");
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::parse_parameters (ParameterHandler &prm)
      {
        ...
        AssertThrow (this->introspection().chemical_composition_field_names().size() == 0,
                     ExcMessage("Rheology model maxwell wierchert only works if there are no chemical compositions."));
        ...
        AssertThrow (number_of_maxwell_arms > 0, ExcMessage("Rheology model maxwell wierchert only works if there is at least one maxwell arm."));

        elastic_shear_moduli = Utilities::possibly_extend_from_1_to_N (dealii::Utilities::string_to_double(
                                                                         Utilities::split_string_list(prm.get("Elastic shear moduli"))), number_of_maxwell_arms, "Elastic shear moduli");

        stress_fractions = Utilities::possibly_extend_from_1_to_N (dealii::Utilities::string_to_double(
                                                                     Utilities::split_string_list(prm.get("Stress fractions"))), number_of_maxwell_arms, "Stress fractions");

        number_of_maxwell_arms = prm.get_integer("Number of maxwell arms");

        //normalize the values because they should all add to one.
        const double stress_fractions_sum = std::accumulate(stress_fractions.begin(), stress_fractions.end(), 0.0);
        for (double &fraction : stress_fractions)
          {
            AssertThrow(fraction >= 0.0, ExcMessage("Stress fractions must be non-negative."));
            fraction /= stress_fractions_sum;
          }
        ...
        AssertThrow((this->introspection().get_number_of_fields_of_type(CompositionalFieldDescription::stress) == 2*SymmetricTensor<2,dim>::n_independent_components),
                    ExcMessage("Maxwell Wiechert rheology requires 3+3 in 2D or 6+6 in 3D fields of type stress"));

        AssertThrow((this->introspection().get_number_of_fields_of_type(CompositionalFieldDescription::stress_fraction) == number_of_maxwell_arms),
                    ExcMessage("Maxwell Wiechert rheology requires m number of fields of type stress_fraction, where m is the number of maxwell arms."));

        std::vector<std::string> stress_fraction_field_names   = this->introspection().get_names_for_fields_of_type(CompositionalFieldDescription::stress_fraction);
        std::vector<unsigned int> stress_fraction_field_indices = this->introspection().get_indices_for_fields_of_type(CompositionalFieldDescription::stress_fraction);


        for (unsigned int i = 0; i < number_of_maxwell_arms; ++i)
          {
            const std::string field_name = "stress_fraction_" + Utilities::int_to_string(i+1);

            AssertThrow(this->introspection().compositional_name_exists(field_name),
                        ExcMessage("The Maxwell weichert rheology requires a compositional field named '" + field_name + "' to be present, "
                                   "representing the stress fraction carried by each of the Maxwell arms " ));
        ...
        for (auto stress_field : stress_field_indices)
          AssertThrow(use_discontinuous_composition_discretization[stress_field],
                      ExcMessage("The material model with elasticity enabled require "
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::create_maxwell_wiechert_additional_outputs (MaterialModel::MaterialModelOutputs<dim> &out) const
      {
        ...
        if (out.template has_additional_output_object<ReactionRateOutputs<dim>>() == false &&
          ...
          {
            const unsigned int n_points = out.n_evaluation_points();
            out.additional_outputs.push_back(std::make_unique<Rheology::StressFractionOutputs<dim>>(n_points, number_of_maxwell_arms));
          }
      }
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::fill_elastic_outputs (const MaterialModel::MaterialModelInputs<dim> &in,
            ...
            for (unsigned int i=0; i < in.n_evaluation_points(); ++i)
              {
                ...
                const double viscosity_ratio = 1 ; //effective_creep_viscosity / max;
                ...
                // The effective creep viscosity is scaled
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::fill_elastic_additional_outputs (const MaterialModel::MaterialModelInputs<dim> &in,
                                                              const std::vector<double> &average_elastic_shear_moduli,
                                                              MaterialModel::MaterialModelOutputs<dim> &out) const
      {
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::fill_reaction_outputs (const MaterialModel::MaterialModelInputs<dim> &in,
                                                    MaterialModel::MaterialModelOutputs<dim> &out) const
      {
      ...
      template <int dim>
      void
      MaxwellWierchert<dim>::fill_reaction_rates (const MaterialModel::MaterialModelInputs<dim> &in,
                                                  MaterialModel::MaterialModelOutputs<dim> &out) const
      {
            ...
            for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
              {
                ...
                std::vector<SymmetricTensor<2, dim>> stress_mw_k(number_of_maxwell_arms);
                SymmetricTensor<2, dim> stress_t;
                ...
                for (unsigned int a = 0; a < number_of_maxwell_arms; ++a)
                  {
                    ...
                    const double f_k_old = in.composition[i][f_index];

                    const double relaxation_k  = viscosities[a] / (viscosities[a] + elastic_timestep()*elastic_shear_moduli[a]);

                    stress_mw_k[a] = 2. * calculate_maxwell_wiechert_viscosity(a) * Utilities::Tensors::consistent_deviator(in.strain_rate[i]) + f_k_old * relaxation_k * stress_0_t;
                    stress_t += stress_mw_k[a];
                  }
                ...
                // TODO: this is not entirely correct. Based on the Bob's computation this
                // needs to be updated (see equation 37).
                const double stress_t_norm = std::fabs(Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(stress_t));

                for (unsigned int a = 0; a < number_of_maxwell_arms; ++a)
                  {
                    const unsigned int f_index = this->introspection().compositional_index_for_name("stress_fraction_" + Utilities::int_to_string(a+1));

                    const double stress_mw_k_norm = std::fabs(Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(stress_mw_k[a]));

                    const double f_k_old = in.composition[i][f_index];
                    const double f_k_new = (stress_t_norm > 0.0) ? (stress_mw_k_norm / stress_t_norm) : f_k_old;

                    reaction_rate_out->reaction_rates[i][f_index] = (f_k_new - f_k_old) / dtc;
                  }
              }
          }
      }
      ...
      template <int dim>
      double
      MaxwellWierchert<dim>::elastic_timestep () const
      {
      ...
      // TODO: see if I can reduce the copies from elasticity
      template <int dim>
      double
      MaxwellWierchert<dim>::calculate_timestep_ratio() const
      {
      ...
      calculate_elastic_viscosity (const double shear_modulus) const
      {
        ...
        const double timestep_ratio = calculate_timestep_ratio();
        return shear_modulus*elastic_timestep()*timestep_ratio;
      }
      ...
      double
      MaxwellWierchert<dim>::
      calculate_maxwell_wiechert_viscosity (unsigned int a) const
      {
        return 1.0 / (1.0/(elastic_timestep()*elastic_shear_moduli[a]) + 1.0/viscosities[a]);
      }
...
#define INSTANTIATE(dim) \
  template class MaxwellWierchert<dim>;
