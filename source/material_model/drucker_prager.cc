...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    void
    DruckerPrager<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
          ...
          if (in.requests_property(MaterialProperties::viscosity))
            {
              ...
              const double edot_ii_strict =
                (this->simulator_is_past_initialization() == false ?
                   ...
                   ((this->get_timestep_number() == 0) && (in.strain_rate[i].norm() <= std::numeric_limits<double>::min()) ?
              ...
              if (std::sqrt(strain_rate_effective) <= std::numeric_limits<double>::min())
              ...
              else
                {
                  ...
                  const double viscosity_pressure_derivative =
                    drucker_prager_plasticity.compute_derivative(angle_of_internal_friction, std::sqrt(strain_rate_effective));
                  ...
                  if (derivatives != nullptr && in.requests_property(MaterialProperties::additional_outputs))
                    {
                      ...
                      derivatives->viscosity_derivative_wrt_strain_rate[i] =
                        deviator_tensor<dim>() * effective_viscosity_strain_rate_derivatives;
    ...
    template <int dim>
    bool
    DruckerPrager<dim>::is_compressible() const
    {
      return false;
    }
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Drucker Prager");
        {
          ...
          prm.enter_subsection("Viscosity");
          {
            ...
            prm.declare_entry("Cohesion", "2e7", Patterns::Double(0.), "The value of the cohesion $C$. Units: \\si{\\pascal}.");
          }
