...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    bool
    EntropyModel<dim>::is_compressible() const
    {
      return true;
    }
    ...
    template <int dim>
    double
    EntropyModel<dim>::equilibrate_temperature(const std::vector<double> &temperature,
                                               const std::vector<double> &mass_fractions,
                                               const std::vector<double> &entropy,
                                               const std::vector<double> &specific_heat,
                                               const double               pressure,
                                               std::vector<double>       &component_equilibrated_S) const
    {
      AssertThrow(material_file_names.size() == temperature.size() && temperature.size() == mass_fractions.size() &&
                    temperature.size() == entropy.size() && temperature.size() == specific_heat.size(),
      ...
      while (equilibration == false)
        {
          if (iteration >= multicomponent_max_iteration)
            {
              ...
              error_message << "The components are not equilibrated after the maximum number of iterations: "
                            << std::to_string(multicomponent_max_iteration)
                            << ". Equilibration failed at pressure = " << std::to_string(pressure)
                            << ". The last equilibrated T = " << std::to_string(std::exp(ln_equilibrated_T))
          ...
          for (unsigned int i = 0; i < material_file_names.size(); ++i)
            {
              component_entropies[i] =
                component_entropies[i] + component_heat_capacities[i] * (ln_equilibrated_T - std::log(component_temperatures[i]));
    ...
    template <int dim>
    void
    EntropyModel<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in, MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      const std::vector<unsigned int> &composition_indices =
        this->introspection().get_indices_for_fields_of_type(CompositionalFieldDescription::chemical_composition);

      AssertThrow(
        composition_indices.size() == material_file_names.size() - 1,
        ExcMessage(
          "The 'entropy model' material model assumes that there exists a background field in addition to the compositional fields, "
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          for (unsigned int j = 0; j < material_file_names.size(); ++j)
            {
              ...
              eos_outputs.compressibilities[j] =
                ((entropy_reader[j]->density_gradient(component_entropy[j], pressure)) * pressure_unit_vector) / eos_outputs.densities[j];
            }
          ...
          volume_fractions = MaterialUtilities::compute_volumes_from_masses(mass_fractions, eos_outputs.densities, true);
          ...
          if (c_p < max_exact_specific_heat)
            ...
            {
              const double fraction_max_c_p =
                std::exp(ln_ratio_max_exact_max_limit_specific_heat *
                         std::exp((std::log(c_p / max_exact_specific_heat)) / ln_ratio_max_exact_max_limit_specific_heat));
              out.specific_heat[i] = max_limit_specific_heat * fraction_max_c_p;
            }
          out.compressibilities[i] =
            MaterialUtilities::average_value(mass_fractions, eos_outputs.compressibilities, MaterialUtilities::arithmetic);
                  ...
                  if (this->get_parameters().use_operator_splitting && reaction_rate_out != nullptr)
                    {
                      ...
                      const unsigned int timestep_number = this->simulator_is_past_initialization() ? this->get_timestep_number() : 0;

                      if (c_is_entropy_field == true && timestep_number > 0)
                        reaction_rate_out->reaction_rates[i][c] =
                          (component_equilibrated_S[c_is_nth_entropy_field] - in.composition[i][entropy_indices[c_is_nth_entropy_field]]) /
                          this->get_timestep();
                    }
          ...
          if (in.requests_property(MaterialProperties::viscosity) || in.requests_property(MaterialProperties::additional_outputs))
            {
              ...
              if (adjusted_inputs.temperature[i] * reference_temperature == 0)
                ...
                {
                  double vis_lateral = std::exp(-lateral_viscosity_prefactor_lookup->lateral_viscosity(depth) * delta_temperature /
                                                (adjusted_inputs.temperature[i] * reference_temperature));
                  ...
                  if (plastic_out != nullptr && in.requests_property(MaterialProperties::additional_outputs))
                    {
                      ...
                      plastic_out->yield_stresses[i]  = drucker_prager_plasticity.compute_yield_stress(pressure, drucker_prager_parameters);
                    }

                  const double strain_rate_effective = std::fabs(Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(
                    Utilities::Tensors::consistent_deviator(in.strain_rate[i])));

                  if (std::sqrt(strain_rate_effective) >= std::numeric_limits<double>::min())
                    {
                      const double eta_plastic =
                        drucker_prager_plasticity.compute_viscosity(pressure, std::sqrt(strain_rate_effective), drucker_prager_parameters);
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Entropy model");
        {
          ...
          prm.declare_entry("Lateral viscosity file name",
                            "constant_lateral_vis_prefactor.txt",
                            Patterns::Anything(),
          ...
          prm.declare_entry("Angle of internal friction",
                            "0.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Maximum limited specific heat capacity",
                            "1e50",
                            Patterns::Double(0.),
                            "The maximum allowed value for the specific heat capacity.");

          prm.declare_entry("Maximum exact specific heat capacity",
                            "1e50",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Maximum iteration for multicomponent equilibration",
                            "50",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Multicomponent equilibration tolerance",
                            "1e-7",
                            Patterns::Double(0.),
        ...
        Rheology::AsciiDepthProfile<dim>::declare_parameters(prm, "Depth dependent viscosity");
    ...
    EntropyModel<dim>::parse_parameters(ParameterHandler &prm)
    {
      if (this->introspection().composition_type_exists(CompositionalFieldDescription::Type::chemical_composition))
        {
          AssertThrow(this->get_parameters().use_operator_splitting == true,
                      ExcMessage(
                        "The 'entropy model' material model requires the use of operator splitting for multiple chemical composition. "
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Entropy model");
        {
          ...
          AssertThrow(
            max_limit_specific_heat >= max_exact_specific_heat,
            ExcMessage(
              "The maximum limited specific heat capacity must be greater than or equal to the maximum exact specific heat capacity."));
