...
namespace aspect
{
  namespace MaterialModel
  {
    template <int dim>
    void
    MulticomponentCompressible<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &in,
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          for (unsigned int c = 0; c < volume_fractions.size(); ++c)
            phase_averaged_thermal_conductivities[c] = MaterialUtilities::phase_average_value(
              phase_function_values, n_phase_transitions_for_each_chemical_composition, thermal_conductivities, c);
          out.thermal_conductivities[i] =
            MaterialUtilities::average_value(volume_fractions, phase_averaged_thermal_conductivities, MaterialUtilities::arithmetic);
    ...
    template <int dim>
    bool
    MulticomponentCompressible<dim>::is_compressible() const
    {
