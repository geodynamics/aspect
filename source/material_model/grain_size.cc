...
namespace aspect
{
  namespace MaterialModel
  {
      ...
      for (unsigned i = 0; i < n_material_data; ++i)
        {
          if (material_file_format == perplex)
            material_lookup.push_back(std::make_unique<MaterialModel::MaterialUtilities::Lookup::PerplexReader>(
              datadirectory + material_file_names[i], use_bilinear_interpolation, this->get_mpi_communicator()));
          else if (material_file_format == hefesto)
            material_lookup.push_back(
              std::make_unique<MaterialModel::MaterialUtilities::Lookup::HeFESToReader>(datadirectory + material_file_names[i],
    ...
    template <int dim>
    unsigned int
    GrainSize<dim>::get_phase_index(const MaterialUtilities::PhaseFunctionInputs<dim> &in) const
    {
      ...
      for (unsigned int j = 0; j < n_phase_transitions[0]; ++j)
        {
          MaterialUtilities::PhaseFunctionInputs<dim> phase_inputs(in.temperature, in.pressure, in.depth, in.pressure_depth_derivative, j);
    ...
    template <int dim>
    double
    GrainSize<dim>::diffusion_viscosity(const double       temperature,
                                        ...
                                        const unsigned int phase_index) const
    {
      ...
      return diffusion_creep_prefactor[phase_index] * std::pow(second_strain_rate_invariant, strain_rate_dependence) *
             std::pow(grain_size, diffusion_creep_grain_size_exponent[phase_index] / diffusion_creep_exponent[phase_index]) * energy_term;
    }
    ...
    template <int dim>
    double
    GrainSize<dim>::dislocation_viscosity(const double                   temperature,
                                          const double                   adiabatic_temperature,
                                          const double                   adiabatic_pressure,
                                          ...
                                          const double                   viscosity_guess) const
    {
      ...
      if (viscosity_guess == 0)
        dis_viscosity =
          dislocation_creep_prefactor[phase_index] * std::pow(second_strain_rate_invariant, strain_rate_dependence) * energy_term;
      else
      ...
      while ((std::abs((dis_viscosity - dis_viscosity_old) / dis_viscosity) > dislocation_viscosity_iteration_threshold) &&
             (i < dislocation_viscosity_iteration_number))
        {
          const SymmetricTensor<2, dim> dislocation_strain_rate =
            diffusion_viscosity / (diffusion_viscosity + dis_viscosity) * shear_strain_rate;
          const double dislocation_strain_rate_invariant =
            std::sqrt(std::max(-Utilities::Tensors::consistent_second_invariant_of_deviatoric_tensor(dislocation_strain_rate), 0.));

          dis_viscosity_old = dis_viscosity;
          dis_viscosity =
            dislocation_creep_prefactor[phase_index] * std::pow(dislocation_strain_rate_invariant, strain_rate_dependence) * energy_term;
          ++i;
        }
    ...
    template <int dim>
    double
    GrainSize<dim>::enthalpy(const double               temperature,
    ...
    template <int dim>
    double
    GrainSize<dim>::seismic_Vp(const double               temperature,
                               const double               pressure,
                               const std::vector<double> &compositional_fields,
                               const Point<dim> & /*position*/) const
    {
    ...
    template <int dim>
    double
    GrainSize<dim>::seismic_Vs(const double               temperature,
                               const double               pressure,
                               const std::vector<double> &compositional_fields,
                               const Point<dim> & /*position*/) const
    {
    ...
    template <int dim>
    double
    GrainSize<dim>::density(const double               temperature,
                            const double               pressure,
                            const std::vector<double> &compositional_fields,
                            const Point<dim> & /*position*/) const
    {
    ...
    template <int dim>
    bool
    GrainSize<dim>::is_compressible() const
    {
      return (reference_compressibility != 0) || use_table_properties;
    }
    ...
    template <int dim>
    double
    GrainSize<dim>::compressibility(const double               temperature,
                                    const double               pressure,
                                    const std::vector<double> &compositional_fields,
                                    const Point<dim>          &position) const
    {
    ...
    template <int dim>
    double
    GrainSize<dim>::thermal_expansion_coefficient(const double               temperature,
                                                  const double               pressure,
                                                  const std::vector<double> &compositional_fields,
                                                  const Point<dim> & /*position*/) const
    {
    ...
    template <int dim>
    double
    GrainSize<dim>::specific_heat(const double               temperature,
                                  const double               pressure,
                                  const std::vector<double> &compositional_fields,
                                  const Point<dim> & /*position*/) const
    {
    ...
    GrainSize<dim>::enthalpy_derivative(const typename Interface<dim>::MaterialModelInputs &in) const
    {
      ...
      if (in.current_cell.state() == IteratorState::valid)
        {
          ...
          in.current_cell->get_dof_values(this->get_current_linearization_point(), solution_values.begin(), solution_values.end());

          // Only create the evaluator the first time we get here
          if (!temperature_evaluator)
            temperature_evaluator = std::make_unique<FEPointEvaluation<1, dim>>(this->get_mapping(),
                                                                                this->get_fe(),
                                                                                update_values,
                                                                                this->introspection().component_indices.temperature);
          if (!pressure_evaluator)
            pressure_evaluator = std::make_unique<FEPointEvaluation<1, dim>>(this->get_mapping(),
                                                                             this->get_fe(),
                                                                             update_values,
          ...
          temperature_evaluator->evaluate(solution_values, EvaluationFlags::values);

          // Initialize the evaluator for the pressure
          pressure_evaluator->reinit(in.current_cell, quadrature_formula.get_points());
          pressure_evaluator->evaluate(solution_values, EvaluationFlags::values);
          ...
          derivative = material_lookup[0]->enthalpy_derivatives(temperatures, pressures, max_latent_heat_substeps);
        }
    ...
    template <int dim>
    void
    GrainSize<dim>::evaluate(const typename Interface<dim>::MaterialModelInputs &in,
                             typename Interface<dim>::MaterialModelOutputs      &out) const
    {
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          ...
          MaterialUtilities::PhaseFunctionInputs<dim> phase_inputs(
            in.temperature[i], adiabatic_pressures[i], depth, rho_g, numbers::invalid_unsigned_int);
          ...
          if (in.requests_property(MaterialProperties::viscosity) || in.requests_property(MaterialProperties::additional_outputs))
            {
              ...
              if (std::abs(second_strain_rate_invariant) > 1e-30)
                {
                  disl_viscosity = dislocation_viscosity(
                    in.temperature[i], adiabatic_temperature, adiabatic_pressures[i], in.strain_rate[i], phase_indices[i], diff_viscosity);
              ...
              if (enable_drucker_prager_rheology)
                {
                  ...
                  const Rheology::DruckerPragerParameters drucker_prager_parameters =
                    drucker_prager_plasticity.compute_drucker_prager_parameters(0, phase_function_values, n_phase_transitions);
                  const double pressure_for_yielding =
                    use_adiabatic_pressure_for_yielding ? adiabatic_pressures[i] : std::max(in.pressure[i], 0.0);

                  const double yield_stress =
                    drucker_prager_plasticity.compute_yield_stress(pressure_for_yielding, drucker_prager_parameters);
      ...
      const std::shared_ptr<DislocationViscosityOutputs<dim>> disl_viscosities_out =
        out.template get_additional_output_object<DislocationViscosityOutputs<dim>>();
      if (in.requests_property(MaterialProperties::additional_outputs))
        grain_size_evolution->fill_additional_outputs(
          in, out, phase_indices, disl_viscosities_out->dislocation_viscosities, out.additional_outputs);
      ...
      if (in.requests_property(MaterialProperties::reaction_terms))
        {
          ...
          const std::function<double(double, double, double, const dealii::SymmetricTensor<2, dim> &, unsigned int, double, double)>
            dislocation_viscosity_ = [this](const double                   temperature,
                                            const double                   adiabatic_temperature,
                                            const double                   adiabatic_pressure,
                                            const SymmetricTensor<2, dim> &strain_rate,
                                            const unsigned int             phase_index,
            ...
            return this->dislocation_viscosity(
              temperature, adiabatic_temperature, adiabatic_pressure, strain_rate, phase_index, diffusion_viscosity, viscosity_guess);
          };

          const std::function<double(double, double, double, double, double, unsigned int)> diffusion_viscosity_ =
            [this](const double       temperature,
                   const double       adiabatic_temperature,
                   const double       adiabatic_pressure,
                   const double       grain_size,
                   const double       second_strain_rate_invariant,
                   const unsigned int phase_index) -> double {
            return this->diffusion_viscosity(
              temperature, adiabatic_temperature, adiabatic_pressure, grain_size, second_strain_rate_invariant, phase_index);
          };
          ...
          grain_size_evolution->calculate_reaction_terms(
            in, adiabatic_pressures, phase_indices, dislocation_viscosity_, diffusion_viscosity_, min_eta, max_eta, out);
        }
      ...
      for (unsigned int i = 0; i < in.n_evaluation_points(); ++i)
        {
          if (!use_table_properties)
                  ...
                  if (material_lookup.size() == 1)
                    {
                      out.thermal_expansion_coefficients[i] =
                        (1 - out.densities[i] * material_lookup[0]->dHdp(in.temperature[i], adiabatic_pressures[i])) / in.temperature[i];
            ...
            {
              out.thermal_expansion_coefficients[i] =
                thermal_expansion_coefficient(in.temperature[i], adiabatic_pressures[i], in.composition[i], in.position[i]);
              out.specific_heat[i] = specific_heat(in.temperature[i], adiabatic_pressures[i], in.composition[i], in.position[i]);
            }

          out.thermal_expansion_coefficients[i] =
            std::clamp(out.thermal_expansion_coefficients[i], min_thermal_expansivity, max_thermal_expansivity);
    ...
    GrainSize<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Grain size model");
        {
          ...
          prm.declare_entry("Dislocation viscosity iteration threshold",
                            "1e-3",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Dislocation viscosity iteration number",
                            "100",
                            Patterns::Integer(0),
                            "We need to perform an iteration inside the computation "
                            "of the dislocation viscosity, because it depends on the "
          ...
          prm.declare_entry("Diffusion creep grain size exponent",
                            ...
                            "List must have one more entry than the Phase transition depths. "
                            "Units: none.");
          prm.declare_entry("Maximum temperature dependence of viscosity",
                            "100.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Derivatives file names",
                            ...
                            "compositional fields (material data is assumed to "
                            "be in order with the ordering of the fields). ");
          prm.declare_entry("Use table properties",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Material file format",
                            "perplex",
                            Patterns::Selection("perplex|hefesto"),
                            "The material file format to be read in the property "
                            "tables.");
          prm.declare_entry("Use enthalpy for material properties",
                            "true",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Bilinear interpolation",
                            "true",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Use Drucker-Prager rheology",
                            ...
                            "or not (if false).");
          prm.declare_entry("Use adiabatic pressure for yield stress",
                            "false",
                            Patterns::Bool(),
    ...
    GrainSize<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("Grain size model");
        {
          ...
          dislocation_creep_exponent = Utilities::string_to_double(Utilities::split_string_list(prm.get("Dislocation creep exponent")));
          dislocation_activation_energy =
            Utilities::string_to_double(Utilities::split_string_list(prm.get("Dislocation activation energy")));
          dislocation_activation_volume =
            Utilities::string_to_double(Utilities::split_string_list(prm.get("Dislocation activation volume")));
          dislocation_creep_prefactor = Utilities::string_to_double(Utilities::split_string_list(prm.get("Dislocation creep prefactor")));
          diffusion_creep_exponent    = Utilities::string_to_double(Utilities::split_string_list(prm.get("Diffusion creep exponent")));
          diffusion_activation_energy = Utilities::string_to_double(Utilities::split_string_list(prm.get("Diffusion activation energy")));
          diffusion_activation_volume = Utilities::string_to_double(Utilities::split_string_list(prm.get("Diffusion activation volume")));
          diffusion_creep_prefactor   = Utilities::string_to_double(Utilities::split_string_list(prm.get("Diffusion creep prefactor")));
          diffusion_creep_grain_size_exponent =
            Utilities::string_to_double(Utilities::split_string_list(prm.get("Diffusion creep grain size exponent")));
          ...
          diffusion_creep_prefactor[diffusion_creep_prefactor.size() - 1] *=
            std::pow(1.0, diffusion_creep_grain_size_exponent[diffusion_creep_grain_size_exponent.size() - 1]);
          ...
          if (use_table_properties && material_file_names.size() > 1)
            {
              AssertThrow(this->introspection().get_indices_for_fields_of_type(CompositionalFieldDescription::grain_size)[0] >=
          ...
          else if (prm.get("Material file format") == "hefesto")
            {
              ...
              AssertThrow(use_enthalpy == false || material_file_names.size() == derivatives_file_names.size(),
                          ExcMessage("When using HeFESTO files, you need to provide as many file names "
                                     ...
                                     "\n   Material file names = " +
                                     prm.get("Material file names") + "\n   Derivatives file names = " + prm.get("Derivatives file names") +
      ...
      if (out.template has_additional_output_object<DislocationViscosityOutputs<dim>>() == false)
        {
          ...
          out.additional_outputs.push_back(std::make_unique<MaterialModel::DislocationViscosityOutputs<dim>>(n_points));
        }
...
#define INSTANTIATE(dim) template class DislocationViscosityOutputs<dim>;
