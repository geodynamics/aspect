...
#  endif
    ...
    template <int dim>
    void
    TwoMergedChunks<dim>::create_coarse_mesh(parallel::distributed::Triangulation<dim> &coarse_grid) const
    {
      if (use_merged_grids)
        {
          ...
          GridGenerator::subdivided_hyper_rectangle(lower_coarse_grid, lower_rep_vec, point1, point4, false);

          // Create the upper box.
          GridGenerator::subdivided_hyper_rectangle(upper_coarse_grid, upper_rep_vec, point3, point2, false);

          // Merge the lower and upper mesh into one coarse_grid.
          // Now we have at least two cells.
          GridGenerator::merge_triangulations(lower_coarse_grid, upper_coarse_grid, coarse_grid);
        }
      else
        {
          const std::vector<unsigned int> lower_rep_vec(lower_repetitions.begin(), lower_repetitions.end());
          GridGenerator::subdivided_hyper_rectangle(coarse_grid, lower_rep_vec, point1, point2, false);
        }
      ...
      coarse_grid.signals.post_refinement.connect([&]() { this->set_boundary_indicators(coarse_grid); });
    }
    ...
    template <int dim>
    void
    TwoMergedChunks<dim>::set_boundary_indicators(parallel::distributed::Triangulation<dim> &triangulation) const
    {
    ...
    TwoMergedChunks<dim>::get_used_boundary_indicators() const
    {
    ...
    TwoMergedChunks<dim>::get_symbolic_boundary_names_map() const
    {
      switch (dim)
        {
          case 2:
            {
              return {{"bottom", 0}, {"top", 1}, {"lowerwest", 2}, {"lowereast", 3}, {"uppereast", 4}, {"upperwest", 5}};
            }
    ...
    template <int dim>
    double
    TwoMergedChunks<dim>::length_scale() const
    {
    ...
    TwoMergedChunks<dim>::point_is_in_domain(const Point<dim> &point) const
    {
      AssertThrow(
        !this->get_parameters().mesh_deformation_enabled ||
          ...
          this->get_timestep_number() == 0 || this->get_timestep_number() == numbers::invalid_unsigned_int,
        ExcMessage(
    ...
    template <int dim>
    void
    TwoMergedChunks<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Chunk with lithosphere boundary indicators");
        {
          ...
          prm.declare_entry("Chunk middle boundary radius",
                            "1",
                            Patterns::Double(0),
          ...
          prm.declare_entry("Outer chunk radius repetitions",
                            "1",
                            Patterns::Integer(1),
                            "Number of cells in radial direction for the upper chunk.");
          prm.declare_entry("Inner chunk radius repetitions",
                            "1",
                            Patterns::Integer(1),
    ...
    TwoMergedChunks<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Chunk with lithosphere boundary indicators");
        {
          ...
          AssertThrow(point3[0] < point2[0], ExcMessage("Middle boundary radius must be less than outer radius."));
