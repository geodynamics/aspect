...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    create_coarse_mesh (parallel::distributed::Triangulation<dim> &coarse_grid) const
    {
      ...
      if (phi == 360)
        {
          if (tailored_spherical_shell == true)
          {
    ...
    SphericalShell<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Spherical shell");
        {
          ...
          prm.declare_entry ("Tailored spherical shell", "false",
                             Patterns::Bool (),
                             "Whether or not the 3D spherical shell mesh is created using "
                             "the extrusion method. It consists in generating a 2D hyper "
                             "ball and extrude it radially according a number of slices and "
                             "slice height specified by the user.");
    ...
    SphericalShell<dim>::parse_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Spherical shell");
        {
          ...
          tailored_spherical_shell = prm.get_bool("Tailored spherical shell");
          ...
          if (tailored_spherical_shell == true && dim == 2)
            AssertThrow (false, ExcMessage("A tailored spherical shell only works for 3D models. "));
          if (tailored_spherical_shell == true && n_slices == 0)
            AssertThrow (false, ExcMessage("You must set a positive number of slices for extrusion. "));
          if (tailored_spherical_shell == true && height == 0)
            AssertThrow (false, ExcMessage("You must set a positive slice height for extrusion. "));
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
