...
namespace aspect
{
  namespace GeometryModel
  {
    namespace internal
    {
      ...
      ChunkGeometry<dim>::push_forward_gradient(const Point<dim> &chart_point) const
      {
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::push_forward(const Point<dim> &r_phi_theta) const
      {
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::pull_back(const Point<dim> &x_y_z) const
      {
        // Only take into account topography when we're not using the ZeroTopography plugin
        if (dynamic_cast<const InitialTopographyModel::ZeroTopography<dim> *>(topo.get()) != nullptr)
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::push_forward_sphere(const Point<dim> &input_vertex) const
      {
      ...
      template <int dim>
      Tensor<1, dim>
      ChunkGeometry<dim>::normal_vector(const typename Triangulation<dim>::face_iterator &face, const Point<dim> &p) const
      {
        ...
        const auto minmax_distance = std::minmax_element(distances_to_center.begin(), distances_to_center.begin() + face->n_vertices());
        const auto min_distance_to_first_vertex =
          std::min_element(distances_to_first_vertex.begin(), distances_to_first_vertex.begin() + face->n_vertices() - 1);
        ...
        if (*minmax_distance.second - *minmax_distance.first < 1.e-10 * *min_distance_to_first_vertex)
          {
            ...
            const Tensor<1, dim> normalized_spherical_normal   = unnormalized_spherical_normal / unnormalized_spherical_normal.norm();
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::pull_back_sphere(const Point<dim> &v) const
      {
        Point<dim> output_vertex;
        switch (dim)
          {
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::push_forward_topo(const Point<dim> &r_phi_theta) const
      {
      ...
      template <int dim>
      Point<dim>
      ChunkGeometry<dim>::pull_back_topo(const Point<dim> &topo_r_phi_theta) const
      {
      ...
      template <int dim>
      double
      ChunkGeometry<dim>::topography_for_point(const Point<dim> &x_y_z) const
      {
        if (dynamic_cast<const InitialTopographyModel::ZeroTopography<dim> *>(topo.get()) != nullptr)
          return 0;
        else
...
#  endif
    ...
    template <int dim>
    void
    Chunk<dim>::create_coarse_mesh(parallel::distributed::Triangulation<dim> &coarse_grid) const
    {
      // First create a box in Cartesian coordinates:
      const std::vector<unsigned int> rep_vec(repetitions.begin(), repetitions.end());
      GridGenerator::subdivided_hyper_rectangle(coarse_grid, rep_vec, point1, point2, true);

      // Then transform this box into a spherical chunk (possibly with
      // topography -- the 'manifold' has that built in):
      GridTools::transform([&](const Point<dim> &p) -> Point<dim> { return manifold->push_forward(p); }, coarse_grid);
    ...
    Chunk<dim>::get_used_boundary_indicators() const
    {
    ...
    Chunk<dim>::get_symbolic_boundary_names_map() const
    {
      switch (dim)
        {
          case 2:
            {
              return {{"bottom", 0}, {"top", 1}, {"west", 2}, {"east", 3}};
            }

          case 3:
            {
              return {{"bottom", 0}, {"top", 1}, {"west", 2}, {"east", 3}, {"south", 4}, {"north", 5}};
            }
        }
    ...
    template <int dim>
    void
    Chunk<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Chunk");
        {
          prm.declare_entry("Chunk inner radius",
                            "0.",
                            Patterns::Double(0.),
                            "Radius at the bottom surface of the chunk. Units: \\si{\\meter}.");
          prm.declare_entry("Chunk outer radius",
                            "1.",
                            Patterns::Double(0.),
                            "Radius at the top surface of the chunk. Units: \\si{\\meter}.");

          prm.declare_entry("Chunk minimum longitude",
                            "0.",
                            Patterns::Double(-180., 360.), // enables crossing of either hemisphere
                            "Minimum longitude of the chunk. Units: \\si{\\degree}.");
          prm.declare_entry("Chunk maximum longitude",
                            "1.",
                            Patterns::Double(-180., 360.), // enables crossing of either hemisphere
                            "Maximum longitude of the chunk. Units: \\si{\\degree}.");

          prm.declare_entry("Chunk minimum latitude",
                            "0.",
                            Patterns::Double(-90., 90.),
                            "Minimum latitude of the chunk. This value is ignored "
                            "if the simulation is in 2d. Units: \\si{\\degree}.");
          prm.declare_entry("Chunk maximum latitude",
                            "1.",
                            Patterns::Double(-90., 90.),
                            "Maximum latitude of the chunk. This value is ignored "
                            "if the simulation is in 2d. Units: \\si{\\degree}.");

          prm.declare_entry("Radius repetitions", "1", Patterns::Integer(1), "Number of cells in radius.");
          prm.declare_entry("Longitude repetitions", "1", Patterns::Integer(1), "Number of cells in longitude.");
          prm.declare_entry("Latitude repetitions",
                            "1",
                            Patterns::Integer(1),
    ...
    Chunk<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Geometry model");
      {
        prm.enter_subsection("Chunk");
        {
          ...
          AssertThrow(point1[0] < point2[0], ExcMessage("Inner radius must be less than outer radius."));
          AssertThrow(point1[1] < point2[1], ExcMessage("Minimum longitude must be less than maximum longitude."));
          AssertThrow(point2[1] - point1[1] < 2. * numbers::PI, ExcMessage("Maximum - minimum longitude should be less than 360 degrees."));
          ...
          if (dim == 3)
            {
              ...
              AssertThrow(point1[2] < point2[2], ExcMessage("Minimum latitude must be less than maximum latitude."));
              AssertThrow(point1[2] > -0.5 * numbers::PI, ExcMessage("Minimum latitude needs to be larger than -90 degrees."));
              AssertThrow(point2[2] < 0.5 * numbers::PI, ExcMessage("Maximum latitude needs to be less than 90 degrees."));
            }
        }
        prm.leave_subsection();
      }
...
namespace aspect
{
  namespace GeometryModel
  {
    namespace internal
    {
#define INSTANTIATE(dim) template class ChunkGeometry<dim>;
