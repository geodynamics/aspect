...
namespace aspect
{
  ...
  print_aspect_header(Stream &stream)
  {
    ...
    stream << DEAL_II_COMPILER_VECTORIZATION_LEVEL << " (" << vectorization_level << ", " << n_vectorization_doubles << " doubles, "
           << n_vectorization_bits << " bits)\n";

    stream << "--     . using Trilinos " << DEAL_II_TRILINOS_VERSION_MAJOR << '.' << DEAL_II_TRILINOS_VERSION_MINOR << '.'
           << DEAL_II_TRILINOS_VERSION_SUBMINOR << '\n';
    stream << "--     . using p4est " << DEAL_II_P4EST_VERSION_MAJOR << '.' << DEAL_II_P4EST_VERSION_MINOR << '.'
           << DEAL_II_P4EST_VERSION_SUBMINOR << '\n';

#ifdef ASPECT_WITH_WORLD_BUILDER
    stream << "--     . using Geodynamic World Builder " << WORLD_BUILDER_VERSION_MAJOR << '.' << WORLD_BUILDER_VERSION_MINOR << '.'
    ...
    stream << "--     . using FastScape " << FASTSCAPE_VERSION_MAJOR << '.' << FASTSCAPE_VERSION_MINOR << '.' << FASTSCAPE_VERSION_SUBMINOR
           << '\n';
#endif
  ...
  template void
  print_aspect_header<std::ostream>(std::ostream &stream);
  template void
  print_aspect_header<std::ofstream>(std::ofstream &stream);
