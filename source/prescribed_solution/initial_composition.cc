...
namespace aspect
{
  namespace PrescribedSolution
  {
                                                ...
                                                std::vector<double>             &solution)
    {
      ...
      const unsigned int first_comp = this->introspection().component_indices.compositional_fields[0];
      ...
      for (unsigned int q = 0; q < positions.size(); ++q)
        {
          ...
          const auto point = geometry_model.cartesian_to_other_coordinates(positions[q], coordinate_system);

          const double indicator = indicator_function.value(Utilities::convert_array_to_point<dim>(point.get_coordinates()));

          if (indicator > 0.5)
            {
              const unsigned int field_index = component - first_comp;

              solution[q] = initial_composition_manager->initial_composition(positions[q], field_index);
