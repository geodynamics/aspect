...
namespace aspect
{
  namespace MeshRefinement
  {
        ...
        if (found == false)
          {
            ...
            AssertThrow(false,
                        ExcMessage("The key given for the isosurface could not be converted. The provided key was: " + property_name +
                                   ". "
                                   "The following keys are allowed for this model: " +
                                   key_list + "."));
          }
      }
            ...
            if (string.compare(0, 4, "min+") == 0)
              {
                ...
                return std::clamp(minimum_refinement_level + Utilities::string_to_int(tmpNumber[1]),
            ...
            else
              {
                AssertThrow(string == "min",
                            ExcMessage("A value of " + string_value +
                                       " was provided, but you can't provide a smaller value than the minimum."));
            ...
            if (string.compare(0, 4, "max-") == 0)
              {
                ...
                AssertThrow(tmpNumber.size() == 2,
                            ExcMessage("Could not convert value '" + string + "' to an int because it contains more than one '-' sign."));
                return std::clamp(static_cast<int>(maximum_refinement_level) - Utilities::string_to_int(tmpNumber[1]),
                                  static_cast<int>(minimum_refinement_level),
                                  static_cast<int>(maximum_refinement_level));
              }
            else
              {
                AssertThrow(string == "max",
                            ExcMessage("A value of " + string_value +
                                       " was provided, but you can't provide a larger value than the maximum."));
                return maximum_refinement_level;
              }
          }
        else
          {
            return std::clamp(static_cast<unsigned int>(Utilities::string_to_int(string)),
                              minimum_refinement_level,
                              maximum_refinement_level);
          }
      }
    ...
    template <int dim>
    void
    Isosurfaces<dim>::declare_parameters(ParameterHandler &prm)
    {
    ...
    Isosurfaces<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        ...
        const unsigned int maximum_refinement_level =
          this->get_parameters().initial_global_refinement + this->get_parameters().initial_adaptive_refinement;
          ...
          for (auto &isosurface_entry : isosurface_entries)
            {
              ...
              AssertThrow(field_entries.size() >= 3,
                          ExcMessage("An isosurface needs to contain at least 3 entries, but isosurface " +
                                     std::to_string(isosurface_entry_number) + " contains  only " + std::to_string(field_entries.size()) +
                                     " entries: " + isosurface_entry + "."));

              // convert a potential min, min+1, min + 1, min+10, max, max-1, etc. to actual integers.
              isosurface.min_refinement =
                internal::min_max_string_to_int(field_entries[0], minimum_refinement_level, maximum_refinement_level);
              isosurface.max_refinement =
                internal::min_max_string_to_int(field_entries[1], minimum_refinement_level, maximum_refinement_level);
              ...
              for (auto field_entry = field_entries.begin() + 2; field_entry != field_entries.end(); ++field_entry)
                {
                  ...
                  AssertThrow(
                    key_and_value.size() == 2,
                    ExcMessage(
                      "The isosurface property must have a key (e.g. Temperature) and two values separated by a | (e.g. (300 | 600)."));
                  ...
                  AssertThrow(values.size() == 2, ExcMessage("Both a maximum and a minimum value are required for each isosurface."));
