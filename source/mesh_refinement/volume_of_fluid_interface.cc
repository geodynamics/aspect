...
namespace aspect
{
  namespace MeshRefinement
  {
    ...
    VolumeOfFluidInterface<dim>::tag_additional_cells() const
    {
      ...
      const std::vector<std::set<typename Triangulation<dim>::active_cell_iterator>> vertex_to_cells(
        GridTools::vertex_to_cell_map(this->get_triangulation()));

      std::set<typename Triangulation<dim>::active_cell_iterator> marked_cells;
      FEValues<dim> fe_values(this->get_mapping(), this->get_fe(), qMidC, update_values | update_quadrature_points);
      ...
      LinearAlgebra::BlockVector interface_contained_local(this->introspection().index_sets.system_partitioning,
      ...
      for (unsigned int f = 0; f < this->get_volume_of_fluid_handler().get_n_fields(); ++f)
        {
          ...
          const FEValuesExtractors::Scalar volume_of_fluid_field =
            ...
            if (!cell->is_artificial())
              {
                ...
                fe_values[volume_of_fluid_field].get_function_values(this->get_solution(), volume_of_fluid_values);
                            ...
                            if (neighbor->is_active() && !neighbor->is_artificial())
                              {
                                ...
                                fe_values[volume_of_fluid_field].get_function_values(this->get_solution(), neighbor_volume_of_fluid_values);
                            ...
                            for (unsigned int subface = 0; subface < face->n_children(); ++subface)
                              {
                                const typename DoFHandler<dim>::active_cell_iterator neighbor_sub =
                                  (cell_has_periodic_neighbor ? cell->periodic_neighbor_child_on_subface(f, subface) :
      ...
      LinearAlgebra::BlockVector interface_contained_global(this->introspection().index_sets.system_partitioning,
      ...
      const FEValuesExtractors::Scalar ic_extract =
        this->get_volume_of_fluid_handler().field_struct_for_field_index(0).volume_fraction.extractor_scalar();
      ...
      for (const auto &cell : this->get_dof_handler().active_cell_iterators())
        if (!cell->is_artificial())
          {
            fe_values.reinit(cell);
            fe_values[ic_extract].get_function_values(interface_contained_global, ic_values);
      ...
      typename std::set<typename parallel::distributed::Triangulation<dim>::active_cell_iterator>::const_iterator mcells =
                                                                                                                    marked_cells.begin(),
                                                                                                                  endmc =
                                                                                                                    marked_cells.end();
          ...
          for (const unsigned int vertex_index : mcell->vertex_indices())
            {
              const std::set<typename Triangulation<dim>::active_cell_iterator> &neighbor_cells =
                vertex_to_cells[mcell->vertex_index(vertex_index)];
    ...
    template <int dim>
    void
    VolumeOfFluidInterface<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh refinement");
      {
        prm.enter_subsection("Volume of fluid interface");
        {
          prm.declare_entry("Strict coarsening",
                            "false",
                            Patterns::Bool(),
    ...
    VolumeOfFluidInterface<dim>::parse_parameters(ParameterHandler &prm)
    {
      AssertThrow(
        ...
        ExcMessage(
          "The 'volume_of_fluid boundary' mesh refinement strategy requires that the 'Use interface tracking' parameter is enabled."));
