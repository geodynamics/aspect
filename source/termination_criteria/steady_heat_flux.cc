...
namespace aspect
{
  namespace TerminationCriteria
  {
    namespace
    {
       ...
       */
      void
      trim_time_heat_flux_list(const double necessary_time_in_steady_state, std::list<std::pair<double, double>> &time_heat_flux_list)
      {
    ...
    SteadyHeatFlux<dim>::execute()
    {
      ...
      const double global_heat_flux_integral = Utilities::MPI::sum(local_boundary_fluxes, this->get_mpi_communicator());
      ...
      if ((time_heat_flux.size() <= 2) || (time_heat_flux.back().first - time_heat_flux.front().first < necessary_time_in_steady_state))
        return false;
          ...
          try
            {
              const std::vector<types::boundary_id> x_boundary_indicators =
                this->get_geometry_model().translate_symbolic_boundary_names_to_ids(
                  Utilities::split_string_list(prm.get("Boundary indicators")));
              boundary_indicators = std::set<types::boundary_id>(x_boundary_indicators.begin(), x_boundary_indicators.end());
            ...
            {
              AssertThrow(false,
                          ExcMessage("While parsing the entry <Termination criteria/Steady "
