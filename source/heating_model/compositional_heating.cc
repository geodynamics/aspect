...
namespace aspect
{
  namespace HeatingModel
  {
    template <int dim>
    void
    CompositionalHeating<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim> &material_model_inputs,
      ...
      for (unsigned int q = 0; q < heating_model_outputs.heating_source_terms.size(); ++q)
        {
          ...
          const std::vector<double> volume_fractions =
            MaterialModel::MaterialUtilities::compute_composition_fractions(material_model_inputs.composition[q],
    ...
    CompositionalHeating<dim>::get_required_properties() const
    {
    ...
    CompositionalHeating<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Heating model");
      {
        ...
        {
          prm.declare_entry("Compositional heating values",
                            "0.",
          ...
          prm.declare_entry("Use compositional field for heat production averaging",
                            "1",
      ...
      prm.enter_subsection("Heating model");
      {
        prm.enter_subsection("Compositional heating");
        {
          const std::vector<int> used_fields = Utilities::possibly_extend_from_1_to_N(
            Utilities::string_to_int(Utilities::split_string_list(prm.get("Use compositional field for heat production averaging"))),
            n_fields,
          ...
          heating_values = Utilities::possibly_extend_from_1_to_N(Utilities::string_to_double(
