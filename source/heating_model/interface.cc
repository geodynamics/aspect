...
namespace aspect
{
  namespace HeatingModel
  {
    template <int dim>
    void
    Interface<dim>::create_additional_material_model_outputs(MaterialModel::MaterialModelOutputs<dim> & /*outputs*/) const
    {}
    ...
    template <int dim>
    void
    Interface<dim>::create_additional_material_model_inputs(MaterialModel::MaterialModelInputs<dim> & /*inputs*/) const
    {}


    template <int dim>
    MaterialModel::MaterialProperties::Property
    Interface<dim>::get_required_properties() const
    {
    ...
    Manager<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      for (auto &model_name : this->plugin_names)
        {
          this->plugin_objects.emplace_back(std::get<dim>(registered_plugins).create_plugin(model_name, "Heating model::Model names"));
    ...
    template <int dim>
    void
    Manager<dim>::create_additional_material_model_inputs_and_outputs(
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Heating model interface", out);
    }



    HeatingModelOutputs::HeatingModelOutputs(const unsigned int n_points, const unsigned int)
