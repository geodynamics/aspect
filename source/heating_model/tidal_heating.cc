...
namespace aspect
{
  namespace HeatingModel
  {
    ...
    template <int dim>
    void
    TidalHeating<dim>::evaluate(const MaterialModel::MaterialModelInputs<dim>  &material_model_inputs,
                                ...
                                HeatingModel::HeatingModelOutputs              &heating_model_outputs) const
    {
       ...
       * This variation of tidal strain rate follows as (maximum_tidal_strain_rate -
       * minimum_tidal_strain_rate)*cos(theta/2)+(maximum_tidal_strain_rate+minimum_tidal_strain_rate)/2 where theta is polar angle from
       * spherical coordinates. If false, constant_tidal_strain_rate is used.
       */
      for (unsigned int q = 0; q < heating_model_outputs.heating_source_terms.size(); ++q)
        {
          ...
          if (strain_rate_distribution == latitudinal_variation)
            {
              ...
              local_tidal_strain_rate = (maximum_tidal_strain_rate - minimum_tidal_strain_rate) / 2. * std::cos(colatitude * 2.) +
                                        (maximum_tidal_strain_rate + minimum_tidal_strain_rate) / 2.;
            }
          ...
          heating_model_outputs.heating_source_terms[q] =
            2. * material_model_outputs.viscosities[q] * local_tidal_strain_rate * local_tidal_strain_rate /
            (1. + ((tidal_frequency * material_model_outputs.viscosities[q]) / elastic_shear_modulus) *
                    ((tidal_frequency * material_model_outputs.viscosities[q]) / elastic_shear_modulus));
    ...
    TidalHeating<dim>::get_required_properties() const
    {
      ...
      prm.enter_subsection("Heating model");
      {
        prm.enter_subsection("Tidal heating");
        {
          ...
          prm.declare_entry("Elastic shear modulus",
                            ...
                            "Units: \\si{\\pascal}.");
          prm.declare_entry("Constant tidal strain rate",
                            "2e-10",
                            Patterns::Double(0),
                            ...
                            "Units: \\si{\\per\\second}.");
          prm.declare_entry("Custom distribution of tidal strain rate",
                            "constant",
          ...
          prm.declare_entry("Maximum tidal strain rate",
                            "2.81e-10",
                            Patterns::Double(0),
                            ...
                            "Units: \\si{\\per\\second}.");
          prm.declare_entry("Minimum tidal strain rate",
                            "1.67e-10",
                            Patterns::Double(0),
          ...
          if (prm.get("Custom distribution of tidal strain rate") == "constant")
          ...
          else if (prm.get("Custom distribution of tidal strain rate") == "latitudinal variation")
            {
              if (dim == 2 && Plugins::plugin_type_matches<GravityModel::RadialWithTidalPotential<dim>>(this->get_gravity_model()))
                throw ExcMessage(
                  "On 2D, tidal heating and gravity with tidal potential have different lateral variations: latitudinal and longitudinal variation, respectively");
