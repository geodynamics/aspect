...
namespace aspect
{
  namespace AdiabaticConditions
  {
    ...
    ComputeProfile<dim>::initialize()
    {
      ...
      // here.
      if ((reference_composition == initial_composition) && (initial_composition_manager == nullptr))
      ...
      for (unsigned int i = 0; i < n_points; ++i)
        {
          if (i == 0)
          ...
          else
            {
              ...
              temperatures[i] =
                (this->include_adiabatic_heating()) ? temperatures[i - 1] * (1 + alpha * gravity * delta_z * one_over_cp) : temperatures[0];
            }
          ...
          if (i > 0)
            in.pressure_gradient[0] = g / (g.norm() != 0.0 ? g.norm() : 1.0) * (pressures[i] - pressures[i - 1]) / delta_z;
          else
    ...
    template <int dim>
    double
    ComputeProfile<dim>::pressure(const Point<dim> &p) const
    {
    ...
    template <int dim>
    double
    ComputeProfile<dim>::temperature(const Point<dim> &p) const
    {
    ...
    template <int dim>
    double
    ComputeProfile<dim>::density(const Point<dim> &p) const
    {
    ...
    template <int dim>
    double
    ComputeProfile<dim>::density_derivative(const Point<dim> &p) const
    {
    ...
    template <int dim>
    double
    ComputeProfile<dim>::get_property(const Point<dim> &p, const std::vector<double> &property) const
    {
      const double z = this->get_geometry_model().depth(p);
    ...
    ComputeProfile<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Adiabatic conditions model");
      {
        prm.enter_subsection("Compute profile");
        {
          ...
          prm.declare_entry("Composition reference profile",
                            "initial composition",
          ...
          prm.declare_entry("Number of points",
                            ...
                            "temperature will be.");
          prm.declare_entry("Use surface condition function",
                            "false",
                            Patterns::Bool(),
