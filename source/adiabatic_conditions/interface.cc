  ...
  namespace AdiabaticConditions
  {
    ...
    create_adiabatic_conditions(ParameterHandler &prm)
    {
      ...
      std::unique_ptr<Interface<dim>> plugin =
        std::get<dim>(registered_plugins).create_plugin(model_name, "Adiabatic Conditions model::Model name");
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Adiabatic conditions interface", out);
    }
...
\
  template void register_adiabatic_conditions<dim>(const std::string &, \
                                                   const std::string &, \
...
\
  template std::unique_ptr<Interface<dim>> create_adiabatic_conditions<dim>(ParameterHandler & prm);
