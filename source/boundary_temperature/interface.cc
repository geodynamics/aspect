  ...
  namespace BoundaryTemperature
  {
        ...
        try
          {
            ...
            fixed_temperature_boundary_indicators =
              std::set<types::boundary_id>(x_fixed_temperature_boundary_indicators.begin(), x_fixed_temperature_boundary_indicators.end());
          ...
          {
            AssertThrow(false,
                        ExcMessage("While parsing the entry <Boundary temperature model/Fixed temperature "
      ...
      for (auto &model_name : this->plugin_names)
        {
          ...
          this->plugin_objects.emplace_back(
            std::get<dim>(registered_plugins).create_plugin(model_name, "Boundary temperature::Model names"));
    ...
    Manager<dim>::boundary_temperature(const types::boundary_id boundary_indicator, const Point<dim> &position) const
    {
      ...
      for (unsigned int i = 0; i < this->plugin_objects.size(); ++p, ++i)
        temperature = model_operators[i](temperature, (*p)->boundary_temperature(boundary_indicator, position));
    ...
    Manager<dim>::minimal_temperature(const std::set<types::boundary_id> &fixed_boundary_ids) const
    {
      ...
      for (const auto &p : this->plugin_objects)
        temperature = std::min(temperature, p->minimal_temperature(fixed_boundary_ids));
    ...
    Manager<dim>::maximal_temperature(const std::set<types::boundary_id> &fixed_boundary_ids) const
    {
      ...
      for (const auto &p : this->plugin_objects)
        temperature = std::max(temperature, p->maximal_temperature(fixed_boundary_ids));
    ...
    Manager<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Boundary temperature model");
      {
        ...
        prm.declare_entry("Allow fixed temperature on outflow boundaries",
                          "true",
                          Patterns::Bool(),
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Boundary temperature interface", out);
    }
  }
}
