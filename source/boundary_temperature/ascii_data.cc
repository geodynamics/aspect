      ...
      if (this->get_time() - first_data_file_model_time >= 0.0)
        {
          ...
          return lookups.find(boundary_id)->second->get_data(position,0,time_weight);
        }
      else
        return 0.0;
    }
...
namespace aspect
{
  namespace BoundaryTemperature
  {
    ASPECT_REGISTER_BOUNDARY_TEMPERATURE_MODEL(AsciiData,
                                               ...
                                               "has to be 'vx', 'vy', 'vz'. "
                                               "The according grid for assigning the boundary "
                                               "data to the model is created by the plugin "
                                               "as a structured regular grid "
                                               "as large as the model domain. Make sure that the "
                                               "provided input data does exactly fit the model size. "
                                               "Note that the data in the input "
                                               "files need to be sorted in a specific order: "
                                               "the first coordinate needs to ascend first, "
                                               "followed by the second and the third at last."
