...
namespace aspect
{
  namespace BoundaryTemperature
  {
    ...
    template <int dim>
    double
    TwoMergedBoxes<dim>::boundary_temperature(const types::boundary_id boundary_indicator, const Point<dim> &) const
    {
      Assert(boundary_indicator < 2 * dim + 2 * (dim - 1),
             ExcMessage("Given boundary indicator needs to be less than 2*dimension+2*(dimension-1)."));
    ...
    template <int dim>
    double
    TwoMergedBoxes<dim>::minimal_temperature(const std::set<types::boundary_id> &fixed_boundary_ids) const
    {
    ...
    template <int dim>
    double
    TwoMergedBoxes<dim>::maximal_temperature(const std::set<types::boundary_id> &fixed_boundary_ids) const
    {
      if (fixed_boundary_ids.empty())
          ...
          if (dim == 3)
            {
              ...
              prm.declare_entry("Front temperature lithosphere",
                                "0.",
                                Patterns::Double(),
                                "Temperature at the additional front lithosphere boundary (at minimal $y$-value). Units: $\\text{K}$.");
              prm.declare_entry("Back temperature lithosphere",
                                "0.",
                                Patterns::Double(),
