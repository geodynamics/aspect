...
namespace aspect
{
  namespace PrescribedDilation
  {
      ...
      for (auto &model_name : this->plugin_names)
        {
          this->plugin_objects.emplace_back(
            std::get<dim>(registered_plugins).create_plugin(model_name, "Prescribed dilation::Model names"));
    ...
    template <int dim>
    void
    Manager<dim>::evaluate(const aspect::MaterialModel::MaterialModelInputs<dim> &in, PrescribedDilationOutputs<dim> &out) const
    {
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Prescribed dilation interface", out);
    }
