  ...
  namespace BoundaryConvectiveHeating
  {
    ...
    namespace
    {
      ...
      std::tuple<aspect::internal::Plugins::UnusablePluginList,
                 ...
                 aspect::internal::Plugins::PluginList<BoundaryTemperature::Interface<3>>>
        registered_temperature_plugins;

      std::tuple<aspect::internal::Plugins::UnusablePluginList,
                 aspect::internal::Plugins::UnusablePluginList,
                 aspect::internal::Plugins::PluginList<BoundaryHeatFlux::Interface<2>>,
                 aspect::internal::Plugins::PluginList<BoundaryHeatFlux::Interface<3>>>
        registered_heat_flux_plugins;
    }
    ...
    {
      std::get<dim>(registered_temperature_plugins).register_plugin(name, description, declare_parameters_function, factory_function);
    }
    ...
    {
      std::get<dim>(registered_heat_flux_plugins).register_plugin(name, description, declare_parameters_function, factory_function);
    }
    ...
    Manager<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      {
        this->plugin_names       = Utilities::split_string_list(prm.get("List of heat transfer coefficient model names"));
        temperature_plugin_names = Utilities::split_string_list(prm.get("List of boundary temperature model names"));
        heat_flux_plugin_names   = Utilities::split_string_list(prm.get("List of boundary heat flux model names"));

        AssertThrow(this->plugin_names.size() <= 1 && temperature_plugin_names.size() <= 1 && heat_flux_plugin_names.size() <= 1,
        ...
        AssertThrow(Utilities::has_unique_entries(this->plugin_names) && Utilities::has_unique_entries(temperature_plugin_names) &&
        ...
        try
          {
            const std::vector<types::boundary_id> x_convective_heating_boundary_indicators =
              this->get_geometry_model().translate_symbolic_boundary_names_to_ids(
                Utilities::split_string_list(prm.get("Convective heating boundary indicators")));
            convective_heating_boundary_indicators = std::set<types::boundary_id>(x_convective_heating_boundary_indicators.begin(),
          ...
          {
            AssertThrow(false,
                        ExcMessage("While parsing the entry <Boundary convective heating model/"
      ...
      for (auto &model_name : this->plugin_names)
        {
          ...
          this->plugin_objects.emplace_back(
            std::get<dim>(registered_plugins).create_plugin(model_name, "Boundary convective heating model::Model names"));
      ...
      for (auto &model_name : temperature_plugin_names)
        {
          ...
          temperature_plugin_objects.emplace_back(
            std::get<dim>(registered_temperature_plugins).create_plugin(model_name, "Boundary temperature::Model names"));
      ...
      for (auto &model_name : heat_flux_plugin_names)
        {
          ...
          heat_flux_plugin_objects.emplace_back(
            std::get<dim>(registered_heat_flux_plugins).create_plugin(model_name, "Boundary heat flux model::Model names"));
                                            ...
                                            const MaterialModel::MaterialModelOutputs<dim> &material_model_outputs) const
    {
      ...
      for (const auto &p : this->plugin_objects)
        coefficients = p->heat_transfer_coefficient(boundary_indicator, material_model_inputs, material_model_outputs);
      return coefficients;
    }
    ...
    template <int dim>
    double
    Manager<dim>::boundary_temperature(const types::boundary_id boundary_indicator, const Point<dim> &position) const
    {
      // This manager class is written to accommodate multiple plugins at least
      // in principle. However, at the moment it only works for exactly
      // one plugin because it does not yet know how to combine results
      ...
      for (const auto &p : temperature_plugin_objects)
        temperature = p->boundary_temperature(boundary_indicator, position);
                            ...
                            const std::vector<Tensor<1, dim>>              &normal_vectors) const
    {
      ...
      for (const auto &p : heat_flux_plugin_objects)
        heat_fluxes = p->heat_flux(boundary_indicator, material_model_inputs, material_model_outputs, normal_vectors);
      return heat_fluxes;
    }
    ...
    Manager<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Boundary convective heating model");
      {
        ...
        const std::string pattern_of_temperature_names = std::get<dim>(registered_temperature_plugins).get_pattern_of_names();
        const std::string pattern_of_heat_flux_names   = std::get<dim>(registered_heat_flux_plugins).get_pattern_of_names();
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Boundary convective heating interface", out);
    }
  }
}
