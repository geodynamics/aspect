...
#  define AssertThrowNetCDF(error_code) AssertThrow(false, ExcInternalError())
...
namespace aspect
{
  namespace Utilities
  {
    ...
    template <int dim>
    StructuredDataLookup<dim>::StructuredDataLookup(const unsigned int n_components, const double scale_factor)
      : n_components(n_components)
      , data(n_components)
      , maximum_component_value(n_components)
      , scale_factor(scale_factor)
    ...
    StructuredDataLookup<dim>::get_interpolation_point_coordinates(const unsigned int dimension) const
    {
      AssertThrow(dimension < dim,
                  ExcMessage("There is no spatial dimension number " + std::to_string(dimension) + " in the current data file."));
    ...
    StructuredDataLookup<dim>::get_column_name_from_index(const unsigned int column_index) const
    {
      AssertThrow(data_component_names.size() > column_index,
                  ExcMessage("There is no data column number " + Utilities::to_string(column_index) +
                             " in the current data file. The data file only contains " + Utilities::to_string(data_component_names.size()) +
                             " named columns."));
            ...
            for (unsigned int n = 1; n < coordinate_values[d].size(); ++n)
              {
                ...
                AssertThrow(current_grid_spacing > 0.0,
                            ExcMessage("Coordinates in dimension " + Utilities::int_to_string(d) + " are not strictly ascending."));
                                      ...
                                      const unsigned int                 root_process)
    {
      ...
      if ((root_process == numbers::invalid_unsigned_int) ||
          ((root_process != numbers::invalid_unsigned_int) && (Utilities::MPI::this_mpi_process(mpi_communicator) == root_process)))
        {
          ...
          Assert(data_table.size() == n_components, ExcMessage("Error: Incorrect number of columns specified."));
          for (unsigned int c = 0; c < n_components; ++c)
            Assert(data_table[c].size() == points_per_direction, ExcMessage("Error: One of the data tables has an incorrect size."));
          ...
          for (unsigned int c = 0; c < n_components; ++c)
            {
              ...
              for (std::size_t idx = 0; idx < n_elements; ++idx)
                maximum_component_value[c] =
                  std::max(maximum_component_value[c], data_table[c](compute_table_indices(points_per_direction, idx)));
            }
      ...
      if (root_process != numbers::invalid_unsigned_int)
        {
          coordinate_values                 = Utilities::MPI::broadcast(mpi_communicator, coordinate_values, root_process);
          n_components                      = Utilities::MPI::broadcast(mpi_communicator, n_components, root_process);
          data_component_names              = Utilities::MPI::broadcast(mpi_communicator, data_component_names, root_process);
          maximum_component_value           = Utilities::MPI::broadcast(mpi_communicator, maximum_component_value, root_process);
          coordinate_values_are_equidistant = Utilities::MPI::broadcast(mpi_communicator, coordinate_values_are_equidistant, root_process);
          points_per_direction              = Utilities::MPI::broadcast(mpi_communicator, points_per_direction, root_process);

          // We can then also prepare the data tables for sharing between
          // processes
          for (unsigned int c = 0; c < n_components; ++c)
            data_table[c].replicate_across_communicator(mpi_communicator, root_process);
        }
          ...
          if (coordinate_values_are_equidistant)
            {
              ...
              data[c] = std::make_unique<Functions::InterpolatedUniformGridData<dim>>(std::move(grid_extent),
            ...
            data[c] = std::make_unique<Functions::InterpolatedTensorProductGridData<dim>>(
              std::move(std::array<std::vector<double>, dim>(this->coordinate_values)), std::move(data_table[c]));
        }
    }
    ...
    template <int dim>
    void
    StructuredDataLookup<dim>::load_ascii(const std::string &filename, const MPI_Comm comm)
    {
      ...
      if (Utilities::MPI::this_mpi_process(comm) == root_process)
        {
                  ...
                  if (n_components == numbers::invalid_unsigned_int)
                    ...
                    AssertThrow(n_components + dim == name_column_index,
                                ExcMessage("The number of expected data columns and the "
                                           ...
                                           Utilities::int_to_string(name_column_index) +
                                           " column "
                                           "names (one for each dimension and one per data column), "
                                           "but it only has " +
                                           Utilities::int_to_string(n_components + dim) + " column names."));
                  break;
                }
                  ...
                  if (name_column_index >= dim)
                    {
                      ...
                      AssertThrow(std::find(column_names.begin(), column_names.end(), column_name_or_data) == column_names.end(),
                                  ExcMessage("There are multiple fields named " + column_name_or_data + " in the data file " + pretty_name +
          ...
          AssertThrow(n_components != numbers::invalid_unsigned_int,
                      ExcMessage("ERROR: number of n_components in " + pretty_name +
                                 " could not be "
          ...
          AssertThrow((number_of_entries) == column_names.size() + dim,
                      ExcMessage("ERROR: The number of columns in the data file " + pretty_name + " is incorrect. It needs to have " +
                                 Utilities::int_to_string(column_names.size() + dim) + " columns, but the first row has " +
                                 Utilities::int_to_string(number_of_entries) + " columns."));
              ...
              if (column_num < dim)
                {
                  ...
                  AssertThrow(old_value == 0. || (std::abs(old_value - temp_data) < 1e-8 * std::abs(old_value)),
                              ExcMessage("Invalid coordinate in column " + Utilities::int_to_string(column_num) + " in row " +
                                         Utilities::int_to_string(row_num) + " in file " + pretty_name +
                  ...
                  if (log_components.find(component) != log_components.end())
                    {
                      AssertThrow(temp_data > 0.0,
                                  ExcMessage(
                                    "The data value in column " + Utilities::int_to_string(component) + " in row " +
                                    Utilities::int_to_string(row_num) + " in file " + filename +
          ...
          AssertThrow(in.eof(),
                      ExcMessage("While reading the data file '" + pretty_name +
                                 "' the ascii data "
      ...
      {
        n_components      = Utilities::MPI::broadcast(comm, n_components, root_process);
        coordinate_values = Utilities::MPI::broadcast(comm, coordinate_values, root_process);
        column_names      = Utilities::MPI::broadcast(comm, column_names, root_process);
      ...
      // objects.
      this->reinit(column_names, std::move(coordinate_values), std::move(data_tables), comm, root_process);
    }
          ...
          for (int varid = 0; varid < nvars; ++varid)
            {
              ...
              status = nc_inq_var(ncid, varid, var_name, &xtype, &var_ndims, var_dimids, &var_natts);
              AssertThrowNetCDF(status);
                  ...
                  if (var_ndims == dim)
                    {
                      ...
                      AssertThrow(use,
                                  ExcMessage("You asked to include column '" + cur_name +
                                             "', but it unfortunately has different dimensions than the first column you chose."));
                    ...
                    AssertThrow(false,
                                ExcMessage("You asked to include column '" + cur_name +
                                           "', but it unfortunately has an incorrect number of dimensions."));
                }
              AssertThrow(found, ExcMessage("You asked to include column '" + cur_name + "', but it was not found!"));
            }
        }
        ...
        for (int d = 0; d < dim; ++d)
          {
            ...
            status = nc_inq_var(ncid, varid, name, &xtype, &ndims, dimids, &natts);
    ...
    template <int dim>
    void
    StructuredDataLookup<dim>::load_file(const std::string &filename, const MPI_Comm communicator)
    {
    ...
    template <int dim>
    double
    StructuredDataLookup<dim>::get_data(const Point<dim> &position, const unsigned int component, const bool crash_if_not_in_range) const
    {
      ...
      if (crash_if_not_in_range)
        {
          ...
          AssertThrow(position[0] >= (x_coordinates[0] * (1. - 10. * std::numeric_limits<double>::epsilon())) &&
                        position[0] <= (x_coordinates[x_coordinates.size() - 1] * (1. + 10. * std::numeric_limits<double>::epsilon())),
                      ExcMessage("The requested x position " + std::to_string(position[0]) +
                                 " is outside the range of the data (minimum value = " + std::to_string(x_coordinates[0]) +
                                 " , maximum value = " + std::to_string(x_coordinates[x_coordinates.size() - 1]) + "). " +
                                 "The requested y position is " + std::to_string(position[1]) + "."));

          AssertThrow(position[1] >= (y_coordinates[0] * (1. - 10. * std::numeric_limits<double>::epsilon())) &&
                        position[1] <= (y_coordinates[y_coordinates.size() - 1] * (1. + 10. * std::numeric_limits<double>::epsilon())),
                      ExcMessage("The requested y position " + std::to_string(position[1]) +
                                 " is outside the range of the data (minimum value = " + std::to_string(y_coordinates[0]) +
                                 " , maximum value = " + std::to_string(y_coordinates[y_coordinates.size() - 1]) + "). " +
    ...
    template <int dim>
    Tensor<1, dim>
    StructuredDataLookup<dim>::get_gradients(const Point<dim> &position, const unsigned int component)
    {
    ...
    template <int dim>
    AsciiDataBase<dim>::AsciiDataBase() = default;
                                           ...
                                           const std::string &subsection_name)
    {
      prm.enter_subsection(subsection_name);
      {
        prm.declare_entry("Data directory",
                          ...
                          "it is missing.");
        prm.declare_entry("Data file name", default_filename, Patterns::Anything(), "The file name of the model data.");
        prm.declare_entry("Scale factor",
                          "1.",
                          Patterns::Double(),
    ...
    template <int dim>
    void
    AsciiDataBase<dim>::parse_parameters(ParameterHandler &prm, const std::string &subsection_name)
    {
      prm.enter_subsection(subsection_name);
      {
    ...
    template <int dim>
    void
    AsciiDataBoundary<dim>::initialize(const std::set<types::boundary_id> &boundary_ids, const unsigned int n_components)
    {
      ...
      for (const auto &boundary_id : boundary_ids)
        {
          lookups.insert(
            std::make_pair(boundary_id, std::make_unique<Utilities::StructuredDataLookup<dim - 1>>(n_components, this->scale_factor)));
          ...
          this->get_pcout() << std::endl << "   Loading Ascii data boundary file " << filename << '.' << std::endl << std::endl;


          AssertThrow(Utilities::fexists(filename, this->get_mpi_communicator()) || filename_is_url(filename),
                      ExcMessage(std::string("Ascii data file <") + pretty_name + "> not found!"));
              ...
              if (Utilities::fexists(filename, this->get_mpi_communicator()))
                {
                  this->get_pcout() << std::endl
                                    << "   Also loading next Ascii data boundary file " << filename << '.' << std::endl
                                    << std::endl;
    ...
    namespace
    {
       ...
       */
      std::string
      replace_placeholders(const std::string &filename_and_path, const std::string &boundary_name, const int filenumber)
      {
        const int maxsize  = filename_and_path.length() + 256;
        char     *filename = static_cast<char *>(std::malloc(maxsize * sizeof(char)));
        int       ret      = std::snprintf(filename, maxsize, filename_and_path.c_str(), boundary_name.c_str(), filenumber);
    ...
    template <int dim>
    std::string
    AsciiDataBoundary<dim>::create_filename(const int filenumber, const types::boundary_id boundary_id) const
    {
      ...
      if (time_dependent == true)
        {
          ...
          const bool need_update = static_cast<int>(time_steps_since_start) > std::abs(current_file_number - first_data_file_number);
          ...
          if (need_update)
            {
              ...
              const int old_file_number = (decreasing_file_order) ? current_file_number - 1 : current_file_number + 1;

              // Calculate new file_number
              current_file_number = (decreasing_file_order) ? first_data_file_number - static_cast<unsigned int>(time_steps_since_start) :
                                                              first_data_file_number + static_cast<unsigned int>(time_steps_since_start);
          ...
          time_weight = time_steps_since_start - std::abs(current_file_number - first_data_file_number);

          Assert((0 <= time_weight) && (time_weight <= 1), ExcMessage("Error in set_current_time. Time_weight has to be in [0,1]"));
        }
    }

    template <int dim>
    void
    AsciiDataBoundary<dim>::update_data(const types::boundary_id boundary_id, const bool load_both_files)
    {
    ...
    AsciiDataBoundary<dim>::end_time_dependence()
    {
      ...
      this->get_pcout() << std::endl
                        ...
                        << "   will continue unchanged from the last known state into the future)." << std::endl
                        << std::endl;
    }
    ...
    namespace
    {
      ...
      template <int dim>
      Point<dim>
      data_coordinates_from_position(const Point<dim> &position, const aspect::GeometryModel::Interface<dim> &geometry_model)
      {
        ...
        if (dim == 3 && (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(geometry_model) ||
                         Plugins::plugin_type_matches<const GeometryModel::TwoMergedChunks<dim>>(geometry_model) ||
                         Plugins::plugin_type_matches<const GeometryModel::EllipsoidalChunk<dim>>(geometry_model)))
          {
      ...
      template <int dim>
      Point<dim - 1>
      boundary_coordinates_from_data_coordinates(const Point<dim> &data_coordinates, const types::boundary_id boundary_indicator)
      {
        const std::array<unsigned int, dim - 1> boundary_dimensions = get_boundary_dimensions<dim>(boundary_indicator);
    ...
    template <int dim>
    double
    AsciiDataBoundary<dim>::get_data_component(const types::boundary_id boundary_indicator,
                                               ...
                                               const unsigned int       component) const
    {
      ...
      Assert(lookups.find(boundary_indicator) != lookups.end(), ExcInternalError());
    ...
    {
      Utilities::AsciiDataBase<dim>::declare_parameters(prm, default_directory, default_filename, subsection_name);
        ...
        if (declare_time_dependent_parameters == true)
          {
            ...
            prm.declare_entry("Data file time step",
                              "1e6",
                              Patterns::Double(0.),
            ...
            prm.declare_entry("First data file model time",
                              "0",
                              Patterns::Double(0.),
      ...
      for (unsigned int i = 0; i < number_of_layer_boundaries; ++i)
        {
          ...
          lookups.push_back(std::make_unique<Utilities::StructuredDataLookup<dim - 1>>(n_components, this->scale_factor));
    ...
    template <int dim>
    double
    AsciiDataLayered<dim>::get_data_component(const Point<dim> &position, const unsigned int component) const
    {
      const Point<dim> data_coordinates = data_coordinates_from_position(position, this->get_geometry_model());
    ...
    template <int dim>
    void
    AsciiDataLayered<dim>::parse_parameters(ParameterHandler &prm, const std::string &subsection_name)
    {
      Utilities::AsciiDataBase<dim>::parse_parameters(prm, subsection_name);
    ...
    AsciiDataInitial<dim>::initialize(const unsigned int n_components)
    {
      ...
      this->get_pcout() << std::endl << "   Loading Ascii data initial file " << filename << '.' << std::endl << std::endl;
      ...
      if (slice_data == true)
        {
          slice_lookup = std::make_unique<Utilities::StructuredDataLookup<3>>(n_components, this->scale_factor);
          slice_lookup->load_file(filename, this->get_mpi_communicator());
        }
      else
        {
          lookup = std::make_unique<Utilities::StructuredDataLookup<dim>>(n_components, this->scale_factor);
    ...
    template <int dim>
    double
    AsciiDataInitial<dim>::get_data_component(const Point<dim> &position, const unsigned int component) const
    {
      ...
      if (slice_data == true)
        {
          ...
          const std::array<double, 3> spherical_position = Utilities::Coordinates::cartesian_to_spherical_coordinates(rotated_position);
                                              ...
                                              const std::string &subsection_name)
    {
      ...
      prm.enter_subsection(subsection_name);
      {
        prm.declare_entry("Slice dataset in 2D plane",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("First point on slice",
                          "0.0,1.0,0.0",
                          Patterns::Anything(),
        ...
        prm.declare_entry("Second point on slice",
                          "1.0,0.0,0.0",
                          Patterns::Anything(),
    ...
    template <int dim>
    void
    AsciiDataInitial<dim>::parse_parameters(ParameterHandler &prm, const std::string &subsection_name)
    {
      Utilities::AsciiDataBase<dim>::parse_parameters(prm, subsection_name);
        ...
        if (slice_data == true)
          {
            ...
            AssertThrow(dim == 2, ExcMessage("The AsciiDataInitial class can only slice data in 2d models."));
    ...
    template <int dim>
    AsciiDataProfile<dim>::AsciiDataProfile() = default;
    ...
    template <int dim>
    double
    AsciiDataProfile<dim>::get_data_component(const Point<1> &position, const unsigned int component) const
    {
