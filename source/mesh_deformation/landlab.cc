  ...
  namespace MeshDeformation
  {
          ...
          if (!this_rank_runs_landlab)
            {
              ...
              // Enter/leave the same timer sections in the same order as the
              // participating-rank branch below.
              this->get_computing_timer().enter_subsection("Landlab: Create Grid");
              this->get_computing_timer().leave_subsection("Landlab: Create Grid");
      ...
      if (this_rank_runs_landlab)
        {
          ...
          this->get_computing_timer().enter_subsection("Landlab: update");
          PyObject *pValue = PythonHelper::call_python_function(pModule, "update_until", pArgs);
          this->get_computing_timer().leave_subsection("Landlab: update");
                                               ...
                                               AffineConstraints<double> &constraints) const
    {
      ...
      if (this_rank_runs_landlab)
        {
          PyObject *pArgs  = PyTuple_Pack(1, PyLong_FromLong(dim));
          this->get_computing_timer().enter_subsection("Landlab: Run initial_topography");
          PyObject *pValue = PythonHelper::call_python_function(pModule, "get_initial_topography", pArgs);
          this->get_computing_timer().leave_subsection("Landlab: Run initial_topography");
          Py_DECREF(pArgs);
