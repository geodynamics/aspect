          ...
          if (rank == 0)
            {
              ...
              AssertThrow(python_copy, ExcMessage("Failed to open destination file for writing: " + copied_module_filename));
...
#else
      AssertThrow(false,
                  ExcMessage("To use the 'Landlab' mesh deformation plugin, ASPECT needs to be configured using ASPECT_WITH_LANDLAB=ON."));
#endif
    }
      ...
      if (!this->remote_point_evaluator)
        {
...

          {
            ...
            const ArrayView<double> data_y = (dim == 3) ? PythonHelper::numpy_to_array_view(pgrid_y) : ArrayView<double>(nullptr, 0);
            const ArrayView<double> data_z =
              (dim == 3 && is_spherical) ? PythonHelper::numpy_to_array_view(pgrid_z) : ArrayView<double>(nullptr, 0);
      ...
      if (this_rank_runs_landlab)
        {
          ...
          std::vector<std::vector<double>> variable_data(variable_names.size(),
                                                         std::vector<double>(current_solution_at_points.size(), 0.0));
          ...
          for (size_t i = 0; i < data.size(); ++i)
            {
              ...
              if (gravity.norm() > 0.0)
                ...
                AssertThrow(false,
                            ExcMessage("The gravity vector has a zero norm. To use the Landlab mesh deformation plugin, "
...
#  endif
    ...
    template <int dim>
    void
    Landlab<dim>::compute_initial_deformation_as_constraints(const Mapping<dim> & /*mapping*/,
                                                             ...
                                                             AffineConstraints<double> &constraints) const
    {
      ...
      LinearAlgebra::Vector initial_deformation_dof_vector =
        this->interpolate_external_vector_field_to_surface_support_points(initial_deformation);
      ...
      for (const types::global_dof_index index : constrained_dofs)
        {
          if (constraints.can_store_line(index))
            if (constraints.is_constrained(index) == false)
              {
                constraints.add_constraint(index, {}, initial_deformation_ghosted(index));
              }
        }
#else
    ...
    Landlab<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Mesh deformation");
      {
        ...
        {
          prm.declare_entry("MPI ranks for Landlab",
                            "1",
                            Patterns::Integer(1),
          ...
          prm.declare_entry("Script name", "", Patterns::Anything(), "Name of the Python module to load (without .py extension).");
        }
      ...
      prm.enter_subsection("Mesh deformation");
      {
        prm.enter_subsection("Landlab");
        {
          ...
          AssertThrow(Utilities::fexists(script_path + script_module_name + ".py"),
                      ExcMessage("The specified script path: " + script_path + script_module_name + ".py" +
                                 " for Landlab does not exist."));
        }
        prm.leave_subsection();
      }
      prm.leave_subsection();
