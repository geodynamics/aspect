 ...
 /*template <int dim>
  ...
  {}*/
                                                             ...
                                                             const std::set<types::boundary_id> &boundary_ids) const
    {
       ...
       */
      std::vector<std::vector<double>> temporary_variables(4, std::vector<double>(array_size));
      ...
      if (Utilities::MPI::this_mpi_process(this->get_mpi_communicator()) == 0)
        {
          ...
          for (unsigned int p=1; p<Utilities::MPI::n_mpi_processes(this->get_mpi_communicator()); ++p)
            {
              MPI_Status status;
              MPI_Recv(&temporary_variables, array_size+1, MPI_DOUBLE, p, 42, this->get_mpi_communicator(), &status);
        ...
        {
          MPI_Send(&temporary_variables, array_size+1, MPI_DOUBLE, 0, 42, this->get_mpi_communicator());
        }
     ...
     /*  Table<dim-1,Tensor<1,dim>> data_table;
        data_table.TableBase<dim-1,Tensor<1,dim>>::reinit(size_idx);
      //  std::vector<Table<dim-1,double> > data_tables(components,data_table);
            ...
            for (unsigned int j=0; j<data_table.size()[0]; ++j)
              {
                ...
                {
                if(k==2)
             	   data_table(idx)[k] = V[nx*i+j];
                else
             	 data_table(idx)[k] = 0;

                std::cout<<data_table(idx)[k]<<", ";
                }
                std::cout<<" : ";
              }
            std::cout<<"  "<<std::endl;
        ...
        //std::array<std::unique_ptr<typename Functions::InterpolatedUniformGridData<dim-1> >, 3> velocities;
        std::unique_ptr<typename Functions::InterpolatedUniformGridData<dim-1> > velocities;

            velocities = std_cxx14::make_unique<Functions::InterpolatedUniformGridData<dim-1>> (grid_extent,
                                                                                                table_intervals,
                                                                                                data_table);*/
