...
namespace aspect
{
  namespace InitialTemperature
  {
    ...
    template <int dim>
    double
    Adiabatic<dim>::top_boundary_layer_age(const Point<dim> &position) const
    {
      ...
      if (top_boundary_layer_age_model == BoundaryLayerAgeModel::ascii_data)
        {
          ...
          age_top = Utilities::AsciiDataBoundary<dim>::get_data_component(surface_boundary_id, position, 0);
        }
      else if (top_boundary_layer_age_model == BoundaryLayerAgeModel::constant)
        {
          age_top = (this->convert_output_to_years() ? age_top_boundary_layer * year_in_seconds : age_top_boundary_layer);
        }
    ...
    template <int dim>
    double
    Adiabatic<dim>::initial_temperature(const Point<dim> &position) const
    {
      const double age_top    = top_boundary_layer_age(position);
      const double age_bottom = (this->convert_output_to_years() ? age_bottom_boundary_layer * year_in_seconds : age_bottom_boundary_layer);
      ...
      const double     adiabatic_bottom_temperature =
        (this->include_adiabatic_heating()) ? this->get_adiabatic_conditions().temperature(bottom_point) : adiabatic_surface_temperature;
      ...
      const double kappa =
        ((this->get_parameters().formulation_temperature_equation ==
          Parameters<dim>::Formulation::TemperatureEquation::reference_density_profile) ?
           out.thermal_conductivities[0] / (this->get_adiabatic_conditions().density(in.position[0]) * out.specific_heat[0]) :
      ...
      if (cooling_model == "half-space cooling")
        {
          ...
          surface_cooling_temperature = age_top > 0.0 ?
                                          (T_surface - adiabatic_surface_temperature) *
                                            std::erfc(this->get_geometry_model().depth(position) / (2 * std::sqrt(kappa * age_top))) :
                                          0.0;
          bottom_heating_temperature =
            (age_bottom > 0.0 && this->get_adiabatic_conditions().is_initialized()) ?
              (T_bottom - adiabatic_bottom_temperature + subadiabaticity) *
                std::erfc((this->get_geometry_model().maximal_depth() - this->get_geometry_model().depth(position)) /
          ...
          if (depth > lithosphere_thickness)
          ...
          else
            {
              const double exponential =
                -kappa * Utilities::fixed_power<2>(numbers::PI) * age_top / Utilities::fixed_power<2>(lithosphere_thickness);
              double sum_terms = 0.;
              for (unsigned int n = 1; n < 11; ++n)
                {
                  ...
                  surface_cooling_temperature =
                    T_surface - adiabatic_surface_temperature +
                    (adiabatic_surface_temperature - T_surface) * (depth / lithosphere_thickness + 2 / numbers::PI * sum_terms);
                }
            }
        }
          ...
          if (Plugins::plugin_type_matches<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()))
              ...
              if (dim == 2)
                {
                  ...
                  mid_point(0) = inner_radius * std::sin(half_opening_angle), mid_point(1) = inner_radius * std::cos(half_opening_angle);
                }
              else if (dim == 3)
                {
                  ...
                  if (shell_geometry_model.opening_angle() == 90)
                    {
                      mid_point(0) = inner_radius * std::sqrt(1. / 3), mid_point(1) = inner_radius * std::sqrt(1. / 3),
          ...
          else if (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(this->get_geometry_model()))
            {
              ...
              if (dim == 2)
                {
                  // choose the center of the perturbation at half angle along the inner radius
                  mid_point(0) = inner_radius * std::cos(longitude_midpoint), mid_point(1) = inner_radius * std::sin(longitude_midpoint);
                }
              else if (dim == 3)
                {
      ...
      const double perturbation = (mid_point.distance(position) < radius) ? amplitude : 0.0;


      // add the subadiabaticity
      const double zero_depth = 0.174;
      const double nondimensional_depth =
        (this->get_geometry_model().depth(position) / this->get_geometry_model().maximal_depth() - zero_depth) / (1.0 - zero_depth);
      ...
      const double temperature_profile =
        (this->include_adiabatic_heating()) ? this->get_adiabatic_conditions().temperature(position) : adiabatic_surface_temperature;
    ...
    Adiabatic<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Age top boundary layer",
                            "0.",
                            Patterns::Double(0.),
                            ...
                            "\\si{\\second} otherwise.");
          prm.declare_entry("Age bottom boundary layer",
                            "0.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Position",
                            ...
                            "radial lines.");
          prm.declare_entry("Subadiabaticity",
                            "0.",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Top boundary layer age model",
                            "constant",
          ...
          prm.declare_entry("Cooling model",
                            "half-space cooling",
          ...
          prm.declare_entry("Lithosphere thickness",
                            "125e3",
                            Patterns::Double(0.),
    ...
    Adiabatic<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      prm.enter_subsection("Initial temperature model");
      {
        ...
        prm.enter_subsection("Adiabatic");
        {
          top_boundary_layer_age_model = BoundaryLayerAgeModel::parse("Top boundary layer age model", prm);
          ...
          if (n_compositional_fields > 0)
            {
              ...
              try
                {
                  function = std::make_unique<Functions::ParsedFunction<1>>(n_compositional_fields);
