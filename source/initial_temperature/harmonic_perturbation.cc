...
namespace aspect
{
  namespace InitialTemperature
  {
    template <int dim>
    double
    HarmonicPerturbation<dim>::initial_temperature(const Point<dim> &position) const
    {
      ...
      const double background_temperature =
        this->include_adiabatic_heating() ? this->get_adiabatic_conditions().temperature(position) : reference_temperature;
      ...
      if (Plugins::plugin_type_matches<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()))
      ...
      else if (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(this->get_geometry_model()))
        {
          ...
          AssertThrow(dim == 2, ExcMessage("Harmonic perturbation only implemented in 2d for chunk geometry"));
    ...
    HarmonicPerturbation<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Vertical wave number",
                            "1",
                            Patterns::Integer(),
          ...
          prm.declare_entry("Magnitude", "1.0", Patterns::Double(0.), "The magnitude of the Harmonic perturbation.");
