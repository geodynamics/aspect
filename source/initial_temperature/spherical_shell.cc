...
namespace aspect
{
  namespace InitialTemperature
  {
    template <int dim>
    double
    SphericalHexagonalPerturbation<dim>::initial_temperature(const Point<dim> &position) const
    {
      ...
      */
      const double scale = ((dim == 3) ? std::max(0.0, std::cos(numbers::PI * std::fabs(position(2) / R1))) : 1.0);
      const double phi   = std::atan2(position(0), position(1));
      const double s_mod =
        s + 0.2 * s * (1 - s) * std::sin(angular_mode * phi + (90 + 2 * rotation_offset) * constants::degree_to_radians) * scale;
    ...
    SphericalHexagonalPerturbation<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Angular mode", "6", Patterns::Integer(), "The number of convection cells with which to perturb the system.");

          prm.declare_entry("Rotation offset",
                            "-45.",
                            Patterns::Double(),
    ...
    SphericalHexagonalPerturbation<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      if (Plugins::plugin_type_matches<const GeometryModel::Sphere<dim>>(this->get_geometry_model()))
        {
          R1 = Plugins::get_plugin_as_type<const GeometryModel::Sphere<dim>>(this->get_geometry_model()).radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()))
        {
          R1 = Plugins::get_plugin_as_type<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()).outer_radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(this->get_geometry_model()))
        {
          R1 = Plugins::get_plugin_as_type<const GeometryModel::Chunk<dim>>(this->get_geometry_model()).outer_radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::EllipsoidalChunk<dim>>(this->get_geometry_model()))
        {
          ...
          AssertThrow(gm.get_eccentricity() == 0.0, ExcNotImplemented("This plugin cannot be used with a non-zero eccentricity. "));
    ...
    template <int dim>
    SphericalGaussianPerturbation<dim>::SphericalGaussianPerturbation()
    {
    ...
    template <int dim>
    double
    SphericalGaussianPerturbation<dim>::initial_temperature(const Point<dim> &position) const
    {
      // Check that a boundary temperature is prescribed
      AssertThrow(this->has_boundary_temperature(),
                  ExcMessage("This initial condition can only be used if a boundary "
      ...
      const double dT =
        this->get_boundary_temperature_manager().maximal_temperature() - this->get_boundary_temperature_manager().minimal_temperature();
      ...
      const double Perturbation =
        (sign * amplitude *
         std::exp(-(Utilities::fixed_power<2>((position(0) * scale / R1 - x)) + Utilities::fixed_power<2>((position(1) * scale / R1 - y))) /
                  sigma));
    ...
    SphericalGaussianPerturbation<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Angle", "0.", Patterns::Double(0.), "The angle where the center of the perturbation is placed.");
          prm.declare_entry("Non-dimensional depth",
                            "0.7",
                            Patterns::Double(0.),
                            "The non-dimensional radial distance where the center of the "
                            "perturbation is placed.");
          prm.declare_entry("Amplitude", "0.01", Patterns::Double(0.), "The amplitude of the perturbation.");
          prm.declare_entry("Sigma", "0.2", Patterns::Double(0.), "The standard deviation of the Gaussian perturbation.");
          prm.declare_entry("Sign", "1.", Patterns::Double(), "The sign of the perturbation.");
          prm.declare_entry("Filename for initial geotherm table",
    ...
    SphericalGaussianPerturbation<dim>::parse_parameters(ParameterHandler &prm)
    {
      ...
      if (Plugins::plugin_type_matches<const GeometryModel::Sphere<dim>>(this->get_geometry_model()))
        ...
        {
          R0 = Plugins::get_plugin_as_type<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()).inner_radius();
          R1 = Plugins::get_plugin_as_type<const GeometryModel::SphericalShell<dim>>(this->get_geometry_model()).outer_radius();
        }
      else if (Plugins::plugin_type_matches<const GeometryModel::Chunk<dim>>(this->get_geometry_model()))
        {
          R0 = Plugins::get_plugin_as_type<const GeometryModel::Chunk<dim>>(this->get_geometry_model()).inner_radius();
          R1 = Plugins::get_plugin_as_type<const GeometryModel::Chunk<dim>>(this->get_geometry_model()).outer_radius();
        }
