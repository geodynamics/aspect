...
namespace aspect
{
  namespace InitialTemperature
  {
    ...
    AdiabaticBoundary<dim>::initial_temperature(const Point<dim> &position) const
    {
      ...
      const double isotherm_depth = Utilities::AsciiDataBoundary<dim>::get_data_component(surface_boundary_id, position, 0);
    ...
    AdiabaticBoundary<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Initial temperature model");
      {
        ...
        {
          prm.declare_entry("Isotherm temperature",
                            "1673.15",
                            Patterns::Double(0.),
          ...
          prm.declare_entry("Adiabatic temperature gradient",
                            "0.0005",
                            Patterns::Double(0.),
