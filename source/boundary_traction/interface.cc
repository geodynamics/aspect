...
namespace aspect
{
  namespace BoundaryTraction
  {
      ...
      for (const auto &plugin : this->plugin_objects)
        {
          if (boundary_indicators[i] == boundary_indicator)
            {
              found_plugin                         = true;
              const Tensor<1, dim> plugin_traction = plugin->boundary_traction(boundary_indicator, position, normal_vector);
        ...
        for (const auto &p : x_boundary_traction_indicators)
          {
            ...
            try
              ...
              {
                AssertThrow(false,
                            ExcMessage("While parsing the entry <Boundary traction model/Prescribed "
      ...
      for (const auto &plugin_name : this->plugin_names)
        {
          ...
          this->plugin_objects.push_back(std::get<dim>(registered_plugins).create_plugin(plugin_name, "Boundary traction::Model names"));
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Boundary traction interface", out);
    }
  }
}
