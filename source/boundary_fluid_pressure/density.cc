...
namespace aspect
{
  namespace BoundaryFluidPressure
  {
    ...
    template <int dim>
    void
    Density<dim>::fluid_pressure_gradient(const types::boundary_id /*boundary_indicator*/,
              ...
              case DensityFormulation::average_density:
                {
                  ...
                  fluid_pressure_gradient_outputs[q] =
                    ((1.0 - phi) * material_model_outputs.densities[q] * gravity + phi * melt_outputs->fluid_densities[q] * gravity) *
                    normal_vectors[q];
                  break;
                }
    ...
    Density<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Boundary fluid pressure model");
      {
        ...
        {
          prm.declare_entry("Density formulation",
                            "solid density",
