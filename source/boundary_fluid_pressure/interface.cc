...
namespace aspect
{
  namespace BoundaryFluidPressure
  {
    ...
    create_boundary_fluid_pressure(ParameterHandler &prm)
    {
      ...
      return std::get<dim>(registered_plugins).create_plugin(model_name, "Boundary fluid pressure model::Plugin name");
    }
    ...
    {
      std::get<dim>(registered_plugins).write_plugin_graph("Boundary fluid pressure interface", out);
    }
...
\
  template void register_boundary_fluid_pressure<dim>(const std::string &, \
                                                      const std::string &, \
...
\
  template std::unique_ptr<Interface<dim>> create_boundary_fluid_pressure<dim>(ParameterHandler & prm);
