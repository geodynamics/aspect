...
namespace aspect
{
  namespace internal
  {
    ...
    setup_quadratures(const Parameters<dim> &parameters, const ReferenceCell<dim> reference_cell)
    {
...
#else
        return reference_cell.template get_gauss_type_quadrature<dim>(degree);
#endif
      };
      ...
      quadratures.pressure   = get_gauss_type_quadrature(
        parameters.use_equal_order_interpolation_for_stokes ? parameters.stokes_velocity_degree + 1 : parameters.stokes_velocity_degree);
      ...
      quadratures.system = get_gauss_type_quadrature(
    ...
    setup_face_quadratures(const Parameters<dim> &parameters, const ReferenceCell<dim> reference_cell)
    {
...
#else
        return reference_cell.face_reference_cell(0).template get_gauss_type_quadrature<dim - 1>(degree);
#endif
      };
      ...
      quadratures.pressure   = get_gauss_type_face_quadrature(
        parameters.use_equal_order_interpolation_for_stokes ? parameters.stokes_velocity_degree + 1 : parameters.stokes_velocity_degree);
    ...
    new_FE_Q_or_DGP(const bool discontinuous, const unsigned int degree)
    {
      if (discontinuous)
    ...
    template <int dim>
    std::shared_ptr<FiniteElement<dim>>
    new_FE_Q_or_DGQ(const bool discontinuous, const unsigned int degree)
    {
      if (discontinuous)
  ...
  construct_default_variables(const Parameters<dim> &parameters)
  {
    ...
    variables.push_back(
      VariableDeclaration<dim>("velocity", std::make_shared<FE_Q<dim>>(parameters.stokes_velocity_degree), dim, n_velocity_blocks));
    ...
    if (parameters.use_equal_order_interpolation_for_stokes == false)
      ...
      variables.push_back(VariableDeclaration<dim>("pressure", std::make_shared<FE_Q<dim>>(parameters.stokes_velocity_degree), 1, 1));
        ...
        if (c > 0 &&
        ...
        else
          {
            std::shared_ptr<FiniteElement<dim>> fe =
              internal::new_FE_Q_or_DGQ<dim>(parameters.use_discontinuous_composition_discretization[c], parameters.composition_degrees[c]);
  ...
  template <int dim>
  Introspection<dim>::Introspection(const std::vector<VariableDeclaration<dim>> &variable_definition, const Parameters<dim> &parameters)
      ...
      if (this->n_compositional_fields > 0)
        {
          ...
          const unsigned int last =
            *std::max_element(this->base_elements.compositional_fields.begin(), this->base_elements.compositional_fields.end());
  ...
  namespace
  {
    ...
    make_composition_component_mask_sequence(const FEVariableCollection<dim>                     &fevs,
                                             const typename Introspection<dim>::ComponentIndices &indices)
    {
  ...
  template <int dim>
  Introspection<dim>::ComponentMasks::ComponentMasks(const FEVariableCollection<dim>            &fevs,
  ...
  {
    Assert(compositional_field_indices_with_base_element.find(base_element_index) != compositional_field_indices_with_base_element.end(),
  ...
  {
    const std::vector<std::string>::const_iterator it = std::find(composition_names.begin(), composition_names.end(), name);
    AssertThrow(it != composition_names.end(),
                ExcMessage("The compositional field " + name + " you asked for is not used in the simulation."));
  ...
  {
    return (std::find(composition_names.begin(), composition_names.end(), name) != composition_names.end() ? true : false);
  }
  ...
  Introspection<dim>::is_composition_component(const unsigned int component_index) const
  {
    ...
    Assert(component_indices.compositional_fields[0] > component_indices.temperature &&
             component_indices.compositional_fields.back() == n_components - 1,
           ExcInternalError());
...
#define INSTANTIATE(dim) \
  ...
  template std::vector<VariableDeclaration<dim>> construct_default_variables(const Parameters<dim> &parameters);
