...
namespace aspect
{
  namespace MaterialModel
  {
    ...
    template <int dim>
    MeltOutputs<dim>::MeltOutputs(const unsigned int n_points, const unsigned int /*n_comp*/)
    ...
    {
      average_property(operation, projection_matrix, expansion_matrix, compaction_viscosities);
      average_property(operation, projection_matrix, expansion_matrix, fluid_viscosities);
      average_property(operation, projection_matrix, expansion_matrix, permeabilities);
      average_property(operation, projection_matrix, expansion_matrix, fluid_densities);
  ...
  namespace Assemblers
  {
    ...
    template <int dim>
    void
    MeltStokesPreconditioner<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                           internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      internal::Assembly::Scratch::StokesPreconditioner<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::StokesPreconditioner<dim> &>(scratch_base);
      internal::Assembly::CopyData::StokesPreconditioner<dim> &data =
        dynamic_cast<internal::Assembly::CopyData::StokesPreconditioner<dim> &>(data_base);
      ...
      const double p_c_scale = Plugins::get_plugin_as_type<const MaterialModel::MeltInterface<dim>>(this->get_material_model())
                                 .p_c_scale(scratch.material_model_inputs, scratch.material_model_outputs, this->get_melt_handler(), true);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          const double K_D =
            this->get_melt_handler().limited_darcy_coefficient(melt_outputs->permeabilities[q] / melt_outputs->fluid_viscosities[q],
                                                               p_c_scale > 0);
            ...
            for (unsigned int j = 0; j < stokes_dofs_per_cell; ++j)
              {
                if (introspection.stokes_dof_info[i].component_index == introspection.stokes_dof_info[j].component_index)
                  data.local_matrix(i, j) += ((one_over_eta * pressure_scaling * pressure_scaling) * scratch.phi_p[i] * scratch.phi_p[j] +
                                              (K_D * pressure_scaling * pressure_scaling) * scratch.grad_phi_p[i] * scratch.grad_phi_p[j] +
                                              (1. / eta + 1. / viscosity_c) * p_c_scale * p_c_scale * pressure_scaling * pressure_scaling *
                                                (scratch.phi_p_c[i] * scratch.phi_p_c[j])) *
                                             JxW;

                // add S between p_c and p_f
                data.local_matrix(i, j) +=
                  ((p_c_scale * one_over_eta * pressure_scaling * pressure_scaling) * scratch.phi_p[i] * scratch.phi_p_c[j] +
                   (p_c_scale * one_over_eta * pressure_scaling * pressure_scaling) * scratch.phi_p_c[i] * scratch.phi_p[j]) *
                  JxW;
              }
        }
    }
    ...
    namespace
    {
                                 ...
                                 const double                                          operator_split_reaction)
      {
        ...
        if (simulator_access->get_parameters().use_operator_splitting)
          melting_rate =
            (simulator_access->get_timestep() > 0 ? operator_split_reaction * solid_density / simulator_access->get_timestep() : 0.0);
        ...
        const Tensor<1, dim> gravity =
          simulator_access->get_gravity_model().gravity_vector(scratch.finite_element_values.quadrature_point(q_point));
        ...
        fluid_pressure_RHS += is_compressible ? (current_u * fluid_density_gradient) * porosity / fluid_density -
    ...
    template <int dim>
    void
    MeltStokesSystem<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                   internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      ...
      if (this->get_parameters().use_operator_splitting)
        scratch.finite_element_values[introspection.extractors.compositional_fields[porosity_index]].get_function_values(
          this->get_reaction_vector(), reactions);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          // Viscosity scalar
          const double eta =
            (scratch.rebuild_stokes_matrix ? scratch.material_model_outputs.viscosities[q] : numbers::signaling_nan<double>());
          const double eta_two_thirds =
            (scratch.rebuild_stokes_matrix ? scratch.material_model_outputs.viscosities[q] * 2.0 / 3.0 : numbers::signaling_nan<double>());
          const Tensor<1, dim> gravity = this->get_gravity_model().gravity_vector(scratch.finite_element_values.quadrature_point(q));

          const double compressibility =
            (this->get_material_model().is_compressible() ? scratch.material_model_outputs.compressibilities[q] :
          ...
          const double         p_f_RHS            = compute_fluid_pressure_rhs(this, scratch, q, K_D, reactions[q]);
          ...
          for (unsigned int i = 0; i < stokes_dofs_per_cell; ++i)
            {
              data.local_rhs(i) +=
                ((bulk_density * gravity * scratch.phi_u[i]) +
                 ...
                 (force != nullptr ? (force->rhs_u[q] * scratch.phi_u[i] + pressure_scaling * force->rhs_p[q] * scratch.phi_p[i] +
                 ...
                 (this->get_material_model().is_compressible() ?
                    (pressure_scaling * compressibility * density_s * (scratch.velocity_values[q] * gravity) * scratch.phi_p[i]) :
                    0) +
                 pressure_scaling * p_f_RHS * scratch.phi_p[i] - pressure_scaling * K_D * density_f * (scratch.grad_phi_p[i] * gravity)) *
                JxW;

              if (elastic_outputs != nullptr && this->get_parameters().enable_elasticity)
                data.local_rhs(i) += (elastic_outputs->elastic_force[q] * scratch.grads_phi_u[i]) * JxW;
                ...
                for (unsigned int j = 0; j < stokes_dofs_per_cell; ++j)
                  {
                    data.local_matrix(i, j) +=
                      ((eta * 2.0 * (scratch.grads_phi_u[i] * scratch.grads_phi_u[j])) -
                       ...
                       - (pressure_scaling * scratch.phi_p[i] * scratch.div_phi_u[j]) +
                       (-pressure_scaling * pressure_scaling / viscosity_c * p_c_scale * p_c_scale * scratch.phi_p_c[i] *
                        scratch.phi_p_c[j]) -
                       ...
                       K_D * pressure_scaling * pressure_scaling * (scratch.grad_phi_p[i] * scratch.grad_phi_p[j]) +
                       (this->get_material_model().is_compressible() ? K_D * pressure_scaling * pressure_scaling / density_f *
    ...
    template <int dim>
    void
    MeltStokesSystem<dim>::create_additional_material_model_outputs(MaterialModel::MaterialModelOutputs<dim> &outputs) const
    {
      ...
      if (this->get_parameters().enable_elasticity && !outputs.template has_additional_output_object<MaterialModel::ElasticOutputs<dim>>())
        {
          outputs.additional_outputs.push_back(std::make_unique<MaterialModel::ElasticOutputs<dim>>(n_points));
        }
    ...
    template <int dim>
    void
    MeltStokesSystemBoundary<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                           internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      internal::Assembly::Scratch::StokesSystem<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::StokesSystem<dim> &>(scratch_base);
              ...
              if (is_velocity_or_pressures(introspection, p_c_component_index, p_f_component_index, component_index_i))
                {
                  ...
                  data.local_rhs(i_stokes) += (scratch.face_finite_element_values[ex_p_f].value(i, q) * this->get_pressure_scaling() * K_D *
                                               (density_f * (normal_vector * gravity) - grad_p_f[q]) * JxW);
                  ++i_stokes;
                }
              ++i;
            }
    ...
    namespace
    {
                          ...
                          const double                                             divergence_u)
      {
        ...
        const double compressibility =
          (simulator_access->get_material_model().is_compressible() ? scratch.material_model_outputs.compressibilities[q_point] : 0.0);
        ...
        double melt_transport_RHS = melting_rate / density + divergence_u + compressibility * density * (current_u * gravity);
    ...
    template <int dim>
    void
    MeltAdvectionSystem<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                      internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          const double       bulk_density =
            (1.0 - porosity) * scratch.material_model_outputs.densities[q] + porosity * melt_outputs->fluid_densities[q];

          const double density_c_P =
            ((scratch.advection_field->is_temperature()) ? bulk_density * scratch.material_model_outputs.specific_heat[q] : 1.0);
          ...
          const double conductivity =
            (scratch.advection_field->is_temperature() ? scratch.material_model_outputs.thermal_conductivities[q] : 0.0);
          const double latent_heat_LHS =
            ((scratch.advection_field->is_temperature()) ? scratch.heating_model_outputs.lhs_latent_heat_terms[q] : 0.0);
          ...
          const double gamma = ((scratch.advection_field->is_temperature()) ? scratch.heating_model_outputs.heating_source_terms[q] : 0.0);
          ...
          const double melt_transport_RHS =
            (this->get_melt_handler().is_porosity(*scratch.advection_field) ? compute_melting_RHS(this, scratch, q, divergence_u) : 0.0);
          ...
          const double melt_transport_LHS =
            (this->get_melt_handler().is_porosity(*scratch.advection_field) &&
               ...
               divergence_u + (this->get_material_model().is_compressible() ?
                                 scratch.material_model_outputs.compressibilities[q] * scratch.material_model_outputs.densities[q] *
                                   current_u * this->get_gravity_model().gravity_vector(scratch.finite_element_values.quadrature_point(q)) :
                                 0.0) :
               0.0);

          const double factor = (use_bdf2_scheme) ? ((2 * time_step + old_time_step) / (time_step + old_time_step)) : 1.0;
          ...
          if (scratch.advection_field->is_temperature() && this->get_melt_handler().is_melt_cell(scratch.cell) &&
            ...
            {
              density_c_P_solid =
                (1.0 - porosity) * scratch.material_model_outputs.densities[q] * scratch.material_model_outputs.specific_heat[q];
              density_c_P_melt = porosity * melt_outputs->fluid_densities[q] * scratch.material_model_outputs.specific_heat[q];
            }
          else if (!scratch.advection_field->is_temperature() && !this->get_melt_handler().is_porosity(*scratch.advection_field) &&
                   scratch.advection_field->advection_method(this->introspection()) ==
                     Parameters<dim>::AdvectionFieldMethod::fem_melt_field)
            {
          ...
          for (unsigned int i = 0; i < advection_dofs_per_cell; ++i)
            {
              data.local_rhs(i) +=
                (field_term_for_rhs * scratch.phi_field[i] + time_step * scratch.phi_field[i] * (gamma + melt_transport_RHS) +
              ...
              for (unsigned int j = 0; j < advection_dofs_per_cell; ++j)
                {
                  data.local_matrix(i, j) +=
                    ((time_step * (conductivity + scratch.artificial_viscosity) * (scratch.grad_phi_field[i] * scratch.grad_phi_field[j])) +
    ...
    template <int dim>
    std::vector<double>
    MeltAdvectionSystem<dim>::compute_residual(internal::Assembly::Scratch::ScratchBase<dim> &scratch_base) const
    {
      internal::Assembly::Scratch::AdvectionSystem<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::AdvectionSystem<dim> &>(scratch_base);
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          if (scratch.advection_field->is_temperature())
          ...
          else
            {
              ...
              const double melt_transport_RHS =
                (this->get_melt_handler().is_porosity(*scratch.advection_field) ? compute_melting_RHS(this, scratch, q, divergence_u) :
                                                                                  0.0);
              ...
              const double melt_transport_LHS =
                (this->get_melt_handler().is_porosity(*scratch.advection_field) ?
                   scratch.current_velocity_divergences[q] +
                     (this->get_material_model().is_compressible() ?
                        scratch.material_model_outputs.compressibilities[q] * scratch.material_model_outputs.densities[q] * u *
                          this->get_gravity_model().gravity_vector(scratch.finite_element_values.quadrature_point(q)) :
                        0.0) :
                   0.0);

              residuals[q] = std::abs(dField_dt + u_grad_field + melt_transport_LHS * field - melt_transport_RHS - dreaction_term_dt);
            }
        }
      return residuals;
    }
    ...
    template <int dim>
    void
    MeltPressureRHSCompatibilityModification<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                                           internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      internal::Assembly::Scratch::StokesSystem<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::StokesSystem<dim> &>(scratch_base);
    ...
    template <int dim>
    void
    MeltBoundaryTraction<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                       internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      internal::Assembly::Scratch::StokesSystem<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::StokesSystem<dim> &>(scratch_base);
          ...
          for (unsigned int q = 0; q < scratch.face_finite_element_values.n_quadrature_points; ++q)
            {
              const Tensor<1, dim> traction =
                this->get_boundary_traction_manager().boundary_traction(face->boundary_id(),
                  ...
                  if (is_velocity_or_pressures(introspection, p_c_component_index, p_f_component_index, component_index_i))
                    {
                      data.local_rhs(i_stokes) +=
                        scratch.face_finite_element_values[introspection.extractors.velocities].value(i, q) * traction * JxW;
                      ++i_stokes;
                    }
                  ++i;
                }
  ...
  template <int dim>
  void
  MeltHandler<dim>::compute_melt_variables(LinearAlgebra::BlockSparseMatrix &system_matrix,
                                           ...
                                           LinearAlgebra::BlockVector       &system_rhs) const
  {
    ...
    Assert(this->include_melt_transport(),
           ExcMessage("'Include melt transport' has to be on to "
...

    {
      ...
      const unsigned int     fluid_velocity_dofs_per_cell =
        fe.element_multiplicity(u_f_variable.base_index) * fe.base_element(u_f_variable.base_index).dofs_per_cell;
        ...
        if (cell->is_locally_owned())
          {
            ...
            fe_values[this->introspection().extractors.compositional_fields[por_idx]].get_function_values(solution, porosity_values);
            fe_values[this->introspection().extractors.velocities].get_function_values(solution, u_s_values);

            fe_values[this->introspection().variable("fluid pressure").extractor_scalar()].get_function_gradients(solution,
                                                                                                                  grad_p_f_values);
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                if (is_melt_cell)
                  ...
                  for (unsigned int i = 0; i < fluid_velocity_dofs_per_cell; ++i)
                    cell_vector(i) += (u_s_values[q]) * phi_u_f[i] * JxW;
              }

            this->get_current_constraints().distribute_local_to_global(
              cell_matrix, cell_vector, cell_u_f_dof_indices, system_matrix, system_rhs, false);
          }
      ...
      SolverControl                   solver_control(system_rhs.block(block_idx).size(), 1e-8 * system_rhs.block(block_idx).l2_norm());
    ...
    // p_s = p_c / (1-phi) + p_f
    {
      ...
      FEValues<dim> fe_values(this->get_mapping(), this->get_fe(), quadrature, update_quadrature_points | update_gradients | update_values);
        ...
        if (cell->is_locally_owned())
          {
            ...
            fe_values[this->introspection().extractors.compositional_fields[por_idx]].get_function_values(solution, porosity_values);
            fe_values[this->introspection().variable("compaction pressure").extractor_scalar()].get_function_values(solution, p_c_values);
            fe_values[this->introspection().variable("fluid pressure").extractor_scalar()].get_function_values(solution, p_f_values);
            in.reinit(fe_values, cell, this->introspection(), solution);
            ...
            for (unsigned int j = 0; j < this->get_fe().base_element(this->introspection().base_elements.pressure).dofs_per_cell; ++j)
              {
                const unsigned int pressure_idx = this->get_fe().component_to_system_index(this->introspection().component_indices.pressure,
  ...
  template <int dim>
  bool
  MeltHandler<dim>::is_porosity(const AdvectionField &advection_field) const
  {
  ...
  namespace internal
  {
    ...
    struct PcConstraintsAssembleData
    {
        ...
        PcConstraintsAssembleData(const FiniteElement<dim> &finite_element,
          ...
          : finite_element_values(mapping, finite_element, quadrature, update_flags)
    ...
    {
      public:
        PcNonZeroDofsAssembler(const std::vector<bool> &is_melt_cell, IndexSet &nonzero_entries)
          ...
          if (is_melt_cell)
            {
              ...
              for (unsigned int j = 0; j < this->get_fe().base_element(pc_base_index).dofs_per_cell; ++j)
                {
                  const unsigned int pressure_idx = this->get_fe().component_to_system_index(pc_component_index,
                                                                                             /*dof index within component=*/j);
        ...
        {
          nonzero_entries.add_indices(data.nonzero_dof_indices.begin(), data.nonzero_dof_indices.end());
        }
  ...
  template <int dim>
  void
  MeltHandler<dim>::add_current_constraints(AffineConstraints<double> &constraints)
  {
        ...
        if (cell->is_locally_owned())
          {
            ...
            material_model_inputs.reinit(finite_element_values, cell, this->introspection(), this->get_current_linearization_point());
            ...
            const double p_c_scale =
              Plugins::get_plugin_as_type<const MaterialModel::MeltInterface<dim>>(this->get_material_model())
                .p_c_scale(material_model_inputs, material_model_outputs, this->get_melt_handler(), false /*=consider_is_melt_cell*/);
    ...
    auto worker = [&](const typename DoFHandler<dim>::active_cell_iterator &cell,
                      ...
                      internal::PcConstraintsCopyData<dim> &data) { assembler.local_save_nonzero_pc_dofs(cell, scratch, data); };

    auto copier = [&](const internal::PcConstraintsCopyData<dim> &data) { assembler.copy_local_to_global(data); };
    ...
    WorkStream::run(
      CellFilter(IteratorFilters::LocallyOwnedCell(), this->get_dof_handler().begin_active()),
      CellFilter(IteratorFilters::LocallyOwnedCell(), this->get_dof_handler().end()),
      worker,
      copier,
      internal::PcConstraintsAssembleData<dim>(
        fe, quadrature_formula, this->get_mapping(), cell_update_flags, this->introspection().n_compositional_fields, stokes_dofs_per_cell),
      internal::PcConstraintsCopyData<dim>(stokes_dofs_per_cell));

    // For the constraints, first pick all relevant p_c dofs:
    IndexSet for_constraints =
  ...
  template <int dim>
  bool
  MeltHandler<dim>::is_melt_cell(const typename DoFHandler<dim>::active_cell_iterator &cell) const
  {
  ...
  template <int dim>
  double
  MeltHandler<dim>::limited_darcy_coefficient(const double K_D, const bool is_melt_cell) const
  {
    const double ref_K_D =
      Plugins::get_plugin_as_type<const MaterialModel::MeltInterface<dim>>(this->get_material_model()).reference_darcy_coefficient();
  ...
  MeltHandler<dim>::get_boundary_fluid_pressure() const
  {
  ...
  template <int dim>
  void
  MeltHandler<dim>::edit_finite_element_variables(const Parameters<dim> &parameters, std::vector<VariableDeclaration<dim>> &variables)
  {
    ...
    variables.insert(
      variables.begin() + 2,
      VariableDeclaration<dim>("compaction pressure",
                               melt_parameters.use_discontinuous_p_c ?
                                 std::shared_ptr<FiniteElement<dim>>(std::make_shared<FE_DGP<dim>>(parameters.stokes_velocity_degree - 1)) :
                                 std::shared_ptr<FiniteElement<dim>>(std::make_shared<FE_Q<dim>>(parameters.stokes_velocity_degree - 1)),
                               1,
                               1));

    variables.insert(variables.begin() + 3,
                     VariableDeclaration<dim>("fluid velocity", std::make_shared<FE_Q<dim>>(parameters.stokes_velocity_degree), dim, 1));
  }
  ...
  template <int dim>
  void
  MeltHandler<dim>::initialize_stokes_dof_info(Introspection<dim> &introspection, const FiniteElement<dim> &finite_element) const
  {
    const unsigned int fluid_pressure_component      = introspection.variable("fluid pressure").first_component_index;
    const unsigned int compaction_pressure_component = introspection.variable("compaction pressure").first_component_index;
    ...
    for (unsigned int i = 0; i < finite_element.dofs_per_cell; ++i)
      {
        ...
        if (Assemblers::is_velocity_or_pressures(introspection, compaction_pressure_component, fluid_pressure_component, component_index))
  ...
  template <int dim>
  void
  MeltHandler<dim>::set_assemblers(Assemblers::Manager<dim> &assemblers) const
  {
    ...
    AssertThrow((this->get_parameters().formulation_mass_conservation ==
                 Parameters<dim>::Formulation::MassConservation::isentropic_compression) ||
                  (this->get_parameters().formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::incompressible),
    ...
    assemblers.stokes_system_on_boundary_face.push_back(std::make_unique<aspect::Assemblers::MeltStokesSystemBoundary<dim>>());
    ...
    if (!this->get_boundary_traction_manager().get_prescribed_boundary_traction_indicators().empty())
      {
        assemblers.stokes_system_on_boundary_face.push_back(std::make_unique<Assemblers::MeltBoundaryTraction<dim>>());
      }

    // add the terms necessary to normalize the pressure
    if (this->pressure_rhs_needs_compatibility_modification())
      {
        assemblers.stokes_system.push_back(std::make_unique<Assemblers::MeltPressureRHSCompatibilityModification<dim>>());
      }
    ...
    for (unsigned int i = 0; i < 1 + this->introspection().n_compositional_fields; ++i)
      {
        if ((i == 0 && this->get_parameters().temperature_method == Parameters<dim>::AdvectionFieldMethod::fem_field) ||
          ...
          assemblers.advection_system[i].push_back(std::make_unique<Assemblers::MeltAdvectionSystem<dim>>());
  ...
  namespace Melt
  {
    ...
    Parameters<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("Melt scaling factor threshold",
                          "1e-7",
                          Patterns::Double(),
        ...
        prm.declare_entry("Heat advection by melt",
                          "false",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Use discontinuous compaction pressure",
                          "true",
                          Patterns::Bool(),
        ...
        prm.declare_entry("Average melt velocity",
                          "true",
                          Patterns::Bool(),
  ...
  MeltHandler<dim>::initialize() const
  {
    ...
    for (unsigned int c = 0; c < this->introspection().n_compositional_fields; ++c)
      {
        const typename Parameters<dim>::AdvectionFieldMethod::Kind method = this->get_parameters().compositional_field_methods[c];

        if (this->get_parameters().use_discontinuous_composition_discretization[c])
          AssertThrow(method != Parameters<dim>::AdvectionFieldMethod::fem_melt_field && !this->is_porosity(AdvectionField::composition(c)),
  ...
  template <int dim>
  void
  MeltHandler<dim>::create_material_model_outputs(MaterialModel::MaterialModelOutputs<dim> &output)
  {
    ...
    output.additional_outputs.push_back(std::make_unique<MaterialModel::MeltOutputs<dim>>(output.n_evaluation_points(), n_comp));
  }


  template <int dim>
  void
  MeltHandler<dim>::apply_free_surface_stabilization_with_melt(const double                                          free_surface_theta,
                                                               const typename DoFHandler<dim>::active_cell_iterator &cell,
                                                               internal::Assembly::Scratch::StokesSystem<dim>       &scratch,
                                                               internal::Assembly::CopyData::StokesSystem<dim>      &data) const
  {
    const std::set<types::boundary_id> tmp_boundary_indicators_requiring_stabilization =
      this->get_mesh_deformation_handler().get_boundary_indicators_requiring_stabilization();
            ...
            for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
              {
                for (unsigned int i = 0, i_stokes = 0; i_stokes < stokes_dofs_per_cell; /*increment at end of loop*/)
                  {
                    ...
                    if (Assemblers::is_velocity_or_pressures(
                          this->introspection(), p_c_component_index, p_f_component_index, component_index_i))
                      {
                        scratch.phi_u[i_stokes] =
                          scratch.face_finite_element_values[this->introspection().extractors.velocities].value(i, q_point);
                        ++i_stokes;
                      }
                    ++i;
                  }
                ...
                const Tensor<1, dim> gravity =
                  this->get_gravity_model().gravity_vector(scratch.face_finite_element_values.quadrature_point(q_point));
                ...
                const double pressure_perturbation =
                  scratch.face_material_model_outputs.densities[q_point] * this->get_timestep() * free_surface_theta * g_norm;
                  ...
                  for (unsigned int j = 0; j < stokes_dofs_per_cell; ++j)
                    {
                      ...
                      const double stress_value = -pressure_perturbation * phi_u_times_g_hat[i] * phi_u_times_n_hat[j] * JxW;
...
\
  template class MeltHandler<dim>; \
\
  namespace MaterialModel \
  { \
    template class MeltInputs<dim>; \
  } \
\
  namespace Melt \
  { \
