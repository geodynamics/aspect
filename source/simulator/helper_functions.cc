...
namespace aspect
{
  ...
  Simulator<dim>::output_statistics()
  {
    ...
    output_statistics_thread = std::thread([statistics_copy_ptr = std::make_unique<TableHandler>(statistics), this]() {
      // First write everything into a string in memory
      std::ostringstream stream;
      statistics_copy_ptr->write_text(stream, TableHandler::table_with_separate_column_description);
      stream.flush();
      ...
      const bool write_everything = ( // We may have never written anything. More precisely, this
        ...
        (statistics_last_hash != std::hash<std::string>()(statistics_contents.substr(0, statistics_last_write_size))));
  ...
  template <int dim>
  double
  Simulator<dim>::compute_pressure_scaling_factor() const
  {
  ...
  template <int dim>
  double
  Simulator<dim>::get_maximal_velocity(const LinearAlgebra::BlockVector &solution) const
  {
    // use a quadrature formula that has one point at
    // the location of each degree of freedom in the
    // velocity element
    const QIterated<dim> quadrature_formula(QTrapezoid<1>(), parameters.stokes_velocity_degree);
    const unsigned int   n_q_points = quadrature_formula.size();
        ...
        if (parameters.timing_output_frequency == 0)
          {
            ...
            pcout << "-- Total wallclock time elapsed including restarts: "
                  << std::round(wall_timer.wall_time() + total_walltime_until_last_snapshot) << 's' << std::endl;
          }
  ...
  Simulator<dim>::exchange_refinement_flags()
  {
    ...
    auto pack = [](const typename DoFHandler<dim>::active_cell_iterator &cell) -> std::uint8_t {
    ...
    auto unpack = [](const typename DoFHandler<dim>::active_cell_iterator &cell, const std::uint8_t &flag) -> void {
  ...
  template <int dim>
  void
  Simulator<dim>::maybe_refine_mesh(const double new_time_step, unsigned int &max_refinement_level)
  {
    /*
     ...
     */
    if ((parameters.additional_refinement_times.size() > 0) && (parameters.additional_refinement_times.front() < time + new_time_step))
      {
        ...
        while ((parameters.additional_refinement_times.size() > 0) &&
               (parameters.additional_refinement_times.front() < time + new_time_step))
          {
            ...
            parameters.additional_refinement_times.erase(parameters.additional_refinement_times.begin());
          }
      }
    // see if this is a time step where regular refinement is requested
    else if ((timestep_number > 0 && parameters.adaptive_refinement_interval > 0 &&
  ...
  Simulator<dim>::maybe_write_timing_output() const
  {
    ...
    else if ((timestep_number > 0) && (timestep_number % parameters.timing_output_frequency == 0))
      write_timing_output = true;
  ...
  template <int dim>
  bool
  Simulator<dim>::maybe_write_checkpoint(const std::time_t last_checkpoint_time, const bool force_writing_checkpoint)
  {
    ...
    if (!write_regular_checkpoint && parameters.checkpoint_time_secs > 0)
      {
        const bool global_do_checkpoint =
          Utilities::MPI::broadcast(mpi_communicator,
                                    (std::time(nullptr) - last_checkpoint_time) >= parameters.checkpoint_time_secs,
                                    /* root= */ 0);
    ...
    if (!write_regular_checkpoint && (parameters.checkpoint_time_secs == 0) && (parameters.checkpoint_steps > 0) &&
    ...
    if ((parameters.additional_checkpoint_times.size() > 0) && (parameters.additional_checkpoint_times.front() < time))
      {
        ...
        while ((parameters.additional_checkpoint_times.size() > 0) && (parameters.additional_checkpoint_times.front() < time))
          parameters.additional_checkpoint_times.erase(parameters.additional_checkpoint_times.begin());
  ...
  Simulator<dim>::get_extrapolated_advection_field_range(const AdvectionField &advection_field) const
  {
    const QIterated<dim> quadrature_formula(QTrapezoid<1>(), advection_field.polynomial_degree(introspection));
    ...
    // latest.
    double min_local_field = std::numeric_limits<double>::max(), max_local_field = std::numeric_limits<double>::lowest();
    ...
    if (timestep_number > 1)
      {
        for (const auto &cell : dof_handler.active_cell_iterators())
          if (cell->is_locally_owned())
            {
              fe_values.reinit(cell);
              fe_values[field].get_function_values(old_solution, old_field_values);
              fe_values[field].get_function_values(old_old_solution, old_old_field_values);

              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  const double extrapolated_field =
                    (1. + time_step / old_time_step) * old_field_values[q] - time_step / old_time_step * old_old_field_values[q];

                  min_local_field = std::min(min_local_field, extrapolated_field);
                  max_local_field = std::max(max_local_field, extrapolated_field);
                }
            }
      }
    else
    ...
    return std::make_pair(Utilities::MPI::min(min_local_field, mpi_communicator), Utilities::MPI::max(max_local_field, mpi_communicator));
  }


  template <int dim>
  void
  Simulator<dim>::interpolate_onto_velocity_system(const TensorFunction<1, dim> &func, LinearAlgebra::Vector &vec) const
  {
    Assert(introspection.block_indices.velocities == 0, ExcNotImplemented());

    const std::vector<Point<dim>> mesh_support_points =
      finite_element.base_element(introspection.base_elements.velocities).get_unit_support_points();
    ...
    if (parameters.use_direct_stokes_solver == false)
      Assert(introspection.block_indices.pressure == 1, ExcNotImplemented());

    IndexSet stokes_dofs(dof_handler.n_dofs());
    stokes_dofs.add_range(0, vec.size());
    const AffineConstraints<double> stokes_hanging_node_constraints = hanging_node_constraints.get_view(stokes_dofs);
  ...
  Simulator<dim>::normalize_pressure(LinearAlgebra::BlockVector &vector) const
  {
    ...
    const FEValuesExtractors::Scalar &extractor_pressure =
      (parameters.include_melt_transport ? introspection.variable("fluid pressure").extractor_scalar() : introspection.extractors.pressure);
    ...
    if (parameters.pressure_normalization == "surface")
      {
        ...
        FEFaceValues<dim> fe_face_values(*mapping, finite_element, quadrature, update_JxW_values | update_values);
                  ...
                  if (face->at_boundary() && face->boundary_id() == top_boundary_id)
                    {
                      ...
                      fe_face_values[extractor_pressure].get_function_values(vector, pressure_values);

                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          const double JxW = fe_face_values.JxW(q);
                          my_pressure += pressure_values[q] * JxW;
                          my_area += JxW;
                        }
                    }
                }
    ...
    else if (parameters.pressure_normalization == "volume")
      {
        ...
        FEValues<dim> fe_values(*mapping, finite_element, quadrature, update_JxW_values | update_values);
        ...
        for (const auto &cell : dof_handler.active_cell_iterators())
          if (cell->is_locally_owned())
            {
              fe_values.reinit(cell);
              fe_values[extractor_pressure].get_function_values(vector, pressure_values);
      ...
      AssertThrow(false, ExcMessage("Invalid pressure normalization method: " + parameters.pressure_normalization));
    ...
    LinearAlgebra::BlockVector distributed_vector(introspection.index_sets.system_partitioning, mpi_communicator);
    distributed_vector = vector;

    if (parameters.use_locally_conservative_discretization == false)
      {
        if (introspection.block_indices.velocities != introspection.block_indices.pressure && !parameters.include_melt_transport)
        ...
        else
          {
            ...
            const unsigned int pressure_component =
              (parameters.include_melt_transport ? introspection.variable("fluid pressure").first_component_index :
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (unsigned int j = 0; j < n_local_pressure_dofs; ++j)
                    {
                      unsigned int support_point_index = finite_element.component_to_system_index(pressure_component,
                                                                                                  /*dof index within component=*/j);
                      ...
                      distributed_vector(local_dof_indices[support_point_index]) =
                        vector(local_dof_indices[support_point_index]) + pressure_adjustment;
                    }
                }
          ...
          if (cell->is_locally_owned())
            {
              ...
              cell->get_dof_indices(local_dof_indices);
              const unsigned int first_pressure_dof = finite_element.component_to_system_index(pressure_component, 0);

              // make sure that this DoF is really owned by the current processor
              // and that it is in fact a pressure dof
              Assert(dof_handler.locally_owned_dofs().is_element(local_dof_indices[first_pressure_dof]), ExcInternalError());

              // then adjust its value
              distributed_vector(local_dof_indices[first_pressure_dof]) =
  ...
  template <int dim>
  void
  Simulator<dim>::denormalize_pressure(const double pressure_adjustment, LinearAlgebra::BlockVector &vector) const
  {
    if (parameters.pressure_normalization == "no")
      return;
    ...
    if (parameters.use_locally_conservative_discretization == false)
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  for (unsigned int j = 0; j < n_local_pressure_dofs; ++j)
                    {
                      const unsigned int local_dof_index = finite_element.component_to_system_index(pressure_component,
                                                                                                    /*dof index within component=*/j);
                      ...
                      if (vector.has_ghost_elements() || dof_handler.locally_owned_dofs().is_element(local_dof_indices[local_dof_index]))
    ...
    else
      {
        ...
        const unsigned int pressure_block_index =
          parameters.include_melt_transport ? introspection.variable("fluid pressure").block_index : introspection.block_indices.pressure;
        vector_backup.block(pressure_block_index) = vector.block(pressure_block_index);
          ...
          if (cell->is_locally_owned())
            {
              ...
              // then adjust its value
              vector(local_dof_indices[first_pressure_dof]) = vector_backup(local_dof_indices[first_pressure_dof]) - pressure_adjustment;
            }
  ...
  Simulator<dim>::make_pressure_rhs_compatible(LinearAlgebra::BlockVector &vector)
  {
    ...
    if ((!parameters.use_locally_conservative_discretization) && (!parameters.include_melt_transport) &&
        (introspection.block_indices.velocities != introspection.block_indices.pressure))
      {
        ...
        vector.block(introspection.block_indices.pressure)
          .add(correction, pressure_shape_function_integrals.block(introspection.block_indices.pressure));
      }
    else if (!parameters.use_locally_conservative_discretization)
      {
        ...
        const IndexSet &idxset  = parameters.include_melt_transport ? introspection.index_sets.locally_owned_fluid_pressure_dofs :
    ...
    else
      {
        ...
        AssertThrow(parameters.use_locally_conservative_discretization, ExcInternalError());
    ...
    if (pressure_block_index == introspection.block_indices.velocities)
      {
        ...
        return system_matrix.block(0, 0).residual(residual.block(0), linearized_stokes_variables.block(0), system_rhs.block(0));
      }
    else
      {
        const double residual_u =
          system_matrix.block(0, 1).residual(residual.block(0), linearized_stokes_variables.block(1), system_rhs.block(0));
  ...
  Simulator<dim>::apply_limiter_to_dg_solutions(const AdvectionField &advection_field)
  {
    ...
    if (!advection_field.is_temperature() && parameters.volume_of_fluid_tracking_enabled)
      if (volume_of_fluid_handler->field_index_for_name(
            introspection.name_for_compositional_index(advection_field.compositional_variable)) != volume_of_fluid_handler->get_n_fields())
        return;
    ...
    const Quadrature<dim> &quadrature_formula_0 =
      (advection_field.is_temperature() ? introspection.quadratures.temperature :
    ...
    FEValues<dim>       fe_values(*mapping, finite_element, quadrature_formula, update_values | update_quadrature_points);
    ...
    FEValues<dim> fe_values_0(*mapping, finite_element, quadrature_formula_0, update_values | update_quadrature_points | update_JxW_values);
    std::vector<double> values_0(n_q_points_0);

    const FEValuesExtractors::Scalar field =
      (advection_field.is_temperature() ? introspection.extractors.temperature :
                                          introspection.extractors.compositional_fields[advection_field.compositional_variable]);

    const double max_solution_exact_global =
      (advection_field.is_temperature() ? parameters.global_temperature_max_preset :
                                          parameters.global_composition_max_preset[advection_field.compositional_variable]);
    const double min_solution_exact_global =
      (advection_field.is_temperature() ? parameters.global_temperature_min_preset :
      ...
      if (cell->is_locally_owned())
        {
          ...
          if (min_solution_local < min_solution_exact_global || max_solution_local > max_solution_exact_global)
            {
              ...
              if (std::abs(max_solution_local - local_solution_average) > std::numeric_limits<double>::min())
                {
                  theta = std::min(theta,
                                   std::abs((max_solution_exact_global - local_solution_average) /
              ...
              if (std::abs(min_solution_local - local_solution_average) > std::numeric_limits<double>::min())
                {
                  theta = std::min(theta,
                                   std::abs((min_solution_exact_global - local_solution_average) /
               ...
               */
              for (unsigned int j = 0; j < finite_element.base_element(advection_field.base_element(introspection)).dofs_per_cell; ++j)
                {
                                                          ...
                                                          std::vector<std::vector<unsigned int>> &support_point_index_by_field) const
  {
    ...
    if (n_fields > 0)
      {
        const unsigned int likely_number_of_support_points =
          dof_handler.get_fe().base_element(advection_fields[0].base_element(introspection)).get_unit_support_points().size();
    ...
    for (unsigned int i = 0; i < advection_fields.size(); ++i)
      {
        const std::vector<Point<dim>> &support_points =
          dof_handler.get_fe().base_element(advection_fields[i].base_element(introspection)).get_unit_support_points();
  ...
  Simulator<dim>::compute_reactions()
  {
    ...
    LinearAlgebra::BlockVector distributed_reaction_vector(introspection.index_sets.system_partitioning, mpi_communicator);
    ...
    if (parameters.reaction_solver_type == Parameters<dim>::ReactionSolverType::fixed_step)
      Assert(reaction_time_step_size > 0, ExcMessage("Reaction time step must be greater than 0."));
    ...
    auto copy_fields_into_one_vector = [n_q_points, n_fields](const MaterialModel::MaterialModelInputs<dim> &in, VectorType &fields) {
      ...
      if (cell->is_locally_owned())
        {
          ...
          if (parameters.reaction_solver_type == Parameters<dim>::ReactionSolverType::ARKode)
            {
              ode.explicit_function = [&](const double /*time*/, const VectorType &y, VectorType &ydot) {
                      ...
                      for (unsigned int c = 0; c < introspection.n_compositional_fields; ++c)
                        {
                          ...
                          in.composition[j][c] =
                            in.composition[j][c] + reaction_time_step_size * reaction_rate_outputs->reaction_rates[j][c];
                          accumulated_reactions_C[j][c] += reaction_time_step_size * reaction_rate_outputs->reaction_rates[j][c];
                        }
                      in.temperature[j] =
                        in.temperature[j] + reaction_time_step_size * heating_model_outputs.rates_of_temperature_change[j];
    ...
    update_solution_vectors_with_reaction_results(introspection.block_indices.temperature, distributed_vector, distributed_reaction_vector);
    ...
    pcout << "in " << average_iteration_count << " substep(s)." << std::endl;
    ...
    if (timestep_number > 1)
      {
        ...
        distr_solution.sadd((1 + time_step / old_time_step), -time_step / old_time_step, distr_old_solution);
  ...
  Simulator<dim>::interpolate_material_output_into_advection_field(const std::vector<AdvectionField> &adv_fields)
  {
    ...
    else if (adv_fields.size() == 1)
      {
        ...
        if (adv_field.is_temperature())
        ...
        else
          {
            ...
            pcout << "   Copying properties into prescribed compositional field " + name_of_field + "... " << std::flush;
          }
      }
    else
      {
        pcout << "   Copying properties into prescribed compositional fields... " << std::flush;
      }
    ...
    FEValues<dim> fe_values(*mapping, dof_handler.get_fe(), quadrature, update_quadrature_points | update_values | update_gradients);
          ...
          for (unsigned int i = 0; i < n_fields; ++i)
            {
              ...
              for (unsigned int j = 0; j < advection_dofs_per_cell; ++j)
                {
                  const unsigned int dof_idx = dof_handler.get_fe().component_to_system_index(adv_field.component_index(introspection),
                                                                                              /*dof index within component=*/j);
                      ...
                      else
                        {
                          ...
                          distributed_vector(local_dof_indices[dof_idx]) =
                            prescribed_field_out
                              ->prescribed_field_outputs[support_point_index_by_field[i][j]][adv_field.compositional_variable];
                        }
                    }
                }
            }
  ...
  template <int dim>
  void
  Simulator<dim>::replace_outflow_boundary_ids(const unsigned int offset, const bool is_composition, const unsigned int composition_index)
  {
    ...
    if (is_composition)
      {
        ...
        if (introspection.compositional_field_methods[composition_index] == Parameters<dim>::AdvectionFieldMethod::fem_melt_field ||
            (parameters.include_melt_transport &&
             introspection.get_indices_for_fields_of_type(CompositionalFieldDescription::porosity)[0] == composition_index))
    ...
    const UpdateFlags update_flags = UpdateFlags(
      consider_darcy_velocity ? update_values | update_gradients | update_normal_vectors | update_quadrature_points | update_JxW_values :
                                update_values | update_normal_vectors | update_quadrature_points | update_JxW_values);

    FEFaceValues<dim> fe_face_values(*mapping, finite_element, quadrature_formula, update_flags);
    ...
    const auto &tangential_velocity_boundaries = boundary_velocity_manager.get_tangential_boundary_velocity_indicators();

    const auto &zero_velocity_boundaries = boundary_velocity_manager.get_zero_boundary_velocity_indicators();

    const auto &prescribed_velocity_boundaries = boundary_velocity_manager.get_prescribed_boundary_velocity_indicators();
        ...
        for (const unsigned int face_number : cell->face_indices())
          {
            ...
            if (face->at_boundary() && tangential_velocity_boundaries.find(face->boundary_id()) == tangential_velocity_boundaries.end() &&
                zero_velocity_boundaries.find(face->boundary_id()) == zero_velocity_boundaries.end())
              {
                ...
                if (consider_fluid_velocity)
                  {
                    fe_face_values[introspection.variable("fluid velocity").extractor_vector()].get_function_values(
                      current_linearization_point, face_current_fluid_velocity_values);
                  }
                ...
                for (unsigned int q = 0; q < fe_face_values.n_quadrature_points; ++q)
                  {
                    ...
                    if (prescribed_velocity_boundaries.find(face->boundary_id()) == prescribed_velocity_boundaries.end())
                      ...
                      boundary_velocity =
                        boundary_velocity_manager.boundary_velocity(face->boundary_id(), fe_face_values.quadrature_point(q));
                    ...
                    if (consider_darcy_velocity)
                      {
                        ...
                        integrated_flow += (boundary_darcy_velocity * fe_face_values.normal_vector(q)) * fe_face_values.JxW(q);
                      }

                    else if (consider_fluid_velocity)
                      {
                        integrated_flow +=
                          (face_current_fluid_velocity_values[q] * fe_face_values.normal_vector(q)) * fe_face_values.JxW(q);
                      }

                    else
                      {
                        integrated_flow += (boundary_velocity * fe_face_values.normal_vector(q)) * fe_face_values.JxW(q);
                      }
                  }
  ...
  namespace
  {
    ...
    bool
    is_element(const typename Container::value_type &t, const Container &container)
    {
  ...
  Simulator<dim>::check_consistency_of_boundary_conditions() const
  {
            ...
            if (velocity_boundary_id == traction_boundary_id)
              {
                ...
                AssertThrow((boundary_velocity_manager.get_component_mask(velocity_boundary_id) &
                            ...
                            ExcMessage("Boundary indicator <" + Utilities::int_to_string(velocity_boundary_id) + "> with symbolic name <" +
      ...
      for (unsigned int j = i + 1; j < boundary_indicator_lists.size(); ++j)
        {
          ...
          AssertThrow(intersection.empty(),
                      ExcMessage("Boundary indicator <" + Utilities::int_to_string(*intersection.begin()) + "> with symbolic name <" +
    ...
    AssertThrow(T_intersection.empty(),
                ExcMessage("Boundary indicator <" + Utilities::int_to_string(*T_intersection.begin()) + "> with symbolic name <" +
    ...
    AssertThrow(Tc_intersection.empty(),
                ExcMessage("Boundary indicator <" + Utilities::int_to_string(*Tc_intersection.begin()) + "> with symbolic name <" +
    ...
    AssertThrow(Tflux_intersection.empty(),
                ExcMessage("Boundary indicator <" + Utilities::int_to_string(*Tflux_intersection.begin()) + "> with symbolic name <" +
      ...
      for (const auto &boundary_indicators : boundary_indicator_lists)
        {
          AssertThrow(is_element(pb.first.first, boundary_indicators) == false,
                      ExcMessage("Boundary indicator <" + Utilities::int_to_string(pb.first.first) + "> with symbolic name <" +
          ...
          AssertThrow(is_element(pb.first.second, boundary_indicators) == false,
                      ExcMessage("Boundary indicator <" + Utilities::int_to_string(pb.first.second) + "> with symbolic name <" +
    ...
    const std::set<types::boundary_id> all_boundary_indicators = geometry_model->get_used_boundary_indicators();

    // next make sure that all listed indicators are actually used by
    // this geometry
    for (const auto &list : boundary_indicator_lists)
      for (const auto &p : list)
        AssertThrow(all_boundary_indicators.find(p) != all_boundary_indicators.end(),
  ...
  Simulator<dim>::compute_initial_newton_residual()
  {
    ...
    LinearAlgebra::Vector temp_velocity_linearization_point(
      ...
      mpi_communicator);
    temp_velocity_linearization_point.swap(current_linearization_point.block(introspection.block_indices.velocities));
    ...
    const double initial_newton_residual =
      std::sqrt(initial_newton_residual_vel * initial_newton_residual_vel + initial_newton_residual_p * initial_newton_residual_p);

    // Swap old content back in:
    current_linearization_point.block(introspection.block_indices.velocities).swap(temp_velocity_linearization_point);

    pcout << "   Initial Newton Stokes residual = " << initial_newton_residual << ", v = " << initial_newton_residual_vel
          << ", p = " << initial_newton_residual_p << std::endl
          << std::endl;
                                      ...
                                      const LinearAlgebra::BlockVector &search_direction)
  {
    ...
    LinearAlgebra::BlockVector current_iterate(introspection.index_sets.system_partitioning, mpi_communicator);
    ...
    do
      {
        ...
        const double test_residual =
          std::sqrt(test_velocity_residual * test_velocity_residual + test_pressure_residual * test_pressure_residual);

        // Determine if the residual has decreased sufficiently.
        const double alpha = 1e-4;
        if (test_residual < (1.0 - alpha * step_length_factor) * dcr.residual ||
            line_search_iteration >= newton_handler->parameters.max_newton_line_search_iterations || use_picard)
          {
          ...
          {
            pcout << "   Line search iteration " << line_search_iteration << ", with norm of the rhs " << test_residual << " and going to "
                  << (1.0 - alpha * step_length_factor) * dcr.residual << ", relative residual: " << test_residual / dcr.initial_residual
                  << std::endl;
        ...
        Assert(line_search_iteration <= newton_handler->parameters.max_newton_line_search_iterations, ExcInternalError());
      }
    while (true);
  }
    ...
    if (EisenstatWalkerChoiceOne)
      {
        ...
        if (std::pow(linear_stokes_solver_tolerance, powerterm) <= 0.1)
          {
            new_linear_stokes_solver_tolerance =
              std::min(maximum_linear_stokes_solver_tolerance, std::fabs(newton_residual - stokes_residual) / (newton_residual_old));
          }
        else
          {
    ...
    else
      {
        if (0.9 * linear_stokes_solver_tolerance * linear_stokes_solver_tolerance <= 0.1)
          {
            new_linear_stokes_solver_tolerance =
              std::min(maximum_linear_stokes_solver_tolerance,
                       0.9 * std::fabs(newton_residual * newton_residual) / (newton_residual_old * newton_residual_old));
          }
        else
          {
            new_linear_stokes_solver_tolerance =
              std::min(newton_handler->parameters.maximum_linear_stokes_solver_tolerance,
    ...
    if (parameters.stokes_solver_type == Parameters<dim>::StokesSolverType::default_solver)
      {
        ...
        if (parameters.include_melt_transport == true ||
            ...
            (material_model->is_compressible() == true &&
             parameters.formulation_mass_conservation ==
               Parameters<dim>::Formulation::MassConservation::implicit_reference_density_profile) ||
...
#define INSTANTIATE(dim) \
  ...
  template void   Simulator<dim>::denormalize_pressure(const double pressure_adjustment, LinearAlgebra::BlockVector &vector) const; \
  ...
  template void Simulator<dim>::compute_unique_advection_support_points( \
