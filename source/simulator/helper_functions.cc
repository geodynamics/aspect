...
namespace aspect
{
  ...
  {
    // Vector for current solution (x) with zero velocity
    LinearAlgebra::BlockVector zero_velocity_solution (introspection.index_sets.stokes_partitioning, mpi_communicator);
    // Vector to store residual
    ...
    if (block_p == introspection.block_indices.velocities)
      {
        ...
        for (unsigned int i=0; i < idxset.n_elements(); ++i)
          {
            types::global_dof_index idx = idxset.nth_index_in_set(i);
            zero_velocity_solution(idx)        = current_linearization_point(idx);
          }
        zero_velocity_solution.block(block_p).compress(VectorOperation::insert);
      }
    else
      zero_velocity_solution.block (block_p) = current_linearization_point.block (block_p);
    ...
    ghosted.block(block_p) = zero_velocity_solution.block(block_p);
    denormalize_pressure (this->pressure_adjustment, zero_velocity_solution, ghosted);
    current_constraints.set_zero (zero_velocity_solution);

    zero_velocity_solution.block (block_p) /= pressure_scaling;
