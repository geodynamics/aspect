...
namespace aspect
{
  ...
  Simulator<dim>::solve_advection(const AdvectionField &advection_field)
  {
    ...
    const double advection_solver_tolerance =
      (advection_field.is_temperature()) ? (parameters.temperature_solver_tolerance) : (parameters.composition_solver_tolerance);

    const double tolerance = std::max(1e-50, advection_solver_tolerance * system_rhs.block(block_idx).l2_norm());
    ...
    SolverGMRES<LinearAlgebra::Vector> solver(solver_control,
                                              SolverGMRES<LinearAlgebra::Vector>::AdditionalData(parameters.advection_gmres_restart_length,
                                                                                                 true));
    ...
    if (system_rhs.block(block_idx).l2_norm() <= std::numeric_limits<double>::min())
      {
        ...
        signals.post_advection_solver(*this, advection_field.is_temperature(), advection_field.compositional_variable, solver_control);

        return 0;
      }

    AssertThrow(system_matrix.block(block_idx, block_idx).linfty_norm() > std::numeric_limits<double>::min(),
                ExcMessage("The " + field_name +
                           " equation can not be solved, because the matrix is zero, "
    ...
    computing_timer.enter_subsection(advection_field.is_temperature() ? "Solve temperature system" : "Solve composition system");
    ...
    if (advection_field.is_temperature())
      ...
      {
        pcout << "   Solving " << introspection.name_for_compositional_index(advection_field.compositional_variable) << " system "
              << "... " << std::flush;
      }
    ...
    LinearAlgebra::Vector temp(introspection.index_sets.system_partitioning[block_idx], mpi_communicator);
    ...
    const double initial_residual =
      system_matrix.block(block_idx, block_idx).residual(temp, distributed_solution.block(block_idx), system_rhs.block(block_idx));
    ...
    if ((advection_field.is_discontinuous(introspection) &&
           ...
           parameters.use_limiter_for_discontinuous_composition_solution[advection_field.compositional_variable]))))
      {
        ...
        computing_timer.leave_subsection(advection_field.is_temperature() ? "Solve temperature system" : "Solve composition system");
  ...
  Simulator<dim>::solve_stokes(LinearAlgebra::BlockVector &solution_vector)
  {
    ...
    outputs = stokes_solver->solve(
      system_matrix, system_rhs, assemble_newton_stokes_system, last_pressure_normalization_adjustment, solution_vector);
    ...
    return {outputs.initial_nonlinear_residual, outputs.final_linear_residual};
  }
