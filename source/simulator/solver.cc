...
namespace aspect
{
  ...
  double Simulator<dim>::solve_stokes ()
  {
    ...
    // pressure = 1). For example the linearization_solution vector or the
    // StokesBlock matrix wrapper. Let us make sure that this holds (and
    // shorten their names):
    ...
    LinearAlgebra::BlockVector linearization_solution (introspection.index_sets.stokes_partitioning, mpi_communicator);

    // copy the velocity and pressure from current_linearization_point into the
    // vector linearization_solution. We need to do the copy because
    // linearization_solution has a different layout than
    // current_linearization_point, which also contains all the other solution
    // variables.
    linearization_solution.block (block_vel) = current_linearization_point.block (block_vel);

    linearization_solution.block (block_p) = current_linearization_point.block (block_p);
    denormalize_pressure (this->pressure_adjustment, linearization_solution, current_linearization_point);

    current_constraints.set_zero (linearization_solution);
    linearization_solution.block (block_p) /= pressure_scaling;
    ...
    const double residual_u = system_matrix.block(0,1).residual (distributed_stokes_solution.block(0),
                                                                 linearization_solution.block(1),
                                                                 system_rhs.block(0));
    ...
    distributed_stokes_solution = linearization_solution;
