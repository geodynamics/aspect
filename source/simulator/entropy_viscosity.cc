...
namespace aspect
{
  ...
  template <int dim>
  double
  Simulator<dim>::get_entropy_variation(const double average_field, const AdvectionField &advection_field) const
  {
    ...
    FEValues<dim>       fe_values(finite_element, quadrature_formula, update_values | update_JxW_values);
    std::vector<double> old_field_values(n_q_points);
    std::vector<double> old_old_field_values(n_q_points);

    double min_entropy = std::numeric_limits<double>::max(), max_entropy = std::numeric_limits<double>::lowest(), area = 0,
      ...
      if (cell->is_locally_owned())
        {
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              const double field_value = (old_field_values[q] + old_old_field_values[q]) / 2;
              const double entropy     = ((field_value - average_field) * (field_value - average_field));
    ...
    const double local_for_sum[2] = {entropy_integrated, area}, local_for_max[2] = {-min_entropy, max_entropy};
    ...
    // average value
    return std::max(global_for_max[1] - average_entropy, average_entropy - (-global_for_max[0]));
  }
  ...
  template <int dim>
  double
  Simulator<dim>::compute_viscosity(internal::Assembly::Scratch::AdvectionSystem<dim> &scratch,
                                    ...
                                    const AdvectionField                              &advection_field) const
  {
    ...
    for (unsigned int i = 0; i < assemblers->advection_system[advection_field.field_index()].size(); ++i)
      {
        ...
        auto *stabilization_assembler = dynamic_cast<Assemblers::AdvectionStabilizationInterface<dim> *>(
          (assemblers->advection_system[advection_field.field_index()][i]).get());
    ...
    const bool                  use_darcy_velocity =
      (advection_field.advection_method(introspection) == Parameters<dim>::AdvectionFieldMethod::fem_darcy_field);
    ...
    if (use_darcy_velocity)
      {
        ...
        AssertThrow(melt_outputs != nullptr, ExcMessage("Darcy field entropy viscosity requires MeltOutputs to be available."));
      }
    ...
    for (unsigned int q = 0; q < scratch.finite_element_values.n_quadrature_points; ++q)
      {
        const Tensor<1, dim> velocity      = (scratch.old_velocity_values[q] + scratch.old_old_velocity_values[q]) / 2;
        ...
        if (parameters.include_melt_transport)
          {
            const Tensor<1, dim> fluid_velocity = (old_fluid_velocity_values[q] + old_old_fluid_velocity_values[q]) / 2;
            velocity_norm                       = std::max(fluid_velocity.norm(), velocity_norm);
          }

        const double strain_rate = ((scratch.old_strain_rates[q] + scratch.old_old_strain_rates[q]) / 2).norm();
        ...
        max_velocity = std::max(velocity_norm + parameters.stabilization_gamma * strain_rate * cell_diameter, max_velocity);
      }
    ...
    if (std::abs(global_u_infty) < 1e-50)
      return parameters.stabilization_beta[advection_field.field_index()] * max_conductivity / geometry_model->length_scale() *
             cell_diameter;

    const double maximum_viscosity =
      parameters.stabilization_beta[advection_field.field_index()] * max_advection_prefactor * max_velocity * cell_diameter;

    if (timestep_number <= 1 || std::abs(global_entropy_variation) < 1e-50 || std::abs(global_field_variation) < 1e-50)
    ...
    else
      {
        ...
        if (parameters.stabilization_alpha == 2)
          entropy_viscosity = (parameters.stabilization_c_R[advection_field.field_index()] * cell_diameter * cell_diameter * max_residual /
                               global_entropy_variation);
        else
          entropy_viscosity = (parameters.stabilization_c_R[advection_field.field_index()] * cell_diameter * global_Omega_diameter *
                               max_velocity * max_residual / (global_u_infty * global_field_variation));
  ...
  template <typename T>
  void
  Simulator<dim>::get_artificial_viscosity(Vector<T>            &viscosity_per_cell,
                                           ...
                                           const bool            skip_interior_cells) const
  {
    ...
    if (advection_field.is_temperature())
    ...
    else
      {
        ...
        if (std::find(parameters.compositional_fields_with_disabled_boundary_entropy_viscosity.begin(),
                      ...
                      field_name) != parameters.compositional_fields_with_disabled_boundary_entropy_viscosity.end())
          skip_EV_dirichlet_boundary_cells = true;
      }

    const std::pair<double, double> global_field_range = get_extrapolated_advection_field_range(advection_field);
    const double                    global_entropy_variation =
      get_entropy_variation((global_field_range.first + global_field_range.second) / 2, advection_field);
    ...
    const UpdateFlags face_update_flags = update_values | update_quadrature_points | update_normal_vectors | update_JxW_values;

    internal::Assembly::Scratch::AdvectionSystem<dim> scratch(finite_element,
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
      {
        ...
        if (cell->is_artificial() || (cell->is_ghost() && parameters.use_artificial_viscosity_smoothing == false))
          {
        ...
        // the smoothing uses both the boundary cell and its neighbor.
        else if (skip_interior_cells && !cell->at_boundary())
          {
            ...
            if (parameters.use_artificial_viscosity_smoothing == false || neighbor_at_boundary == false)
              {
                viscosity_per_cell[cell->active_cell_index()] = numbers::signaling_nan<T>();
                continue;
              }
        ...
        if (parameters.advection_stabilization_method == Parameters<dim>::AdvectionStabilizationMethod::entropy_viscosity &&
            skip_EV_dirichlet_boundary_cells == true)
          {
            const std::set<types::boundary_id> &dirichlet_boundaries =
              (advection_field.is_temperature() == true) ? boundary_temperature_manager.get_fixed_temperature_boundary_indicators() :
              ...
              if (cell->at_boundary(face_no) == true &&
                  dirichlet_boundaries.find(cell->face(face_no)->boundary_id()) != dirichlet_boundaries.end())
                {
                  ...
                  (*scratch.face_finite_element_values)[introspection.extractors.velocities].get_function_values(
                    old_old_solution, face_old_old_velocity_values);
        ...
        scratch.finite_element_values[introspection.extractors.pressure].get_function_values(old_old_solution, scratch.old_old_pressure);

        for (unsigned int c = 0; c < introspection.n_compositional_fields; ++c)
          {
            scratch.finite_element_values[introspection.extractors.compositional_fields[c]].get_function_values(
              old_solution, scratch.old_composition_values[c]);
            scratch.finite_element_values[introspection.extractors.compositional_fields[c]].get_function_values(
              old_old_solution, scratch.old_old_composition_values[c]);
          }

        scratch.finite_element_values[introspection.extractors.velocities].get_function_values(old_solution, scratch.old_velocity_values);
        ...
        scratch.old_field_values =
          (advection_field.is_temperature() ? scratch.old_temperature_values :
                                              scratch.old_composition_values[advection_field.compositional_variable]);
        scratch.old_old_field_values =
          (advection_field.is_temperature() ? scratch.old_old_temperature_values :
                                              scratch.old_old_composition_values[advection_field.compositional_variable]);

        scratch.finite_element_values[solution_field].get_function_gradients(old_solution, scratch.old_field_grads);
        scratch.finite_element_values[solution_field].get_function_gradients(old_old_solution, scratch.old_old_field_grads);

        if (update_flags & update_hessians)
          {
            scratch.finite_element_values[solution_field].get_function_laplacians(old_solution, scratch.old_field_laplacians);
            scratch.finite_element_values[solution_field].get_function_laplacians(old_old_solution, scratch.old_old_field_laplacians);
          }

        if (parameters.include_melt_transport && melt_handler->is_porosity(advection_field))
          {
            scratch.finite_element_values[introspection.extractors.velocities].get_function_divergences(
              current_linearization_point, scratch.current_velocity_divergences);
          }
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
          {
            ...
            scratch.material_model_inputs.pressure_gradient[q] =
              (scratch.old_pressure_gradients[q] + scratch.old_old_pressure_gradients[q]) / 2;

            for (unsigned int c = 0; c < introspection.n_compositional_fields; ++c)
              scratch.material_model_inputs.composition[q][c] =
                (scratch.old_composition_values[c][q] + scratch.old_old_composition_values[c][q]) / 2;
        ...
        for (unsigned int i = 0; i < assemblers->advection_system[advection_field.field_index()].size(); ++i)
          assemblers->advection_system[advection_field.field_index()][i]->create_additional_material_model_outputs(
            scratch.material_model_outputs);
        ...
        else if (parameters.formulation_temperature_equation == Parameters<dim>::Formulation::TemperatureEquation::real_density)
          {
            // use real density
          }
        else
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  if (advection_field.is_temperature())
                    ...
                    {
                      norm_of_advection_term = std::max(scratch.current_velocity_values[q].norm(), norm_of_advection_term);
                ...
                if (cell->at_boundary(face_no) == false)
                  {
                    if (cell->neighbor(face_no)->is_active())
                      viscosity_per_cell[cell->active_cell_index()] =
                        std::max(viscosity_per_cell[cell->active_cell_index()],
