  ...
  namespace internal
  {
    namespace Assembly
    {
      namespace Scratch
      {
        ...
        template <int dim>
        sys_StokesPreconditioner<dim>::
        sys_StokesPreconditioner (const sys_StokesPreconditioner &scratch)
          :
        ...
        template <int dim>
        sys_StokesSystem<dim>::
        sys_StokesSystem (const sys_StokesSystem<dim> &scratch)
          :
          sys_StokesPreconditioner<dim> (scratch),
          phi_u (scratch.phi_u),
          grads_phi_u (scratch.grads_phi_u),
        ...
        template <int dim>
        sys_TemperatureSystem<dim>::
        sys_TemperatureSystem (const sys_TemperatureSystem &scratch)
          :
          system_fe_values (scratch.system_fe_values.get_mapping(),
                            scratch.system_fe_values.get_fe(),
                            scratch.system_fe_values.get_quadrature(),
...
//                                const Quadrature<dim>    &system_quadrature,
//                                const Mapping<dim>       &mapping,
...
//                              const UpdateFlags         system_update_flags)
//          :
//          system_fe_values (mapping, system_fe, system_quadrature,
//                            system_update_flags),
//          grads_phi_u (system_fe.dofs_per_cell),
//          phi_p (system_fe.dofs_per_cell),
...
//        template <int dim>
//        sys_PreconditionerScratch<dim>::
//        sys_PreconditionerScratch (const sys_PreconditionerScratch &scratch)
//          :
//          system_fe_values (scratch.system_fe_values.get_mapping(),
//                            scratch.system_fe_values.get_fe(),
//                            scratch.system_fe_values.get_quadrature(),
...
//        {}
        ...
        // FEValues objects for those two cases.
//        template <int dim>
//        struct sys_SystemScratch : public sys_PreconditionerScratch<dim>
//        {
//          sys_SystemScratch (const FiniteElement<dim> &system_fe,
//                         const Mapping<dim>       &mapping,
//                         const Quadrature<dim>    &system_quadrature,
//                         const UpdateFlags         system_update_flags);
//
...
//
//        template <int dim>
//        sys_SystemScratch<dim>::
//        sys_SystemScratch (const FiniteElement<dim> &system_fe,
//                      const Mapping<dim>       &mapping,
//                      const Quadrature<dim>    &system_quadrature,
//                      const UpdateFlags         system_update_flags)
//          :
//          sys_SystemPreconditioner<dim> (system_fe, system_quadrature,
//                                     mapping,
//                                     system_update_flags),
//          phi_u (system_fe.dofs_per_cell),
//          div_phi_u (system_fe.dofs_per_cell),
...
//          old_strain_rates(system_quadrature.size()),
//          old_old_strain_rates(system_quadrature.size()),
//          old_old_temperature_values(system_quadrature.size()),
//          old_temperature_grads(system_quadrature.size()),
//          old_old_temperature_grads(system_quadrature.size()),
//          old_temperature_laplacians(system_quadrature.size()),
//          old_old_temperature_laplacians(system_quadrature.size())
//        {}
//
//
//        template <int dim>
//        sys_SystemScratch<dim>::
//        sys_SystemScratch (const sys_SystemScratch<dim> &scratch)
//          :
...
//        {}
      ...
      namespace CopyData
      {
        ...
        struct sys_StokesPreconditioner
        {
          sys_StokesPreconditioner (const FiniteElement<dim> &stokes_fe);
          sys_StokesPreconditioner (const sys_StokesPreconditioner &data);
        ...
        struct sys_StokesSystem : public sys_StokesPreconditioner<dim>
        {
          sys_StokesSystem (const FiniteElement<dim> &stokes_fe);
          sys_StokesSystem (const sys_StokesSystem<dim> &data);
        ...
        sys_StokesSystem (const FiniteElement<dim> &stokes_fe)
          :
          sys_StokesPreconditioner<dim> (stokes_fe),
        ...
        sys_StokesSystem (const sys_StokesSystem<dim> &data)
          :
          sys_StokesPreconditioner<dim> (data),
        ...
        struct sys_TemperatureSystem
        {
          sys_TemperatureSystem (const FiniteElement<dim> &temperature_fe);
          sys_TemperatureSystem (const sys_TemperatureSystem &data);
...
//        {
//          sys_PreconditionerCopy (const FiniteElement<dim> &system_fe);
//          sys_PreconditionerCopy (const sys_PreconditionerCopy &data);
//
//          FullMatrix<double>          local_matrix;
//          std::vector<unsigned int>   local_dof_indices;
//        };
...
//
//        template <int dim>
//        struct SystemCopy : public PreconditionerCopy<dim>
//        {
//          SystemCopy (const FiniteElement<dim> &system_fe);
//          SystemCopy (const SystemCopy<dim> &data);
//
//          Vector<double> local_rhs;
//          Vector<double> local_pressure_shape_function_integrals;
//        };
...
//        SystemCopy (const FiniteElement<dim> &system_fe)
//          :
//          PreconditionerCopy<dim> (system_fe),
...
//        {}
  ...
  template <int dim>
  double
  Simulator<dim>::sys_get_entropy_variation (const double average_temperature) const
  {
...
//TODO: make the temperature be something useful here
  ...
  template <int dim>
  void
  Simulator<dim>::sys_assemble_stokes_preconditioner ()
  {
    ...
    run (CellFilter (IteratorFilters::LocallyOwnedCell(),
         ...
         std_cxx1x::bind (&Simulator<dim>::
                          sys_local_assemble_stokes_preconditioner,
                          this,
                          std_cxx1x::_1,
                          std_cxx1x::_2,
                          std_cxx1x::_3),
         std_cxx1x::bind (&Simulator<dim>::
                          sys_copy_local_to_global_stokes_preconditioner,
                          this,
                          std_cxx1x::_1),
  ...
  template <int dim>
  void
  Simulator<dim>::sys_build_stokes_preconditioner ()
  {
    ...
    sys_assemble_stokes_preconditioner ();
  ...
  template <int dim>
  void Simulator<dim>::sys_assemble_stokes_system ()
  {
    ...
    run (CellFilter (IteratorFilters::LocallyOwnedCell(),
         ...
         CellFilter (IteratorFilters::LocallyOwnedCell(),
                     system_dof_handler.end()),
         std_cxx1x::bind (&Simulator<dim>::
                          sys_local_assemble_stokes_system,
                          this,
                          std_cxx1x::_1,
                          std_cxx1x::_2,
                          std_cxx1x::_3),
         std_cxx1x::bind (&Simulator<dim>::
                          sys_copy_local_to_global_stokes_system,
                          this,
                          std_cxx1x::_1),
         internal::Assembly::Scratch::
    ...
    for (unsigned int q=0; q<n_q_points; ++q)
      {
        ...
        const double ext_T
          = aspect::internal::sys_bdf2_extrapolate(
  ...
  template <int dim>
  void Simulator<dim>::sys_assemble_temperature_system ()
  {
    ...
    global_T_range = sys_get_extrapolated_temperature_range();
    ...
    run (CellFilter (IteratorFilters::LocallyOwnedCell(),
         ...
         CellFilter (IteratorFilters::LocallyOwnedCell(),
                     system_dof_handler.end()),
         std_cxx1x::bind (&Simulator<dim>::
                          sys_local_assemble_temperature_system,
                          this,
                          global_T_range,
                          sys_get_maximal_velocity(old_system_solution),
                          ...
                          std_cxx1x::_3),
         std_cxx1x::bind (&Simulator<dim>::
                          sys_copy_local_to_global_temperature_system,
                          this,
                          std_cxx1x::_1),
         internal::Assembly::Scratch::
         sys_TemperatureSystem<dim> (system_fe, mapping, QGauss<dim>(parameters.temperature_degree+2)),
         internal::Assembly::CopyData::
         sys_TemperatureSystem<dim> (system_fe));
...
namespace aspect
{
  template void Simulator<deal_II_dimension>::sys_local_assemble_stokes_preconditioner (
    const DoFHandler<deal_II_dimension>::active_cell_iterator &cell,
    internal::Assembly::Scratch::sys_StokesPreconditioner<deal_II_dimension> &scratch,
    internal::Assembly::CopyData::sys_StokesPreconditioner<deal_II_dimension> &data);

  template void Simulator<deal_II_dimension>::sys_copy_local_to_global_stokes_preconditioner (
    const internal::Assembly::CopyData::sys_StokesPreconditioner<deal_II_dimension> &data);

  template void Simulator<deal_II_dimension>::sys_assemble_stokes_preconditioner ();

  template void Simulator<deal_II_dimension>::sys_build_stokes_preconditioner ();

  template void Simulator<deal_II_dimension>::sys_local_assemble_stokes_system (
    const DoFHandler<deal_II_dimension>::active_cell_iterator &cell,
    internal::Assembly::Scratch::sys_StokesSystem<deal_II_dimension>  &scratch,
    internal::Assembly::CopyData::sys_StokesSystem<deal_II_dimension> &data);

  template void Simulator<deal_II_dimension>::sys_copy_local_to_global_stokes_system (
    const internal::Assembly::CopyData::sys_StokesSystem<deal_II_dimension> &data);

  template void Simulator<deal_II_dimension>::sys_assemble_stokes_system ();

  template void Simulator<deal_II_dimension>::sys_copy_local_to_global_temperature_system (
    const internal::Assembly::CopyData::sys_TemperatureSystem<deal_II_dimension> &data);

  template void Simulator<deal_II_dimension>::sys_assemble_temperature_system ();

  template void Simulator<deal_II_dimension>::sys_local_assemble_temperature_system (
    const std::pair<double,double> global_T_range,
    const double                   global_max_velocity,
    const double                   global_entropy_variation,
    const DoFHandler<deal_II_dimension>::active_cell_iterator &cell,
    internal::Assembly::Scratch::sys_TemperatureSystem<deal_II_dimension>  &scratch,
    internal::Assembly::CopyData::sys_TemperatureSystem<deal_II_dimension> &data);
}
