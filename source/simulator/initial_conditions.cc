...
namespace aspect
{
  ...
  Simulator<dim>::set_initial_temperature_and_compositional_fields()
  {
    ...
    for (unsigned int n = 0; n < 1 + introspection.n_compositional_fields; ++n)
      {
        const AdvectionField advf = ((n == 0) ? AdvectionField::temperature() : AdvectionField::composition(n - 1));

        const unsigned int base_element = advf.base_element(introspection);

        // get the temperature/composition support points
        const std::vector<Point<dim>> support_points = finite_element.base_element(base_element).get_unit_support_points();
        Assert(support_points.size() != 0, ExcInternalError());

        // create an FEValues object with just the temperature/composition element
        FEValues<dim> fe_values(*mapping, finite_element, support_points, update_quadrature_points);
        ...
        const VectorFunctionFromScalarFunctionObject<dim, double> &advf_init_function =
          (advf.is_temperature() ?
             VectorFunctionFromScalarFunctionObject<dim, double>(
               [&](const Point<dim> &p) -> double { return initial_temperature_manager->initial_temperature(p); },
               introspection.component_indices.temperature,
               introspection.n_components) :
             VectorFunctionFromScalarFunctionObject<dim, double>(
               [&](const Point<dim> &p) -> double { return initial_composition_manager->initial_composition(p, n - 1); },
               introspection.component_indices.compositional_fields[n - 1],
               introspection.n_components));

        const ComponentMask advf_mask =
          (advf.is_temperature() ? introspection.component_masks.temperature : introspection.component_masks.compositional_fields[n - 1]);

        try
          {
            VectorTools::interpolate(*mapping, dof_handler, advf_init_function, initial_solution, advf_mask);
          }
        ...
        // and abort the program
          ...
          {
            const std::string field_name = (advf.is_temperature() ? "temperature" : introspection.name_for_compositional_index(n - 1));

            std::cerr << std::endl << std::endl << "----------------------------------------------------" << std::endl;
            std::cerr << "Exception on MPI process <" << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
                      << "> while setting the initial condition for field <" << field_name << ">: " << std::endl
                      << exc.what() << std::endl
                      << "Aborting!" << std::endl
                      << "----------------------------------------------------" << std::endl;
                ...
                for (unsigned int i = 0; i < finite_element.base_element(base_element).dofs_per_cell; ++i)
                  {
                    ...
                    for (unsigned int m = 0; m < parameters.normalized_fields.size(); ++m)
                      sum +=
                        initial_composition_manager->initial_composition(fe_values.quadrature_point(i), parameters.normalized_fields[m]);
        ...
        if (Utilities::MPI::max(normalize_composition ? 1 : 0, mpi_communicator) == 1)
          {
            const double global_max = Utilities::MPI::max(max_sum_comp, mpi_communicator);

            if (n == 1)
              pcout << "Sum of compositional fields is not one, fields will be normalized" << std::endl;
    ...
    for (unsigned int n = 0; n < 1 + introspection.n_compositional_fields; ++n)
      {
        AdvectionField advf = ((n == 0) ? AdvectionField::temperature() : AdvectionField::composition(n - 1));
  ...
  template <int dim>
  void
  Simulator<dim>::interpolate_particle_properties(const std::vector<AdvectionField> &advection_fields)
  {
        ...
        for (unsigned int advection_field = 0; advection_field < advection_fields.size(); ++advection_field)
          {
            if (parameters.mapped_particle_properties.size() != 0)
              {
                const std::pair<std::string, unsigned int> particle_property_and_component =
                  parameters.mapped_particle_properties.find(advection_fields[advection_field].compositional_variable)->second;
                ...
                if (particle_property_manager.get_data_info().fieldname_exists(particle_property_and_component.first))
                  {
                    Assert(advection_field_has_been_found[advection_field] == false,
                           ExcMessage("The field " + advection_fields[advection_field].name(introspection) +
                                      " is mapped to particle properties in more than one particle manager. This is not supported."));

                    const unsigned int particle_property_index =
                      particle_property_manager.get_data_info().get_position_by_field_name(particle_property_and_component.first) +
              ...
              {
                Assert(
                  particle_managers.size() == 1,
                  ExcMessage(
                    "Automatically mapping particle properties to compositional fields is only supported if there is exactly one set of particles. "
      ...
      if (advection_field_has_been_found[advection_field] != true)
        {
          ...
          for (unsigned int particle_manager = 0; particle_manager < particle_managers.size(); ++particle_manager)
            {
              const Particle::Property::Manager<dim> &particle_property_manager =
                particle_managers[particle_manager].get_property_manager();
          ...
          const std::pair<std::string, unsigned int> wrong_property =
            parameters.mapped_particle_properties.find(advection_fields[advection_field].compositional_variable)->second;

          Assert(false,
                 ExcMessage(
                   "The field " + advection_fields[advection_field].name(introspection) +
                   " is marked as advected by particles, "
                   "but no particle property exists that is mapped to this field. The active particle properties in your model are "
                   "the following:" +
                   property_names.str() + "The property you selected is '" + wrong_property.first +
                   "'. "
    ...
    for (const auto &data : base_element_to_field_indices)
      {
        ...
        std::vector<std::vector<std::pair<unsigned int, unsigned int>>> property_indices_for_base_element(particle_managers.size());
        for (unsigned int particle_manager = 0; particle_manager < particle_managers.size(); ++particle_manager)
          for (const std::pair<unsigned int, unsigned int> &field_and_particle_property : particle_property_indices[particle_manager])
            if (std::find(advection_field_indices.begin(), advection_field_indices.end(), field_and_particle_property.first) !=
                advection_field_indices.end())
              property_indices_for_base_element[particle_manager].push_back(field_and_particle_property);

        // get the temperature/composition support points
        const std::vector<Point<dim>> support_points = finite_element.base_element(base_element_index).get_unit_support_points();
        Assert(support_points.size() != 0, ExcInternalError());
              ...
              for (unsigned int particle_manager = 0; particle_manager < particle_managers.size(); ++particle_manager)
                {
                  try
                    {
                      particle_properties = particle_managers[particle_manager].get_interpolator().properties_at_points(
                        particle_managers[particle_manager].get_particle_handler(),
                        quadrature_points,
                  ...
                  // generated, catch it, print an error message, and abort the program.
                  catch (std::exception &exc)
                    {
                      std::cerr << std::endl << std::endl << "----------------------------------------------------" << std::endl;
                      std::cerr << "Exception on MPI process <" << Utilities::MPI::this_mpi_process(MPI_COMM_WORLD)
                                << "> while interpolating particle properties: " << std::endl
                                << exc.what() << std::endl
                                << "Aborting!" << std::endl
                                << "----------------------------------------------------" << std::endl;
                    ...
                    for (unsigned int i = 0; i < n_dofs_per_cell; ++i)
                      {
                        const unsigned int system_local_dof =
                          finite_element.component_to_system_index(advection_fields[field_and_particle_property.first].component_index(
                                                                     introspection),
    ...
    for (unsigned int b = 0; b < introspection.n_blocks; ++b)
      if (particle_blocks[b] == false)
        Assert(particle_solution.block(b).l2_norm() == 0, ExcInternalError());
  ...
  template <int dim>
  void
  Simulator<dim>::compute_initial_pressure_field()
  {
    ...
    if (parameters.use_locally_conservative_discretization == false)
      {
        ...
        const FEVariable<dim> &pressure_variable =
          parameters.include_melt_transport ? introspection.variable("fluid pressure") : introspection.variable("pressure");
        ...
        VectorFunctionFromScalarFunctionObject<dim> vector_function_object(
          [&](const Point<dim> &p) -> double { return adiabatic_conditions->pressure(p); }, pressure_comp, introspection.n_components);

        VectorTools::interpolate(*mapping, dof_handler, vector_function_object, system_tmp, pressure_component_mask);
    ...
    else
      {
        ...
        // is discontinuous.
        Assert(finite_element.base_element(introspection.base_elements.pressure).dofs_per_face == 0, ExcNotImplemented());
