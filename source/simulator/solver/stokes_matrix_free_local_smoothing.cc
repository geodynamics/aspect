...
namespace aspect
{
  ...
  template <int dim, int velocity_degree>
  StokesMatrixFreeHandlerLocalSmoothingImplementation<dim, velocity_degree>::StokesMatrixFreeHandlerLocalSmoothingImplementation(
    ...
    fe_projection(FE_DGQ<dim>(
                    parameters.material_averaging == MaterialModel::MaterialAveraging::AveragingOperation::project_to_Q1 ||
                        parameters.material_averaging == MaterialModel::MaterialAveraging::AveragingOperation::project_to_Q1_only_viscosity
  ...
  StokesMatrixFreeHandlerLocalSmoothingImplementation<dim, velocity_degree>::evaluate_material_model()
  {
    ...
    MGTransferType<dim, GMGNumberType> transfer;
        ...
        for (const unsigned int q : velocity.quadrature_point_indices())
          {
            ...
            if (is_compressible || this->get_parameters().enable_prescribed_dilation)
              for (unsigned int d = 0; d < dim; ++d)
  ...
  StokesMatrixFreeHandlerLocalSmoothingImplementation<dim, velocity_degree>::solve(
    ...
    LinearAlgebra::BlockVector       &solution_vector)
  {
    ...
    using GMGPreconditioner = PreconditionMG<dim, VectorType, MGTransferType<dim, GMGNumberType>>;
  ...
  StokesMatrixFreeHandlerLocalSmoothingImplementation<dim, velocity_degree>::setup_dofs()
  {
    ...
    // user with a reasonable error message:
    {
      ...
      have_periodic_hanging_nodes = (Utilities::MPI::max(have_periodic_hanging_nodes ? 1 : 0, this->get_mpi_communicator())) == 1;
    ...
    // Set up velocity DoFHandler
    {
      ...
      this->get_geometry_model().make_periodicity_constraints(dof_handler_v, constraints_v);
...
#else
      IndexSet locally_relevant_dofs;
      DoFTools::extract_locally_relevant_dofs(dof_handler_p, locally_relevant_dofs);
#endif

      constraints_p.reinit(dof_handler_p.locally_owned_dofs(), locally_relevant_dofs);

      this->get_geometry_model().make_periodicity_constraints(dof_handler_p, constraints_p);
      ...
      for (unsigned int level = 0; level < n_levels; ++level)
        {
            ...
            if (!no_flux_boundaries.empty())
              {
                ...
                const IndexSet &refinement_edge_indices = mg_constrained_dofs_A_block.get_refinement_edge_indices(level);
...

          {
            ...
            matrix_free_level->reinit(
              mapping, stokes_dofs, stokes_constraints, QGauss<1>(this->get_parameters().stokes_velocity_degree + 1), additional_data);
          }
          {
            mg_matrices_A_block[level].clear();
            const std::vector<unsigned int> selected_dof_handler = {/*velocity =*/0};
            mg_matrices_A_block[level].initialize(matrix_free_level, mg_constrained_dofs_A_block, level, selected_dof_handler);
          }
          {
