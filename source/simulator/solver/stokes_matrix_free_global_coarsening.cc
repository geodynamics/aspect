...
namespace aspect
{
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::declare_parameters(ParameterHandler &prm)
  {
    ...
    prm.enter_subsection("Matrix Free");
    {
      ...
      prm.declare_entry("Execute solver timings",
                        "false",
                        Patterns::Bool(),
  ...
  template <int dim, int velocity_degree>
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::StokesMatrixFreeHandlerGlobalCoarseningImplementation(
    ...
    fe_projection(FE_DGQ<dim>(parameters.material_averaging == MaterialModel::MaterialAveraging::AveragingOperation::project_to_Q1 ||
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::evaluate_material_model()
  {
    ...
    MGLevelObject<MGTwoLevelTransfer<dim, dealii::LinearAlgebra::distributed::Vector<GMGNumberType>>> transfers(min_level, max_level);
      ...
      for (auto l = min_level; l <= max_level; ++l)
        {
          ...
          std::shared_ptr<MatrixFree<dim, double>> mf(new MatrixFree<dim, double>());
          mf->reinit(get_level_triangulation_mapping(), dofhandlers_projection[l], cs, QGauss<1>(degree + 1));
          temp_ops[l].initialize(mf);
        }

      GCMGTransferType<dim, GMGNumberType> transfer(transfers, [&](const auto l, auto &vec) { temp_ops[l].initialize_dof_vector(vec); });

      transfer.interpolate_to_mg(dof_handler_projection, level_viscosity_vector, active_viscosity_vector);
    };
    ...
    FEValues<dim> fe_values_projection(this->get_mapping(), fe_projection, quadrature_formula, update_values);
            ...
            for (unsigned int i = 0; i < n_components_filled; ++i)
              {
                ...
                if (dof_handler_projection.get_fe().degree == 0)
                ...
                else
                  {
                    ...
                    fe_values_projection.get_function_values(level_viscosity_vector[level], local_dof_indices, values_on_quad);

                    // Do not allow viscosity to be greater than or less than the limits
                    // of the evaluated viscosity on the active level.
                    for (unsigned int q = 0; q < n_q_points; ++q)
                      level_cell_data[level].viscosity(cell, q)[i] = std::clamp(values_on_quad[q],
          ...
          if (active_cell_data.enable_prescribed_dilation)
            {
              if (out.template has_additional_output_object<MaterialModel::PrescribedPlasticDilation<dim>>() == false)
                out.additional_outputs.push_back(
                  std::make_unique<MaterialModel::PrescribedPlasticDilation<dim>>(out.n_evaluation_points()));
                      ...
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          ...
                          else if ((this->get_newton_handler().parameters.velocity_block_stabilization &
                                    Newton::Parameters::Stabilization::PD) != Newton::Parameters::Stabilization::none)
                            effective_strain_rate = deviator(effective_strain_rate);

                          // use the spd factor when the stabilization is PD or SPD.
                          const double alpha =
                            (this->get_newton_handler().parameters.velocity_block_stabilization & Newton::Parameters::Stabilization::PD) !=
                                Newton::Parameters::Stabilization::none ?
                          ...
                          active_cell_data.newton_factor_wrt_pressure_table(cell, q)[i] =
                            derivatives->viscosity_derivative_wrt_pressure[q] * derivatives->viscosity_derivative_averaging_weights[q] *
                            newton_derivative_scaling_factor;
                          Assert(std::isfinite(active_cell_data.newton_factor_wrt_pressure_table(cell, q)[i]),
                                 ExcMessage(
                                   "active_cell_data.newton_factor_wrt_pressure_table is not finite: " +
                          ...
                          for (unsigned int m = 0; m < dim; ++m)
                            for (unsigned int n = 0; n < dim; ++n)
                              {
                                active_cell_data.strain_rate_table(cell, q)[m][n][i] = effective_strain_rate[m][n];

                                active_cell_data.newton_factor_wrt_strain_rate_table(cell, q)[m][n][i] =
                                  derivatives->viscosity_derivative_wrt_strain_rate[q][m][n] *
                                  derivatives->viscosity_derivative_averaging_weights[q] * newton_derivative_scaling_factor * alpha;
                                ...
                                Assert(std::isfinite(active_cell_data.newton_factor_wrt_strain_rate_table(cell, q)[m][n][i]),
                                       ExcMessage(
                                         "active_cell_data.newton_factor_wrt_strain_rate_table has an element which is not finite: " +
                          ...
                          if (active_cell_data.enable_prescribed_dilation)
                            {
                              ...
                              Assert(std::isfinite(active_cell_data.dilation_derivative_wrt_pressure_table(cell, q)[i]),
                                     ExcMessage(
                                       "active_cell_data.dilation_derivative_wrt_pressure_table is not finite: " +
                                ...
                                for (unsigned int n = 0; n < dim; ++n)
                                  {
                                    active_cell_data.dilation_derivative_wrt_strain_rate_table(cell, q)[m][n][i] =
                                      derivatives->dilation_derivative_wrt_strain_rate[q][m][n] * newton_derivative_scaling_factor;
                                    Assert(std::isfinite(active_cell_data.dilation_derivative_wrt_pressure_table(cell, q)[i]),
                                           ExcMessage("active_cell_data.dilation_derivative_wrt_strain_rate_table is not finite: " +
                                                      std::to_string(active_cell_data.dilation_derivative_wrt_pressure_table(cell, q)[i]) +
                                                      ". Relevant variables are derivatives->dilation_derivative_wrt_strain_rate[q] = " +
                                                      std::to_string(derivatives->dilation_derivative_wrt_pressure[q]) +
                      ...
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          ...
                          Assert(std::isfinite(active_cell_data.dilation_lhs_term_table(cell, q)[i]),
                                 ExcMessage("active_cell_data.dilation_lhs_term_table is not finite: " +
                                            std::to_string(active_cell_data.dilation_lhs_term_table(cell, q)[i])));
                        }
                    }
                }
            }
      ...
      if (active_cell_data.apply_stabilization_free_surface_faces == true)
        {
          ...
          active_cell_data.free_surface_boundary_indicators = this->get_mesh_deformation_handler().get_free_surface_boundary_indicators();
              ...
              for (unsigned int i = 0; i < n_components_filled; ++i)
                {
                  ...
                  typename DoFHandler<dim>::active_cell_iterator matrix_free_cell = cell_face_pair.first;
                  typename DoFHandler<dim>::active_cell_iterator simulator_cell(&(this->get_triangulation()),
                                                                                matrix_free_cell->level(),
                                                                                matrix_free_cell->index(),
                                                                                &(this->get_dof_handler()));
                  ...
                  face_material_inputs.reinit(fe_face_values, simulator_cell, this->introspection(), this->get_solution());
                  face_material_inputs.requested_properties = MaterialModel::MaterialProperties::density;
                  ...
                  for (unsigned int q = 0; q < n_face_q_points; ++q)
                    {
                      ...
                      const double pressure_perturbation =
                        face_material_outputs.densities[q] * this->get_timestep() * free_surface_theta * g_norm;
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::correct_stokes_rhs()
  {
    ...
    FEEvaluation<dim, velocity_degree, velocity_degree + 1, dim, double>   velocity(*stokes_matrix.get_matrix_free(), 0);
    FEEvaluation<dim, velocity_degree - 1, velocity_degree + 1, 1, double> pressure(*stokes_matrix.get_matrix_free(), 1);

    const bool use_viscosity_at_quadrature_points = (active_cell_data.viscosity.size(1) == velocity.n_q_points);
    ...
    for (unsigned int cell = 0; cell < n_cells; ++cell)
      {
        ...
        for (const unsigned int q : velocity.quadrature_point_indices())
          {
            ...
            SymmetricTensor<2, dim, VectorizedArray<double>> sym_grad_u = velocity.get_symmetric_gradient(q);
        ...
        velocity.integrate_scatter(EvaluationFlags::gradients, rhs_correction.block(0));

        pressure.integrate_scatter(EvaluationFlags::values, rhs_correction.block(1));
      }
    ...
    if (active_cell_data.apply_stabilization_free_surface_faces)
      {
        ...
        FEFaceEvaluation<dim, velocity_degree, velocity_degree + 1, dim, double> velocity_boundary(*stokes_matrix.get_matrix_free());
        ...
        for (unsigned int face = n_faces_interior; face < n_faces_boundary + n_faces_interior; ++face)
          {
            ...
            velocity_boundary.integrate_scatter(EvaluationFlags::values, rhs_correction.block(0));
          }
      }
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::solve(
    ...
    LinearAlgebra::BlockVector       &solution_vector)
  {
    ...
    if (print_details)
      {
        ...
        const std::string  solver   = (this->get_parameters().stokes_krylov_type == Parameters<dim>::StokesKrylovType::idr_s) ?
                                        ("IDR(" + std::to_string(this->get_parameters().idr_s_parameter) + ")") :
                                        "GMRES";
    ...
    // MG object
    Multigrid<VectorType> mg_A(mg_matrix_A, mg_coarse_A, *mg_transfer_A_block, mg_smoother_A, mg_smoother_A);

    Multigrid<VectorType> mg_Schur(mg_matrix_Schur, mg_coarse_Schur, *mg_transfer_Schur_complement, mg_smoother_Schur, mg_smoother_Schur);
    ...
    LinearAlgebra::BlockVector distributed_stokes_rhs(this->introspection().index_sets.stokes_partitioning, this->get_mpi_communicator());
    ...
    Assert(!this->get_parameters().include_melt_transport || this->introspection().variable("compaction pressure").block_index == 1,
           ExcNotImplemented());
    ...
    if (solve_newton_system == false)
      {
        ...
        this->denormalize_pressure(last_pressure_normalization_adjustment, linearized_stokes_initial_guess);
      }
    else
      {
        ...
        Assert(this->get_nonlinear_iteration() != 0, ExcMessage("The Newton solver may not be active in the first nonlinear iteration"));
    ...
    if (solve_newton_system == false)
      {
        ...
        solver_tolerance =
          this->get_parameters().linear_stokes_solver_tolerance * std::sqrt(residual_u * residual_u + residual_p * residual_p);
      }
    else
      {
    ...
    SolverControl solver_control_cheap(this->get_parameters().n_cheap_stokes_solver_steps, solver_tolerance, true);
    SolverControl solver_control_expensive(this->get_parameters().n_expensive_stokes_solver_steps, solver_tolerance);
    ...
    const internal::BlockSchurPreconditioner<internal::InverseVelocityBlock<GMGPreconditioner, VectorType, ABlockMatrixType>,
                                             SchurApproximationType,
                                             BTBlockOperatorType,
                                             dealii::LinearAlgebra::distributed::BlockVector<double>>
      preconditioner_cheap(inverse_velocity_block_cheap, schur_approximation_cheap, BT_block);

    const internal::BlockSchurPreconditioner<internal::InverseVelocityBlock<GMGPreconditioner, VectorType, ABlockMatrixType>,
                                             SchurApproximationType,
                                             BTBlockOperatorType,
                                             dealii::LinearAlgebra::distributed::BlockVector<double>>
      preconditioner_expensive(inverse_velocity_block_expensive, schur_approximation_expensive, BT_block);
    ...
    if (do_timings)
      {
        ...
        auto time_this = [&](const char *name, int repeats, const std::function<void()> &body, const std::function<void()> &prepare) {
          this->get_pcout() << "Timing " << name << ' ' << n_timings << " time(s) and repeat " << repeats
        ...
        // stokes vmult
        {
          ...
          time_this(
            "stokes_vmult", 10, [&]() { stokes_matrix.vmult(tmp_dst, tmp_src); }, [&]() { tmp_src = tmp_dst; });
        }
        ...
        // stokes preconditioner
        {
          ...
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_src = rhs_copy;
          time_this(
            "stokes_preconditioner", 1, [&]() { preconditioner_cheap.vmult(tmp_dst, tmp_src); }, [&]() { tmp_src = tmp_dst; });
        }
        // A preconditioner
        {
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_dst = solution_copy;
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_src = rhs_copy;
          time_this(
            "A_preconditioner", 1, [&]() { prec_A.vmult(tmp_dst.block(0), tmp_src.block(0)); }, [&]() { tmp_src = tmp_dst; });
        }
        // S preconditioner
        {
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_dst = solution_copy;
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_src = rhs_copy;
          time_this(
            "S_preconditioner", 5, [&]() { prec_Schur.vmult(tmp_dst.block(1), tmp_src.block(1)); }, [&]() { tmp_src = tmp_dst; });
        }
        // Solve
        {
          ...
          dealii::LinearAlgebra::distributed::BlockVector<double> tmp_src = rhs_copy;
          time_this(
            "Stokes_solve_cheap_idr",
            1,
            [&] {
              SolverIDR<dealii::LinearAlgebra::distributed::BlockVector<double>> solver(
                solver_control_cheap,
                mem,
                SolverIDR<dealii::LinearAlgebra::distributed::BlockVector<double>>::AdditionalData(this->get_parameters().idr_s_parameter));
          ...
          time_this(
            "Stokes_solve_cheap_gmres",
            1,
            [&] {
              SolverGMRES<dealii::LinearAlgebra::distributed::BlockVector<double>> solver(
                solver_control_cheap,
                mem,
                SolverGMRES<dealii::LinearAlgebra::distributed::BlockVector<double>>::AdditionalData(
                  this->get_parameters().stokes_gmres_restart_length + 2, true));
    ...
    try
      {
        ...
        if (this->get_parameters().stokes_krylov_type == Parameters<dim>::StokesKrylovType::gmres)
          {
            ...
            solver.solve(stokes_matrix, solution_copy, rhs_copy, preconditioner_cheap);
          }
        ...
        this->get_pcout() << (solver_control_cheap.last_step() != numbers::invalid_unsigned_int ? solver_control_cheap.last_step() : 0)
                          << "+0"
                          << " iterations." << std::endl;
    ...
    // it in n_expensive_stokes_solver_steps steps or less.
    catch (const SolverControl::NoConvergence &exc)
      {
        ...
        const unsigned int number_of_temporary_vectors =
          (this->get_parameters().include_melt_transport == false ? this->get_parameters().stokes_gmres_restart_length :
                                                                    std::max(this->get_parameters().stokes_gmres_restart_length, 100U));

        SolverFGMRES<dealii::LinearAlgebra::distributed::BlockVector<double>> solver(
          solver_control_expensive,
          mem,
          SolverFGMRES<dealii::LinearAlgebra::distributed::BlockVector<double>>::AdditionalData(number_of_temporary_vectors));
        ...
        try
          {
            ...
            solver.solve(stokes_matrix, solution_copy, rhs_copy, preconditioner_expensive);

            // Success. Print expensive iterations to screen.
            this->get_pcout() << solver_control_expensive.last_step() << " iterations." << std::endl;
        ...
        // if the solver fails trigger the post stokes solver signal and throw an exception
        catch (const std::exception &exc)
          {
            this->get_signals().post_stokes_solver(sim,
                                                   ...
                                                   inverse_velocity_block_cheap.n_iterations() +
    ...
    if (print_details)
      {
        this->get_pcout() << "     Schur complement preconditioner: " << schur_approximation_cheap.n_iterations() << '+'
                          << schur_approximation_expensive.n_iterations() << " iterations." << std::endl;
        this->get_pcout() << "     A block preconditioner: " << inverse_velocity_block_cheap.n_iterations() << '+'
                          << inverse_velocity_block_expensive.n_iterations() << " iterations." << std::endl;
      }
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::setup_dofs()
  {
      ...
      for (auto l = min_level; l <= max_level; ++l)
        {
          ...
          // velocity:
          {
            ...
            this->get_geometry_model().make_periodicity_constraints(dof_handler, constraint);
                ...
                if (component_mask != ComponentMask(this->introspection().n_components, false))
                  {
                    ...
                    VectorTools::interpolate_boundary_values(
                      mapping, dof_handler, boundary_id, Functions::ZeroFunction<dim>(dim), constraint, velocity_mask);
                  }
                else
                  {
            ...
            for (const auto id : dirichlet_boundary)
              {
                VectorTools::interpolate_boundary_values(mapping, dof_handler, id, Functions::ZeroFunction<dim>(dim), constraint);
              }


            Assert(this->get_boundary_velocity_manager().get_active_plugins().size() == 0, ExcNotImplemented());
          ...
          matrix_free = std::make_shared<MatrixFree<dim, double>>();
          matrix_free_objects.push_back(matrix_free);
          mg_matrices_Schur_complement[l].reinit(
            mapping, dofhandlers_v[l], dofhandlers_p[l], constraints_v[l], constraints_p[l], matrix_free);
    ...
    // Matrixfree object
    {
      ...
      matrix_free->reinit(this->get_mapping(),
                          stokes_dofs,
                          stokes_constraints,
    ...
    // Build MG transfer
    {
      {
        for (unsigned int l = min_level; l < max_level; ++l)
          transfers_v[l + 1].reinit(dofhandlers_v[l + 1], dofhandlers_v[l], constraints_v[l + 1], constraints_v[l]);
      ...
      {
        for (unsigned int l = min_level; l < max_level; ++l)
          transfers_p[l + 1].reinit(dofhandlers_p[l + 1], dofhandlers_p[l], constraints_p[l + 1], constraints_p[l]);
  ...
  StokesMatrixFreeHandlerGlobalCoarseningImplementation<dim, velocity_degree>::get_dof_handler_memory_consumption() const
  {
    ...
    for (auto l = min_level; l <= max_level; ++l)
      total +=
        dofhandlers_v[l].memory_consumption() + dofhandlers_p[l].memory_consumption() + dofhandlers_projection[l].memory_consumption();
