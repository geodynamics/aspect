...
namespace aspect
{
  namespace internal
  {
    ...
    {
      void
      copy(aspect::LinearAlgebra::Vector &out, const dealii::LinearAlgebra::distributed::Vector<double> &in)
      {
      ...
      void
      copy(dealii::LinearAlgebra::distributed::Vector<double> &out, const aspect::LinearAlgebra::Vector &in)
      {
      ...
      void
      copy(aspect::LinearAlgebra::BlockVector &out, const dealii::LinearAlgebra::distributed::BlockVector<double> &in)
      {
      ...
      void
      copy(dealii::LinearAlgebra::distributed::BlockVector<double> &out, const aspect::LinearAlgebra::BlockVector &in)
      {
        const unsigned int n_blocks = in.n_blocks();
        for (unsigned int b = 0; b < n_blocks; ++b)
          copy(out.block(b), in.block(b));
  ...
  namespace MatrixFreeStokesOperators
  {
    ...
    {
      return viscosity.memory_consumption() + newton_factor_wrt_pressure_table.memory_consumption() +
             strain_rate_table.memory_consumption() + newton_factor_wrt_strain_rate_table.memory_consumption() +
             dilation_derivative_wrt_pressure_table.memory_consumption() + dilation_derivative_wrt_strain_rate_table.memory_consumption();
    }
                          ...
                          double                                                     &maximum_viscosity)
    {
      ...
      const bool active_no_averaging = (material_averaging == MaterialModel::MaterialAveraging::AveragingOperation::none);
          ...
          [&](const typename DoFHandler<dim>::active_cell_iterator &cell,
            ...
            if (dof_handler_projection.get_fe().degree == 0)
              MaterialModel::MaterialAveraging::average(
                material_averaging, FEQ_cell, quadrature_formula, mapping, in.requested_properties, out);
      ...
      FEValues<dim> fe_values_projection(mapping, dof_handler_projection.get_fe(), quadrature_formula, update_values);
        ...
        for (unsigned int cell = 0; cell < n_cells; ++cell)
          {
            ...
            for (unsigned int i = 0; i < n_components_filled; ++i)
              {
                typename DoFHandler<dim>::active_cell_iterator FEQ_cell = matrix_free.get_cell_iterator(cell, i);
...
#ifdef DEBUG
                {
                  ...
                  typename DoFHandler<dim>::active_cell_iterator s_cell     = matrix_free_schur.get_cell_iterator(cell, i, 1);
                  double                                         distance_s = s_cell->center().distance(FEQ_cell->center());
                  Assert(distance_s < 1e-10, ExcInternalError());

                  typename DoFHandler<dim>::active_cell_iterator A_cell     = matrix_free_A.get_cell_iterator(cell, i);
                ...
                if (active_no_averaging)
                ...
                else
                  {
                    ...
                    fe_values_projection.get_function_values(active_viscosity_vector, local_dof_indices, values_on_quad);

                    // Do not allow viscosity to be greater than or less than the limits
                    // of the evaluated viscosity on the active level.
                    for (unsigned int q = 0; q < n_q_points; ++q)
                      active_cell_data.viscosity(cell, q)[i] = std::clamp(values_on_quad[q], minimum_viscosity, maximum_viscosity);
                  }
              }
          }
      }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::set_cell_data(const OperatorCellData<dim, number> &data)
  {
    this->cell_data = &data;
  }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::compute_diagonal()
  {
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::local_apply(
    ...
    const std::pair<unsigned int, unsigned int>                   &cell_range) const
  {
    ...
    const bool use_viscosity_at_quadrature_points = (cell_data->viscosity.size(1) == u_eval.n_q_points);
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        if (cell_data->enable_newton_derivatives && cell_data->average_newton_factors)
          {
            ...
            for (const unsigned int q : u_eval.quadrature_point_indices())
              {
                ...
                deta_deps_times_sym_grad_u += cell_data->newton_factor_wrt_strain_rate_table(cell, q) * sym_grad_u;
        ...
        for (const unsigned int q : u_eval.quadrature_point_indices())
          {
            ...
            const SymmetricTensor<2, dim, VectorizedArray<number>> sym_grad_u = u_eval.get_symmetric_gradient(q);
            ...
            VectorizedArray<number> pressure_terms = -cell_data->pressure_scaling * div_u;

            if (cell_data->enable_prescribed_dilation)
              pressure_terms -=
                cell_data->pressure_scaling * cell_data->pressure_scaling * cell_data->dilation_lhs_term_table(cell, q) * val_p;

            // Terms to be tested by the symmetric gradients of phi_u:
            SymmetricTensor<2, dim, VectorizedArray<number>> velocity_terms = viscosity_x_2 * sym_grad_u;
            ...
            if (cell_data->enable_newton_derivatives)
              {
                ...
                velocity_terms +=
                  (cell_data->symmetrize_newton_system ? (cell_data->strain_rate_table(cell, q) * deta_deps_times_sym_grad_u +
                ...
                if (cell_data->is_compressible || cell_data->enable_prescribed_dilation)
                  {
                    ...
                    if (cell_data->symmetrize_newton_system)
                    ...
                    else
                      {
                        for (unsigned int d = 0; d < dim; ++d)
                          velocity_terms[d][d] -=
                            2.0 * one_third * trace(cell_data->strain_rate_table(cell, q)) * deta_deps_times_sym_grad_u;
                      }

                    if (cell_data->enable_prescribed_dilation)
                      {
                        pressure_terms +=
                          ((cell_data->dilation_derivative_wrt_strain_rate_table(cell, q) * sym_grad_u) +
                           (cell_data->dilation_derivative_wrt_pressure_table(cell, q) * cell_data->pressure_scaling * val_p)) *
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::local_apply_face(
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::local_apply_boundary_face(
    const dealii::MatrixFree<dim, number>                         &data,
    dealii::LinearAlgebra::distributed::BlockVector<number>       &dst,
    const dealii::LinearAlgebra::distributed::BlockVector<number> &src,
    ...
    for (unsigned int face = face_range.first; face < face_range.second; ++face)
      {
        ...
        if (cell_data->free_surface_boundary_indicators.find(boundary_id) == cell_data->free_surface_boundary_indicators.end())
          continue;
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::StokesOperator<dim, degree_v, number>::apply_add(
    dealii::LinearAlgebra::distributed::BlockVector<number>       &dst,
    const dealii::LinearAlgebra::distributed::BlockVector<number> &src) const
  {
    if (cell_data->apply_stabilization_free_surface_faces)
      MatrixFreeOperators::Base<dim, dealii::LinearAlgebra::distributed::BlockVector<number>>::data->loop(
        &StokesOperator::local_apply,
      ...
      MatrixFreeOperators::Base<dim, dealii::LinearAlgebra::distributed::BlockVector<number>>::data->cell_loop(&StokesOperator::local_apply,
                                                                                                               this,
                                                                                                               dst,
                                                                                                               src);
  }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::BTBlockOperator<dim, degree_v, number>::set_cell_data(const OperatorCellData<dim, number> &data)
  {
    this->cell_data = &data;
  }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::BTBlockOperator<dim, degree_v, number>::compute_diagonal()
  {
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::BTBlockOperator<dim, degree_v, number>::local_apply(
    const dealii::MatrixFree<dim, number>                         &data,
    dealii::LinearAlgebra::distributed::BlockVector<number>       &dst,
    const dealii::LinearAlgebra::distributed::BlockVector<number> &src,
    const std::pair<unsigned int, unsigned int>                   &cell_range) const
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::BTBlockOperator<dim, degree_v, number>::local_apply_face(
    const dealii::MatrixFree<dim, number> &,
    dealii::LinearAlgebra::distributed::BlockVector<number> &,
    const dealii::LinearAlgebra::distributed::BlockVector<number> &,
    const std::pair<unsigned int, unsigned int> &) const
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::BTBlockOperator<dim, degree_v, number>::apply_add(
    dealii::LinearAlgebra::distributed::BlockVector<number>       &dst,
    const dealii::LinearAlgebra::distributed::BlockVector<number> &src) const
  {
    MatrixFreeOperators::Base<dim, dealii::LinearAlgebra::distributed::BlockVector<number>>::data->cell_loop(&BTBlockOperator::local_apply,
                                                                                                             this,
                                                                                                             dst,
                                                                                                             src);
  }
  ...
  template <int dim, int degree_p, typename number>
  void
  MatrixFreeStokesOperators::MassMatrixOperator<dim, degree_p, number>::set_cell_data(const OperatorCellData<dim, number> &data)
  {
    this->cell_data = &data;
  }
  ...
  template <int dim, int degree_p, typename number>
  void
  MatrixFreeStokesOperators::MassMatrixOperator<dim, degree_p, number>::local_apply(
    const dealii::MatrixFree<dim, number>                    &data,
  ...
  template <int dim, int degree_p, typename number>
  void
  MatrixFreeStokesOperators::MassMatrixOperator<dim, degree_p, number>::inner_cell_operation(
    FEEvaluation<dim, degree_p, degree_p + 2, 1, number> &pressure) const
  {
    const bool use_viscosity_at_quadrature_points = (cell_data->viscosity.size(1) == pressure.n_q_points);
  ...
  template <int dim, int degree_p, typename number>
  void
  MatrixFreeStokesOperators::MassMatrixOperator<dim, degree_p, number>::apply_add(
    dealii::LinearAlgebra::distributed::Vector<number>       &dst,
    const dealii::LinearAlgebra::distributed::Vector<number> &src) const
  {
    MatrixFreeOperators::Base<dim, dealii::LinearAlgebra::distributed::Vector<number>>::data->cell_loop(&MassMatrixOperator::local_apply,
                                                                                                        this,
                                                                                                        dst,
                                                                                                        src);
  }
  ...
  template <int dim, int degree_p, typename number>
  void
  MatrixFreeStokesOperators::MassMatrixOperator<dim, degree_p, number>::compute_diagonal()
  {
    this->inverse_diagonal_entries = std::make_shared<DiagonalMatrix<dealii::LinearAlgebra::distributed::Vector<number>>>();
    this->diagonal_entries         = std::make_shared<DiagonalMatrix<dealii::LinearAlgebra::distributed::Vector<number>>>();
    ...
    MatrixFreeTools::
      compute_diagonal<dim, degree_p, degree_p + 2, 1, number, VectorizedArray<number>, dealii::LinearAlgebra::distributed::Vector<number>>(
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::set_cell_data(const OperatorCellData<dim, number> &data)
  {
    this->cell_data = &data;
  }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::inner_cell_operation(
    FEEvaluation<dim, degree_v, degree_v + 1, dim, number> &velocity) const
  {
    const bool use_viscosity_at_quadrature_points = (cell_data->viscosity.size(1) == velocity.n_q_points);
    ...
    for (const unsigned int q : velocity.quadrature_point_indices())
      {
        ...
        SymmetricTensor<2, dim, VectorizedArray<number>> sym_grad_u = velocity.get_symmetric_gradient(q);
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::local_apply(
    const dealii::MatrixFree<dim, number>                    &data,
    dealii::LinearAlgebra::distributed::Vector<number>       &dst,
    const dealii::LinearAlgebra::distributed::Vector<number> &src,
    const std::pair<unsigned int, unsigned int>              &cell_range) const
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::apply_add(
    dealii::LinearAlgebra::distributed::Vector<number>       &dst,
    const dealii::LinearAlgebra::distributed::Vector<number> &src) const
  {
    MatrixFreeOperators::Base<dim, dealii::LinearAlgebra::distributed::Vector<number>>::data->cell_loop(&ABlockOperator::local_apply,
                                                                                                        this,
                                                                                                        dst,
                                                                                                        src);
  }
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::compute_diagonal()
  {
    this->inverse_diagonal_entries = std::make_shared<DiagonalMatrix<dealii::LinearAlgebra::distributed::Vector<number>>>();
    dealii::LinearAlgebra::distributed::Vector<number> &inverse_diagonal = this->inverse_diagonal_entries->get_vector();
    this->data->initialize_dof_vector(inverse_diagonal);

    MatrixFreeTools::compute_diagonal(*(this->get_matrix_free()),
                                      inverse_diagonal,
                                      &MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::cell_operation,
                                      this);
  ...
  template <int dim, int degree_v, typename number>
  void
  MatrixFreeStokesOperators::ABlockOperator<dim, degree_v, number>::set_diagonal(
