...
namespace aspect
{
  namespace StokesSolver
  {
    namespace internal
    {
      ...
      class StokesBlock
      {
        public:
           ...
           */
          void
          vmult(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const;

          void
          Tvmult(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const;

          void
          vmult_add(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const;

          void
          Tvmult_add(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const;
           ...
           */
          double
          residual(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &x, const LinearAlgebra::BlockVector &b) const;
      ...
      void
      StokesBlock::vmult(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const
      {
      ...
      void
      StokesBlock::Tvmult(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const
      {
        Assert(src.n_blocks() == 2, ExcInternalError());
        Assert(dst.n_blocks() == 2, ExcInternalError());
      ...
      void
      StokesBlock::vmult_add(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const
      {
        Assert(src.n_blocks() == 2, ExcInternalError());
        Assert(dst.n_blocks() == 2, ExcInternalError());
      ...
      void
      StokesBlock::Tvmult_add(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &src) const
      {
        Assert(src.n_blocks() == 2, ExcInternalError());
        Assert(dst.n_blocks() == 2, ExcInternalError());
      ...
      double
      StokesBlock::residual(LinearAlgebra::BlockVector &dst, const LinearAlgebra::BlockVector &x, const LinearAlgebra::BlockVector &b) const
      {
      ...
      class SchurComplementOperator
      {
        public:
          ...
          virtual void
          vmult(LinearAlgebra::Vector &dst, const LinearAlgebra::Vector &src) const = 0;
          virtual unsigned int
          n_iterations() const = 0;
      };
      ...
      class WeightedBFBT : public SchurComplementOperator
      {
        public:
          ...
          void
          vmult(LinearAlgebra::Vector &dst, const LinearAlgebra::Vector &src) const override;

          unsigned int
          n_iterations() const override;
      ...
      template <class PreconditionerMp>
      WeightedBFBT<PreconditionerMp>::WeightedBFBT(const LinearAlgebra::SparseMatrix      &pressure_laplace_matrix,
                                                   const PreconditionerMp                 &laplace_preconditioner,
                                                   const double                            solver_tolerance,
                                                   const LinearAlgebra::Vector            &inverse_lumped_mass_matrix,
                                                   const LinearAlgebra::BlockSparseMatrix &system_matrix)
      ...
      template <class PreconditionerMp>
      void
      WeightedBFBT<PreconditionerMp>::vmult(LinearAlgebra::Vector &dst, const LinearAlgebra::Vector &src) const
      {
        ...
        try
          {
            ...
            wtmp.reinit(inverse_lumped_mass_matrix);
            {
              ...
              solver.solve(pressure_laplace_matrix, ptmp, src, laplace_preconditioner);
              ...
              solver.solve(pressure_laplace_matrix, dst, ptmp, laplace_preconditioner);
              n_iterations_ += solver_control.last_step();
            }
          }
      ...
      template <class PreconditionerMp>
      InverseWeightedMassMatrix<PreconditionerMp>::InverseWeightedMassMatrix(const LinearAlgebra::SparseMatrix &mp_matrix,
                                                                             const PreconditionerMp            &mp_preconditioner,
                                                                             const double                       solver_tolerance)
        : n_iterations_(0)
        , mp_matrix(mp_matrix)
      ...
      template <class PreconditionerMp>
      void
      InverseWeightedMassMatrix<PreconditionerMp>::vmult(LinearAlgebra::Vector &dst, const LinearAlgebra::Vector &src) const
      {
                            ...
                            LinearAlgebra::BlockVector             &solution_vector)
    {
      ...
      if (solve_newton_system == false)
        {
          ...
          outputs.initial_nonlinear_residual =
            stokes_block.residual(distributed_stokes_solution, linearized_stokes_initial_guess, system_rhs);
          ...
          // pressure).
          const double velocity_residual = system_matrix.block(velocity_block_index, pressure_block_index)
                                             .residual(distributed_stokes_solution.block(velocity_block_index),
      ...
      SolverControl solver_control_cheap(this->get_parameters().n_cheap_stokes_solver_steps, solver_tolerance);
      ...
      aspect::internal::InverseVelocityBlock<LinearAlgebra::PreconditionAMG, LinearAlgebra::Vector, LinearAlgebra::SparseMatrix>
        inverse_velocity_block_cheap(system_matrix.block(velocity_block_index, velocity_block_index),
                                     *sim.Amg_preconditioner,
                                     /* do_solve_A = */ false,
                                     sim.stokes_A_block_is_symmetric(),
                                     this->get_parameters().linear_solver_A_block_tolerance);
      const aspect::internal::BlockSchurPreconditioner<
        aspect::internal::InverseVelocityBlock<LinearAlgebra::PreconditionAMG, LinearAlgebra::Vector, LinearAlgebra::SparseMatrix>,
        internal::SchurComplementOperator,
        LinearAlgebra::SparseMatrix,
        LinearAlgebra::BlockVector>
        preconditioner_cheap(inverse_velocity_block_cheap, *schur, system_matrix.block(0, 1));

      // create an expensive preconditioner that solves for the A block with CG
      aspect::internal::InverseVelocityBlock<LinearAlgebra::PreconditionAMG, LinearAlgebra::Vector, LinearAlgebra::SparseMatrix>
        inverse_velocity_block_expensive(system_matrix.block(velocity_block_index, velocity_block_index),
                                         *sim.Amg_preconditioner,
                                         /* do_solve_A = */ true,
                                         sim.stokes_A_block_is_symmetric(),
                                         this->get_parameters().linear_solver_A_block_tolerance);
      const aspect::internal::BlockSchurPreconditioner<
        ...
        LinearAlgebra::SparseMatrix,
        LinearAlgebra::BlockVector>
        preconditioner_expensive(inverse_velocity_block_expensive, *schur, system_matrix.block(0, 1));
      ...
      try
        {
          ...
          SolverFGMRES<LinearAlgebra::BlockVector> solver(solver_control_cheap,
                                                          mem,
                                                          SolverFGMRES<LinearAlgebra::BlockVector>::AdditionalData(
                                                            this->get_parameters().stokes_gmres_restart_length));

          solver.solve(stokes_block, distributed_stokes_solution, distributed_stokes_rhs, preconditioner_cheap);
          ...
          try
            {
              ...
              SolverFGMRES<LinearAlgebra::BlockVector> solver(
                solver_control_expensive, mem, SolverFGMRES<LinearAlgebra::BlockVector>::AdditionalData(number_of_temporary_vectors));

              solver.solve(stokes_block, distributed_stokes_solution, distributed_stokes_rhs, preconditioner_expensive);
...
#define INSTANTIATE(dim) template class MatrixBased<dim>;
