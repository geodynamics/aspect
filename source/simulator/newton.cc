...
namespace aspect
{
  ...
  template <int dim>
  void
  NewtonHandler<dim>::set_assemblers(Assemblers::Manager<dim> &assemblers) const
  {
    ...
    if (this->get_material_model().is_compressible() || this->get_parameters().enable_prescribed_dilation ||
        this->get_prescribed_dilation_manager().get_active_plugin_names().size() > 0)
      {
        ...
        if (this->get_parameters().use_full_A_block_preconditioner == false)
          assemblers.stokes_preconditioner.push_back(std::make_unique<aspect::Assemblers::NewtonStokesCompressiblePreconditioner<dim>>());

        assemblers.stokes_system.push_back(std::make_unique<aspect::Assemblers::NewtonStokesCompressibleStrainRateViscosityTerm<dim>>());
      }
    ...
    else if (this->get_parameters().formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::incompressible)
      {
        // do nothing, because we assembled div u =0 above already
      }
    else if (this->get_parameters().formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::isentropic_compression)
      {
  ...
  template <int dim>
  void
  NewtonHandler<dim>::create_material_model_outputs(MaterialModel::MaterialModelOutputs<dim> &output)
  {
    if (output.template has_additional_output_object<MaterialModel::MaterialModelDerivatives<dim>>())
      return;

    output.additional_outputs.push_back(std::make_unique<MaterialModel::MaterialModelDerivatives<dim>>(output.n_evaluation_points()));
  }
  ...
  namespace Newton
  {
    ...
    Parameters::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("Solver parameters");
      {
        ...
        {
          prm.declare_entry("Nonlinear Newton solver switch tolerance",
                            "1e-5",
                            Patterns::Double(0., 1.),
          ...
          prm.declare_entry("Max pre-Newton nonlinear iterations",
                            "10",
                            Patterns::Integer(0),
          ...
          prm.declare_entry("Max Newton line search iterations",
                            "5",
                            Patterns::Integer(0),
          ...
          prm.declare_entry("Use Newton residual scaling method",
                            "false",
                            Patterns::Bool(),
          ...
          prm.declare_entry("Maximum linear Stokes solver tolerance",
                            "1e-2",
                            Patterns::Double(0., 1.),
          ...
          prm.declare_entry("Stabilization preconditioner",
                            "SPD",
          ...
          prm.declare_entry("Stabilization velocity block",
                            "SPD",
                            Patterns::Selection("SPD|PD|symmetric|none"),
          ...
          prm.declare_entry("Use Newton failsafe",
                            "false",
                            Patterns::Bool(),
...
\
  template class NewtonHandler<dim>; \
\
  namespace MaterialModel \
  { \
