...
namespace aspect
{
  ...
  namespace
  {
    ...
    class FunctorDepthAverageAdiabat : public internal::FunctorBase<dim>
    {
      public:
        ...
        FunctorDepthAverageAdiabat(const Property &property, const AdiabaticConditions::Interface<dim> &adiabat)
          : property(property)
          , adiabat(adiabat)
        {}
    ...
    {
      public:
        FunctorDepthAverageVelocityMagnitude(const FEValuesExtractors::Vector &field, bool convert_to_years)
          : field_(field)
                   ...
                   std::vector<double>              &output) override
        {
          ...
          for (unsigned int q = 0; q < output.size(); ++q)
            output[q] = std::sqrt(velocity_values[q] * velocity_values[q]) * (convert_to_years_ ? year_in_seconds : 1.0);
        }
          ...
          for (unsigned int q = 0; q < output.size(); ++q)
            {
              ...
              output[q] = std::max(0.0, velocity_values[q] * vertically_down) * (convert_to_years_ ? year_in_seconds : 1.0);
            }
        }
          ...
          for (unsigned int q = 0; q < output.size(); ++q)
            {
              ...
              output[q] = std::max(0.0, velocity_values[q] * vertically_up) * (convert_to_years_ ? year_in_seconds : 1.0);
            }
        }
    ...
    class FunctorDepthAverageVsVp : public internal::FunctorBase<dim>
    {
      public:
        ...
        {
          outputs.additional_outputs.push_back(std::make_unique<MaterialModel::SeismicAdditionalOutputs<dim>>(n_points));
        }
          ...
          for (unsigned int q = 0; q < output.size(); ++q)
            {
              ...
              const double advective_flux   = (velocity_values[q] * vertical) * in.temperature[q] * out.densities[q] * out.specific_heat[q];
              const double conductive_flux  = -(temperature_gradients[q] * vertical) * out.thermal_conductivities[q];
    ...
    {
      public:
        FunctorDepthAverageVerticalMassFlux(const FEValuesExtractors::Vector &velocity_field, const GravityModel::Interface<dim> *gm)
          : velocity_field_(velocity_field)
          , gravity_model(gm)
        {}
  ...
  namespace internal
  {
    ...
    template <int dim>
    FunctorBase<dim>::~FunctorBase() = default;
    ...
    template <int dim>
    QAnisotropic<dim>
    get_quadrature_formula(const unsigned int lateral_quadrature_degree, const unsigned int depth_dimension);
    ...
    get_quadrature_formula(const unsigned int lateral_quadrature_degree, const unsigned int depth_dimension)
    {
      ...
      if (depth_dimension == dim)
        return QAnisotropic<dim>(QGauss<1>(lateral_quadrature_degree), QIterated<1>(QMidpoint<1>(), 10));
      else if (depth_dimension == 1)
        return QAnisotropic<dim>(QIterated<1>(QMidpoint<1>(), 10), QGauss<1>(lateral_quadrature_degree));
                                                  ...
                                                  std::vector<std::unique_ptr<internal::FunctorBase<dim>>> &functors) const
  {
    ...
    std::vector<std::vector<double>> values(n_properties, std::vector<double>(n_slices, 0.0));
    ...
    const unsigned int lateral_quadrature_degree =
      static_cast<unsigned int>(std::ceil((this->introspection().polynomial_degree.max_degree + 1.0) / 2.0));

    std::unique_ptr<Quadrature<dim>> quadrature_formula;
    if (geometry_unique_depth_direction != numbers::invalid_unsigned_int)
      quadrature_formula = std::make_unique<Quadrature<dim>>(
        internal::get_quadrature_formula<dim>(lateral_quadrature_degree, geometry_unique_depth_direction));
    else
    ...
    std::vector<std::vector<double>> output_values(n_properties, std::vector<double>(n_q_points));
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              ...
              unsigned int layer_index =
                std::distance(depth_bounds.begin(), std::lower_bound(depth_bounds.begin(), depth_bounds.end(), depth));
              if (layer_index > 0)
                layer_index -= 1;
    ...
    if (print_under_res_warning)
      {
        this->get_pcout() << std::endl
                          << "**** Warning: When computing depth averages, there is at least one depth band" << std::endl
                          << "     that does not have any quadrature points in it." << std::endl
                          << "     Consider reducing the number of depth layers for averaging." << std::endl
                          << std::endl;
      }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "density"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "temperature"))[0];
  }
  ...
  template <int dim>
  void
  LateralAveraging<dim>::get_composition_averages(const unsigned int c, std::vector<double> &values) const
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "C_" + Utilities::int_to_string(c)))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "viscosity"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "log_viscosity"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "velocity_magnitude"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "sinking_velocity"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "rising_velocity"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "Vs"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "Vp"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "vertical_heat_flux"))[0];
  }
  ...
  {
    values = compute_lateral_averages(values.size(), std::vector<std::string>(1, "vertical_mass_flux"))[0];
  }
  ...
  template <int dim>
  std::vector<std::vector<double>>
  LateralAveraging<dim>::compute_lateral_averages(const unsigned int n_slices, const std::vector<std::string> &property_names) const
  {
    ...
    for (const auto &property_name : property_names)
      {
        if (property_name == "temperature")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageField<dim>>(this->introspection().extractors.temperature));
          }
          ...
          {
            const unsigned int c = this->introspection().compositional_index_for_name(property_name);

            functors.push_back(std::make_unique<FunctorDepthAverageField<dim>>(this->introspection().extractors.compositional_fields[c]));
          }
        else if (property_name == "velocity_magnitude")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageVelocityMagnitude<dim>>(this->introspection().extractors.velocities,
                                                                                           this->convert_output_to_years()));
          }
        else if (property_name == "sinking_velocity")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageSinkingVelocity<dim>>(this->introspection().extractors.velocities,
                                                                                         &this->get_gravity_model(),
                                                                                         this->convert_output_to_years()));
          }
        else if (property_name == "rising_velocity")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageRisingVelocity<dim>>(this->introspection().extractors.velocities,
                                                                                        &this->get_gravity_model(),
                                                                                        this->convert_output_to_years()));
          }
          ...
          {
            functors.push_back(std::make_unique<FunctorDepthAverageVerticalHeatFlux<dim>>(this->introspection().extractors.velocities,
                                                                                          this->introspection().extractors.temperature,
                                                                                          &this->get_gravity_model()));
          }
        else if (property_name == "vertical_mass_flux")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageVerticalMassFlux<dim>>(this->introspection().extractors.velocities,
                                                                                          &this->get_gravity_model()));
          }
          ...
          {
            const unsigned int c = this->introspection().compositional_index_for_name(property_name.substr(0, property_name.size() - 5));

            functors.push_back(
              std::make_unique<FunctorDepthAverageFieldMass<dim>>(this->introspection().extractors.compositional_fields[c]));
          }
        else if (property_name == "adiabatic_temperature")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageAdiabat<dim>>(FunctorDepthAverageAdiabat<dim>::temperature,
                                                                                 this->get_adiabatic_conditions()));
          }
        else if (property_name == "adiabatic_pressure")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageAdiabat<dim>>(FunctorDepthAverageAdiabat<dim>::pressure,
                                                                                 this->get_adiabatic_conditions()));
          }
        else if (property_name == "adiabatic_density")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageAdiabat<dim>>(FunctorDepthAverageAdiabat<dim>::density,
                                                                                 this->get_adiabatic_conditions()));
          }
        else if (property_name == "adiabatic_density_derivative")
          {
            functors.push_back(std::make_unique<FunctorDepthAverageAdiabat<dim>>(FunctorDepthAverageAdiabat<dim>::density_derivative,
                                                                                 this->get_adiabatic_conditions()));
          }
        else
          {
