...
namespace aspect
{
  namespace internal
  {
    namespace Assembly
    {
      ...
      {
        template <int dim>
        StokesPreconditioner<dim>::StokesPreconditioner(const FiniteElement<dim> &finite_element,
        ...
        template <int dim>
        StokesPreconditioner<dim>::StokesPreconditioner(const StokesPreconditioner &scratch)
        ...
        template <int dim>
        StokesPreconditioner<dim>::~StokesPreconditioner() = default;


        template <int dim>
        void
        StokesPreconditioner<dim>::reinit(const typename DoFHandler<dim>::active_cell_iterator &cell_ref)
        {
          this->cell        = cell_ref;
        ...
        template <int dim>
        StokesSystem<dim>::StokesSystem(const FiniteElement<dim>  &finite_element,
                                        const Mapping<dim>        &mapping,
                                        const Quadrature<dim>     &quadrature,
          ...
          face_finite_element_values(mapping, finite_element, face_quadrature, face_update_flags)
          ,
        ...
        template <int dim>
        void
        StokesSystem<dim>::reinit(const typename DoFHandler<dim>::active_cell_iterator &cell_ref, const unsigned face_number_ref)
        {
          this->cell        = cell_ref;
        ...
        template <int dim>
        AdvectionSystem<dim>::AdvectionSystem(const FiniteElement<dim>  &finite_element,
          ...
          , face_finite_element_values(face_quadrature.size() > 0 ?
                                         std::make_unique<FEFaceValues<dim>>(mapping, finite_element, face_quadrature, face_update_flags) :
                                         nullptr)
          , neighbor_face_finite_element_values(
              face_quadrature.size() > 0 ?
                std::make_unique<FEFaceValues<dim>>(mapping, finite_element, face_quadrature, face_update_flags) :
                nullptr)
          , subface_finite_element_values(
              face_quadrature.size() > 0 ?
                std::make_unique<FESubfaceValues<dim>>(mapping, finite_element, face_quadrature, face_update_flags) :
                nullptr)
          , local_dof_indices(finite_element.dofs_per_cell)
          ,
          ...
          , face_phi_field((face_quadrature.size() > 0 ? advection_element.dofs_per_cell : 0), numbers::signaling_nan<double>())
          , face_grad_phi_field((face_quadrature.size() > 0 ? advection_element.dofs_per_cell : 0),
                                numbers::signaling_nan<Tensor<1, dim>>())
          , neighbor_face_phi_field((face_quadrature.size() > 0 ? advection_element.dofs_per_cell : 0), numbers::signaling_nan<double>())
          ...
          , old_composition_values(n_compositional_fields, std::vector<double>(quadrature.size(), numbers::signaling_nan<double>()))
          , old_old_composition_values(n_compositional_fields, std::vector<double>(quadrature.size(), numbers::signaling_nan<double>()))
          ...
          , current_composition_values(n_compositional_fields, std::vector<double>(quadrature.size(), numbers::signaling_nan<double>()))
        ...
        template <int dim>
        AdvectionSystem<dim>::AdvectionSystem(const AdvectionSystem &scratch)
          : ScratchBase<dim>(scratch)
          ,
          ...
          , face_finite_element_values(scratch.face_finite_element_values.get() ?
                                         ...
                                         nullptr)
          , neighbor_face_finite_element_values(
              scratch.neighbor_face_finite_element_values.get() ?
                ...
                nullptr)
          , subface_finite_element_values(
              scratch.subface_finite_element_values.get() ?
        ...
        template <int dim>
        void
        AdvectionSystem<dim>::reinit(const typename DoFHandler<dim>::active_cell_iterator &cell_ref)
        {
          this->cell        = cell_ref;
          this->face_number = numbers::invalid_unsigned_int;
          finite_element_values.reinit(cell_ref);
      ...
      namespace CopyData
      {
        ...
        template <int dim>
        StokesPreconditioner<dim>::StokesPreconditioner(const unsigned int stokes_dofs_per_cell)
          : local_matrix(stokes_dofs_per_cell, stokes_dofs_per_cell)
        ...
        template <int dim>
        StokesPreconditioner<dim>::StokesPreconditioner(const StokesPreconditioner &data)
        ...
        template <int dim>
        void
        StokesPreconditioner<dim>::extract_stokes_dof_indices(const std::vector<types::global_dof_index> &all_dof_indices,
        ...
        template <int dim>
        StokesSystem<dim>::StokesSystem(const unsigned int stokes_dofs_per_cell, const bool do_pressure_rhs_compatibility_modification)
          : StokesPreconditioner<dim>(stokes_dofs_per_cell)
          , local_rhs(stokes_dofs_per_cell)
          , local_pressure_shape_function_integrals(do_pressure_rhs_compatibility_modification ? stokes_dofs_per_cell : 0)
        {}
        ...
        template <int dim>
        AdvectionSystem<dim>::AdvectionSystem(const FiniteElement<dim> &finite_element, const bool field_is_discontinuous)
          : local_matrix(finite_element.dofs_per_cell, finite_element.dofs_per_cell)
          , local_matrices_int_ext((field_is_discontinuous ? Assemblers::n_interface_matrices<dim>(finite_element.reference_cell()) : 0),
                                   FullMatrix<double>(finite_element.dofs_per_cell, finite_element.dofs_per_cell))
          , local_matrices_ext_int((field_is_discontinuous ? Assemblers::n_interface_matrices<dim>(finite_element.reference_cell()) : 0),
                                   FullMatrix<double>(finite_element.dofs_per_cell, finite_element.dofs_per_cell))
          , local_matrices_ext_ext((field_is_discontinuous ? Assemblers::n_interface_matrices<dim>(finite_element.reference_cell()) : 0),
                                   FullMatrix<double>(finite_element.dofs_per_cell, finite_element.dofs_per_cell))
          , local_rhs(finite_element.dofs_per_cell)
          ,

          assembled_matrices((field_is_discontinuous ? Assemblers::n_interface_matrices<dim>(finite_element.reference_cell()) : 0), false)
          ,

          local_dof_indices(finite_element.dofs_per_cell)
          , neighbor_dof_indices((field_is_discontinuous ? Assemblers::n_interface_matrices<dim>(finite_element.reference_cell()) : 0),
  ...
  namespace Assemblers
  {
    ...
    n_interface_matrices(const ReferenceCell<dim> &reference_cell)
    {
      ...
      Assert((reference_cell == ReferenceCells::Triangle) || (reference_cell == ReferenceCells::Quadrilateral) ||
               (reference_cell == ReferenceCells::Tetrahedron) || (reference_cell == ReferenceCells::Hexahedron),
             ExcNotImplemented());
      return (reference_cell.n_faces() * reference_cell.face_reference_cell(0).n_isotropic_children());
    }
    ...
    nth_interface_matrix(const ReferenceCell<dim> &reference_cell, const unsigned int face)
    {
      ...
      return (face * reference_cell.face_reference_cell(0).n_isotropic_children());
    }
    ...
    {
      AssertIndexRange(face, reference_cell.n_faces());
      AssertIndexRange(sub_face, reference_cell.face_reference_cell(0).n_isotropic_children());
      return (face * reference_cell.face_reference_cell(0).n_isotropic_children() + sub_face);
    }
    ...
    template <int dim>
    AdvectionStabilizationInterface<dim>::~AdvectionStabilizationInterface() = default;
...
\
    template unsigned int \
    nth_interface_matrix<dim>(const ReferenceCell<dim> &reference_cell, const unsigned int face); \
\
    template unsigned int \
    nth_interface_matrix<dim>(const ReferenceCell<dim> &reference_cell, const unsigned int face, const unsigned int sub_face); \
  }
