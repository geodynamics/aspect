...
namespace aspect
{
  namespace Assemblers
  {
    template <int dim>
    void
    StokesPreconditionerAnisotropicViscosity<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
            ...
            for (unsigned int j = 0; j < stokes_dofs_per_cell; ++j)
              if (introspection.stokes_dof_info[i].component_index == introspection.stokes_dof_info[j].component_index)
                data.local_matrix(i, j) += (2.0 * eta * (scratch.grads_phi_u[i] * stress_strain_director * scratch.grads_phi_u[j]) +
                                            one_over_eta * pressure_scaling * pressure_scaling * (scratch.phi_p[i] * scratch.phi_p[j])) *
                                           JxW;
        }
    }
    ...
    template <int dim>
    void
    StokesPreconditionerAnisotropicViscosity<dim>::create_additional_material_model_outputs(
      MaterialModel::MaterialModelOutputs<dim> &outputs) const
    {
      ...
      if (outputs.template has_additional_output_object<MaterialModel::AnisotropicViscosity<dim>>() == false)
        {
          outputs.additional_outputs.push_back(std::make_unique<MaterialModel::AnisotropicViscosity<dim>>(n_points));
        }
    }
    ...
    template <int dim>
    void
    StokesCompressiblePreconditionerAnisotropicViscosity<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                                                       internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
      internal::Assembly::Scratch::StokesPreconditioner<dim> &scratch =
        dynamic_cast<internal::Assembly::Scratch::StokesPreconditioner<dim> &>(scratch_base);
            ...
            for (unsigned int j = 0; j < stokes_dofs_per_cell; ++j)
              if (introspection.stokes_dof_info[i].component_index == introspection.stokes_dof_info[j].component_index)
                data.local_matrix(i, j) +=
                  (-eta_two_thirds * (scratch.div_phi_u[i] * trace(stress_strain_director * scratch.grads_phi_u[j]))) * JxW;
        }
    }
    ...
    template <int dim>
    void
    StokesIncompressibleTermsAnisotropicViscosity<dim>::execute(internal::Assembly::Scratch::ScratchBase<dim>   &scratch_base,
                                                                internal::Assembly::CopyData::CopyDataBase<dim> &data_base) const
    {
    ...
    template <int dim>
    void
    StokesIncompressibleTermsAnisotropicViscosity<dim>::create_additional_material_model_outputs(
      MaterialModel::MaterialModelOutputs<dim> &outputs) const
    {
      const unsigned int n_points = outputs.viscosities.size();
    ...
    template <int dim>
    void
    StokesCompressibleStrainRateViscosityTermAnisotropicViscosity<dim>::execute(
