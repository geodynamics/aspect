...
namespace aspect
{
  namespace Assemblers
  {
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          if (advection_field.is_temperature())
            {
              const double density_c_P = scratch.material_model_outputs.densities[q] * scratch.material_model_outputs.specific_heat[q];
              ...
              for (unsigned int i = 0; i < advection_dofs_per_cell; ++i)
                {
                  data.local_rhs(i) += field_term_for_rhs * scratch.phi_field[i] * JxW;

                  for (unsigned int j = 0; j < advection_dofs_per_cell; ++j)
                    {
                      data.local_matrix(i, j) += ((time_step * scratch.material_model_outputs.thermal_conductivities[q] *
            ...
            {
              const double rho_T = scratch.material_model_outputs.densities[q] * scratch.material_model_inputs.temperature[q];
              ...
              for (unsigned int i = 0; i < advection_dofs_per_cell; ++i)
                {
                  data.local_rhs(i) += (field_term_for_rhs * scratch.phi_field[i] + diffusion_term * scratch.phi_field[i] +
      ...
      for (unsigned int q = 0; q < n_q_points; ++q)
        {
          ...
          const double diffusion_term = (this->get_timestep() == 0.0) ?
                                          0.0 :
                                          (scratch.material_model_inputs.temperature[q] - scratch.old_temperature_values[q]) /
                                            this->get_timestep() * scratch.material_model_outputs.densities[q] *
    ...
    EntropyAdvectionSystem<dim>::diffusion_prefactors(internal::Assembly::Scratch::ScratchBase<dim> &scratch_base) const
    {
      ...
      for (unsigned int i = 0; i < prefactors.size(); ++i)
        prefactors[i] = scratch.material_model_outputs.thermal_conductivities[i] * scratch.material_model_inputs.temperature[i] /
    ...
    {
      AssertThrow(Plugins::plugin_type_matches<MaterialModel::EntropyModel<dim>>(simulator_access.get_material_model()) ||
                    Plugins::plugin_type_matches<MaterialModel::Compositing<dim>>(simulator_access.get_material_model()),
      ...
      if (simulator_access.introspection().composition_type_exists(CompositionalFieldDescription::entropy))
        {
          ...
          const std::vector<unsigned int> &entropy_indices =
            simulator_access.introspection().get_indices_for_fields_of_type(CompositionalFieldDescription::entropy);
...
#define INSTANTIATE(dim) \
  ...
  template void set_assemblers_entropy_advection(const SimulatorAccess<dim> &, Assemblers::Manager<dim> &);
