...
namespace aspect
{
  namespace
  {
    ...
    void
    initialize_simulator(const Simulator<dim> &simulator, std::vector<std::unique_ptr<AssemblerType>> &assemblers)
    {
  ...
  template <int dim>
  void
  Simulator<dim>::set_stokes_assemblers()
  {
    ...
    if (material_model->is_compressible() || parameters.enable_prescribed_dilation ||
        prescribed_dilation_manager.get_active_plugin_names().size() > 0)
      {
        ...
        if (parameters.use_full_A_block_preconditioner == false)
          assemblers->stokes_preconditioner.push_back(std::make_unique<aspect::Assemblers::StokesCompressiblePreconditioner<dim>>());

        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesCompressibleStrainRateViscosityTerm<dim>>());
      }

    if (parameters.formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::implicit_reference_density_profile)
      {
        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesImplicitReferenceDensityCompressibilityTerm<dim>>());
      }
    else if (parameters.formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::reference_density_profile)
      {
        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesReferenceDensityCompressibilityTerm<dim>>());
      }
    else if (parameters.formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::incompressible)
      {
      ...
      {
        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesIsentropicCompressionTerm<dim>>());
      }
    else if (parameters.formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::hydrostatic_compression)
      {
        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesHydrostaticCompressionTerm<dim>>());
      }
    else if (parameters.formulation_mass_conservation == Parameters<dim>::Formulation::MassConservation::projected_density_field)
      {
        CitationInfo::add("pda");
        assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesProjectedDensityFieldTerm<dim>>());
      }
    else
      AssertThrow(false,
    ...
    if (!boundary_traction_manager.get_prescribed_boundary_traction_indicators().empty())
      {
        assemblers->stokes_system_on_boundary_face.push_back(std::make_unique<aspect::Assemblers::StokesBoundaryTraction<dim>>());
      }

    // add the terms necessary to normalize the pressure
    if (do_pressure_rhs_compatibility_modification)
      assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesPressureRHSCompatibilityModification<dim>>());

    if (prescribed_dilation_manager.get_active_plugin_names().size() > 0)
      assemblers->stokes_system.push_back(std::make_unique<aspect::Assemblers::StokesPrescribedDilation<dim>>());
  }

  template <int dim>
  void
  Simulator<dim>::set_advection_assemblers()
  {
    ...
    for (unsigned int i = 0; i < 1 + introspection.n_compositional_fields; ++i)
      {
        if ((i == 0 && parameters.temperature_method == Parameters<dim>::AdvectionFieldMethod::fem_field) ||
          ...
          assemblers->advection_system[i].push_back(std::make_unique<aspect::Assemblers::AdvectionSystem<dim>>());
        ...
        if ((i == 0 && parameters.temperature_method == Parameters<dim>::AdvectionFieldMethod::prescribed_field_with_diffusion) ||
            (i > 0 &&
             parameters.compositional_field_methods[i - 1] == Parameters<dim>::AdvectionFieldMethod::prescribed_field_with_diffusion))
          assemblers->advection_system[i].push_back(std::make_unique<aspect::Assemblers::DiffusionSystem<dim>>());
        ...
        if (i > 0 && parameters.compositional_field_methods[i - 1] == Parameters<dim>::AdvectionFieldMethod::fem_darcy_field)
          assemblers->advection_system[i].push_back(std::make_unique<aspect::Assemblers::DarcySystem<dim>>());
        ...
        if (parameters.use_discontinuous_temperature_discretization || parameters.fixed_heat_flux_boundary_indicators.size() != 0 ||
    ...
    if (parameters.use_discontinuous_temperature_discretization || parameters.have_discontinuous_composition_discretization)
      {
        ...
        AssertThrow(geometry_model->get_periodic_boundary_pairs().size() == 0 || Utilities::MPI::n_mpi_processes(mpi_communicator) == 1 ||
                      no_dc_field_method || (parameters.initial_adaptive_refinement == 0 && parameters.adaptive_refinement_interval == 0),
  ...
  Simulator<dim>::set_assemblers()
  {
    ...
    if (!parameters.include_melt_transport && !assemble_newton_stokes_system)
      {
        set_advection_assemblers();
        set_stokes_assemblers();
      }
    else if (parameters.include_melt_transport && !assemble_newton_stokes_system)
      {
      ...
      {
        AssertThrow(false, ExcMessage("The melt implementation does not support the Newton solver at the moment."));
      }
    else
  ...
  template <int dim>
  void
  Simulator<dim>::local_assemble_stokes_preconditioner(const typename DoFHandler<dim>::active_cell_iterator    &cell,
                                                       ...
                                                       internal::Assembly::CopyData::StokesPreconditioner<dim> &data)
  {
    ...
    scratch.material_model_inputs.reinit(scratch.finite_element_values, cell, this->introspection, current_linearization_point);
    ...
    scratch.material_model_inputs.requested_properties =
      MaterialModel::MaterialProperties::equation_of_state_properties | MaterialModel::MaterialProperties::viscosity |
      (parameters.include_melt_transport || assemble_newton_stokes_system ? MaterialModel::MaterialProperties::additional_outputs :
  ...
  template <int dim>
  void
  Simulator<dim>::copy_local_to_global_stokes_preconditioner(const internal::Assembly::CopyData::StokesPreconditioner<dim> &data)
  {
    current_constraints.distribute_local_to_global(data.local_matrix, data.local_dof_indices, system_preconditioner_matrix);
  ...
  Simulator<dim>::assemble_stokes_preconditioner()
  {
    ...
    const UpdateFlags cell_update_flags = ((update_JxW_values | update_values | update_gradients | update_quadrature_points) |
    ...
    WorkStream::run(CellFilter(IteratorFilters::LocallyOwnedCell(), dof_handler.begin_active()),
                    ...
                    worker,
                    copier,
                    internal::Assembly::Scratch::StokesPreconditioner<dim>(finite_element,
                                                                           quadrature_formula,
                                                                           *mapping,
                                                                           cell_update_flags,
                                                                           ...
                                                                           parameters.use_bfbt),
                    internal::Assembly::CopyData::StokesPreconditioner<dim>(stokes_dofs_per_cell));
...
#  if !DEAL_II_VERSION_GTE(9, 7, 0)
    ...
    DoFTools::extract_constant_modes(dof_handler, introspection.component_masks.velocities, constant_modes);
    Amg_data.constant_modes = constant_modes;
#  else
    Amg_data.constant_modes = DoFTools::extract_constant_modes(dof_handler, introspection.component_masks.velocities);
#  endif
    ...
    if (parameters.include_melt_transport == false)
      {
        if (parameters.use_bfbt)
          ...
          {
            LinearAlgebra::PreconditionILU *Mp_preconditioner_ILU = dynamic_cast<LinearAlgebra::PreconditionILU *>(Mp_preconditioner.get());
...
#  if !DEAL_II_VERSION_GTE(9, 7, 0)
        std::vector<std::vector<bool>> constant_modes;
        DoFTools::extract_constant_modes(dof_handler, cm_pressure, constant_modes);
        Amg_data.constant_modes = constant_modes;
#  else
        Amg_data.constant_modes = DoFTools::extract_constant_modes(dof_handler, cm_pressure);
#  endif
#endif
    ...
    if (parameters.use_full_A_block_preconditioner)
      Amg_preconditioner->initialize(system_matrix.block(0, 0), Amg_data);
    else
      Amg_preconditioner->initialize(system_preconditioner_matrix.block(0, 0), Amg_data);
  ...
  template <int dim>
  void
  Simulator<dim>::local_assemble_stokes_system(const typename DoFHandler<dim>::active_cell_iterator &cell,
                                               ...
                                               internal::Assembly::CopyData::StokesSystem<dim>      &data)
  {
    ...
    const bool need_viscosity = assemble_newton_stokes_system || this->parameters.enable_prescribed_dilation || rebuild_stokes_matrix;

    scratch.material_model_inputs.reinit(scratch.finite_element_values, cell, this->introspection, current_linearization_point);

    scratch.material_model_inputs.requested_properties =
      MaterialModel::MaterialProperties::equation_of_state_properties | MaterialModel::MaterialProperties::additional_outputs |
        ...
        for (unsigned int q = 0; q < scratch.finite_element_values.n_quadrature_points; ++q)
          {
            ...
            scratch.reference_densities_depth_derivative[q] =
              adiabatic_conditions->density_derivative(scratch.material_model_inputs.position[q]);
          }
      }
              ...
              if (assemblers->stokes_system_assembler_on_boundary_face_properties.need_face_material_model_data)
                {
                  const bool need_viscosity =
                    rebuild_stokes_matrix | assemblers->stokes_system_assembler_on_boundary_face_properties.need_viscosity;
                  ...
                  for (unsigned int i = 0; i < assemblers->stokes_system_on_boundary_face.size(); ++i)
                    assemblers->stokes_system_on_boundary_face[i]->create_additional_material_model_outputs(
                  ...
                  material_model->evaluate(scratch.face_material_model_inputs, scratch.face_material_model_outputs);
  ...
  template <int dim>
  void
  Simulator<dim>::copy_local_to_global_stokes_system(const internal::Assembly::CopyData::StokesSystem<dim> &data)
  {
    if (rebuild_stokes_matrix == true)
      current_constraints.distribute_local_to_global(data.local_matrix, data.local_rhs, data.local_dof_indices, system_matrix, system_rhs);
    else
      current_constraints.distribute_local_to_global(data.local_rhs, data.local_dof_indices, system_rhs);
  ...
  Simulator<dim>::assemble_stokes_system()
  {
    ...
    const UpdateFlags cell_update_flags = (update_values | update_gradients | update_quadrature_points | update_JxW_values) |
    ...
    auto worker = [&](const typename DoFHandler<dim>::active_cell_iterator &cell,
                      internal::Assembly::Scratch::StokesSystem<dim>       &scratch,
                      internal::Assembly::CopyData::StokesSystem<dim> &data) { this->local_assemble_stokes_system(cell, scratch, data); };

    auto copier = [&](const internal::Assembly::CopyData::StokesSystem<dim> &data) { this->copy_local_to_global_stokes_system(data); };

    WorkStream::run(CellFilter(IteratorFilters::LocallyOwnedCell(), dof_handler.begin_active()),
                    CellFilter(IteratorFilters::LocallyOwnedCell(), dof_handler.end()),
                    worker,
                    copier,
                    internal::Assembly::Scratch::StokesSystem<dim>(finite_element,
                                                                   *mapping,
                                                                   quadrature_formula,
                                                                   ...
                                                                   parameters.use_bfbt),
                    internal::Assembly::CopyData::StokesSystem<dim>(stokes_dofs_per_cell, do_pressure_rhs_compatibility_modification));
  ...
  {
    computing_timer.enter_subsection(advection_field.is_temperature() ? "Build temperature preconditioner" :
    ...
    computing_timer.leave_subsection(advection_field.is_temperature() ? "Build temperature preconditioner" :
                                                                        "Build composition preconditioner");
  }
  ...
  template <int dim>
  void
  Simulator<dim>::local_assemble_advection_system(const AdvectionField                                 &advection_field,
                                                  ...
                                                  internal::Assembly::CopyData::AdvectionSystem<dim>   &data)
  {
    ...
    scratch.finite_element_values[solution_field].get_function_values(old_solution, scratch.old_field_values);
    scratch.finite_element_values[solution_field].get_function_values(old_old_solution, scratch.old_old_field_values);
    ...
    if (parameters.include_melt_transport)
      scratch.material_model_inputs.requested_properties =
        scratch.material_model_inputs.requested_properties | MaterialModel::MaterialProperties::additional_outputs;

    for (const auto &heating_model : heating_model_manager.get_active_plugins())
      scratch.material_model_inputs.requested_properties =
        scratch.material_model_inputs.requested_properties | heating_model->get_required_properties();

    material_model->evaluate(scratch.material_model_inputs, scratch.material_model_outputs);
    if (parameters.formulation_temperature_equation == Parameters<dim>::Formulation::TemperatureEquation::reference_density_profile)
      {
    ...
    if (!parameters.use_operator_splitting &&
        !(introspection.compositional_name_exists("ve_stress_xx") &&
          parameters.mapped_particle_properties.count(introspection.compositional_index_for_name("ve_stress_xx"))))
      {
    ...
    // then also work on possible face terms. if necessary, initialize
    // the material model data on faces
    const bool has_boundary_face_assemblers =
      !assemblers->advection_system_on_boundary_face[advection_field.field_index()].empty() &&
      assemblers->advection_system_assembler_on_face_properties[advection_field.field_index()].need_face_finite_element_evaluation;
    const bool has_interior_face_assemblers =
      !assemblers->advection_system_on_interior_face[advection_field.field_index()].empty() &&
      assemblers->advection_system_assembler_on_face_properties[advection_field.field_index()].need_face_finite_element_evaluation;
    ...
    for (scratch.face_number = 0; scratch.face_number < cell->n_faces(); ++scratch.face_number)
      {
        ...
        if ((has_boundary_face_assemblers && face->at_boundary()) || (has_interior_face_assemblers && !face->at_boundary()))
          {
            (*scratch.face_finite_element_values).reinit(cell, scratch.face_number);

            (*scratch.face_finite_element_values)[introspection.extractors.velocities].get_function_values(
              current_linearization_point, scratch.face_current_velocity_values);

            // get the mesh velocity, as we need to subtract it off of the advection systems
            if (parameters.mesh_deformation_enabled)
              (*scratch.face_finite_element_values)[introspection.extractors.velocities].get_function_values(
                mesh_deformation->mesh_velocity, scratch.face_mesh_velocity_values);
            ...
            if (assemblers->advection_system_assembler_on_face_properties[advection_field.field_index()].need_face_material_model_data)
              {
                ...
                for (unsigned int i = 0; i < assemblers->advection_system_on_boundary_face[advection_field.field_index()].size(); ++i)
                  assemblers->advection_system_on_boundary_face[advection_field.field_index()][i]->create_additional_material_model_outputs(
                    scratch.face_material_model_outputs);

                for (unsigned int i = 0; i < assemblers->advection_system_on_interior_face[advection_field.field_index()].size(); ++i)
                  assemblers->advection_system_on_interior_face[advection_field.field_index()][i]->create_additional_material_model_outputs(
                    scratch.face_material_model_outputs);
                ...
                for (const auto &heating_model : heating_model_manager.get_active_plugins())
                  scratch.face_material_model_inputs.requested_properties =
                    scratch.face_material_model_inputs.requested_properties | heating_model->get_required_properties();
                    ...
                    for (unsigned int q = 0; q < n_q_points; ++q)
                      {
                        scratch.face_material_model_outputs.densities[q] =
                          adiabatic_conditions->density(scratch.face_material_model_inputs.position[q]);
                      }
                  }
  ...
  template <int dim>
  void
  Simulator<dim>::copy_local_to_global_advection_system(const AdvectionField                                     &advection_field,
          ...
          if (data.assembled_matrices[f])
            {
              for (unsigned int i = 0; i < data.local_dof_indices.size(); ++i)
                for (unsigned int j = 0; j < data.neighbor_dof_indices[f].size(); ++j)
                  {
                    system_matrix.add(data.local_dof_indices[i], data.neighbor_dof_indices[f][j], data.local_matrices_int_ext[f](i, j));
                    system_matrix.add(data.neighbor_dof_indices[f][j], data.local_dof_indices[i], data.local_matrices_ext_int[f](j, i));
                  }

              for (unsigned int i = 0; i < data.neighbor_dof_indices[f].size(); ++i)
                for (unsigned int j = 0; j < data.neighbor_dof_indices[f].size(); ++j)
                  system_matrix.add(data.neighbor_dof_indices[f][i], data.neighbor_dof_indices[f][j], data.local_matrices_ext_ext[f](i, j));
            }
      }
  }
  ...
  {
    computing_timer.enter_subsection(advection_field.is_temperature() ? "Assemble temperature system" : "Assemble composition system");
    ...
    if (!advection_field.is_temperature() && sparsity_block_idx != block_idx)
      {
        ...
        // at this time.
        system_matrix.block(block_idx, block_idx).copy_from(system_matrix.block(sparsity_block_idx, sparsity_block_idx));
      }
    ...
    const bool allocate_face_quadrature =
      (!assemblers->advection_system_on_boundary_face.empty() || !assemblers->advection_system_on_interior_face.empty()) &&
      assemblers->advection_system_assembler_on_face_properties[advection_field.field_index()].need_face_finite_element_evaluation;
    const bool allocate_neighbor_contributions =
      !assemblers->advection_system_on_interior_face.empty() &&
      assemblers->advection_system_assembler_on_face_properties[advection_field.field_index()].need_face_finite_element_evaluation;

    const bool use_supg = (parameters.advection_stabilization_method == Parameters<dim>::AdvectionStabilizationMethod::supg);

    // When using SUPG, we need to compute hessians to be able to compute the residual:
    const UpdateFlags update_flags =
      update_values | update_gradients | update_quadrature_points | update_JxW_values | ((use_supg) ? update_hessians : UpdateFlags(0));

    const UpdateFlags face_update_flags =
      (allocate_face_quadrature ? update_values | update_gradients | update_quadrature_points | update_normal_vectors | update_JxW_values :
                                  update_default);
    ...
    WorkStream::run(
      ...
      worker,
      copier,
      internal::Assembly::Scratch::AdvectionSystem<dim>(finite_element,
                                                        ...
                                                        *mapping,
                                                         ...
                                                         */
                                                        (allocate_face_quadrature ? QGauss<dim - 1>(advection_quadrature_degree) :
                                                                                    Quadrature<dim - 1>()),
                                                        update_flags,
                                                        face_update_flags,
                                                        introspection.n_compositional_fields,
                                                        advection_field),
      internal::Assembly::CopyData::AdvectionSystem<dim>(finite_element.base_element(advection_field.base_element(introspection)),
                                                         allocate_neighbor_contributions));

    system_matrix.compress(VectorOperation::add);
    system_rhs.compress(VectorOperation::add);

    computing_timer.leave_subsection(advection_field.is_temperature() ? "Assemble temperature system" : "Assemble composition system");
  }
}
...
#define INSTANTIATE(dim) \
  ...
  template void Simulator<dim>::local_assemble_stokes_preconditioner(const DoFHandler<dim>::active_cell_iterator             &cell, \
                                                                     internal::Assembly::Scratch::StokesPreconditioner<dim>  &scratch, \
                                                                     internal::Assembly::CopyData::StokesPreconditioner<dim> &data); \
  ...
  template void Simulator<dim>::local_assemble_stokes_system(const DoFHandler<dim>::active_cell_iterator     &cell, \
                                                             internal::Assembly::Scratch::StokesSystem<dim>  &scratch, \
                                                             internal::Assembly::CopyData::StokesSystem<dim> &data); \
  template void Simulator<dim>::copy_local_to_global_stokes_system(const internal::Assembly::CopyData::StokesSystem<dim> &data); \
  ...
  template void Simulator<dim>::build_advection_preconditioner(const AdvectionField &, \
                                                               ...
                                                               const double                            diagonal_strengthening); \
  template void Simulator<dim>::local_assemble_advection_system(const AdvectionField                               &advection_field, \
                                                                const Vector<double>                               &viscosity_per_cell, \
                                                                const DoFHandler<dim>::active_cell_iterator        &cell, \
                                                                internal::Assembly::Scratch::AdvectionSystem<dim>  &scratch, \
                                                                internal::Assembly::CopyData::AdvectionSystem<dim> &data); \
  template void Simulator<dim>::copy_local_to_global_advection_system(const AdvectionField &advection_field, \
                                                                      const internal::Assembly::CopyData::AdvectionSystem<dim> &data); \
