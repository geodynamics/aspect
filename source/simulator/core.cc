...
namespace aspect
{
  namespace
  {
    ...
    construct_variables(const Parameters<dim> &parameters, SimulatorSignals<dim> &signals, std::unique_ptr<MeltHandler<dim>> &melt_handler)
    {
      std::vector<VariableDeclaration<dim>> variables = construct_default_variables(parameters);
      if (melt_handler)
    ...
    smoothing_flags(const bool global_coarsening)
    {
      if (global_coarsening)
        ...
        return static_cast<typename Triangulation<dim>::MeshSmoothing>(Triangulation<dim>::limit_level_difference_at_vertices |
   ...
   */
  template <int dim>
  Simulator<dim>::IntermediaryConstructorAction::IntermediaryConstructorAction(const std::function<void()> &action)
  {
    action();
  }
   ...
   **/
  template <int dim>
  Simulator<dim>::Simulator(const MPI_Comm mpi_communicator_, ParameterHandler &prm)
    : simulator_is_past_initialization(false)
    , assemblers(std::make_unique<Assemblers::Manager<dim>>())
    , parameters(prm, mpi_communicator_)
    , melt_handler(parameters.include_melt_transport ? std::make_unique<MeltHandler<dim>>(prm) : nullptr)
    , newton_handler(Parameters<dim>::is_defect_correction(parameters.nonlinear_solver) ? std::make_unique<NewtonHandler<dim>>() : nullptr)
    , post_signal_creation(std::bind(&internals::SimulatorSignals::call_connector_functions<dim>, std::ref(signals)))
    , volume_of_fluid_handler(parameters.volume_of_fluid_tracking_enabled ? std::make_unique<VolumeOfFluidHandler<dim>>(*this, prm) :
                                                                            nullptr)
    ...
    , pcout(iostream_tee_stream, (Utilities::MPI::this_mpi_process(mpi_communicator) == 0))
    ,
    ...
    post_geometry_model_creation_action(
      std::bind(&Parameters<dim>::parse_geometry_dependent_parameters, std::ref(parameters), std::ref(prm), std::cref(*geometry_model)))
    ...
    ,
#ifdef ASPECT_WITH_WORLD_BUILDER
    world_builder(parameters.world_builder_file != "" ? std::make_shared<WorldBuilder::World>(parameters.world_builder_file) : nullptr)
    ,
#endif
    ...
    triangulation(mpi_communicator,
                  smoothing_flags<dim>(parameters.stokes_gmg_type == Parameters<dim>::StokesGMGType::global_coarsening),
                  settings(parameters))
    ,
    ...
    , assemble_newton_stokes_system(Parameters<dim>::is_defect_correction(parameters.nonlinear_solver) ? true : false)
    , rebuild_stokes_preconditioner(true)
  {
    ...
    if (parameters.enable_elasticity)
      {
        ...
        MaterialModel::MaterialModelOutputs<dim> out(1, introspection.n_compositional_fields);
        ...
        AssertThrow(elastic_outputs != nullptr, ExcMessage("Elasticity is enabled, but not supported by the material model."));
      }
    ...
    initial_temperature_manager = std::make_shared<InitialTemperature::Manager<dim>>();
    initial_composition_manager = std::make_shared<InitialComposition::Manager<dim>>();
    ...
    const bool particles_are_needed =
      ...
      (std::find(parameters.compositional_field_methods.begin(),
                 ...
                 Parameters<dim>::AdvectionFieldMethod::particles) != parameters.compositional_field_methods.end());

    if (particles_are_needed)
      {
        AssertThrow(parameters.n_particle_managers <= ASPECT_MAX_NUM_PARTICLE_SYSTEMS,
                    ExcMessage(
                      "You have selected " + std::to_string(parameters.n_particle_managers) +
                      " particle managers, but ASPECT "
                      "has been compiled with a maximum of " +
                      std::to_string(ASPECT_MAX_NUM_PARTICLE_SYSTEMS) +
                      ". "
    ...
    std::set<types::boundary_id> open_velocity_boundary_indicators = geometry_model->get_used_boundary_indicators();
    ...
    do_pressure_rhs_compatibility_modification =
      ((material_model->is_compressible() && !parameters.include_melt_transport) ||
       (parameters.include_melt_transport && !material_model->is_compressible()) || parameters.enable_prescribed_dilation ||
       prescribed_dilation_manager.get_active_plugin_names().size() > 0) &&
    ...
    if (Utilities::MPI::this_mpi_process(mpi_communicator) == 0)
      {
        ...
        AssertThrow(prm_out, ExcMessage(std::string("Could not open file <") + parameters.output_directory + "parameters.prm>."));
        prm.print_parameters(prm_out, ParameterHandler::PRM);

        std::ofstream json_out((parameters.output_directory + "parameters.json"));
        AssertThrow(json_out, ExcMessage(std::string("Could not open file <") + parameters.output_directory + "parameters.json>."));
  ...
  Simulator<dim>::start_timestep()
  {
    // first produce some output for the screen to show where we are
    {
      ...
      pcout << "*** Timestep " << timestep_number << ":  t=" << (time * multiplier) << ' ' << unit << ", dt=" << (time_step * multiplier)
            << ' ' << unit << std::endl;
    }
  ...
  Simulator<dim>::compute_current_constraints()
  {
    ...
    AffineConstraints<double> new_current_constraints(dof_handler.locally_owned_dofs(), introspection.index_sets.system_relevant_set);
        ...
        for (const auto p : boundary_temperature_manager.get_fixed_temperature_boundary_indicators())
          {
            VectorFunctionFromScalarFunctionObject<dim> vector_function_object(
              [&](const dealii::Point<dim> &x) -> double { return boundary_temperature_manager.boundary_temperature(p, x); },
              introspection.component_masks.temperature.first_selected_component(),
              introspection.n_components);

            VectorTools::interpolate_boundary_values(
              *mapping, dof_handler, p, vector_function_object, new_current_constraints, introspection.component_masks.temperature);
          }
      }
      ...
      for (unsigned int c = 0; c < introspection.n_compositional_fields; ++c)
        {
          ...
          if (parameters.use_discontinuous_composition_discretization[c] == false)
            for (const auto p : boundary_composition_manager.get_fixed_composition_boundary_indicators())
              {
                VectorFunctionFromScalarFunctionObject<dim> vector_function_object(
                  [&](const Point<dim> &x) -> double { return boundary_composition_manager.boundary_composition(p, x, c); },
    ...
    const bool mesh_has_changed = (current_constraints.get_local_lines().size() != new_current_constraints.get_local_lines().size()) ||
    ...
    const bool any_constrained_dofs_set_changed = (Utilities::MPI::sum(constraints_changed ? 1 : 0, mpi_communicator) > 0);
  ...
  {
    Table<2, DoFTools::Coupling> coupling(introspection.n_components, introspection.n_components);

    // Start by assuming nothing couples
    coupling.fill(DoFTools::none);

    const typename Introspection<dim>::ComponentIndices &x = introspection.component_indices;
    ...
    if (solver_scheme_solves_stokes_equations(parameters))
      {
        ...
        Assert(!(parameters.include_melt_transport && is_stokes_matrix_free()), ExcNotImplemented());
        ...
        if (is_stokes_matrix_free())
            ...
            for (unsigned int d = 0; d < dim; ++d)
              {
                ...
                coupling[x.velocities[d]][introspection.variable("compaction pressure").first_component_index] = DoFTools::always;
                coupling[introspection.variable("compaction pressure").first_component_index][x.velocities[d]] = DoFTools::always;
                coupling[x.velocities[d]][introspection.variable("fluid pressure").first_component_index]      = DoFTools::always;
                coupling[introspection.variable("fluid pressure").first_component_index][x.velocities[d]]      = DoFTools::always;
              }
        ...
        else
          {
            ...
            // necessary entries.
            if (parameters.use_equal_order_interpolation_for_stokes == true || parameters.enable_prescribed_dilation == true)
    ...
    if (solver_scheme_solves_advection_equations(parameters) && compositional_fields_need_matrix_block(introspection))
      {
    ...
    if (parameters.volume_of_fluid_tracking_enabled)
      {
        const unsigned int volume_of_fluid_block =
          volume_of_fluid_handler->field_struct_for_field_index(0).volume_fraction.first_component_index;
  ...
  Simulator<dim>::setup_system_matrix(const std::vector<IndexSet> &system_partitioning)
  {
    ...
    if ((parameters.use_discontinuous_temperature_discretization) || (parameters.have_discontinuous_composition_discretization) ||
      ...
      {
        Table<2, DoFTools::Coupling> face_coupling(introspection.n_components, introspection.n_components);
        face_coupling.fill(DoFTools::none);

        const typename Introspection<dim>::ComponentIndices &x = introspection.component_indices;
        if (parameters.use_discontinuous_temperature_discretization && solver_scheme_solves_advection_equations(parameters) &&
            parameters.temperature_method != Parameters<dim>::AdvectionFieldMethod::prescribed_field &&
            parameters.temperature_method != Parameters<dim>::AdvectionFieldMethod::static_field)
        ...
        for (const unsigned int base_element_index : introspection.get_composition_base_element_indices())
          {
            ...
            if (parameters.use_discontinuous_composition_discretization[first_c] && solver_scheme_solves_advection_equations(parameters) &&
                block_needed)
        ...
        DoFTools::make_flux_sparsity_pattern(
          dof_handler, dsp, current_constraints, false, coupling, face_coupling, Utilities::MPI::this_mpi_process(mpi_communicator));
        ...
        if (solver_scheme_solves_advection_equations(parameters))
          {
            ...
            Table<2, DoFTools::Coupling> composition_coupling(introspection.n_components, introspection.n_components);
            ...
            for (const unsigned int base_element_index : introspection.get_composition_base_element_indices())
              {
                ...
                dsp.block(block, block)
                  .reinit(system_relevant_partitioning[block].size(),
    ...
    else
      {
        DoFTools::make_sparsity_pattern(
          dof_handler, coupling, dsp, current_constraints, false, Utilities::MPI::this_mpi_process(mpi_communicator));
        ...
        if (solver_scheme_solves_advection_equations(parameters) && compositional_fields_need_matrix_block(introspection))
          {
            ...
            DoFTools::make_sparsity_pattern(
              dof_handler, composition_coupling, dsp, current_constraints, true, Utilities::MPI::this_mpi_process(mpi_communicator));
          }
      }
  ...
  Simulator<dim>::setup_system_preconditioner(const std::vector<IndexSet> &system_partitioning)
  {
    ...
    // unnecessary, so we remove those entries here.
    {
      ...
      dsp.block(block, block).reinit(system_partitioning[block].size(), system_partitioning[block].size(), system_partitioning[block]);
    }
    // compositions:
    for (unsigned int c = 0; c < introspection.n_compositional_fields; ++c)
      {
  ...
  Simulator<dim>::compute_initial_velocity_boundary_constraints(AffineConstraints<double> &constraints)
  {
    ...
    // into current_constraints in start_timestep().
    {
      ...
      // Precompute no-normal-flux constraints for all boundaries that are not deforming.
      // All other boundaries are handled in compute_current_velocity_boundary_constraints() below.
      const auto &unmoving_tangential_velocity_boundaries =
      ...
      VectorTools::compute_no_normal_flux_constraints(dof_handler,
                                                      ...
                                                      unmoving_tangential_velocity_boundaries,
                                                      constraints,
                                                      *mapping,
  ...
  Simulator<dim>::compute_current_velocity_boundary_constraints(AffineConstraints<double> &constraints)
  {
    ...
    // If mesh deformation is enabled, we need to recompute no-normal-flux
    // constraints on deforming boundaries, because the normal vector changes.
    // We also cannot use the manifold for the normal vector because the
    // manifold may not represent the actual geometry.
    if (parameters.mesh_deformation_enabled == true)
      {
        signals.pre_compute_no_normal_flux_constraints(triangulation);
    ...
    for (const auto boundary_id : boundary_velocity_manager.get_prescribed_boundary_velocity_indicators())
      {
        Utilities::VectorFunctionFromVelocityFunctionObject<dim> vel(introspection.n_components,
                                                                     [&](const dealii::Point<dim> &x) -> Tensor<1, dim> {
                                                                       if (!assemble_newton_stokes_system ||
                                                                           (assemble_newton_stokes_system && nonlinear_iteration == 0))
        ...
        VectorTools::interpolate_boundary_values(
          *mapping, dof_handler, boundary_id, vel, constraints, boundary_velocity_manager.get_component_mask(boundary_id));
      }
  }
  ...
  Simulator<dim>::setup_dofs()
  {
    ...
    DoFRenumbering::component_wise(dof_handler, introspection.get_components_to_blocks());
    ...
    // large
    {
      ...
      pcout << "Number of active cells: " << triangulation.n_global_active_cells() << " (on " << triangulation.n_global_levels()
            << " levels)" << std::endl
            << "Number of degrees of freedom: " << dof_handler.n_dofs() << " (" << introspection.system_dofs_per_block[0];
    ...
    geometry_model->make_periodicity_constraints(dof_handler, constraints);
    ...
    old_solution.reinit(introspection.index_sets.system_partitioning,
                        ...
                        mpi_communicator);
    old_old_solution.reinit(introspection.index_sets.system_partitioning,
                            introspection.index_sets.system_relevant_partitioning,
                            mpi_communicator);
    current_linearization_point.reinit(introspection.index_sets.system_partitioning,
                                       introspection.index_sets.system_relevant_partitioning,
                                       mpi_communicator);

    if (parameters.use_operator_splitting)
      operator_split_reaction_vector.reinit(introspection.index_sets.system_partitioning,
                                            introspection.index_sets.system_relevant_partitioning,
                                            mpi_communicator);
  ...
  Simulator<dim>::setup_introspection()
  {
    ...
    introspection.system_dofs_per_block = DoFTools::count_dofs_per_fe_block(dof_handler, introspection.get_components_to_blocks());
...
#else
      DoFTools::extract_locally_relevant_dofs(dof_handler, introspection.index_sets.system_relevant_set);
#endif
      introspection.index_sets.system_relevant_partitioning =
      ...
      if (!parameters.include_melt_transport)
        {
          if (parameters.use_direct_stokes_solver)
            introspection.index_sets.locally_owned_pressure_dofs =
              system_index_set & Utilities::extract_locally_active_dofs_with_component(dof_handler, introspection.component_masks.pressure);
          else
            introspection.index_sets.locally_owned_pressure_dofs =
              introspection.index_sets.system_partitioning[introspection.block_indices.pressure];
        }
      ...
      if (parameters.include_melt_transport)
        {
          ...
          introspection.index_sets.locally_owned_fluid_pressure_dofs =
            system_index_set &
            Utilities::extract_locally_active_dofs_with_component(dof_handler, introspection.variable("fluid pressure").component_mask);
        }



      introspection.index_sets.stokes_partitioning.clear();
      introspection.index_sets.stokes_partitioning.push_back(
      ...
      if (!parameters.use_direct_stokes_solver)
        {
          if (parameters.include_melt_transport)
            ...
            introspection.index_sets.stokes_partitioning.push_back(
              introspection.index_sets.system_partitioning[introspection.variable("fluid pressure").block_index]);
          else
            introspection.index_sets.stokes_partitioning.push_back(
              introspection.index_sets.system_partitioning[introspection.block_indices.pressure]);
        }

      Assert(!parameters.use_direct_stokes_solver || parameters.include_melt_transport ||
  ...
  Simulator<dim>::postprocess()
  {
    ...
    std::list<std::pair<std::string, std::string>> output_list = postprocess_manager.execute(statistics);
          ...
          for (const auto &p : output_list)
            {
              ...
              const unsigned int             n_lines             = std::max(first_column_lines.size(), second_column_lines.size());
              ...
              for (unsigned int line = 0; line < n_lines; ++line)
                {
                  ...
                  if (p.first.empty() == false)
                    pcout << std::left << std::setw(width) << (line < first_column_lines.size() ? first_column_lines[line] : "") << ' ';
  ...
  {
#if !DEAL_II_VERSION_GTE(9, 7, 0)
    parallel::distributed::SolutionTransfer<dim, LinearAlgebra::BlockVector> system_trans(dof_handler);
...

    {
      ...
      if (parameters.coarsening_fraction == 0.0)
      ...
      else
        {
          ...
          for (typename Triangulation<dim>::active_cell_iterator cell = triangulation.begin_active(0);
               cell != triangulation.end_active(parameters.min_grid_level);
               ++cell)
            cell->clear_coarsen_flag();
        }
      ...
      // to new mesh:
      std::vector<const LinearAlgebra::BlockVector *> x_system = {&solution, &old_solution};
...
#if !DEAL_II_VERSION_GTE(9, 7, 0)
          mesh_deformation_trans = std::make_unique<parallel::distributed::SolutionTransfer<dim, LinearAlgebra::Vector>>(
...

    {
      ...
      std::vector<LinearAlgebra::BlockVector *> system_tmp = {&distributed_system, &old_distributed_system};
      ...
      if (parameters.mesh_deformation_enabled)
        {
          ...
          LinearAlgebra::Vector distributed_mesh_displacements, distributed_old_mesh_displacements, distributed_initial_topography;

          distributed_mesh_displacements.reinit(mesh_deformation->mesh_locally_owned, mpi_communicator);
          distributed_old_mesh_displacements.reinit(mesh_deformation->mesh_locally_owned, mpi_communicator);
          distributed_initial_topography.reinit(mesh_deformation->mesh_locally_owned, mpi_communicator);

          std::vector<LinearAlgebra::Vector *> system_tmp = {&distributed_mesh_displacements,
        ...
        switch (parameters.nonlinear_solver_failure_strategy)
          {
            case Parameters<dim>::NonlinearSolverFailureStrategy::continue_with_next_timestep:
              {
                pcout << "   Continuing to the next timestep even though solution is not fully converged." << std::endl << std::endl;
            ...
            case Parameters<dim>::NonlinearSolverFailureStrategy::cut_timestep_size:
              {
                if (timestep_number == 0)
                  {
                    pcout << "   Error: Can not cut the timestep in step 0. Aborting." << std::endl;
  ...
  Simulator<dim>::run()
  {
    ...
    unsigned int max_refinement_level = parameters.initial_global_refinement + parameters.initial_adaptive_refinement;
    ...
    if (parameters.resume_computation == true)
      {
        ...
        AssertThrow(last_checkpoint_id != numbers::invalid_unsigned_int,
                    ExcMessage(
                      "You requested to restart the simulation from a checkpoint, but no matching written checkpoint has been found."));
        ...
        while ((parameters.additional_refinement_times.size() > 0) && (parameters.additional_refinement_times.front() < time + time_step))
          {
        ...
        while ((parameters.additional_checkpoint_times.size() > 0) && (parameters.additional_checkpoint_times.front() < time + time_step))
          {
  ...
  start_time_iteration:
    ...
    do
      {
        ...
        if (!(parameters.skip_solvers_on_initial_refinement && pre_refinement_step < parameters.initial_adaptive_refinement))
          {
            start_timestep();
        ...
        if (!time_stepping_manager.should_repeat_time_step() && !parameters.run_postprocessors_on_nonlinear_iterations)
          postprocess();
        ...
        const bool write_checkpoint_due_to_termination = should_terminate && time_stepping_manager.need_checkpoint_on_terminate();

        const bool checkpoint_written = maybe_write_checkpoint(last_checkpoint_time, write_checkpoint_due_to_termination);
    ...
    resource_output << "-- Total wallclock time elapsed including restarts: " << std::round(wallclock_time) << 's' << std::endl;
    ...
    if (resource_usage > 1.0)
      resource_output << "\n-- Approximate resource usage including restarts:             " << resource_usage << " core hours" << std::endl
                      << "-- Approximate economic cost assuming 0.10 $/core hour:       " << resource_usage * 0.1 << " $" << std::endl
                      << "-- Approximate energy usage assuming 5 Wh/core hour:          " << resource_usage * 0.005 << " kWh" << std::endl
                      << "-- Approximate energy carbon footprint assuming 300 gCO2/kWh: " << resource_usage * 0.005 * 0.3 << " kgCO2"
                      << std::endl
                      << "-- Please use ASPECT responsibly." << std::endl
                      << std::endl;
...
\
  template Simulator<dim>::Simulator(const MPI_Comm mpi_communicator_, ParameterHandler &prm); \
\
  template Simulator<dim>::~Simulator(); \
\
  template void Simulator<dim>::start_timestep(); \
\
  template void Simulator<dim>::compute_current_constraints(); \
\
  template Table<2, DoFTools::Coupling> Simulator<dim>::setup_system_matrix_coupling() const; \
\
  template void Simulator<dim>::setup_system_matrix(const std::vector<IndexSet> &system_partitioning); \
\
  template void Simulator<dim>::setup_system_preconditioner(const std::vector<IndexSet> &system_partitioning); \
\
  template void Simulator<dim>::compute_initial_velocity_boundary_constraints(AffineConstraints<double> &constraints); \
\
  template void Simulator<dim>::compute_current_velocity_boundary_constraints(AffineConstraints<double> &constraints); \
\
  template void Simulator<dim>::setup_dofs(); \
\
  template void Simulator<dim>::setup_introspection(); \
\
  template void Simulator<dim>::postprocess(); \
\
  template void Simulator<dim>::refine_mesh(const unsigned int max_grid_level); \
\
  template void Simulator<dim>::solve_timestep(); \
\
  template void Simulator<dim>::run();
